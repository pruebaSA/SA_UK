namespace System.Security.Cryptography.Xml
{
    ...
    public class EncryptedXml
    {
        ...
        public EncryptedXml(XmlDocument document, Evidence evidence)
        {
            ...
            this.m_xmlDsigSearchDepth = System.Security.Cryptography.Xml.Utils.GetXmlDsigSearchDepth();
        }
        ...
        {
            SecurityManager.ResolvePolicy(this.m_evidence).PermitOnly();
        ...
        public byte[] EncryptData(byte[] plaintext, SymmetricAlgorithm symmetricAlgorithm)
        {
            ...
            System.Buffer.BlockCopy(iV, 0, dst, 0, iV.Length);
            System.Buffer.BlockCopy(src, 0, dst, iV.Length, src.Length);
            return dst;
        }
        ...
        private byte[] GetCipherValue(CipherData cipherData)
        {
            ...
            {
                string baseURI = this.m_document?.BaseURI;
            ...
            else if (cipherData.CipherReference.Uri[0] == '#')
            {
                ...
                string baseUri = this.m_document?.BaseURI;
                decInputStream = cipherData.CipherReference.TransformChain.TransformToOctetStream(input, this.m_xmlResolver, baseUri);
            }
            else
            {
        ...
        public virtual byte[] GetDecryptionIV(EncryptedData encryptedData, string symmetricAlgorithmUri)
        {
            ...
            System.Buffer.BlockCopy(this.GetCipherValue(encryptedData.CipherData), 0, dst, 0, dst.Length);
            return dst;
        }
        ...
        public virtual XmlElement GetIdElement(XmlDocument document, string idValue) => 
            SignedXml.DefaultGetIdElement(document, idValue);

        private bool IsOverXmlDsigRecursionLimit() => 
            (this.m_xmlDsigSearchDepthCounter > this.m_xmlDsigSearchDepth);
        ...
        public void ReplaceData(XmlElement inputElement, byte[] decryptedData)
        {
            ...
            {
                XmlNode newChild = parentNode.OwnerDocument.CreateElement(parentNode.Prefix, parentNode.LocalName, parentNode.NamespaceURI);
                try
                {
                    parentNode.AppendChild(newChild);
                    newChild.InnerXml = this.m_encoding.GetString(decryptedData);
                    XmlNode firstChild = newChild.FirstChild;
                    XmlNode nextSibling = inputElement.NextSibling;
                    XmlNode node5 = null;
                    while (firstChild != null)
                    {
                        node5 = firstChild.NextSibling;
