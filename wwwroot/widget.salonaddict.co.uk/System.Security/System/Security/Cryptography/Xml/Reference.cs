    ...
    public class Reference
    {
        ...
        internal byte[] CalculateHashValue(XmlDocument document, CanonicalXmlNodeList refList)
        {
            ...
            try
            {
                switch (this.m_refTargetType)
                {
                    case System.Security.Cryptography.Xml.ReferenceTargetType.Stream:
                        ...
                        inputStream = this.TransformChain.TransformToOctetStream((Stream) this.m_refTarget, resolver, securityUrl);
                        goto Label_04AA;

                    case System.Security.Cryptography.Xml.ReferenceTargetType.XmlElement:
                        resolver = this.SignedXml.ResolverSet ? this.SignedXml.m_xmlResolver : new XmlSecureResolver(new XmlUrlResolver(), securityUrl);
                        inputStream = this.TransformChain.TransformToOctetStream(System.Security.Cryptography.Xml.Utils.PreProcessElementInput((XmlElement) this.m_refTarget, resolver, securityUrl), resolver, securityUrl);
                        goto Label_04AA;
                    ...
                    case System.Security.Cryptography.Xml.ReferenceTargetType.UriReference:
                        ...
                        resolver = this.SignedXml.ResolverSet ? this.SignedXml.m_xmlResolver : new XmlSecureResolver(new XmlUrlResolver(), securityUrl);
                        inputStream = this.TransformChain.TransformToOctetStream((Stream) null, resolver, securityUrl);
                        goto Label_04AA;
                ...
                if (this.m_uri.Length == 0)
                {
                    ...
                    inputStream = this.TransformChain.TransformToOctetStream(document2, resolver, securityUrl);
                    goto Label_04AA;
                }
                if (this.m_uri[0] == '#')
                {
                    ...
                    if (idFromLocalUri == "xpointer(/)")
                    {
                        ...
                        resolver = this.SignedXml.ResolverSet ? this.SignedXml.m_xmlResolver : new XmlSecureResolver(new XmlUrlResolver(), securityUrl);
                        inputStream = this.TransformChain.TransformToOctetStream(System.Security.Cryptography.Xml.Utils.PreProcessDocumentInput(document, resolver, securityUrl), resolver, securityUrl);
                    }
                    else
                    {
                        ...
                        if (discardComments)
                        {
                            ...
                            inputStream = this.TransformChain.TransformToOctetStream(document4, resolver, securityUrl);
                        }
                        else
                        {
                            inputStream = this.TransformChain.TransformToOctetStream(document3, resolver, securityUrl);
                        }
                    }
                    goto Label_04AA;
                }
                if (!System.Security.Cryptography.Xml.Utils.AllowDetachedSignature())
                {
                ...
                if (request != null)
                {
                    ...
                    if (response != null)
                    {
                        ...
                        {
                            resolver = this.SignedXml.ResolverSet ? this.SignedXml.m_xmlResolver : new XmlSecureResolver(new XmlUrlResolver(), securityUrl);
                            inputStream = this.TransformChain.TransformToOctetStream(input, resolver, this.m_uri);
                            goto Label_04AA;
                        }
                    }
                }
            Label_0494:
                throw new CryptographicException(SecurityResources.GetResourceString("Cryptography_Xml_UriNotResolved"), this.m_uri);
            Label_04AA:
                buffer = this.m_hashAlgorithm.ComputeHash(inputStream);
            }
            finally
            {
                if (inputStream != null)
