namespace System.Security.Cryptography.Pkcs
{
    ...
    public sealed class SubjectIdentifier
    {
        ...
        internal SubjectIdentifier(System.Security.Cryptography.CAPIBase.CERT_ID certId)
        {
        ...
        internal SubjectIdentifier(System.Security.Cryptography.CAPIBase.CERT_INFO certInfo) : this(certInfo.Issuer, certInfo.SerialNumber)
        {
        }

        internal SubjectIdentifier(System.Security.Cryptography.CAPIBase.CMSG_SIGNER_INFO signerInfo) : this(signerInfo.Issuer, signerInfo.SerialNumber)
        {
        }

        internal unsafe SubjectIdentifier(System.Security.Cryptography.CAPIBase.CRYPTOAPI_BLOB issuer, System.Security.Cryptography.CAPIBase.CRYPTOAPI_BLOB serialNumber)
        {
            System.Security.Cryptography.CAPIBase.CERT_ISSUER_SERIAL_NUMBER cert_issuer_serial_number;
            bool flag = true;
            ...
            if (flag)
            {
                ...
                if (System.Security.Cryptography.CAPI.DecodeObject(new IntPtr(7L), issuer.pbData, issuer.cbData, out invalidHandle, out cbDecodedValue))
                {
                    using (invalidHandle)
                    {
                        System.Security.Cryptography.CAPIBase.CERT_NAME_INFO cert_name_info = (System.Security.Cryptography.CAPIBase.CERT_NAME_INFO) Marshal.PtrToStructure(invalidHandle.DangerousGetHandle(), typeof(System.Security.Cryptography.CAPIBase.CERT_NAME_INFO));
                        for (uint j = 0; j < cert_name_info.cRDN; j++)
                        {
                            System.Security.Cryptography.CAPIBase.CERT_RDN cert_rdn = (System.Security.Cryptography.CAPIBase.CERT_RDN) Marshal.PtrToStructure(new IntPtr(((long) cert_name_info.rgRDN) + (j * Marshal.SizeOf(typeof(System.Security.Cryptography.CAPIBase.CERT_RDN)))), typeof(System.Security.Cryptography.CAPIBase.CERT_RDN));
                            for (uint k = 0; k < cert_rdn.cRDNAttr; k++)
                            {
                                System.Security.Cryptography.CAPIBase.CERT_RDN_ATTR cert_rdn_attr = (System.Security.Cryptography.CAPIBase.CERT_RDN_ATTR) Marshal.PtrToStructure(new IntPtr(((long) cert_rdn.rgRDNAttr) + (k * Marshal.SizeOf(typeof(System.Security.Cryptography.CAPIBase.CERT_RDN_ATTR)))), typeof(System.Security.Cryptography.CAPIBase.CERT_RDN_ATTR));
