    ...
    internal static class UriTemplateHelpers
    {
        ...
        public static void DisambiguateSamePath(UriTemplate[] array, int a, int b, bool allowDuplicateEquivalentUriTemplates)
        {
            ...
            if ((b - a) != 1)
            {
                ...
                if (array[a].queries.Count == 0)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR2.GetString(SR2.UTTDuplicate, new object[] { array[a].ToString(), array[a - 1].ToString() })));
                }
                if ((b - a) != 1)
                {
                    goto Label_009C;
        ...
        private static void EnsureQueriesAreDistinct(UriTemplate[] array, int a, int b, bool allowDuplicateEquivalentUriTemplates)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR2.GetString(SR2.UTTOtherAmbiguousQueries, new object[] { array[a].ToString() })));
            }
            ...
            for (int m = 0; m < (b - a); m++)
            {
                for (int n = m + 1; n < (b - a); n++)
                {
                    if (Same(strArray[m], strArray[n]))
                    {
                        ...
                        {
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR2.GetString(SR2.UTTAmbiguousQueries, new object[] { array[a + m].ToString(), array[n + a].ToString() })));
                        }
                        if (!allowDuplicateEquivalentUriTemplates)
                        {
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR2.GetString(SR2.UTTDuplicate, new object[] { array[a + m].ToString(), array[n + a].ToString() })));
                        }
                    }
                }
            }
        ...
        public static string GetUriPath(Uri uri) => 
            uri.GetComponents(UriComponents.KeepDelimiter | UriComponents.Path, UriFormat.Unescaped);
        ...
        public static UriTemplatePartType IdentifyPartType(string part)
        {
            ...
            if (index == -1)
            {
                if (num2 != -1)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new FormatException(SR2.GetString(SR2.UTInvalidFormatSegmentOrQueryPart, new object[] { part })));
                }
        ...
        private class UriTemplateQueryComparer : IComparer<UriTemplate>
        {
            public int Compare(UriTemplate x, UriTemplate y) => 
                Comparer<int>.Default.Compare(x.queries.Count, y.queries.Count);
        }
