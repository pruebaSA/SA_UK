namespace System.ServiceModel.Syndication
{
    ...
    public class Atom10FeedFormatter : SyndicationFeedFormatter, IXmlSerializable
    {
        ...
        private TextSyndicationContent ReadTextContentFrom(XmlReader reader, string context) => 
            ReadTextContentFrom(reader, context, this.PreserveAttributeExtensions);
        ...
        private static TextSyndicationContent ReadTextContentFromHelper(XmlReader reader, string type, string context, bool preserveAttributeExtensions)
        {
            ...
            {
                case "text":
                    plaintext = TextSyndicationContentKind.Plaintext;
                    break;

                case "html":
                    plaintext = TextSyndicationContentKind.Html;
                    break;

                case "xhtml":
                    plaintext = TextSyndicationContentKind.XHtml;
                    break;

                default:
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException(FeedUtils.AddLineInfo(reader, SR2.GetString(SR2.Atom10SpecRequiresTextConstruct, new object[] { context, type }))));
            }
            ...
            string text = (plaintext == TextSyndicationContentKind.XHtml) ? reader.ReadInnerXml() : reader.ReadElementString();
            TextSyndicationContent content = new TextSyndicationContent(text, plaintext);
            if (dictionary != null)
            {
