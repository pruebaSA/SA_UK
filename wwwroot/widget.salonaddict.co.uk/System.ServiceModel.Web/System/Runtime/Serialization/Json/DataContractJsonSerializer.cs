namespace System.Runtime.Serialization.Json
{
    ...
    public sealed class DataContractJsonSerializer : XmlObjectSerializer
    {
        ...
        internal static bool CheckIfJsonNameRequiresMapping(XmlDictionaryString jsonName) => 
            ((jsonName != null) && CheckIfJsonNameRequiresMapping(jsonName.Value));

        internal static void CheckIfTypeIsReference(DataContract dataContract)
        {
            if (dataContract.IsReference)
            {
                throw System.ServiceModel.DiagnosticUtility.ExceptionUtility.ThrowHelperError(XmlObjectSerializer.CreateSerializationException(SR2.GetString(SR2.JsonUnsupportedForIsReference, new object[] { DataContract.GetClrTypeFullName(dataContract.UnderlyingType), dataContract.IsReference })));
            }
        }

        internal static bool CheckIfXmlNameRequiresMapping(string xmlName) => 
            ((xmlName != null) && CheckIfJsonNameRequiresMapping(ConvertXmlNameToJsonName(xmlName)));

        internal static bool CheckIfXmlNameRequiresMapping(XmlDictionaryString xmlName) => 
            ((xmlName != null) && CheckIfXmlNameRequiresMapping(xmlName.Value));

        internal static string ConvertXmlNameToJsonName(string xmlName) => 
            XmlConvert.DecodeName(xmlName);
        ...
        private void Initialize(Type type, IEnumerable<Type> knownTypes, int maxItemsInObjectGraph, bool ignoreExtensionDataObject, IDataContractSurrogate dataContractSurrogate, bool alwaysEmitTypeInformation)
        {
            ...
            {
                throw System.ServiceModel.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException("maxItemsInObjectGraph", System.Runtime.Serialization.SR.GetString("ValueMustBeNonNegative")));
            }
        ...
        internal override bool InternalIsStartObject(XmlReaderDelegator reader) => 
            (base.IsRootElement(reader, this.RootContract, this.RootName, XmlDictionaryString.Empty) || IsJsonLocalName(reader, this.RootName.Value));

        internal override object InternalReadObject(XmlReaderDelegator xmlReader, bool verifyObjectName)
        {
            if (this.MaxItemsInObjectGraph == 0)
            {
                throw System.ServiceModel.DiagnosticUtility.ExceptionUtility.ThrowHelperError(XmlObjectSerializer.CreateSerializationException(System.Runtime.Serialization.SR.GetString("ExceededMaxItemsQuota", new object[] { this.MaxItemsInObjectGraph })));
            }
            if (verifyObjectName)
            {
                if (!this.InternalIsStartObject(xmlReader))
                {
                    throw System.ServiceModel.DiagnosticUtility.ExceptionUtility.ThrowHelperError(XmlObjectSerializer.CreateSerializationExceptionWithReaderDetails(System.Runtime.Serialization.SR.GetString("ExpectingElement", new object[] { XmlDictionaryString.Empty, this.RootName }), xmlReader));
                }
            }
            else if (!base.IsStartElement(xmlReader))
            {
                throw System.ServiceModel.DiagnosticUtility.ExceptionUtility.ThrowHelperError(XmlObjectSerializer.CreateSerializationExceptionWithReaderDetails(System.Runtime.Serialization.SR.GetString("ExpectingElementAtDeserialize", new object[] { XmlNodeType.Element }), xmlReader));
            }
        ...
        internal override void InternalWriteObjectContent(XmlWriterDelegator writer, object graph)
        {
            ...
            else
            {
                ...
                if (rootContract.CanContainReferences)
                {
                    ...
                    json2.SerializeWithXsiTypeAtTopLevel(rootContract, writer, graph, underlyingType.TypeHandle);
                }
                else
                {
        ...
        public override bool IsStartObject(XmlDictionaryReader reader) => 
            base.IsStartObjectHandleExceptions(new JsonReaderDelegator(reader));
        ...
        internal static object ReadJsonValue(DataContract contract, XmlReaderDelegator reader, XmlObjectSerializerReadContextComplexJson context) => 
            JsonDataContract.GetJsonDataContract(contract).ReadJsonValue(reader, context);
        ...
        public override object ReadObject(XmlDictionaryReader reader) => 
            base.ReadObjectHandleExceptions(new JsonReaderDelegator(reader), true);

        public override object ReadObject(XmlReader reader) => 
            base.ReadObjectHandleExceptions(new JsonReaderDelegator(reader), true);

        public override object ReadObject(XmlDictionaryReader reader, bool verifyObjectName) => 
            base.ReadObjectHandleExceptions(new JsonReaderDelegator(reader), verifyObjectName);
        ...
        internal bool AlwaysEmitTypeInformation =>
            this.alwaysEmitTypeInformation;
        ...
        private XmlDictionaryString RootName =>
            (this.rootName ?? JsonGlobals.rootDictionaryString);
    }
}
