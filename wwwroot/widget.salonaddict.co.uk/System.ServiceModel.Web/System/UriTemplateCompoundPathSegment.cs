namespace System
{
    ...
    internal class UriTemplateCompoundPathSegment : UriTemplatePathSegment, IComparable<UriTemplateCompoundPathSegment>
    {
        ...
        private int ComparePrefixToOtherPrefix(UriTemplateCompoundPathSegment other) => 
            string.Compare(other.firstLiteral, this.firstLiteral, StringComparison.OrdinalIgnoreCase);
        ...
        private int CompareToOtherThatHasNoPrefixNorSuffix(UriTemplateCompoundPathSegment other) => 
            (other.varLitPairs.Count - this.varLitPairs.Count);
        ...
        public static UriTemplateCompoundPathSegment CreateFromUriTemplate(string segment, UriTemplate template)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new FormatException(SR2.GetString(SR2.UTInvalidWildcardInVariableOrLiteral, new object[] { template.originalTemplate, "*" })));
            }
            ...
            do
            {
                ...
                if (num2 < (index + 2))
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new FormatException(SR2.GetString(SR2.UTInvalidFormatSegmentOrQueryPart, new object[] { segment })));
                }
                string varName = template.AddPathVariable(UriTemplatePartType.Compound, segment.Substring(index + 1, (num2 - index) - 1), out flag2);
                if (flag2)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR2.GetString(SR2.UTDefaultValueToCompoundSegmentVar, new object[] { template, originalSegment, varName })));
                }
                ...
                if (index > 0)
                {
                    if (index == (num2 + 1))
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgument("template", SR2.GetString(SR2.UTDoesNotSupportAdjacentVarsInCompoundSegment, new object[] { template, segment }));
                    }
        ...
        public override void Lookup(string segment, NameValueCollection boundParameters)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR2.GetString(SR2.UTCSRLookupBeforeMatch, new object[0])));
            }
        }
