namespace System.EnterpriseServices.Thunk
{
    ...
    internal class Callback
    {
        ...
        public unsafe IMessage DoCallback(object otp, IMessage msg, IntPtr ctx, [MarshalAs(UnmanagedType.U1)] bool fIsAutoDone, MemberInfo mb, [MarshalAs(UnmanagedType.U1)] bool bHasGit)
        {
            ...
            *((int*) (&data2 + 12)) = _pfn;
            try
            {
                ...
                int errorCode = **(*((int*) unknownPtr2))(unknownPtr2, &IID_IContextCallback, &callbackPtr);
                if (errorCode < 0)
                {
                ...
                *((int*) (&data2 + 8)) = data.Pin().ToInt32();
                int num = **(((int*) callbackPtr))[12](callbackPtr, __unep@?FilteringCallbackFunction@Thunk@EnterpriseServices@System@@$$FYGJPAUtagComCallData@123@@Z, &data2, &_guid, comSlotForMethodInfo, unknownPtr);
                message = data.msg;
        ...
        private static unsafe int modopt(IsLong) MarshalCallback(tagComCallData* pData)
        {
            ...
            int num = CoGetMarshalSizeMax((uint modopt(IsLong)*) &num2, &IID_IUnknown, unknownPtr, 3, null, 0);
            if (num >= 0)
            {
                num2 += 4;
                try
                ...
                fixed (byte* numRef = data.buffer)
                {
                    try
                    {
                        num = MarshalInterface(numRef, (int) num2, unknownPtr, 3, 0);
                    }
                    fault
                    {
                        numRef = null;
        ...
        public unsafe byte[] SwitchMarshal(IntPtr ctx, IntPtr pUnk)
        {
            ...
            *((int*) (&data2 + 12)) = _pfnMarshal;
            try
            {
                ...
                int errorCode = **(*((int*) unknownPtr))(unknownPtr, &IID_IContextCallback, &callbackPtr);
                if (errorCode < 0)
                {
                    Marshal.ThrowExceptionForHR(errorCode);
                }
                ...
                *((int*) (&data2 + 8)) = data.Pin().ToInt32();
                int num = **(((int*) callbackPtr))[12](callbackPtr, __unep@?FilteringCallbackFunction@Thunk@EnterpriseServices@System@@$$FYGJPAUtagComCallData@123@@Z, &data2, &IID_IUnknown, 2, unknownPtr2);
                if (num < 0)
                {
                    Marshal.ThrowExceptionForHR(num);
                }
