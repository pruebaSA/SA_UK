namespace System.EnterpriseServices.CompensatingResourceManager
{
    ...
    internal class CrmLogControl
    {
        ...
        public unsafe CrmLogControl()
        {
            ...
            this._pCtrl = null;
            int errorCode = CoCreateInstance(&CLSID_CRMClerk, null, 0x15, &IID_ICrmLogControl, (void**) &controlPtr);
            if (errorCode < 0)
            {
        ...
        public unsafe CrmLogControl(IntPtr p)
        {
            ...
            int errorCode = **(*((int*) unknownPtr))(unknownPtr, &IID_ICrmLogControl, &controlPtr);
            if (errorCode < 0)
            {
                Marshal.ThrowExceptionForHR(errorCode);
            }
        ...
        {
            int errorCode = **(((int*) this._pCtrl))[0x18](this._pCtrl);
            if (errorCode < 0)
            {
                Marshal.ThrowExceptionForHR(errorCode);
            }
        ...
        {
            int errorCode = **(((int*) this._pCtrl))[0x20](this._pCtrl);
            if (errorCode < 0)
            {
                Marshal.ThrowExceptionForHR(errorCode);
            }
        ...
        {
            int errorCode = **(((int*) this._pCtrl))[0x1c](this._pCtrl);
            if (errorCode < 0)
            {
                Marshal.ThrowExceptionForHR(errorCode);
            }
        ...
        public unsafe CrmMonitorLogRecords GetMonitor() => 
            new CrmMonitorLogRecords(new IntPtr((int) this._pCtrl));

        public unsafe string GetTransactionUOW()
        {
            char* chPtr;
            ICrmLogControl* controlPtr = this._pCtrl;
            int errorCode = **(((int*) controlPtr))[12](controlPtr, &chPtr);
            if (errorCode < 0)
            {
                Marshal.ThrowExceptionForHR(errorCode);
            }
        ...
        public unsafe void RegisterCompensator(string progid, string desc, int modopt(IsLong) flags)
        {
            ...
            try
            {
                ...
                ICrmLogControl* controlPtr = this._pCtrl;
                int errorCode = **(((int*) controlPtr))[0x10](controlPtr, chPtr2, chPtr, flags);
                int num2 = errorCode;
                if (errorCode < 0)
                {
                    Marshal.ThrowExceptionForHR(errorCode);
        ...
        public unsafe void WriteLogRecord(byte[] b)
        {
            ...
            *((int*) &gblob) = b.Length;
            fixed (byte* numRef = b)
            {
                *((int*) (&gblob + 4)) = numRef;
                ICrmLogControl* controlPtr = this._pCtrl;
                int errorCode = **(((int*) controlPtr))[0x24](controlPtr, &gblob, 1);
                if (errorCode < 0)
                {
                    Marshal.ThrowExceptionForHR(errorCode);
                }
