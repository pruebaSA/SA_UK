namespace System.EnterpriseServices.CompensatingResourceManager
{
    ...
    internal class CrmMonitorLogRecords
    {
        ...
        public unsafe CrmMonitorLogRecords(IntPtr mon)
        {
            ...
            int errorCode = **(*((int*) unknownPtr))(unknownPtr, &IID_ICrmMonitorLogRecords, &recordsPtr);
            if (errorCode < 0)
            {
        ...
        public unsafe int GetCount()
        {
            int num2;
            ICrmMonitorLogRecords* recordsPtr = this._pMon;
            int errorCode = **(((int*) recordsPtr))[12](recordsPtr, &num2);
            if (errorCode < 0)
            {
                Marshal.ThrowExceptionForHR(errorCode);
            }
        ...
        public unsafe _LogRecord GetLogRecord(int index)
        {
            ...
            ICrmMonitorLogRecords* recordsPtr = this._pMon;
            int errorCode = **(((int*) recordsPtr))[0x18](recordsPtr, index, &read);
            if (errorCode < 0)
            {
                Marshal.ThrowExceptionForHR(errorCode);
            }
            _LogRecord record = new _LogRecord {
                ...
                blobUserData = { cbSize = *((int*) (&read + 8)) }
            };
        ...
        public unsafe int GetTransactionState()
        {
            int num2;
            ICrmMonitorLogRecords* recordsPtr = this._pMon;
            int errorCode = **(((int*) recordsPtr))[0x10](recordsPtr, &num2);
            if (errorCode < 0)
            {
                Marshal.ThrowExceptionForHR(errorCode);
            }
