namespace System.EnterpriseServices
{
    ...
    internal class RegistrationDriver
    {
        ...
        internal bool AssemblyHasStrongName(Assembly asm) => 
            (asm.GetName().GetPublicKeyToken().Length > 0);
        ...
        private void InstallTypeLibrary_MTS(ApplicationSpec spec)
        {
            ...
            try
            {
                ...
                coll = (ICatalogCollection) this._appColl.GetCollection(CollectionName.Components, obj2.Key());
                Populate(coll);
                IComponentUtil utilInterface = (IComponentUtil) coll.GetUtilInterface();
                foreach (Type type in spec.NormalTypes)
                {
                    ...
                    for (int i = 0; i < coll.Count(); i++)
                    {
                        ...
                        Guid guid2 = new Guid((string) obj3.Key());
                        if (guid2 == guid)
                        {
                            flag = true;
                            break;
                    ...
                    if (!flag)
                    {
                        utilInterface.ImportComponent("{" + guid + "}");
                    }
                }
            }
            catch (COMException exception)
            {
                throw this.WrapCOMException(coll, exception, Resource.FormatString("Reg_TypeLibInstallErr", spec.TypeLib, spec.Name));
            }
            catch (Exception exception2)
            {
                throw new RegistrationException(Resource.FormatString("Reg_TypeLibInstallErr", spec.TypeLib, spec.Name), exception2);
            }
            catch
            {
        ...
        internal static void Populate(ICatalogCollection coll)
        {
            ...
            {
                if (exception.ErrorCode != System.EnterpriseServices.Util.COMADMIN_E_OBJECTERRORS)
                {
                    throw;
                }
            }
        ...
        private void PrepDriver(ref ApplicationSpec spec)
        {
            ...
            if (Platform.IsLessThan(Platform.Whistler))
            {
                ...
                {
                    throw new RegistrationException(Resource.FormatString("Reg_CatalogErr"), exception2);
                }
                catch
                {
                    throw new RegistrationException(Resource.FormatString("Err_NonClsException", "RegistrationDriver.PrepDriver"));
            ...
            {
                throw new RegistrationException(Resource.FormatString("Reg_CatalogErr"), exception3);
            }
            catch
            {
                throw new RegistrationException(Resource.FormatString("Err_NonClsException", "RegistrationDriver.PrepDriver"));
            ...
            if (((spec.Partition == null) || (spec.Partition.Length == 0)) && (spec.ID != null))
            {
                try
                {
                    ...
                    {
                        if (System.EnterpriseServices.Util.DISP_E_UNKNOWNNAME == exception4.ErrorCode)
                        {
            ...
            if ((spec.Partition != null) && (spec.Partition.Length != 0))
            {
                try
                {
                    ...
                    {
                        if (System.EnterpriseServices.Util.DISP_E_UNKNOWNNAME != exception5.ErrorCode)
                        {
                            throw;
                        }
                ...
                {
                    throw new RegistrationException(Resource.FormatString("Reg_PartitionErr", spec.Partition), exception6);
                }
                catch
                {
                    throw new RegistrationException(Resource.FormatString("Err_NonClsException", "RegistrationDriver.PrepDriver"));
            ...
            {
                throw new RegistrationException(Resource.FormatString("Reg_CatalogErr"), exception7);
            }
            catch
            {
                throw new RegistrationException(Resource.FormatString("Err_NonClsException", "RegistrationDriver.PrepDriver"));
                ...
                if (!Platform.IsLessThan(Platform.Whistler) && (this._cat != null))
                {
                    foreach (Type type in spec.ConfigurableTypes)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                if (System.EnterpriseServices.Util.DISP_E_UNKNOWNNAME != exception.ErrorCode)
                                {
                                    throw;
                                }
                        ...
                        {
                            if (System.EnterpriseServices.Util.REGDB_E_CLASSNOTREG != exception2.ErrorCode)
                            {
                                throw;
                            }
                        }
        ...
        private RegistrationException WrapCOMException(ICatalogCollection coll, COMException e, string msg)
        {
            ...
            if (e.ErrorCode == System.EnterpriseServices.Util.COMADMIN_E_OBJECTERRORS)
            {
