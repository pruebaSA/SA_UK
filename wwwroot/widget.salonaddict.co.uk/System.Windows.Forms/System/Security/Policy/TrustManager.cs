namespace System.Security.Policy
{
    ...
    internal class TrustManager : IApplicationTrustManager, ISecurityEncodable
    {
        ...
        private static bool AppRequestsBeyondDefaultTrust(ApplicationSecurityInfo info)
        {
            try
            {
                PermissionSet target = SecurityManager.ResolveSystemPolicy(info.ApplicationEvidence);
                PermissionSet requestedPermissionSet = GetRequestedPermissionSet(info);
                if ((target == null) && (requestedPermissionSet != null))
                {
                    return true;
                }
                if ((target != null) && (requestedPermissionSet == null))
                ...
                return !requestedPermissionSet.IsSubsetOf(target);
            }
            catch (Exception)
            {
                return true;
        ...
        private static ApplicationTrust CreateApplicationTrust(ActivationContext activationContext, ApplicationSecurityInfo info, ApplicationTrustExtraInfo appTrustExtraInfo, bool trust, bool persist) => 
            new ApplicationTrust(activationContext.Identity) { 
        ...
        {
            System.Collections.IEnumerator hostEnumerator = info.ApplicationEvidence.GetHostEnumerator();
            while (hostEnumerator.MoveNext())
            {
                if (hostEnumerator.Current.GetType() == typeof(Url))
                {
                    Url current = (Url) hostEnumerator.Current;
        ...
        private static bool IsInternetZone(string zoneName) => 
            (string.Compare(zoneName, "Internet", true, CultureInfo.InvariantCulture) == 0);
