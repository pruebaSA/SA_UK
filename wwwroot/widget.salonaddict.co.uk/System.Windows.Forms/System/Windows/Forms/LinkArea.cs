namespace System.Windows.Forms
{
    ...
    public struct LinkArea
    {
        ...
        public bool IsEmpty =>
            ((this.length == this.start) && (this.start == 0));
        ...
        public override string ToString() => 
            ("{Start=" + this.Start.ToString(CultureInfo.CurrentCulture) + ", Length=" + this.Length.ToString(CultureInfo.CurrentCulture) + "}");

        public static bool operator ==(LinkArea linkArea1, LinkArea linkArea2) => 
            ((linkArea1.start == linkArea2.start) && (linkArea1.length == linkArea2.length));

        public static bool operator !=(LinkArea linkArea1, LinkArea linkArea2) => 
            !(linkArea1 == linkArea2);

        public override int GetHashCode() => 
            ((this.start << 4) | this.length);
        public class LinkAreaConverter : TypeConverter
        {
            ...
            public override object CreateInstance(ITypeDescriptorContext context, IDictionary propertyValues) => 
                new LinkArea((int) propertyValues["Start"], (int) propertyValues["Length"]);

            public override bool GetCreateInstanceSupported(ITypeDescriptorContext context) => 
                true;

            public override PropertyDescriptorCollection GetProperties(ITypeDescriptorContext context, object value, Attribute[] attributes) => 
                TypeDescriptor.GetProperties(typeof(LinkArea), attributes).Sort(new string[] { "Start", "Length" });
