namespace System.Windows.Forms.Internal
{
    ...
    [UIPermission(SecurityAction.Assert, Unrestricted=true), SecurityPermission(SecurityAction.Assert, Flags=SecurityPermissionFlag.UnmanagedCode), FileIOPermission(SecurityAction.Assert, Unrestricted=true), EnvironmentPermission(SecurityAction.Assert, Unrestricted=true), ReflectionPermission(SecurityAction.Assert, TypeInformation=true, MemberAccess=true)]
    internal sealed class DbgUtil
    {
        ...
        public static string StackFramesToStr() => 
            StackFramesToStr(gdipInitMaxFrameCount);
                ...
                for (int i = index; i < maxFrameCount; i++)
                {
                    ...
                    if (frame2 != null)
                    {
                        ...
                        if (method != null)
                        {
                            ...
                            str = str + string.Format(CultureInfo.CurrentCulture, "at {0}:{1} {2}.{3}({4})\r\n", new object[] { fileName, frame2.GetFileLineNumber(), method.DeclaringType, method.Name, str2 });
                        }
                    }
                }
            }
        ...
        public static string StackTraceToStr(string message) => 
            StackTraceToStr(message, gdipInitMaxFrameCount);
