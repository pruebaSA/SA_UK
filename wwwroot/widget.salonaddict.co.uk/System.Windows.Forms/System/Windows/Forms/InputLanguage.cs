namespace System.Windows.Forms
{
    ...
    public sealed class InputLanguage
    {
        ...
        internal static InputLanguageChangedEventArgs CreateInputLanguageChangedEventArgs(Message m) => 
            new InputLanguageChangedEventArgs(new InputLanguage(m.LParam), (byte) ((long) m.WParam));
        ...
        public override bool Equals(object value) => 
            ((value is InputLanguage) && (this.handle == ((InputLanguage) value).handle));
        ...
        public static InputLanguage FromCulture(CultureInfo culture)
        {
            ...
            {
                if ((((int) language.handle) & 0xffff) == keyboardLayoutId)
                {
                    return language;
                }
            }
        ...
        private static string PadWithZeroes(string input, int length) => 
            ("0000000000000000".Substring(0, length - input.Length) + input);

        public CultureInfo Culture =>
            new CultureInfo(((int) this.handle) & 0xffff);
        ...
        public string LayoutName
        {
            get
            {
                ...
                int num = ((int) handle) & 0xffff;
                int num2 = (((int) handle) >> 0x10) & 0xfff;
                    ...
                    else
                    {
                        ...
                        if (key2 != null)
                        {
                            ...
                            foreach (string str3 in valueNames)
                            {
                                ...
                                if (((num3 == ((int) handle)) || ((num3 & 0xfffffff) == (((int) handle) & 0xfffffff))) || ((num3 & 0xffff) == num))
                                {
                                    handle = (IntPtr) Convert.ToInt32((string) key2.GetValue(str3), 0x10);
                                    num = ((int) handle) & 0xffff;
                                    num2 = (((int) handle) >> 0x10) & 0xfff;
                                    break;
                                }
                            }
                            key2.Close();
