namespace System.Windows.Forms.ComponentModel.Com2Interop
{
    ...
    internal class Com2IManagedPerPropertyBrowsingHandler : Com2ExtendedBrowsingHandler
    {
        internal static Attribute[] GetComponentAttributes(NativeMethods.IManagedPerPropertyBrowsing target, int dispid)
        {
            ...
            int length = stringsFromPtr.Length;
            for (int i = 0; i < stringsFromPtr.Length; i++)
            {
                ...
                if (c == null)
                {
                    ...
                    int num5 = typeName.LastIndexOf('.');
                    if (num5 == -1)
                    {
                        continue;
                    }
                    string name = typeName.Substring(num5 + 1);
                    if (assembly == null)
                    {
                        c = System.Type.GetType(typeName.Substring(0, num5) + str2);
                    }
                    else
                    {
                        c = assembly.GetType(typeName.Substring(0, num5) + str2);
                    }
        ...
        private void OnGetAttributes(Com2PropertyDescriptor sender, GetAttributesEvent attrEvent)
        {
            ...
            {
                Attribute[] componentAttributes = GetComponentAttributes((NativeMethods.IManagedPerPropertyBrowsing) targetObject, sender.DISPID);
