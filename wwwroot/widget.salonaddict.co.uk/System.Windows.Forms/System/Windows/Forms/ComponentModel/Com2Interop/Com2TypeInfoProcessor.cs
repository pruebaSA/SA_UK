namespace System.Windows.Forms.ComponentModel.Com2Interop
{
    ...
    internal class Com2TypeInfoProcessor
    {
        ...
        public static UnsafeNativeMethods.ITypeInfo FindTypeInfo(object obj, bool wantCoClass)
        {
            ...
            for (int i = 0; (classInfo == null) && (i < 2); i++)
            {
                if (wantCoClass == (i == 0))
                {
                    ...
                    {
                        NativeMethods.IProvideClassInfo info2 = (NativeMethods.IProvideClassInfo) obj;
                        try
                        {
        ...
        public static UnsafeNativeMethods.ITypeInfo[] FindTypeInfos(object obj, bool wantCoClass)
        {
            ...
            {
                NativeMethods.IProvideMultipleClassInfo info2 = (NativeMethods.IProvideMultipleClassInfo) obj;
                if (!NativeMethods.Succeeded(info2.GetMultiTypeInfoCount(ref pcti)) || (pcti == 0))
                {
                    pcti = 0;
                }
                if (pcti > 0)
                {
                    ...
                    {
                        if (!NativeMethods.Failed(info2.GetInfoOfIndex(i, 1, ref pTypeInfo, 0, 0, IntPtr.Zero, IntPtr.Zero)))
                        {
        ...
        private static Guid GetGuidForTypeInfo(UnsafeNativeMethods.ITypeInfo typeInfo, StructCache structCache, int[] versions)
        {
            ...
            NativeMethods.tagTYPEATTR data = null;
            try
            {
                if (structCache == null)
                {
                    data = new NativeMethods.tagTYPEATTR();
                }
                else
                {
                    data = (NativeMethods.tagTYPEATTR) structCache.GetStruct(typeof(NativeMethods.tagTYPEATTR));
                }
        ...
        public static int GetNameDispId(UnsafeNativeMethods.IDispatch obj)
        {
            ...
            if (rgszNames != null)
            {
                ...
                Guid empty = Guid.Empty;
                if (NativeMethods.Succeeded(obj.GetIDsOfNames(ref empty, rgszNames, 1, SafeNativeMethods.GetThreadLCID(), rgDispId)))
                {
                    num = rgDispId[0];
                }
            }
        ...
        private static System.Type GetValueTypeFromTypeDesc(NativeMethods.tagTYPEDESC typeDesc, UnsafeNativeMethods.ITypeInfo typeInfo, object[] typeData, StructCache structCache)
        {
            IntPtr unionMember;
            int hr = 0;
            switch (((NativeMethods.tagVT) typeDesc.vt))
            {
                ...
                {
                    NativeMethods.tagTYPEDESC data = (NativeMethods.tagTYPEDESC) structCache.GetStruct(typeof(NativeMethods.tagTYPEDESC));
                    try
                    {
                        try
                        {
                        ...
                        {
                            data = new NativeMethods.tagTYPEDESC {
                        ...
                        if (data.vt == 12)
                        {
                            return VTToType((NativeMethods.tagVT) data.vt);
                        }
            ...
            try
            {
                if (pTypeInfo != null)
                {
                    ...
                    NativeMethods.tagTYPEATTR gtypeattr = (NativeMethods.tagTYPEATTR) structCache.GetStruct(typeof(NativeMethods.tagTYPEATTR));
        ...
        private static PropertyDescriptor[] InternalGetProperties(object obj, UnsafeNativeMethods.ITypeInfo typeInfo, int dispidToGet, ref int defaultIndex)
        {
            ...
            foreach (PropInfo info in propInfoList.Values)
            {
                ...
                descriptorArray[info.Index] = new Com2PropertyDescriptor(info.DispId, info.Name, array, info.ReadOnly != 2, info.ValueType, info.TypeData, !NativeMethods.Succeeded(hr));
                if (info.IsDefault)
                {
        ...
        private static void ProcessFunctions(UnsafeNativeMethods.ITypeInfo typeInfo, IDictionary propInfoList, int dispidToGet, int nameDispID, ref bool addAboutBox, StructCache structCache)
        {
            IntPtr zero = IntPtr.Zero;
            int typeAttr = typeInfo.GetTypeAttr(ref zero);
            if (!NativeMethods.Succeeded(typeAttr) || (zero == IntPtr.Zero))
            {
                throw new ExternalException(System.Windows.Forms.SR.GetString("TYPEINFOPROCESSORGetTypeAttrFailed", new object[] { typeAttr }), typeAttr);
            }
            NativeMethods.tagTYPEATTR data = (NativeMethods.tagTYPEATTR) structCache.GetStruct(typeof(NativeMethods.tagTYPEATTR));
            UnsafeNativeMethods.PtrToStructure(zero, data);
            try
            {
                if (data != null)
                {
                    NativeMethods.tagFUNCDESC gfuncdesc = (NativeMethods.tagFUNCDESC) structCache.GetStruct(typeof(NativeMethods.tagFUNCDESC));
                    NativeMethods.tagELEMDESC structure = (NativeMethods.tagELEMDESC) structCache.GetStruct(typeof(NativeMethods.tagELEMDESC));
                    for (int i = 0; i < data.cFuncs; i++)
                    {
                        IntPtr pFuncDesc = IntPtr.Zero;
                        if (NativeMethods.Succeeded(typeInfo.GetFuncDesc(i, ref pFuncDesc)) && (pFuncDesc != IntPtr.Zero))
                        {
            ...
            if (enumTypeInfo != null)
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        NativeMethods.tagVARDESC gvardesc = (NativeMethods.tagVARDESC) structCache.GetStruct(typeof(NativeMethods.tagVARDESC));
                        ...
                        for (int i = 0; i < cVars; i++)
                        {
                            ...
                            if (NativeMethods.Succeeded(enumTypeInfo.GetVarDesc(i, ref pVarDesc)) && (pVarDesc != IntPtr.Zero))
                            {
                                try
                                {
                                    ...
                                    if (((gvardesc != null) && (gvardesc.varkind == 2)) && (gvardesc.unionMember != IntPtr.Zero))
                                    {
                                        ...
                                        if (NativeMethods.Succeeded(enumTypeInfo.GetDocumentation(gvardesc.memid, null, ref pBstrDocString, null, null)))
                                        {
                                            string str4;
                                            try
                                            {
        ...
        private static void ProcessVariables(UnsafeNativeMethods.ITypeInfo typeInfo, IDictionary propInfoList, int dispidToGet, int nameDispID, StructCache structCache)
        {
            ...
            try
            {
                if (data != null)
                {
                    ...
                    {
                        IntPtr pVarDesc = IntPtr.Zero;
                        if (NativeMethods.Succeeded(typeInfo.GetVarDesc(i, ref pVarDesc)) && (pVarDesc != IntPtr.Zero))
                        {
                            UnsafeNativeMethods.PtrToStructure(pVarDesc, gvardesc);
                            try
                            {
        ...
        private static System.Type VTToType(NativeMethods.tagVT vt)
        {
            NativeMethods.tagVT gvt = vt;
            if (gvt <= NativeMethods.tagVT.VT_VECTOR)
            {
                switch (gvt)
                {
            ...
            else if (((gvt == NativeMethods.tagVT.VT_ARRAY) || (gvt == NativeMethods.tagVT.VT_BYREF)) || (gvt == NativeMethods.tagVT.VT_RESERVED))
            {
            }
        ...
        internal class CachedProperties
        {
            ...
            public PropertyDescriptor[] Properties =>
                this.ClonePropertyDescriptors(this.props);
        }
