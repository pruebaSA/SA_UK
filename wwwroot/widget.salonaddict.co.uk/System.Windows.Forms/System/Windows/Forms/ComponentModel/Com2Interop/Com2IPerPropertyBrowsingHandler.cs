namespace System.Windows.Forms.ComponentModel.Com2Interop
{
    ...
    internal class Com2IPerPropertyBrowsingHandler : Com2ExtendedBrowsingHandler
    {
        ...
        {
            NativeMethods.IPerPropertyBrowsing targetObject = sender.TargetObject as NativeMethods.IPerPropertyBrowsing;
        ...
        private void OnGetDisplayValue(Com2PropertyDescriptor sender, GetNameItemEvent gnievent)
        {
            try
            {
                if ((sender.TargetObject is NativeMethods.IPerPropertyBrowsing) && (!(sender.Converter is Com2IPerPropertyEnumConverter) && !sender.ConvertingNativeType))
                {
                    bool success = true;
                    string str = GetDisplayString((NativeMethods.IPerPropertyBrowsing) sender.TargetObject, sender.DISPID, ref success);
                    if (success)
                    {
                        gnievent.Name = str;
                    }
        ...
        private void OnGetTypeConverterAndTypeEditor(Com2PropertyDescriptor sender, GetTypeConverterAndTypeEditorEvent gveevent)
        {
            ...
            {
                NativeMethods.IPerPropertyBrowsing targetObject = (NativeMethods.IPerPropertyBrowsing) sender.TargetObject;
                bool flag = false;
                NativeMethods.CA_STRUCT pCaStringsOut = new NativeMethods.CA_STRUCT();
                NativeMethods.CA_STRUCT pCaCookiesOut = new NativeMethods.CA_STRUCT();
                int errorCode = 0;
                try
                {
        ...
        private class Com2IPerPropertyBrowsingEnum : Com2Enum
        {
            ...
            private void EnsureArrays()
            {
                if (!this.arraysFetched)
                {
                    ...
                    try
                    {
                        ...
                        NativeMethods.IPerPropertyBrowsing targetObject = (NativeMethods.IPerPropertyBrowsing) this.target.TargetObject;
                        int length = 0;
                        if (items.Length > 0)
                        {
                            object[] sourceArray = new object[objArray2.Length];
                            NativeMethods.VARIANT pVarOut = new NativeMethods.VARIANT();
            ...
            public override string ToString(object v)
            {
                ...
                {
                    bool success = false;
                    string str = Com2IPerPropertyBrowsingHandler.GetDisplayString((NativeMethods.IPerPropertyBrowsing) this.target.TargetObject, this.target.DISPID, ref success);
                    if (success)
                    {
                        return str;
                    }
        ...
        private class Com2IPerPropertyEnumConverter : Com2EnumConverter
        {
            ...
            public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture, object value, System.Type destType)
            {
                if ((destType == typeof(string)) && !this.itemsEnum.arraysFetched)
                {
                    ...
                    {
                        bool success = false;
                        string str = Com2IPerPropertyBrowsingHandler.GetDisplayString((NativeMethods.IPerPropertyBrowsing) this.itemsEnum.target.TargetObject, this.itemsEnum.target.DISPID, ref success);
                        if (success)
                        {
                            return str;
                        }
