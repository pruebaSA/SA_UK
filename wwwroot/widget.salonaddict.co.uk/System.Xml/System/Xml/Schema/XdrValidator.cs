namespace System.Xml.Schema
{
    ...
    internal sealed class XdrValidator : BaseValidator
    {
                    ...
                    if (value.Length != 0)
                    {
                        ...
                        if (tokenizedType == XmlTokenizedType.ENTITY)
                        {
                            if (datatype.Variety == XmlSchemaDatatypeVariety.List)
                            {
                                ...
                                {
                                    BaseValidator.ProcessEntity(sinfo, str, sender, eventhandler, baseUri, lineNo, linePos);
                                }
                            }
                            else
                            {
        ...
        private XmlQualifiedName QualifiedName(string name, string ns) => 
            new XmlQualifiedName(name, XmlSchemaDatatype.XdrCanonizeUri(ns, base.NameTable, base.SchemaNames));
        ...
        private SchemaElementDecl ThoroughGetElementDecl()
        {
            ...
            if ((elementDecl == null) && base.schemaInfo.TargetNamespaces.Contains(base.context.Namespace))
            {
