namespace System.Xml.Schema
{
    ...
    internal sealed class XdrBuilder : SchemaBuilder
    {
        ...
        internal static bool IsXdrSchema(string uri) => 
            (((uri.Length >= "x-schema:".Length) && (string.Compare(uri, 0, "x-schema:", 0, "x-schema:".Length, StringComparison.Ordinal) == 0)) && !uri.StartsWith("x-schema:#", StringComparison.Ordinal));
        ...
        private static bool ParseInteger(string str, ref uint n) => 
            uint.TryParse(str, NumberStyles.AllowTrailingWhite | NumberStyles.AllowLeadingWhite, NumberFormatInfo.InvariantInfo, out n);
        ...
        private static void XDR_BeginAttribute(XdrBuilder builder)
        {
            ...
            if (pAttdef == null)
            {
                XmlQualifiedName name2 = name;
                if (ns.Length == 0)
                {
                    name2 = new XmlQualifiedName(name.Name, builder._TargetNamespace);
                }
                SchemaAttDef def2 = (SchemaAttDef) builder._SchemaInfo.AttributeDecls[name2];
                if (def2 != null)
                {
                    pAttdef = def2.Clone();
                    pAttdef.Name = name;
        ...
        private static void XDR_BuildAttributeType_Name(XdrBuilder builder, object obj, string prefix)
        {
            ...
            else
            {
                ...
                builder._AttributeDef._AttDef.Name = key;
                if (builder._SchemaInfo.AttributeDecls[key] == null)
                {
        ...
        {
            XmlQualifiedName name = (XmlQualifiedName) obj;
            if ((builder._SchemaInfo.ElementDecls[name] == null) && (((SchemaElementDecl) builder._UndeclaredElements[name]) == null))
            {
                SchemaElementDecl decl = new SchemaElementDecl(name, prefix, SchemaType.DTD);
                builder._UndeclaredElements.Add(name, decl);
            }
            ...
            builder._contentValidator.AddName(name, null);
        }
        ...
        {
            XmlQualifiedName key = (XmlQualifiedName) obj;
            if (builder._SchemaInfo.ElementDecls[key] != null)
            {
        ...
        private static void XDR_EndRoot(XdrBuilder builder)
        {
            ...
            {
                XmlQualifiedName name = builder._UndefinedAttributeTypes._TypeName;
                if (name.Namespace.Length == 0)
                {
                    name = new XmlQualifiedName(name.Name, builder._TargetNamespace);
                }
                SchemaAttDef def = (SchemaAttDef) builder._SchemaInfo.AttributeDecls[name];
                if (def != null)
                {
                    builder._UndefinedAttributeTypes._Attdef = def.Clone();
                    builder._UndefinedAttributeTypes._Attdef.Name = name;
                    builder.XDR_CheckAttributeDefault(builder._UndefinedAttributeTypes, builder._UndefinedAttributeTypes._Attdef);
                }
                else
                {
                    builder.SendValidationEvent("Sch_UndeclaredAttribute", name.Name);
                }
