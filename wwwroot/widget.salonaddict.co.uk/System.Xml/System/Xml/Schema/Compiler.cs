    ...
    internal sealed class Compiler : BaseProcessor
    {
        ...
        private bool BuildParticleContentModel(ParticleContentValidator contentValidator, XmlSchemaParticle particle)
        {
            ...
            else if (particle is XmlSchemaGroupBase)
            {
                ...
                foreach (XmlSchemaParticle particle2 in items)
                {
                    ...
                    flag = this.BuildParticleContentModel(contentValidator, particle2);
                }
        ...
        private XmlSchemaParticle CannonicalizeAll(XmlSchemaAll all, bool root)
        {
            if (all.Items.Count > 0)
            {
                ...
                {
                    XmlSchemaParticle item = this.CannonicalizeParticle(element, false);
        ...
        private XmlSchemaParticle CannonicalizeChoice(XmlSchemaChoice choice, bool root)
        {
            ...
            if (choice.Items.Count > 0)
            {
                ...
                {
                    XmlSchemaParticle item = this.CannonicalizeParticle(particle, false);
                    if (item != XmlSchemaParticle.Empty)
                    {
        ...
        private XmlSchemaParticle CannonicalizeElement(XmlSchemaElement element)
        {
            ...
            XmlSchemaChoice to = new XmlSchemaChoice();
            foreach (XmlSchemaElement element2 in group.Members)
            {
                to.Items.Add(element2);
            }
        ...
        private XmlSchemaParticle CannonicalizeGroupRef(XmlSchemaGroupRef groupRef, bool root)
        {
            ...
            base3.LineNumber = groupRef.LineNumber;
            base3.LinePosition = groupRef.LinePosition;
        ...
        private void CleanupAttributes(XmlSchemaObjectCollection attributes)
        {
            foreach (XmlSchemaObject obj2 in attributes)
            {
                ...
                {
                    this.CleanupAttribute((XmlSchemaAttribute) obj2);
                }
            }
        }
        ...
        private XmlSchemaSimpleType[] CompileBaseMemberTypes(XmlSchemaSimpleType simpleType)
        {
            ...
            if (memberTypes != null)
            {
                foreach (XmlQualifiedName name in memberTypes)
                {
                    ...
                    if (unionMember == null)
                    {
                        throw new XmlSchemaException("Sch_UndeclaredSimpleType", name.ToString(), content);
                    }
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        foreach (XmlSchemaAttribute attribute in complexType.AttributeUses.Values)
                        {
                            ...
                            else if (((decl.AttDefs[attribute.QualifiedName] == null) && (attribute.AttDef != null)) && ((attribute.AttDef.Name != XmlQualifiedName.Empty) && (attribute.AttDef != SchemaAttDef.Empty)))
                            {
        ...
        private void CompileLocalAttributes(XmlSchemaComplexType baseType, XmlSchemaComplexType derivedType, XmlSchemaObjectCollection attributes, XmlSchemaAnyAttribute anyAttribute, XmlSchemaDerivationMethod derivedBy)
        {
            ...
            if (baseType != null)
            {
                ...
                {
                    if ((anyAttribute != null) && (((attributeWildcard == null) || !XmlSchemaAnyAttribute.IsSubset(anyAttribute, attributeWildcard)) || !this.IsProcessContentsRestricted(baseType, anyAttribute, attributeWildcard)))
                    {
        ...
        private void CompileSubstitutionGroup(XmlSchemaSubstitutionGroup substitutionGroup)
        {
            ...
            if (!substitutionGroup.Members.Contains(item))
            {
                ...
                try
                {
                    ...
                    {
                        XmlSchemaSubstitutionGroup group = (XmlSchemaSubstitutionGroup) this.examplars[element3.QualifiedName];
                        if (group != null)
                        {
                            this.CompileSubstitutionGroup(group);
                            foreach (XmlSchemaElement element4 in group.Members)
                            {
                    ...
                    if (list != null)
                    {
                        foreach (XmlSchemaElement element5 in list)
                        {
                            substitutionGroup.Members.Add(element5);
                        }
                    }
        ...
        private bool IsProcessContentsRestricted(XmlSchemaComplexType baseType, XmlSchemaAnyAttribute derivedAttributeWildcard, XmlSchemaAnyAttribute baseAttributeWildcard) => 
            ((baseType == XmlSchemaComplexType.AnyType) || (derivedAttributeWildcard.ProcessContentsCorrect >= baseAttributeWildcard.ProcessContentsCorrect));
        ...
        private void Output(SchemaInfo schemaInfo)
        {
            foreach (XmlSchema schema in this.schemasToCompile.Values)
            {
                ...
                schemaInfo.TargetNamespaces[targetNamespace] = targetNamespace;
            }
            foreach (XmlSchemaElement element in this.elements.Values)
            {
