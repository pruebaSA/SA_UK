    ...
    public sealed class XmlSchemaInference
    {
        ...
        internal XmlSchemaElement FindMatchingElement(bool bCreatingNewType, XmlReader xtr, XmlSchemaComplexType ct, ref int lastUsedSeqItem, ref bool bParticleChanged, XmlSchema parentSchema, bool setMaxoccurs)
        {
            ...
            if (sequence.Items[0].GetType() == typeof(XmlSchemaChoice))
            {
                ...
                {
                    XmlSchemaElement element2 = particle as XmlSchemaElement;
                    if (element2 == null)
                    {
            ...
            XmlSchemaParticle particle2 = sequence.Items[num] as XmlSchemaParticle;
            XmlSchemaElement xse = particle2 as XmlSchemaElement;
            if (xse == null)
            {
                throw new XmlSchemaInferenceException("SchInf_UnknownParticle", 0, 0);
            }
            ...
            num++;
            while (num < sequence.Items.Count)
            {
                particle2 = sequence.Items[num] as XmlSchemaParticle;
                xse = particle2 as XmlSchemaElement;
                if (xse == null)
                {
                    throw new XmlSchemaInferenceException("SchInf_UnknownParticle", 0, 0);
                }
                ...
                {
                    lastUsedSeqItem = num;
                    foreach (XmlSchemaElement element7 in objects)
                    {
                        element7.MinOccurs = 0M;
                    }
                    this.InferElement(xse, false, parentSchema);
                ...
                {
                    lastUsedSeqItem = num;
                    foreach (XmlSchemaElement element8 in objects)
                    {
                        element8.MinOccurs = 0M;
                    }
                    XmlSchemaElement element9 = this.FindGlobalElement(namespaceURI, xtr.LocalName, out parentSchema);
            ...
            XmlSchemaElement element11 = null;
            ...
            else
            {
                ...
                if (el != null)
                {
                    element11 = this.FindGlobalElement(namespaceURI, xtr.LocalName, out parentSchema);
                }
            }
            if (el != null)
            {
                ...
                this.InferElement(element11, false, parentSchema);
                foreach (XmlSchemaElement element12 in sequence.Items)
                {
                    item.Items.Add(this.CreateNewElementforChoice(element12));
                }
                sequence.Items.Clear();
                sequence.Items.Add(item);
                return el;
            }
            lastUsedSeqItem = num2 = lastUsedSeqItem + 1;
            el = this.AddElement(xtr.LocalName, xtr.Prefix, xtr.NamespaceURI, parentSchema, sequence.Items, num2);
            if (!bCreatingNewType)
            {
                el.MinOccurs = 0M;
            }
        ...
        public XmlSchemaSet InferSchema(XmlReader instanceDocument) => 
            this.InferSchema1(instanceDocument, new XmlSchemaSet(this.nametable));
