    ...
    internal abstract class FacetsChecker
    {
            ...
            foreach (XmlSchemaFacet facet in facets)
            {
                ...
                switch (facet.FacetType)
                {
                    case FacetType.Length:
                        compiler.CompileLengthFacet(facet);
                        break;

                    case FacetType.MinLength:
                        compiler.CompileMinLengthFacet(facet);
                        break;

                    case FacetType.MaxLength:
                        compiler.CompileMaxLengthFacet(facet);
                        break;

                    case FacetType.Pattern:
                        compiler.CompilePatternFacet(facet as XmlSchemaPatternFacet);
                        break;

                    case FacetType.Whitespace:
                        compiler.CompileWhitespaceFacet(facet);
                        break;

                    case FacetType.Enumeration:
                        compiler.CompileEnumerationFacet(facet, nsmgr, nameTable);
                        break;

                    case FacetType.MinExclusive:
                        compiler.CompileMinExclusiveFacet(facet);
                        break;

                    case FacetType.MinInclusive:
                        compiler.CompileMinInclusiveFacet(facet);
                        break;

                    case FacetType.MaxExclusive:
                        compiler.CompileMaxExclusiveFacet(facet);
                        break;

                    case FacetType.MaxInclusive:
                        compiler.CompileMaxInclusiveFacet(facet);
                        break;

                    case FacetType.TotalDigits:
                        compiler.CompileTotalDigitsFacet(facet);
                        break;

                    case FacetType.FractionDigits:
                        compiler.CompileFractionDigitsFacet(facet);
                        break;
        ...
        private struct FacetsCompiler
        {
            ...
            internal void CompileLengthFacet(XmlSchemaFacet facet)
            {
                ...
                if ((((this.baseFlags & RestrictionFlags.MinLength) != 0) || ((this.baseFlags & RestrictionFlags.MaxLength) != 0)) && ((this.datatype.Restriction.MaxLength < this.derivedRestriction.Length) || (this.datatype.Restriction.MinLength > this.derivedRestriction.Length)))
                {
            ...
            internal void FinishFacetCompile()
            {
                if (!this.firstPattern)
                {
                    ...
                    {
                        throw new XmlSchemaException("Sch_PatternFacetInvalid", exception.Message, this.pattern_facet);
                    }
                }
            }
