namespace System.Xml.Schema
{
    ...
    internal sealed class DtdValidator : BaseValidator
    {
            ...
            try
            {
                ...
                if (datatype != null)
                {
                    ...
                    if (tokenizedType == XmlTokenizedType.ENTITY)
                    {
                        ...
                        string str = (baseUri == null) ? string.Empty : baseUri.ToString();
                        if (datatype.Variety == XmlSchemaDatatypeVariety.List)
                        {
                            string[] strArray = (string[]) defaultValueTyped;
                            foreach (string str2 in strArray)
                            {
                                BaseValidator.ProcessEntity(sinfo, str2, readerAdapter, readerAdapter.EventHandler, str, attdef.ValueLineNum, attdef.ValueLinePos);
                            }
                        }
                        else
                        {
                            BaseValidator.ProcessEntity(sinfo, (string) defaultValueTyped, readerAdapter, readerAdapter.EventHandler, str, attdef.ValueLineNum, attdef.ValueLinePos);
                        }
                    }
        ...
        private SchemaEntity GetEntity(XmlQualifiedName qname, bool fParameterEntity)
        {
            if (fParameterEntity)
            {
                return (SchemaEntity) base.schemaInfo.ParameterEntities[qname];
            }
            return (SchemaEntity) base.schemaInfo.GeneralEntities[qname];
        }
        ...
        public static void SetDefaultTypedValue(SchemaAttDef attdef, IDtdParserAdapter readerAdapter)
        {
            try
            {
                ...
                if (datatype != null)
                {
                    ...
                    attdef.DefaultValueTyped = datatype.ParseValue(defaultValueExpanded, readerAdapter.NameTable, readerAdapter.NamespaceManager);
                }
            }
            catch (Exception)
            {
