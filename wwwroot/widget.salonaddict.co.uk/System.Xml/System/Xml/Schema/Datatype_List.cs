    ...
    internal class Datatype_List : Datatype_anySimpleType
    {
        ...
        internal override Exception TryParseValue(string s, XmlNameTable nameTable, IXmlNamespaceResolver nsmgr, out object typedValue)
        {
            ...
            if (exception == null)
            {
                ...
                {
                    foreach (string str in XmlConvert.SplitString(s))
                    {
                        object obj3;
                        exception = this.itemType.TryParseValue(str, nameTable, nsmgr, out obj3);
                        if (exception != null)
                        {
                            return exception;
                        }
                ...
                {
                    foreach (string str2 in XmlConvert.SplitString(s))
                    {
                        exception = this.itemType.TryParseValue(str2, nameTable, nsmgr, out typedValue);
                        if (exception != null)
                        {
                            return exception;
                        }
        ...
        internal override System.Xml.Schema.FacetsChecker FacetsChecker =>
            DatatypeImplementation.listFacetsChecker;

        internal DatatypeImplementation ItemType =>
            this.itemType;

        internal override Type ListValueType =>
            this.itemType.ListValueType;

        public override XmlTokenizedType TokenizedType =>
            this.itemType.TokenizedType;
