namespace System.Xml.Serialization
{
    ...
    public class XmlCodeExporter : CodeExporter
    {
        ...
        {
            CodeMemberField field = new CodeMemberField(member.GetTypeName(base.CodeProvider), member.Name);
        ...
        private object ImportDefaultValue(TypeMapping mapping, string defaultValue)
        {
            ...
            if (mapping is PrimitiveMapping)
            {
                ...
                if (!mapping3.TypeDesc.HasCustomFormatter)
                {
                    ...
                    return type.GetMethod("To" + mapping3.TypeDesc.FormatterName, new Type[] { typeof(string) })?.Invoke(type, new object[] { defaultValue });
                }
                if (mapping3.TypeDesc.HasDefaultSupport)
                {
