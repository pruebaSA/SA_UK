namespace System.Xml
{
    ...
    internal sealed class DocumentSchemaValidator : IXmlNamespaceResolver
    {
        ...
        private XmlSchemaObject FindSchemaInfo(XmlElement elementToValidate)
        {
            ...
            if (this.nodeSequenceToValidate == null)
            {
                XmlSchemaComplexType schemaType = schemaInfo.SchemaType as XmlSchemaComplexType;
                if (schemaType == null)
                {
                    return null;
                }
                if (!schemaType.HasWildCard && !schemaType.HasDuplicateDecls)
                {
                    return this.GetTypeFromParent(elementToValidate, schemaType);
                }
                this.CheckNodeSequenceCapacity(currentIndex);
                this.nodeSequenceToValidate[currentIndex++] = parentNode;
            }
        ...
        private XmlSchemaElement GetTypeFromParent(XmlElement elementToValidate, XmlSchemaComplexType parentSchemaType) => 
            (parentSchemaType.LocalElements[new XmlQualifiedName(elementToValidate.LocalName, elementToValidate.NamespaceURI)] as XmlSchemaElement);
