namespace System.Xml
{
    ...
    public class XmlTextWriter : XmlWriter
    {
        ...
        private unsafe void ValidateName(string name, bool NCName)
        {
            ...
            if (this.namespaces)
            {
                if ((this.xmlCharType.charProperties[name[num2]] & 4) == 0)
                {
                    goto Label_00E9;
                }
                while (true)
                {
                    num2++;
                    while ((num2 < length) && ((this.xmlCharType.charProperties[name[num2]] & 8) != 0))
                    {
                        num2++;
                    }
                    if (num2 == length)
                    ...
                    if (((name[num2] != ':') || NCName) || ((num3 != -1) || ((num2 + 1) >= length)))
                    {
                        goto Label_00E9;
                    }
                    num3 = num2;
            ...
            if (((this.xmlCharType.charProperties[name[0]] & 4) != 0) || (name[0] == ':'))
            {
                num2++;
                while ((num2 < length) && (((this.xmlCharType.charProperties[name[num2]] & 8) != 0) || (name[num2] == ':')))
                {
                    num2++;
                }
                if (num2 == length)
