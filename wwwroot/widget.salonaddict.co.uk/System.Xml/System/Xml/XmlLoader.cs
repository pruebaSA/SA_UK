namespace System.Xml
{
    ...
    internal class XmlLoader
    {
        ...
        private XmlParserContext GetContext(XmlNode node)
        {
            ...
            return new XmlParserContext(nameTable, nsMgr, documentType?.Name, documentType?.PublicId, documentType?.SystemId, documentType?.InternalSubset, baseURI, xmlLang, none);
        }
                ...
                switch (reader.NodeType)
                {
                    ...
                    case XmlNodeType.EntityReference:
                        ...
                        if (reader.CanResolveEntity)
                        {
                            ...
                            {
                                node.AppendChildForLoad(this.doc.CreateTextNode(""), this.doc);
                            }
                        }
                        break;
                ...
                switch (reader.NodeType)
                {
                    ...
                    case XmlNodeType.EntityReference:
                        ...
                        if (reader.CanResolveEntity)
                        {
                            ...
                            if (node.FirstChild == null)
                            {
                                node.AppendChildForLoad(direct ? new XmlText("") : this.doc.CreateTextNode(""), this.doc);
                            }
                        }
                        break;
        ...
        private void LoadDocumentType(SchemaInfo schInfo, XmlDocumentType dtNode)
        {
            ...
            if (schInfo != null)
            {
                ...
                {
                    foreach (SchemaNotation notation in schInfo.Notations.Values)
                    {
                ...
                {
                    foreach (SchemaEntity entity in schInfo.GeneralEntities.Values)
                    {
                ...
                {
                    foreach (SchemaEntity entity3 in schInfo.ParameterEntities.Values)
                    {
                ...
                IDictionaryEnumerator enumerator = schInfo.ElementDecls.GetEnumerator();
                if (enumerator != null)
                {
                    enumerator.Reset();
        ...
        private XmlDocumentType LoadDocumentTypeNode()
        {
            ...
            {
                this.LoadDocumentType(dtdSchemaInfo, dtNode);
                return dtNode;
            }
        ...
        private XmlEntityReference LoadEntityReferenceNode(bool direct)
        {
            ...
            if (this.reader.CanResolveEntity)
            {
                ...
                {
                    reference.AppendChildForLoad(this.doc.CreateTextNode(""), this.doc);
                }
            }
            return reference;
        }
        ...
        internal static Exception UnexpectedNodeType(XmlNodeType nodetype) => 
            new InvalidOperationException(string.Format(CultureInfo.InvariantCulture, Res.GetString("Xml_UnexpectedNodeType"), new object[] { nodetype.ToString() }));
    }
}
