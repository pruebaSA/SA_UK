namespace MS.Internal.Xml.Cache
{
    ...
    internal sealed class XPathDocumentBuilder : XmlRawWriter
    {
        ...
        {
            foreach (SchemaElementDecl decl in schInfo.ElementDecls.Values)
            {
                if (decl.AttDefs != null)
                {
                    foreach (SchemaAttDef def in decl.AttDefs.Values)
                    {
                        if (def.Datatype.TokenizedType == XmlTokenizedType.ID)
                        {
                            ...
                            this.elemIdMap.Add(decl.Name, def.Name);
                            break;
                        }
                    }
                }
