namespace System.Runtime.Remoting.Channels
{
    ...
    internal static class CoreChannel
    {
        ...
        internal static IMessage DeserializeBinaryResponseMessage(Stream inputStream, IMethodCallMessage reqMsg, bool bStrictBinding) => 
            ((IMessage) CreateBinaryFormatter(false, bStrictBinding).UnsafeDeserializeMethodResponse(inputStream, null, reqMsg));

        internal static IMessage DeserializeMessage(string mimeType, Stream xstm, bool methodRequest, IMessage msg) => 
            DeserializeMessage(mimeType, xstm, methodRequest, msg, null);
        ...
        internal static string GetResourceString(string key) => 
            SystemResMgr?.GetString(key, null);

        internal static Header[] GetSoapHeaders(IMessage reqMsg) => 
            GetMessagePropertiesAsSoapHeader(reqMsg);
        ...
        internal static bool IsLocalIpAddress(IPAddress remoteAddress) => 
            s_MachineIpAddress?.Equals(remoteAddress);
        ...
        internal static bool IsClientSKUInstallation
        {
            get
            {
                if (!s_isClientSKUInstallationInitialized)
                {
                    ...
                    new RegistryPermission(RegistryPermissionAccess.Read, @"HKEY_LOCAL_MACHINE\" + name).Assert();
                    try
                    {
                    ...
                    if (!flag)
                    {
                        string str2 = @"SOFTWARE\Microsoft\NET Framework Setup\DotNetClient\v3.5";
                        new RegistryPermission(RegistryPermissionAccess.Read, @"HKEY_LOCAL_MACHINE\" + str2).Assert();
                        try
                        {
                            RegistryKey key2 = Registry.LocalMachine.OpenSubKey(str2);
                            if (key2 != null)
                            {
                                object obj3 = key2.GetValue("Install");
                                if (obj3 is int)
                                {
                                    flag2 = ((int) obj3) == 1;
                                }
