namespace Microsoft.SqlServer.Server
{
    ...
    internal static class ValueUtilsSmi
    {
        ...
        private static SqlBinary GetSqlBinary_Unchecked(SmiEventSink_Default sink, ITypedGettersV3 getters, int ordinal) => 
            new SqlBinary(GetByteArray_Unchecked(sink, getters, ordinal));
        ...
        internal static object NullUdtInstance(SmiMetaData metaData) => 
            metaData.Type.InvokeMember("Null", BindingFlags.GetProperty | BindingFlags.Public | BindingFlags.Static, null, null, new object[0], CultureInfo.InvariantCulture);
        ...
        private static void SetBytes_FromSqlReader(SmiEventSink_Default sink, SmiTypedGetterSetter setters, int ordinal, SmiMetaData metaData, SqlDataReader reader, int offset)
        {
            ...
            for (long i = 0L; ((num4 < 0) || (i < num4)) && ((0L != (num6 = reader.GetBytesInternal(ordinal, dataIndex, buffer, 0, length))) && (0L != num2)); i += num2)
            {
                num2 = setters.SetBytes(sink, ordinal, dataIndex, buffer, 0, (int) num6);
                sink.ProcessMessagesAndThrow();
                dataIndex += num2;
            }
            setters.SetBytesLength(sink, ordinal, dataIndex);
            sink.ProcessMessagesAndThrow();
        }
