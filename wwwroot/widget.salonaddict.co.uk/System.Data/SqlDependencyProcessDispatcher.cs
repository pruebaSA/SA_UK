...
internal class SqlDependencyProcessDispatcher : MarshalByRefObject
{
    ...
    internal bool StartWithDefault(string connectionString, out string server, out DbConnectionPoolIdentity identity, out string user, out string database, ref string service, string appDomainKey, SqlDependencyPerAppDomainDispatcher dispatcher, out bool errorOccurred, out bool appDomainStart) => 
        this.Start(connectionString, out server, out identity, out user, out database, ref service, appDomainKey, dispatcher, out errorOccurred, out appDomainStart, true);
    ...
    private class SqlConnectionContainer
    {
        ...
        internal SqlConnectionContainer(SqlDependencyProcessDispatcher.SqlConnectionContainerHashHelper hashHelper, string appDomainKey, bool useDefaults)
        {
            ...
            try
            {
                ...
                SqlConnectionString connectionOptions = (SqlConnectionString) this._con.ConnectionOptions;
                connectionOptions.CreatePermissionSet().Assert();
        ...
        private void CreateQueueAndService(bool restart)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    command.CommandText = "CREATE PROCEDURE " + this._sprocName + " AS BEGIN BEGIN TRANSACTION; RECEIVE TOP(0) conversation_handle FROM " + this._escapedQueueName + "; IF (SELECT COUNT(*) FROM " + this._escapedQueueName + " WHERE message_type_name = 'http://schemas.microsoft.com/SQL/ServiceBroker/DialogTimer') > 0 BEGIN DROP SERVICE " + this._escapedQueueName + "; DROP QUEUE " + this._escapedQueueName + "; DROP PROCEDURE " + this._sprocName + "; END COMMIT TRANSACTION; END";
                    if (!restart)
                    {
                    ...
                    command.CommandText = "IF OBJECT_ID('" + this._queue + "') IS NULL BEGIN CREATE QUEUE " + this._escapedQueueName + " WITH ACTIVATION (PROCEDURE_NAME=" + this._sprocName + ", MAX_QUEUE_READERS=1, EXECUTE AS OWNER); END; IF (SELECT COUNT(*) FROM sys.services WHERE NAME='" + this._queue + "') = 0 BEGIN CREATE SERVICE " + this._escapedQueueName + " ON QUEUE " + this._escapedQueueName + " ([http://schemas.microsoft.com/SQL/Notifications/PostQueryNotification]); IF (SELECT COUNT(*) FROM sys.database_principals WHERE name='sql_dependency_subscriber' AND type='R') <> 0 BEGIN GRANT SEND ON SERVICE::" + this._escapedQueueName + " TO sql_dependency_subscriber; END;  END; BEGIN DIALOG @dialog_handle FROM SERVICE " + this._escapedQueueName + " TO SERVICE '" + this._queue + "'";
                                ...
                                if (sqlXml != null)
                                {
                                    ...
                                    if (sqlNotification != null)
                                    {
                                        ...
                                        if (index >= 0)
                                        {
                                            ...
                                            SqlDependencyPerAppDomainDispatcher dispatcher = SqlDependencyProcessDispatcher._staticInstance._sqlDependencyPerAppDomainDispatchers[str3];
                                            if (dispatcher != null)
                                            {
                                                try
                                                {
                    ...
                    while ((MessageAttributes.All != none) && reader.MoveToNextAttribute())
                    {
                        ...
                        Label_014F:;
                            try
                            {
                                ...
                                SqlNotificationInfo info2 = (SqlNotificationInfo) Enum.Parse(typeof(SqlNotificationInfo), str3, true);
