    ...
    internal sealed class XSDSchema : XMLSchema
    {
                ...
                foreach (object obj2 in schema.Items)
                {
                    ...
                    if (obj2 is XmlSchemaType)
                    {
                        ...
                        if (node != null)
                        {
                            ...
                            SimpleType simpleType = column?.SimpleType;
                            if (simpleType != null)
                            {
                                SimpleType otherSimpleType = new SimpleType(node);
                                string errorStr = simpleType.HasConflictingDefinition(otherSimpleType);
        ...
        internal void HandleElementColumn(XmlSchemaElement elem, DataTable table, bool isBase)
        {
            ...
            if (node != null)
            {
                ...
                if (flag)
                {
                    int index = this.GetIntegerAttribute(elem, "Ordinal", -1);
            ...
            if (!this.FromInference || (table.XmlText == null))
            {
                ...
                if (strType != null)
                {
                    ...
                    if (attrs != null)
                    {
                        ...
                        {
                            string str3;
                            if ((attrs[i].NamespaceURI == "urn:schemas-microsoft-com:xml-msdata") && ((str3 = attrs[i].LocalName) != null))
                            {
                                if (str3 != "AllowDBNull")
                                {
                                    if (str3 == "Ordinal")
                            ...
                            continue;
                        Label_014F:
                            index = Convert.ToInt32(attrs[i].Value);
                            continue;
                        Label_0161:
                            s = attrs[i].Value;
                        }
                        ...
                        if ((((localName != "DefaultValue") && (localName != "Ordinal")) && ((localName != "Locale") && (localName != "RemotingFormat"))) && ((localName != "Expression") || !(instance is DataColumn)))
                        {
                            ...
                            else
                            {
                                ...
                                if (descriptor != null)
                                {
                                    ...
                                    {
                                        type = converter.ConvertFromString(typeName);
                                    }
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            throw ExceptionBuilder.CannotConvert(typeName, propertyType.FullName);
                                        }
                                        type = new CultureInfo(typeName);
                                    }
        ...
        private sealed class NameType : IComparable
        {
            ...
            public int CompareTo(object obj) => 
                string.Compare(this.name, (string) obj, StringComparison.Ordinal);
        }
    }
}
