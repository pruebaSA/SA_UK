    ...
    internal sealed class Select
    {
        ...
        private int Eval(BinaryNode expr, DataRow row, DataRowVersion version)
        {
            ...
            if ((expr.op != 13) && (expr.op != 0x27))
            {
                ...
                {
                    type3 = expr.ResultSqlType(storageType, right, lc, rc, expr.op);
                }
                else
                {
                    type3 = expr.ResultType(storageType, right, lc, rc, expr.op);
                }
                if (type3 == StorageType.Empty)
                {
                    expr.SetTypeMismatchError(expr.op, obj3.GetType(), obj2.GetType());
                }
                NameNode node = null;
                CompareInfo comparer = ((((lc && !rc) && ((storageType == StorageType.String) && (right == StorageType.Guid))) && (((node = expr.right as NameNode) != null) && (node.column.DataType == typeof(Guid)))) || (((rc && !lc) && ((right == StorageType.String) && (storageType == StorageType.Guid))) && (((node = expr.left as NameNode) != null) && (node.column.DataType == typeof(Guid))))) ? CultureInfo.InvariantCulture.CompareInfo : null;
                num = expr.BinaryCompare(obj3, obj2, type3, expr.op, comparer);
            }
            switch (expr.op)
            {
                case 7:
