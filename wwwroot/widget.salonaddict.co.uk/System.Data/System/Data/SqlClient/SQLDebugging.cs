namespace System.Data.SqlClient
{
    ...
    [ComVisible(true), Guid("afef65ad-4577-447a-a148-83acadd3d4b9"), ClassInterface(ClassInterfaceType.None), PermissionSet(SecurityAction.LinkDemand, Name="FullTrust")]
    public sealed class SQLDebugging : ISQLDebug
    {
        ...
        private IntPtr CreateSD(ref IntPtr pDacl)
        {
            ...
            if (ptr != IntPtr.Zero)
            {
                ...
                flag = NativeMethods.AllocateAndInitializeSid(ptr, 1, 11, 0, 0, 0, 0, 0, 0, 0, ref pSid);
                if (flag && (pSid != IntPtr.Zero))
                {
                    flag = NativeMethods.AllocateAndInitializeSid(ptr, 2, 0x20, 0x220, 0, 0, 0, 0, 0, 0, ref ptr4);
                    if (flag && (ptr4 != IntPtr.Zero))
                    {
                        ...
                        if (zero != IntPtr.Zero)
                        {
                            ...
                            cb = (0x2c + NativeMethods.GetLengthSid(pSid)) + NativeMethods.GetLengthSid(ptr4);
                            pDacl = Marshal.AllocHGlobal(cb);
                            if ((((pDacl != IntPtr.Zero) && NativeMethods.InitializeAcl(pDacl, cb, 2)) && (NativeMethods.AddAccessDeniedAce(pDacl, 2, 0x40000, pSid) && NativeMethods.AddAccessAllowedAce(pDacl, 2, 0x80000000, pSid))) && ((NativeMethods.AddAccessAllowedAce(pDacl, 2, 0x10000000, ptr4) && NativeMethods.InitializeSecurityDescriptor(zero, 1)) && NativeMethods.SetSecurityDescriptorDacl(zero, true, pDacl, false)))
                            {
                                flag = true;
                            }
                        }
        ...
        bool ISQLDebug.SQLDebug(int dwpidDebugger, int dwpidDebuggee, [MarshalAs(UnmanagedType.LPStr)] string pszMachineName, [MarshalAs(UnmanagedType.LPStr)] string pszSDIDLLName, int dwOption, int cbData, byte[] rgbData)
        {
            ...
            zero = NativeMethods.CreateFileMappingA(ADP.InvalidPtr, ptr2, 4, 0, Marshal.SizeOf(typeof(MEMMAP)), str);
            if (IntPtr.Zero != zero)
            {
                ptr = NativeMethods.MapViewOfFile(zero, 6, 0, 0, IntPtr.Zero);
                if (IntPtr.Zero != ptr)
                {
                    int ofs = 0;
                    ...
                    NativeMethods.UnmapViewOfFile(ptr);
                    flag = true;
                }
            }
