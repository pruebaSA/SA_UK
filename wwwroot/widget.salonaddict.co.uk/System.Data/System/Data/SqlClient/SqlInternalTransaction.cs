namespace System.Data.SqlClient
{
    ...
    internal sealed class SqlInternalTransaction
    {
        ...
        internal string TraceString() => 
            string.Format(null, "(ObjId={0}, tranId={1}, state={2}, type={3}, open={4}, disp={5}", new object[] { this.ObjectID, this._transactionId, this._transactionState, this._transactionType, this._openResultCount, this._disposing });
        ...
        internal bool HasParentTransaction =>
            ((TransactionType.LocalFromAPI == this._transactionType) || ((TransactionType.LocalFromTSQL == this._transactionType) && (this._parent != null)));

        internal bool IsAborted =>
            (TransactionState.Aborted == this._transactionState);

        internal bool IsActive =>
            (TransactionState.Active == this._transactionState);

        internal bool IsCommitted =>
            (TransactionState.Committed == this._transactionState);
        ...
        internal bool IsContext =>
            (TransactionType.Context == this._transactionType);

        internal bool IsDelegated =>
            (TransactionType.Delegated == this._transactionType);

        internal bool IsDistributed =>
            (TransactionType.Distributed == this._transactionType);

        internal bool IsLocal =>
            (((TransactionType.LocalFromTSQL == this._transactionType) || (TransactionType.LocalFromAPI == this._transactionType)) || (TransactionType.Context == this._transactionType));

        internal bool IsOrphaned =>
            (this._parent?.Target == null);

        internal bool IsZombied =>
            (null == this._innerConnection);
