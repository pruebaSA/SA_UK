namespace System.Data.SqlClient
{
    ...
    public sealed class SqlBulkCopy : IDisposable
    {
        ...
        private BulkCopySimpleResultSet CreateAndExecuteInitialQuery()
        {
            ...
            string str = "select @@trancount; SET FMTONLY ON select * from " + this.DestinationTableName + " SET FMTONLY OFF ";
            if (this._connection.IsShiloh)
            {
                string str4;
                if (this._connection.IsKatmaiOrNewer)
                {
                    str4 = "sp_tablecollations_100";
                }
                else if (this._connection.IsYukonOrNewer)
                {
                    str4 = "sp_tablecollations_90";
                }
                else
                {
                    str4 = "sp_tablecollations";
                }
                ...
                if (((str3.Length > 0) && ('#' == str3[0])) && ADP.IsEmpty(str5))
                {
                    str = str + string.Format(null, "exec tempdb..{0} N'{1}.{2}'", new object[] { str4, str2, str3 });
                }
                else
                {
                    str = str + string.Format(null, "exec {0}..{1} N'{2}.{3}'", new object[] { str5, str4, str2, str3 });
                }
            }
            Bid.Trace("<sc.SqlBulkCopy.CreateAndExecuteInitialQuery|INFO> Initial Query: '%ls' \n", str);
            this._parser.TdsExecuteSQLBatch(str, this.BulkCopyTimeout, null, this._stateObj);
        ...
        private bool IsCopyOption(SqlBulkCopyOptions copyOption) => 
            ((this._copyOptions & copyOption) == copyOption);
