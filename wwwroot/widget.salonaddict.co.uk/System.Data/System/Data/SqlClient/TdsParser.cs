namespace System.Data.SqlClient
{
    ...
    internal sealed class TdsParser
    {
        ...
        internal static void ClearPoolCallback(object state)
        {
            if (state != null)
            {
                ((DbConnectionPoolGroup) state).Clear();
            }
        }
        ...
        private PreLoginHandshakeStatus ConsumePreLoginHandshake(bool encrypt, bool trustServerCert, out bool marsCapable)
        {
            ...
            if ((this._physicalStateObj._inBytesPacket > 0x8000) || (this._physicalStateObj._inBytesPacket <= 0))
            {
                throw SQL.ParsingError();
            }
            byte[] buff = new byte[this._physicalStateObj._inBytesPacket];
            ...
            Label_0257:
                ...
                if (SNINativeMethodWrapper.SNIWaitForSSLHandshakeToComplete(this._physicalStateObj.Handle, TdsParserStaticMethods.GetTimeoutMilliseconds(this._physicalStateObj.TimeoutTime)) != 0)
                {
                    this.Errors.Add(this.ProcessSNIError(this._physicalStateObj));
                    this.ThrowExceptionAndWarning(this._physicalStateObj);
                }
        ...
        {
            byte[] buffer = null;
            using (SqlDataReader reader = this.TdsExecuteTransactionManagerRequest(null, TdsEnums.TransactionManagerRequestType.GetDTCAddress, null, TdsEnums.TransactionManagerIsolationLevel.Unspecified, timeout, null, stateObj, false))
            {
        ...
        internal int IncrementNonTransactedOpenResultCount() => 
            Interlocked.Increment(ref this._nonTransactedOpenResultCount);
        ...
        Label_0150:
            while (num2 > 0)
            {
                ...
                if ((buff == null) || (buff.Length < (offst + num)))
                {
                    ...
                    if (buff != null)
                    {
                        System.Buffer.BlockCopy(buff, 0, dst, 0, offst * 2);
                    }
                    buff = dst;
                }
                if (num > 0)
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 3:
                    {
                        ...
                        b[index++] = (byte) (0xff000000L & currentThreadId);
                        b[index++] = (byte) (0xff0000 & currentThreadId);
                        b[index++] = (byte) (0xff00 & currentThreadId);
                        b[index++] = (byte) (0xff & currentThreadId);
                        num2 += 4;
                        num4 = 4;
                        goto Label_0196;
                    }
                        ...
                        for (int i = 0; i < rpcArray.Length; i++)
                        {
                            ...
                            for (int j = 0; j < parameters.Length; j++)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        int actualSize;
                                        int num3 = internalMetaType.IsSizeInCharacters ? (param.GetParameterSize() * 2) : param.GetParameterSize();
                                        ...
                                        else
                                        {
                                            if (internalMetaType.SqlDbType == SqlDbType.Udt)
                                            {
                                                ...
                                                if (!flag3)
                                                {
                                                    ...
                                                    num3 = b.Length;
                                                    if ((num3 < 0) || ((num3 >= 0xffff) && (num != -1)))
                                                    {
                                                        throw new IndexOutOfRangeException();
                                                    }
                                                }
                                                BitConverter.GetBytes((long) num3);
                                            ...
                                            {
                                                num = (num3 > actualSize) ? num3 : actualSize;
        ...
        {
            Encoding encoding = this._defaultEncoding;
            SqlCollation collation = this._defaultCollation;
            int num6 = this._defaultCodePage;
            int num5 = this._defaultLCID;
            try
            {
                ...
                ulong length = (ulong) metadata.length;
                ulong num = 0L;
                ulong byteCount = 0L;
                if (ADP.IsNull(value))
                {
                ...
                else
                {
                    switch (metaType.NullableType)
                    {
                        ...
                        case 0xad:
                        case 240:
                            num = (value is byte[]) ? ((ulong) ((byte[]) value).Length) : ((ulong) ((SqlBinary) value).Length);
                            break;
                        ...
                        case 0xaf:
                            ...
                            if (value is string)
                            {
                                num = (ulong) ((string) value).Length;
                                byteCount = (ulong) this._defaultEncoding.GetByteCount((string) value);
                            }
                            else
                            {
                                SqlString str6 = (SqlString) value;
                                num = (ulong) str6.Value.Length;
                                SqlString str5 = (SqlString) value;
                                byteCount = (ulong) this._defaultEncoding.GetByteCount(str5.Value);
                            }
                            if (byteCount > length)
                            {
                                if (encoding == null)
                                ...
                                this._defaultCollation = collation;
                                this._defaultCodePage = num6;
                                this._defaultLCID = num5;
                                if (value is string)
                                {
                                    num = (ulong) ((string) value).Length;
                                    byteCount = (ulong) this._defaultEncoding.GetByteCount((string) value);
                        ...
                        case 0xe7:
                        case 0xef:
                            num = (ulong) (((value is string) ? ((long) ((string) value).Length) : ((long) ((SqlString) value).Value.Length)) * 2L);
                            break;
                    ...
                    if (metaType.IsLong)
                    {
                        switch (metaType.SqlDbType)
                        {
                            ...
                            case SqlDbType.Text:
                                ...
                                this.WriteTokenLength(metadata.tdsType, (byteCount == 0L) ? ((int) num) : ((int) byteCount), stateObj);
                                break;
                    ...
                    {
                        this.WriteSqlValue(value, metaType, (int) num, (int) byteCount, 0, stateObj);
                    }
                    else if ((metaType.SqlDbType != SqlDbType.Udt) || metaType.IsLong)
                    {
                        this.WriteValue(value, metaType, metadata.scale, (int) num, (int) byteCount, 0, stateObj);
                    }
                    else
                    {
                        this.WriteShort((int) num, stateObj);
                        this.WriteByteArray((byte[]) value, (int) num, 0, stateObj);
                    }
                }
            }
            finally
        ...
        internal void WriteByteArray(byte[] b, int len, int offsetBuffer, TdsParserStateObject stateObj)
        {
            ...
            while (len > 0)
            {
                if ((stateObj._outBytesUsed + len) > stateObj._outBuff.Length)
                {
                    ...
                    System.Buffer.BlockCopy(b, srcOffset, stateObj._outBuff, stateObj._outBytesUsed, count);
                    srcOffset += count;
                ...
                {
                    System.Buffer.BlockCopy(b, srcOffset, stateObj._outBuff, stateObj._outBytesUsed, len);
