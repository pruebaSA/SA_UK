namespace IFRAME.Admin
{
    ...
    public class InvoiceForwardPage : IFRMAdminPage
    {
        ...
        private void BindSalonInvoice(SalonInvoiceDB value)
        {
            ...
            this.ltrTotalAmountDue.Text = (((double) value.TotalAmountDue) / 100.0).ToString("C");
        }
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            if (this.Page.IsValid)
            {
                ...
                salonInvoiceById.PaidOn = new DateTime?(DateTime.Now);
                salonInvoiceById.TotalPaid = 0;
                ...
                if (salonInvoiceById.InvoiceType == "U")
                {
                    WSPDB wSPById = IoC.Resolve<IBillingManager>().GetWSPById(salonInvoiceById.PlanId);
                    wSPById.Active = true;
                    wSPById.UpdatedOn = DateTime.Now;
                    wSPById.UpdatedBy = workingUser.Username;
