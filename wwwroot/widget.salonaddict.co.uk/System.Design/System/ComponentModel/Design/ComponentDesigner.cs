    ...
    public class ComponentDesigner : ITreeDesigner, IDesigner, IDisposable, IDesignerFilter, IComponentInitializer
    {
        ...
        private void OnComponentRename(object sender, ComponentRenameEventArgs e)
        {
            if (this.Component is IPersistComponentSettings)
            {
                ...
                if (!this.settingsKeyExplicitlySet && ((e.Component == this.Component) || (e.Component == rootComponent)))
                {
        ...
        protected bool Inherited =>
            !this.InheritanceAttribute.Equals(System.ComponentModel.InheritanceAttribute.NotInherited);
        ...
        private string SettingsKey
        {
            get
            {
                if (string.IsNullOrEmpty((string) this.ShadowProperties["SettingsKey"]))
                {
                    ...
                    IDesignerHost service = this.GetService(typeof(IDesignerHost)) as IDesignerHost;
                    IComponent rootComponent = service?.RootComponent;
                    if ((settings != null) && (rootComponent != null))
                    {
                        if (string.IsNullOrEmpty(settings.SettingsKey))
                        {
                            if ((rootComponent != null) && (rootComponent != settings))
                            {
                                this.ShadowProperties["SettingsKey"] = string.Format(CultureInfo.CurrentCulture, "{0}.{1}", new object[] { rootComponent.Site.Name, this.Component.Site.Name });
                            }
                            else
                            {
        ...
        protected sealed class ShadowPropertyCollection
        {
            ...
            public bool Contains(string propertyName) => 
                ((this.properties != null) && this.properties.ContainsKey(propertyName));
