    ...
    public class DesignerSerializationManager : IDesignerSerializationManager, IServiceProvider
    {
        ...
        public object GetSerializer(Type objectType, Type serializerType)
        {
                ...
                if (currentSerializer == null)
                {
                    foreach (Attribute attribute in TypeDescriptor.GetAttributes(objectType))
                    {
                        if (attribute is DesignerSerializerAttribute)
                        {
                            ...
                            if (((serializerBaseTypeName != null) && (this.GetType(serializerBaseTypeName) == serializerType)) && ((attribute2.SerializerTypeName != null) && (attribute2.SerializerTypeName.Length > 0)))
                            {
                                Type type = this.GetType(attribute2.SerializerTypeName);
                                if (type != null)
                                {
                                    currentSerializer = Activator.CreateInstance(type, BindingFlags.CreateInstance | BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance, null, null, null);
                                    break;
                                }
                            }
                        }
            ...
            if ((this.defaultProviderTable == null) || !this.defaultProviderTable.ContainsKey(serializerType))
            {
                ...
                if (attribute3 != null)
                {
                    c = this.GetType(attribute3.ProviderTypeName);
        ...
        object IDesignerSerializationManager.GetSerializer(Type objectType, Type serializerType) => 
            this.GetSerializer(objectType, serializerType);
        ...
        private sealed class WrappedPropertyDescriptor : PropertyDescriptor
        {
            ...
            public override bool CanResetValue(object component) => 
                this.property.CanResetValue(this.target);

            public override object GetValue(object component) => 
                this.property.GetValue(this.target);
            ...
            public override bool ShouldSerializeValue(object component) => 
                this.property.ShouldSerializeValue(this.target);
