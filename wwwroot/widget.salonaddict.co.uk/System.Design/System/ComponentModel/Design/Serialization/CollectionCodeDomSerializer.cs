namespace System.ComponentModel.Design.Serialization
{
    ...
    public class CollectionCodeDomSerializer : CodeDomSerializer
    {
        ...
        private static MethodInfo ChooseMethodByType(List<MethodInfo> methods, ICollection values)
        {
            ...
            {
                Type reflectionType = TypeDescriptor.GetReflectionType(obj2);
        ...
        protected virtual object SerializeCollection(IDesignerSerializationManager manager, CodeExpression targetExpression, Type targetType, ICollection originalCollection, ICollection valuesToSerialize)
        {
            ...
            if (valuesToSerialize.Count > 0)
            {
                ...
                MethodInfo info2 = ChooseMethodByType(list, valuesToSerialize);
                if (info2 != null)
                {
                    obj2 = this.SerializeViaAddRange(manager, targetExpression, targetType, info2.GetParameters()[0], valuesToSerialize);
                    flag = true;
                }
                else
                {
                    MethodInfo info3 = ChooseMethodByType(list2, valuesToSerialize);
                    if (info3 != null)
                    {
                        obj2 = this.SerializeViaAdd(manager, targetExpression, targetType, info3.GetParameters()[0], valuesToSerialize);
                        flag = true;
                    }
                }
            ...
            using (CodeDomSerializerBase.TraceScope("CollectionCodeDomSerializer::SerializeViaAdd"))
            {
                ...
                foreach (object obj2 in valuesToSerialize)
                {
                    ...
                    if (flag)
                    {
                        ...
                        if (context != null)
                        {
                            context2 = new ExpressionContext(context.Expression, parameterType, context.Owner);
                        ...
                        if (((obj2 != null) && !parameterType.IsAssignableFrom(obj2.GetType())) && obj2.GetType().IsPrimitive)
                        {
                            expression3 = new CodeCastExpression(parameterType, expression3);
                        }
                        if (expression3 != null)
                        {
