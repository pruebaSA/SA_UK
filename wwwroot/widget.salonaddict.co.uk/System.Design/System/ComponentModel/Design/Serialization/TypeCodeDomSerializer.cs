    ...
    public class TypeCodeDomSerializer : CodeDomSerializerBase
    {
        ...
        public virtual object Deserialize(IDesignerSerializationManager manager, CodeTypeDeclaration declaration)
        {
            ...
            using (CodeDomSerializerBase.TraceScope("TypeCodeDomSerializer::Deserialize"))
            {
                ...
                if (type == null)
                {
                    base.Error(manager, System.Design.SR.GetString("SerializerTypeNotFound", new object[] { name }), "SerializerTypeNotFound");
                }
                if (TypeDescriptor.GetReflectionType(type).IsAbstract)
                {
                    base.Error(manager, System.Design.SR.GetString("SerializerTypeAbstract", new object[] { type.FullName }), "SerializerTypeAbstract");
                }
                ...
                this._nameTable = new HybridDictionary(declaration.Members.Count, caseInsensitive);
