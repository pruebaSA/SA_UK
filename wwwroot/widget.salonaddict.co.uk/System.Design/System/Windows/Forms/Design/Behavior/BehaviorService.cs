namespace System.Windows.Forms.Design.Behavior
{
    ...
    public sealed class BehaviorService : IDisposable
    {
        ...
        private bool OnMouseDoubleClick(MouseButtons button, Point mouseLoc) => 
            this.GetAppropriateBehavior(this.hitTestedGlyph)?.OnMouseDoubleClick(this.hitTestedGlyph, button, mouseLoc);

        private bool OnMouseDown(MouseButtons button, Point mouseLoc) => 
            this.GetAppropriateBehavior(this.hitTestedGlyph)?.OnMouseDown(this.hitTestedGlyph, button, mouseLoc);

        private bool OnMouseEnter(Glyph g) => 
            this.GetAppropriateBehavior(g)?.OnMouseEnter(g);

        private bool OnMouseHover(Point mouseLoc) => 
            this.GetAppropriateBehavior(this.hitTestedGlyph)?.OnMouseHover(this.hitTestedGlyph, mouseLoc);

        private bool OnMouseLeave(Glyph g)
        {
            this.UnHookMouseEvent();
            return this.GetAppropriateBehavior(g)?.OnMouseLeave(g);
        }

        private bool OnMouseMove(MouseButtons button, Point mouseLoc)
        {
            this.HookMouseEvent();
            return this.GetAppropriateBehavior(this.hitTestedGlyph)?.OnMouseMove(this.hitTestedGlyph, button, mouseLoc);
        }
        ...
        private bool OnMouseUp(MouseButtons button)
        {
            this.dragEnterReplies.Clear();
            this.validDragArgs = null;
            return this.GetAppropriateBehavior(this.hitTestedGlyph)?.OnMouseUp(this.hitTestedGlyph, button);
        }
        ...
        private class AdornerWindow : Control
        {
            ...
            internal AdornerWindow(BehaviorService behaviorService, Control designerFrame)
            {
                ...
                designerFrame.HandleDestroyed += new EventHandler(this.OnDesignerFrameHandleDestroyed);
            }
            ...
            protected override void Dispose(bool disposing)
            {
                ...
                {
                    this.designerFrame.HandleDestroyed -= new EventHandler(this.OnDesignerFrameHandleDestroyed);
            ...
            protected override void WndProc(ref Message m)
            {
                ...
                Point pt = new Point((short) System.Design.NativeMethods.Util.LOWORD((int) m.LParam), (short) System.Design.NativeMethods.Util.HIWORD((int) m.LParam));
            ...
            private bool DesignerFrameValid =>
                (((this.designerFrame != null) && !this.designerFrame.IsDisposed) && this.designerFrame.IsHandleCreated);
                ...
                private unsafe IntPtr MouseHookProc(int nCode, IntPtr wparam, IntPtr lparam)
                {
                    if (this.isHooked && (nCode == 0))
                    {
                        ...
                        if (mousehookstructPtr != null)
                        {
                            try
                            {
                                if (this.ProcessMouseMessage(mousehookstructPtr->hWnd, (int) wparam, mousehookstructPtr->pt_x, mousehookstructPtr->pt_y))
                                {
                                    return (IntPtr) 1;
                                }
                            }
        ...
        private class MenuCommandHandler : IMenuCommandService
        {
            ...
            bool IMenuCommandService.GlobalInvoke(CommandID commandID) => 
                this.menuService.GlobalInvoke(commandID);
