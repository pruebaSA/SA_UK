namespace System.Windows.Forms.Design
{
    ...
    internal class CommandSet : IDisposable
    {
        ...
        protected PropertyDescriptor GetProperty(object comp, string propName) => 
            TypeDescriptor.GetProperties(comp)[propName];
        ...
        protected void OnMenuDesignerProperties(object sender, EventArgs e)
        {
            ...
            if (!this.CheckComponentEditor(primarySelection, true))
            {
                ...
                if ((service != null) && service.GlobalInvoke(StandardCommands.PropertiesWindow))
                {
                }
            }
        }
                ...
                if (host != null)
                {
                    ...
                    using (DesignerTransaction transaction = host.CreateTransaction(System.Design.SR.GetString("CommandSetPaste")))
                    {
                        ...
                        if ((is3 != null) && (is3.Count > 0))
                        {
                            ...
                            host.GetDesigner(rootComponent);
                            flag2 = false;
                                ...
                                if (flag2)
                                {
                                    ...
                                    if (!flag3)
                                    {
                                        ...
                                        if (designer3 != null)
                                        {
                                            ...
                                            else if (list4.Contains(component) && (Array.IndexOf(array, component5) == -1))
                                            {
                                                list2.Add(component);
                                            }
        ...
        private void UpdatePastePositions(ArrayList controls)
        {
            if (controls.Count != 0)
            {
                ...
                if (parent != null)
                {
                    ...
                    Size empty = Size.Empty;
                    Point point5 = new Point(clientSize.Width / 2, clientSize.Height / 2);
                    point5.X -= (point2.X - location.X) / 2;
                    point5.Y -= (point2.Y - location.Y) / 2;
                    do
                    {
                        flag = false;
