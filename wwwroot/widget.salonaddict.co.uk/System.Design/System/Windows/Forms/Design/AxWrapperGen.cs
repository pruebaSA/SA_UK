    ...
    public class AxWrapperGen
    {
        ...
        public AxWrapperGen(System.Type axType)
        {
            ...
            if (customAttributes.Length > 0)
            {
                ...
                int length = attribute.Value.IndexOfAny(new char[1]);
                string name = attribute.Value.Substring(0, length);
            ...
            {
                if (type.GetCustomAttributes(typeof(CoClassAttribute), false).Length > 0)
                {
            ...
            try
            {
                ...
                if (customAttributes.Length > 0)
                {
                    InterfaceTypeAttribute attribute2 = (InterfaceTypeAttribute) customAttributes[0];
                    this.dispInterface = attribute2.Value == ComInterfaceType.InterfaceIsIDispatch;
                }
            }
        ...
        private bool ClassAlreadyExistsInNamespace(CodeNamespace ns, string clsName) => 
            classesInNamespace.Contains(clsName);
        ...
        private bool IsDispidKnown(int dp, string propName) => 
            (((((dp == -513) || (dp == -501)) || ((dp == -512) || (dp == -514))) || (((dp == -516) || (dp == -611)) || ((dp == -517) || (dp == -515)))) || ((dp == 0) && propName.Equals(this.defMember)));
        ...
        private bool IsPropertyBindable(PropertyInfo pinfo, out bool isDefaultBind)
        {
            ...
            if (getMethod != null)
            {
                ...
                if ((customAttributes != null) && (customAttributes.Length > 0))
                {
                    TypeLibFuncFlags flags = ((TypeLibFuncAttribute) customAttributes[0]).Value;
                    isDefaultBind = (flags & TypeLibFuncFlags.FDefaultBind) != 0;
                    if (isDefaultBind || ((flags & TypeLibFuncFlags.FBindable) != 0))
                    {
                        return true;
                    }
                }
        ...
        private bool IsPropertyBrowsable(PropertyInfo pinfo, ComAliasEnum alias)
        {
            ...
            {
                TypeLibFuncFlags flags = ((TypeLibFuncAttribute) customAttributes[0]).Value;
                if (((flags & TypeLibFuncFlags.FNonBrowsable) != 0) || ((flags & TypeLibFuncFlags.FHidden) != 0))
                {
                    return false;
                }
            }
        ...
        private string ResolveConflict(string name, System.Type returnType, out bool fOverride, out bool fUseNew)
        {
            ...
            try
            {
                if (axHostPropDescs == null)
                {
                    ...
                    {
                        axHostPropDescs.Add(info.Name + info.PropertyType.GetHashCode(), info);
                    }
                }
        ...
        private void WriteEventMembersDecl(CodeNamespace ns, CodeTypeDeclaration cls)
        {
            ...
            for (int i = 0; i < methods.Length; i++)
            {
                ...
                if (!flag)
                {
                    ...
                    if ((customAttributes != null) && (customAttributes.Length != 0))
                    {
                        DispIdAttribute attribute = (DispIdAttribute) customAttributes[0];
        ...
        private void WriteMethod(CodeTypeDeclaration cls, MethodInfo method, bool hasPropInfo, bool removeOptionals)
        {
            ...
            {
                DispIdAttribute attribute = (DispIdAttribute) customAttributes[0];
                if ((attribute.Value == -552) && (method.GetParameters().Length == 0))
                {
        ...
        private void WriteProperty(CodeTypeDeclaration cls, PropertyInfo pinfo, bool useLet)
        {
            ...
            DispIdAttribute attribute = null;
            if (nopersist == null)
            {
            ...
            if ((customAttributes != null) && (customAttributes.Length > 0))
            {
                attribute = (DispIdAttribute) customAttributes[0];
                declaration = new CodeAttributeDeclaration(typeof(DispIdAttribute).FullName, new CodeAttributeArgument[] { new CodeAttributeArgument(new CodePrimitiveExpression(attribute.Value)) });
            }
            bool fOverride = false;
            bool fUseNew = false;
            ...
            if (fOverride || fUseNew)
            {
                ...
                if (!this.IsDispidKnown(attribute.Value, pinfo.Name))
                {
                    str = "Ctl";
                    fOverride = false;
                    fUseNew = false;
        ...
        private class AxMethodGenerator
        {
            ...
            public CodeExpression DoMethodInvoke(CodeMemberMethod method, string methodName, CodeExpression targetObject, List<CodeExpression> parameters) => 
                this.DoMethodInvokeCore(method, methodName, this._method.ReturnType, targetObject, parameters);
                ...
                foreach (AxParameterData data in this.Parameters)
                {
                    ...
                    else
                    {
                        ...
                        CodeVariableReferenceExpression item = new CodeVariableReferenceExpression(data.Name) {
                            UserData = { [typeof(AxParameterData)] = data }
                        };
            ...
            private static CodeExpression GetDefaultExpressionForInvoke(MethodInfo method, AxParameterData parameterInfo)
            {
                ...
                if (defaultValue == Missing.Value)
                {
                    if (!parameterBaseType.IsPrimitive)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                defaultValue = Enum.GetValues(parameterBaseType).GetValue(0);
                            }
                        }
                    }
                    else
                    {
                        defaultValue = GetPrimitiveDefaultValue(parameterBaseType);
                    }
                }
                else if (parameterBaseType.IsPrimitive)
                {
                    defaultValue = GetClsPrimitiveValue(defaultValue);
                    defaultValue = GetDefaultValueForUnsignedType(parameterBaseType, defaultValue);
                }
                else if (((defaultValue != null) && parameterBaseType.IsInstanceOfType(defaultValue)) && (((defaultValue is DateTime) || (defaultValue is decimal)) || (defaultValue is bool)))
                {
                    if (defaultValue is DateTime)
                    {
                        CodeExpression[] parameters = new CodeExpression[1];
                        DateTime time = (DateTime) defaultValue;
                    ...
                    {
                        return new CodeObjectCreateExpression(typeof(decimal), new CodeExpression[] { new CodeCastExpression(typeof(double), new CodePrimitiveExpression(decimal.ToDouble((decimal) defaultValue))) });
                    }
                ...
                CodeExpression expression = new CodePrimitiveExpression(defaultValue);
        ...
        private static class ComAliasConverter
        {
            ...
            public static AxWrapperGen.ComAliasEnum GetComAliasEnum(MemberInfo memberInfo, System.Type type, ICustomAttributeProvider attrProvider)
            {
                ...
                {
                    customAttributes = attrProvider.GetCustomAttributes(typeof(ComAliasNameAttribute), false);
                }
                if ((customAttributes != null) && (customAttributes.Length > 0))
                {
                    ComAliasNameAttribute attribute = (ComAliasNameAttribute) customAttributes[0];
                    str = attribute.Value;
                }
                ...
                if (attrProvider != null)
                {
                    customAttributes = attrProvider.GetCustomAttributes(typeof(DispIdAttribute), false);
                    if ((customAttributes != null) && (customAttributes.Length > 0))
                    {
                        DispIdAttribute attribute2 = (DispIdAttribute) customAttributes[0];
                        num = attribute2.Value;
                    }
                }
