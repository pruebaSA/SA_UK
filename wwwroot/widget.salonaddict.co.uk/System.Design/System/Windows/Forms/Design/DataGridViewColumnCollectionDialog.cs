    ...
    internal class DataGridViewColumnCollectionDialog : Form
    {
        ...
        private void CommitChanges()
        {
            if (this.formIsDirty)
            {
                try
                {
                    ...
                    IContainer container = this.liveDataGridView.Site?.Container;
        ...
        internal class ListBoxItem : ICustomTypeDescriptor, IComponent, IDisposable
        {
            ...
            AttributeCollection ICustomTypeDescriptor.GetAttributes() => 
                TypeDescriptor.GetAttributes(this.column);

            string ICustomTypeDescriptor.GetClassName() => 
                TypeDescriptor.GetClassName(this.column);

            string ICustomTypeDescriptor.GetComponentName() => 
                TypeDescriptor.GetComponentName(this.column);

            TypeConverter ICustomTypeDescriptor.GetConverter() => 
                TypeDescriptor.GetConverter(this.column);

            EventDescriptor ICustomTypeDescriptor.GetDefaultEvent() => 
                TypeDescriptor.GetDefaultEvent(this.column);

            PropertyDescriptor ICustomTypeDescriptor.GetDefaultProperty() => 
                TypeDescriptor.GetDefaultProperty(this.column);

            object ICustomTypeDescriptor.GetEditor(System.Type type) => 
                TypeDescriptor.GetEditor(this.column, type);

            EventDescriptorCollection ICustomTypeDescriptor.GetEvents() => 
                TypeDescriptor.GetEvents(this.column);

            EventDescriptorCollection ICustomTypeDescriptor.GetEvents(Attribute[] attrs) => 
                TypeDescriptor.GetEvents(this.column, attrs);
            ...
            {
                get => 
                    this.owner.liveDataGridView.Site;
                set
                {
                }
            }
