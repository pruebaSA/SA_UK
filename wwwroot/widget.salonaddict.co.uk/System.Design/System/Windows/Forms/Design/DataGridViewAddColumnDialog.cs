        ...
        private void PopulateDataColumns()
        {
            ...
            if (this.liveDataGridView.DataSource != null)
            {
                ...
                if (itemProperties != null)
                {
                    ...
                    {
                        if (!typeof(IList).IsAssignableFrom(itemProperties[i].PropertyType) || TypeDescriptor.GetConverter(typeof(Image)).CanConvertFrom(itemProperties[i].PropertyType))
                        {
                            this.dataColumns.Items.Add(new ListBoxItem(itemProperties[i].PropertyType, itemProperties[i].Name));
                        }
                    }
                }
            }
