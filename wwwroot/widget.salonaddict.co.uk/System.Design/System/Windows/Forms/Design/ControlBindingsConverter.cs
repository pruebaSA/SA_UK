namespace System.Windows.Forms.Design
{
    ...
    internal class ControlBindingsConverter : TypeConverter
    {
        ...
        public override PropertyDescriptorCollection GetProperties(ITypeDescriptorContext context, object value, Attribute[] attributes)
        {
            ...
            ControlBindingsCollection bindingss = (ControlBindingsCollection) value;
            IBindableComponent bindableComponent = bindingss.BindableComponent;
            bindableComponent.GetType();
            PropertyDescriptorCollection properties = TypeDescriptor.GetProperties(bindableComponent, (Attribute[]) null);
            ArrayList list = new ArrayList();
            for (int i = 0; i < properties.Count; i++)
            {
                Binding binding = bindingss[properties[i].Name];
                ...
                if (((BindableAttribute) properties[i].Attributes[typeof(BindableAttribute)]).Bindable || !((DesignBinding) descriptor.GetValue(bindingss)).IsNull)
                {
                    list.Add(descriptor);
                }
            }
