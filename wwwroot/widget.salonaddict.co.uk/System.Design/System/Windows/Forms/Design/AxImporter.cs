namespace System.Windows.Forms.Design
{
    ...
    public class AxImporter
    {
                ...
                if (!string.Equals(name.Name, "mscorlib", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (comReference == null)
                    {
                        ...
                        catch (FileNotFoundException)
                        {
                            ...
                            assembly = Assembly.Load(AssemblyName.GetAssemblyName(Path.Combine(Path.GetDirectoryName(assemPath), name.Name + ".dll")));
                        }
        ...
        private string GetAxReference(System.Runtime.InteropServices.ComTypes.ITypeLib typeLib) => 
            this.options.references?.ResolveActiveXReference((UCOMITypeLib) typeLib);
        ...
        private string GetAxTypeFromAssembly(string fileName, Guid clsid)
        {
            foreach (System.Type type in this.GetCopiedAssembly(fileName, true, false).GetTypes())
            {
                ...
                {
                    AxHost.ClsidAttribute attribute = (AxHost.ClsidAttribute) type.GetCustomAttributes(typeof(AxHost.ClsidAttribute), false)[0];
                    if (attribute.Value == ("{" + clsid.ToString() + "}"))
                    {
        ...
        private string GetComReference(AssemblyName name) => 
            this.options.references?.ResolveComReference(name);

        private string GetComReference(System.Runtime.InteropServices.ComTypes.ITypeLib typeLib) => 
            this.options.references?.ResolveComReference((UCOMITypeLib) typeLib);
        ...
        private string GetManagedReference(string assemName) => 
            this.options.references?.ResolveManagedReference(assemName);
            ...
            if (assem == null)
            {
                ...
                try
                {
                    ...
                    else if (asmCodeBase != null)
                    {
                        ...
                        assem = Assembly.LoadFrom(asmCodeBase);
                    }
                    if (assem != null)
                    {
