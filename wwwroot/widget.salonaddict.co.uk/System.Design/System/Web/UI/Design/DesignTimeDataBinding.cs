namespace System.Web.UI.Design
{
    ...
    internal sealed class DesignTimeDataBinding
    {
        ...
        public static string CreateBindExpression(string field, string format)
        {
            ...
            {
                return string.Format(CultureInfo.InvariantCulture, "Bind(\"{0}\", \"{1}\")", new object[] { str, format });
            }
            return string.Format(CultureInfo.InvariantCulture, "Bind(\"{0}\")", new object[] { str });
        }
        ...
        public static string CreateEvalExpression(string field, string format)
        {
            ...
            if ((format != null) && (format.Length != 0))
            {
                return string.Format(CultureInfo.InvariantCulture, "Eval(\"{0}\", \"{1}\")", new object[] { str, format });
            }
            return string.Format(CultureInfo.InvariantCulture, "Eval(\"{0}\")", new object[] { str });
        }
