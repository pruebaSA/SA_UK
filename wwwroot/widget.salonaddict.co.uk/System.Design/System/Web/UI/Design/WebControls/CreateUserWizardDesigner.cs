namespace System.Web.UI.Design.WebControls
{
    ...
    public class CreateUserWizardDesigner : WizardDesigner
    {
        ...
        private string ConvertTableToHtmlTable(Table originalTable, System.Web.UI.Control container) => 
            this.ConvertTableToHtmlTable(originalTable, container, null);
        ...
        private bool CustomizeCompleteStepCallback(object context)
        {
            ...
            if (contentTemplate == null)
            {
                try
                {
                    ...
                    Table originalTable = (Table) ((Table) step.Controls[0].Controls[0]).Rows[0].Cells[0].Controls[0];
                    if (viewControl.ControlStyleCreated)
                    {
                        Style controlStyle = viewControl.ControlStyle;
                        originalTable.ForeColor = controlStyle.ForeColor;
                        originalTable.BackColor = controlStyle.BackColor;
                        originalTable.Font.CopyFrom(controlStyle.Font);
                        originalTable.Font.Size = new FontUnit(Unit.Percentage(100.0));
                    }
                    Style stepStyle = viewControl.StepStyle;
                    if (!stepStyle.IsEmpty)
                    {
                        originalTable.ForeColor = stepStyle.ForeColor;
                        originalTable.BackColor = stepStyle.BackColor;
                        originalTable.Font.CopyFrom(stepStyle.Font);
                        originalTable.Font.Size = new FontUnit(Unit.Percentage(100.0));
                    }
                    builder.Append(this.ConvertTableToHtmlTable(originalTable, step.ContentTemplateContainer, _completeStepConverter));
        ...
        private bool CustomizeCreateUserStepCallback(object context)
        {
            try
            {
                ...
                if (contentTemplate == null)
                {
                    ...
                    accessor.GetDesignModeState();
                    StringBuilder builder = new StringBuilder();
                    TemplatedWizardStep createUserStep = viewControl.CreateUserStep;
                    Table originalTable = (Table) ((Table) createUserStep.Controls[0].Controls[0]).Rows[0].Cells[0].Controls[0];
                    if (viewControl.ControlStyleCreated)
                    {
                        Style controlStyle = viewControl.ControlStyle;
                        originalTable.ForeColor = controlStyle.ForeColor;
                    ...
                    builder.Append(this.ConvertTableToHtmlTable(originalTable, createUserStep.ContentTemplateContainer));
                    contentTemplate = ControlParser.ParseTemplate(service, builder.ToString());
                    Hashtable hashtable2 = new Hashtable(1) {
                        { 
                            "ConvertToTemplate",
                    ...
                    ((IControlDesignerAccessor) viewControl).SetDesignModeState(hashtable2);
                }
                TypeDescriptor.GetProperties(this._createUserWizard.CreateUserStep)["ContentTemplate"].SetValue(this._createUserWizard.CreateUserStep, contentTemplate);
                this.UpdateDesignTimeHtml();
                return true;
            }
            catch (Exception)
        ...
        protected override string GetErrorDesignTimeHtml(Exception e) => 
            base.CreatePlaceHolderDesignTimeHtml(System.Design.SR.GetString("Control_ErrorRenderingShort") + "<br />" + e.Message);
        ...
        internal override bool InRegionEditingMode(Wizard viewControl) => 
            (!base.SupportsDesignerRegions || IsStepEmpty(this._createUserWizard.ActiveStep));
