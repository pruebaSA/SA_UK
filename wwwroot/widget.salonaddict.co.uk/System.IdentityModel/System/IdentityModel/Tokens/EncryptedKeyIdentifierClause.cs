namespace System.IdentityModel.Tokens
{
    ...
    public sealed class EncryptedKeyIdentifierClause : BinaryKeyIdentifierClause
    {
        ...
        public bool Matches(byte[] encryptedKey, string encryptionMethod, string carriedKeyName) => 
            ((base.Matches(encryptedKey) && (this.encryptionMethod == encryptionMethod)) && (this.carriedKeyName == carriedKeyName));

        public override string ToString() => 
            string.Format(CultureInfo.InvariantCulture, "EncryptedKeyIdentifierClause(EncryptedKey = {0}, Method '{1}')", new object[] { Convert.ToBase64String(base.GetRawBuffer()), this.EncryptionMethod });

        public string CarriedKeyName =>
            this.carriedKeyName;

        public SecurityKeyIdentifier EncryptingKeyIdentifier =>
            this.encryptingKeyIdentifier;
