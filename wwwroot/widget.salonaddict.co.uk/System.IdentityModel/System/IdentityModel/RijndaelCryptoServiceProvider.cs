    ...
    internal class RijndaelCryptoServiceProvider : Rijndael
    {
        ...
        private class RijndaelCryptoTransform : ICryptoTransform, IDisposable
        {
            ...
            public unsafe RijndaelCryptoTransform(byte[] rgbKey, byte[] rgbIV, PaddingMode paddingMode, int blockSizeBits, bool encrypt)
            {
                ...
                try
                {
                    ThrowIfFalse("AESCryptAcquireContextFailed", NativeMethods.CryptAcquireContextW(out phProv, null, null, 0x18, 0xf0000000));
                    ...
                    fixed (void* voidRef = dst)
                    {
                        PLAINTEXTKEYBLOBHEADER* plaintextkeyblobheaderPtr = (PLAINTEXTKEYBLOBHEADER*) voidRef;
                        ...
                        phKey = System.IdentityModel.SafeKeyHandle.SafeCryptImportKey(phProv, voidRef, cbData);
                    }
                    fixed (void* voidRef2 = rgbIV)
                    {
                        ThrowIfFalse("AESCryptSetKeyParamFailed", NativeMethods.CryptSetKeyParam(phKey, 1, voidRef2, 0));
                    }
            ...
            private unsafe int DecryptData(byte[] inputBuffer, int inputOffset, int inputCount, byte[] outputBuffer, int outputOffset, bool final)
            {
                ...
                if (dwDataLen > 0)
                {
                    ...
                    fixed (void* voidRef = ((void*) &(outputBuffer[outputOffset])))
                    {
                        ThrowIfFalse("AESCryptDecryptFailed", NativeMethods.CryptDecrypt(this.keyHandle, IntPtr.Zero, flag, 0, voidRef, ref dwDataLen));
                    }
                }
                if (!flag && final)
                {
            ...
            private unsafe int EncryptData(byte[] inputBuffer, int inputOffset, int inputCount, byte[] outputBuffer, int outputOffset, bool final)
            {
                ...
                try
                {
                    ...
                    fixed (void* voidRef = ((void*) &(dst[dstOffset])))
                    {
                        ThrowIfFalse("AESCryptEncryptFailed", NativeMethods.CryptEncrypt(this.keyHandle, IntPtr.Zero, final, 0, voidRef, ref dwCount, dst.Length - dstOffset));
                    }
                    flag2 = false;
                }
                finally
