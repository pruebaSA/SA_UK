namespace System.IdentityModel
{
    ...
    internal static class SecurityUtils
    {
        ...
        internal static byte[] CloneBuffer(byte[] buffer) => 
            CloneBuffer(buffer, 0, buffer.Length);
        ...
        internal static ReadOnlyCollection<IAuthorizationPolicy> CreateAuthorizationPolicies(ClaimSet claimSet) => 
            CreateAuthorizationPolicies(claimSet, MaxUtcDateTime);
        ...
        internal static AuthorizationContext CreateDefaultAuthorizationContext(IList<IAuthorizationPolicy> authorizationPolicies)
        {
                    ...
                    for (int i = 0; i < authorizationPolicies.Count; i++)
                    {
                        if (objArray[i] != obj2)
                        {
                            ...
                            else if (policy.Evaluate(evaluationContext, ref objArray[i]))
                            {
                                ...
                                {
                                    DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Verbose, TraceCode.AuthorizationPolicyEvaluated, System.IdentityModel.SR.GetString("AuthorizationPolicyEvaluated", new object[] { policy.Id }));
                                }
                            }
                        }
                    }
            ...
            {
                DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Information, TraceCode.AuthorizationContextCreated, System.IdentityModel.SR.GetString("AuthorizationContextCreated", new object[] { context.Id }));
            }
            return context;
        }
        ...
        private class SimpleAuthorizationContext : AuthorizationContext
        {
            ...
            public override ReadOnlyCollection<ClaimSet> ClaimSets =>
                this.policy.Issuances;

            public override DateTime ExpirationTime =>
                this.policy.ExpirationTime;
