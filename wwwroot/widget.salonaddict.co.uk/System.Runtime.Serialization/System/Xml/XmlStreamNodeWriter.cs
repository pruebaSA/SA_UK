namespace System.Xml
{
    ...
    internal abstract class XmlStreamNodeWriter : XmlNodeWriter
    {
        ...
        [SecurityCritical]
        private unsafe int GetByteCount(char* chars, int count) => 
            this.encoding?.GetByteCount(chars, count);

        [SecurityCritical]
        private unsafe int GetBytes(char* chars, int charCount, byte* bytes, int byteCount) => 
            this.encoding?.GetBytes(chars, charCount, bytes, byteCount);
        ...
        protected unsafe int UnsafeGetUTF8Chars(char* chars, int charCount, byte[] buffer, int offset)
        {
            ...
            fixed (byte* numRef = &(buffer[offset]))
            {
                ...
                do
                {
                    ...
                    bytes += this.GetBytes(chPtr2, (int) ((long) ((chars - chPtr2) / 2)), bytes, (int) ((long) ((numPtr2 - bytes) / 1)));
                }
                while (chars < chPtr);
        ...
        protected unsafe int UnsafeGetUTF8Length(char* chars, int charCount)
        {
            ...
            return (((int) ((long) ((chars - (chPtr - charCount)) / 2))) + this.GetByteCount(chars, (int) ((long) ((chPtr - chars) / 2))));
        }
        ...
        public void WriteBytes(byte[] byteBuffer, int byteOffset, int byteCount)
        {
            if (byteCount < 0x200)
            {
                ...
                System.Buffer.BlockCopy(byteBuffer, byteOffset, dst, num, byteCount);
        ...
        protected unsafe void WriteUTF8Char(int ch)
        {
            ...
            {
                char* chars = (char*) stackalloc byte[2];
                chars[0] = (char) ch;
            ...
            else
            {
                ...
                char* chPtr2 = (char*) stackalloc byte[(2 * 2)];
        ...
        protected void WriteUTF8Chars(byte[] chars, int charOffset, int charCount)
        {
            if (charCount < 0x200)
            {
                ...
                System.Buffer.BlockCopy(chars, charOffset, dst, num, charCount);
        ...
        public int Position =>
            (((int) this.stream.Position) + this.offset);
