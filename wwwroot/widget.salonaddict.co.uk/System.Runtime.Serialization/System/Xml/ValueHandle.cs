namespace System.Xml
{
    ...
    internal class ValueHandle
    {
        ...
        private decimal GetDecimal() => 
            this.bufferReader.GetDecimal(this.offset);

        private XmlDictionaryString GetDictionaryString() => 
            this.bufferReader.GetDictionaryString(this.offset);

        private double GetDouble() => 
            this.bufferReader.GetDouble(this.offset);

        private string GetEscapedCharsText() => 
            this.bufferReader.GetEscapedString(this.offset, this.length);

        private Guid GetGuid() => 
            this.bufferReader.GetGuid(this.offset);

        private int GetInt16() => 
            this.bufferReader.GetInt16(this.offset);

        private int GetInt32() => 
            this.bufferReader.GetInt32(this.offset);

        private long GetInt64() => 
            this.bufferReader.GetInt64(this.offset);

        private int GetInt8() => 
            this.bufferReader.GetInt8(this.offset);

        private string GetQNameDictionaryText() => 
            (PrefixHandle.GetString(PrefixHandle.GetAlphaPrefix(this.length)) + ":" + this.bufferReader.GetDictionaryString(this.offset));

        private float GetSingle() => 
            this.bufferReader.GetSingle(this.offset);
        ...
        private ulong GetUInt64() => 
            this.bufferReader.GetUInt64(this.offset);

        private string GetUnicodeCharsText() => 
            this.bufferReader.GetUnicodeString(this.offset, this.length);

        private UniqueId GetUniqueId() => 
            this.bufferReader.GetUniqueId(this.offset);
            ...
            if ((this.type == ValueHandleType.UTF8) && ((this.length % 4) == 0))
            {
                try
                {
                    ...
                    if (count != bytes.Length)
                    {
                        ...
                        System.Buffer.BlockCopy(bytes, 0, dst, 0, count);
                        bytes = dst;
                    }
                    return bytes;
                }
        ...
        public object[] ToList() => 
            this.bufferReader.GetList(this.offset, this.length);
