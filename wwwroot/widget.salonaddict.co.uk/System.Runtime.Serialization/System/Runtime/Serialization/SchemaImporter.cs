namespace System.Runtime.Serialization
{
    ...
    internal class SchemaImporter
    {
        ...
        private void CheckISerializableBase(XmlQualifiedName typeName, XmlSchemaSequence rootSequence, XmlSchemaObjectCollection attributes)
        {
            ...
            if (attributes != null)
            {
                for (int i = 0; i < attributes.Count; i++)
                {
                    ...
                    {
                        flag = true;
                        this.CheckSchemaImport(typeName, refName);
                        break;
                    }
                }
            }
        ...
        private void CheckSchemaImport(XmlQualifiedName typeName, XmlQualifiedName referencedConstruct)
        {
            string str = referencedConstruct.Namespace;
            if ((!SchemaHelper.NamespacesEqual(str, typeName.Namespace) && !SchemaHelper.NamespacesEqual(str, "http://www.w3.org/2001/XMLSchema")) && !SchemaHelper.NamespacesEqual(str, "http://www.w3.org/2001/XMLSchema-instance"))
            {
                foreach (object obj2 in SchemaHelper.GetSchemaWithType(this.SchemaObjects, this.schemaSet, typeName).Includes)
                {
                    if ((obj2 is XmlSchemaImport) && SchemaHelper.NamespacesEqual(str, ((XmlSchemaImport) obj2).Namespace))
                    {
                        return;
                    }
                }
                ThrowTypeCannotBeImportedException(typeName.Name, typeName.Namespace, System.Runtime.Serialization.SR.GetString("SchemaDoesNotContainImportStatement", new object[] { referencedConstruct.Name, str }));
            }
        }
        ...
        private void ImportBaseContract(XmlQualifiedName baseTypeName, ClassDataContract dataContract)
        {
            ...
            this.CheckSchemaImport(dataContract.StableName, baseTypeName);
        ...
        private void ImportClassMember(XmlSchemaElement element, ClassDataContract dataContract)
        {
            ...
            else
            {
                ...
                this.CheckSchemaImport(stableName, contract.StableName);
            }
        ...
        private CollectionDataContract ImportCollection(XmlQualifiedName typeName, XmlSchemaSequence rootSequence, XmlSchemaObjectCollection attributes, XmlSchemaAnnotation annotation, bool isReference)
        {
            ...
            if (element.SchemaTypeName.IsEmpty)
            {
                if (element.SchemaType != null)
                {
                    dataContract.ItemContract = this.ImportAnonymousElement(element, new XmlQualifiedName(element.Name, typeName.Namespace));
                }
                else if (!element.RefName.IsEmpty)
                {
            ...
            else
            {
                ...
                this.CheckSchemaImport(typeName, dataContract.ItemContract.StableName);
            }
        ...
        private GenericInfo ImportGenericInfo(XmlElement typeElement, XmlSchemaType type)
        {
            ...
            string localName = namedItem?.Value;
            if (localName == null)
            {
        ...
        private DataContract ImportSimpleTypeRestriction(XmlQualifiedName typeName, XmlSchemaSimpleTypeRestriction restriction)
        {
            ...
            {
                return this.ImportType(restriction.BaseTypeName);
            }
            if (restriction.BaseType != null)
            {
                return this.ImportType(restriction.BaseType);
            }
        ...
        private XmlDataContract ImportSpecialXmlDataType(XmlSchemaType xsdType, bool isAnonymous)
        {
            if (isAnonymous)
            {
                ...
                if (this.IsXmlAnyElementType(type))
                {
                    ...
                    Type type2;
                    XmlSchemaAny any = ((XmlSchemaSequence) type.Particle).Items[0] as XmlSchemaAny;
                    if (any.MinOccurs == 1M)
                    {
                        name = new XmlQualifiedName("XDocument", "http://schemas.datacontract.org/2004/07/System.Xml.Linq");
                    }
                    else
                    {
                        name = new XmlQualifiedName("XElement", "http://schemas.datacontract.org/2004/07/System.Xml.Linq");
                    }
                    if (this.dataContractSet.TryGetReferencedType(name, null, out type2) && Globals.TypeOfIXmlSerializable.IsAssignableFrom(type2))
                    {
        ...
        private DataContract ImportType(XmlSchemaType type) => 
            this.ImportType(type, type.QualifiedName, false);
        ...
        private DataContract ImportXmlDataType(XmlQualifiedName typeName, XmlSchemaType xsdType, bool isAnonymous)
        {
            ...
            if (dataContract == null)
            {
                ...
                else
                {
                    ...
                    {
                        DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Verbose, TraceCode.XsdImportAnnotationFailed, System.Runtime.Serialization.SR.GetString("TraceCodeXsdImportAnnotationFailed"), new StringTraceRecord("Type", typeName.Namespace + ":" + typeName.Name));
                    }
                }
                if (!isAnonymous)
                {
