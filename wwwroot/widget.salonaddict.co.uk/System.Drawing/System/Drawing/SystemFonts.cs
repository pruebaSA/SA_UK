namespace System.Drawing
{
    ...
    public sealed class SystemFonts
    {
        ...
        private static bool IsCriticalFontException(Exception ex) => 
            ((((!(ex is ExternalException) && !(ex is ArgumentException)) && (!(ex is OutOfMemoryException) && !(ex is InvalidOperationException))) && !(ex is NotImplementedException)) && !(ex is FileNotFoundException));
        ...
        public static Font DialogFont
        {
            get
            {
                ...
                else
                {
                    try
                    {
                        defaultFont = new Font("Tahoma", 8f);
                    }
                    catch (ArgumentException)
                    {
                    }
