namespace System.Drawing
{
    ...
    public sealed class BufferedGraphicsContext : IDisposable
    {
        ...
        {
            bool flag = false;
            byte[] lppe = new byte[sizeof(NativeMethods.PALETTEENTRY) * 0x100];
            fixed (byte* numRef = pbmi.bmiColors)
            {
                fixed (byte* numRef2 = lppe)
                {
                    NativeMethods.RGBQUAD* rgbquadPtr = (NativeMethods.RGBQUAD*) numRef;
                    NativeMethods.PALETTEENTRY* paletteentryPtr = (NativeMethods.PALETTEENTRY*) numRef2;
        ...
        private bool ShouldUseTempManager(Rectangle targetBounds) => 
            ((targetBounds.Width * targetBounds.Height) > (this.MaximumBuffer.Width * this.MaximumBuffer.Height));
