namespace System.Configuration.Install
{
    ...
    public class ManagedInstallerClass : IManagedInstaller
    {
        private static string GetHelp(Installer installerWithHelp) => 
            (Res.GetString("InstallHelpMessageStart") + Environment.NewLine + installerWithHelp.HelpText + Environment.NewLine + Res.GetString("InstallHelpMessageEnd") + Environment.NewLine);
        ...
        int IManagedInstaller.ManagedInstall(string argString, int hInstall)
        {
            ...
            catch (Exception innerException)
            {
                ...
                if (((hRecord != 0) && (NativeMethods.MsiRecordSetInteger(hRecord, 1, 0x3e9) == 0)) && (NativeMethods.MsiRecordSetStringW(hRecord, 2, builder.ToString()) == 0))
                {
                    NativeMethods.MsiProcessMessage(hInstall, 0x1000000, hRecord);
                }
                return -1;
            }
            return 0;
