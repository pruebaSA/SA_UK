namespace System.DirectoryServices.ActiveDirectory
{
    ...
    internal sealed class Locator
    {
        ...
        private static Hashtable DnsGetDcWrapper(string domainName, string siteName, long dcFlags)
        {
            ...
            int errorCode = 0;
            errorCode = NativeMethods.DsGetDcOpen(domainName, optionFlags, siteName, IntPtr.Zero, null, (int) dcFlags, out zero);
            if (errorCode == 0)
            {
                try
                {
                    errorCode = NativeMethods.DsGetDcNext(zero, ref sockAddressCount, out sockAdresses, out dnsHostName);
                    switch (errorCode)
                    {
                        case 0:
                        case 0x44d:
                    ...
                    switch (errorCode)
                    {
                        ...
                        default:
                            ...
                            finally
                            {
                                ...
                                {
                                    errorCode = NativeMethods.NetApiBufferFree(dnsHostName);
                                }
                            }
                            break;
                    }
        ...
        private static Hashtable DnsQueryWrapper(string domainName, string siteName, long dcFlags)
        {
            ...
            errorCode = NativeMethods.DnsQuery(recordName, 0x21, options, IntPtr.Zero, out zero, IntPtr.Zero);
            if (errorCode == 0)
            {
                try
                {
        ...
        internal static int DsGetDcNameWrapper(string computerName, string domainName, string siteName, long flags, out DomainControllerInfo domainControllerInfo)
        {
            ...
            num = NativeMethods.DsGetDcName(computerName, domainName, IntPtr.Zero, siteName, (int) (flags | 0x40000000L), out zero);
            if (num == 0)
            {
                try
                {
