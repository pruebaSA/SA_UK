namespace System.DirectoryServices.ActiveDirectory
{
    ...
    public class DirectoryContext
    {
        ...
        internal static string GetLoggedOnDomain()
        {
            ...
            int status = NativeMethods.LsaConnectUntrusted(out handle);
            switch (status)
            {
                case 0:
                    int num2;
                    ...
                    status = NativeMethods.LsaCallAuthenticationPackage(handle, 0, structure, submitBufferLength, out zero, out num2, out num3);
                    try
                    {
                        if ((status != 0) || (num3 != 0))
                        {
