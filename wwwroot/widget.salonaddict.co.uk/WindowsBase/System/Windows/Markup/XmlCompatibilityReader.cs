namespace System.Windows.Markup
{
    ...
    [FriendAccessAllowed]
    internal sealed class XmlCompatibilityReader : System.Windows.Markup.XmlWrappingReader
    {
        ...
        private void HandleAlternateContent(int elementDepth, ref bool more)
        {
            ...
            {
                this.Error(System.Windows.SR.Get("XCRInvalidACChild", new object[] { base.Reader.Name }), new object[0]);
            }
            if (base.Reader.IsEmptyElement)
            {
                this.Error(System.Windows.SR.Get("XCRChoiceNotFound"), new object[0]);
            }
        ...
        private void HandleChoice(int elementDepth, ref bool more)
        {
            if (!this.Scope.InAlternateContent)
            {
                this.Error(System.Windows.SR.Get("XCRChoiceOnlyInAC"), new object[0]);
            }
            if (this.Scope.FallbackSeen)
            {
                this.Error(System.Windows.SR.Get("XCRChoiceAfterFallback"), new object[0]);
            }
            string attribute = base.Reader.GetAttribute(this.Requires);
            if (attribute == null)
            {
                this.Error(System.Windows.SR.Get("XCRRequiresAttribNotFound"), new object[0]);
            }
            if (string.IsNullOrEmpty(attribute))
            {
                this.Error(System.Windows.SR.Get("XCRInvalidRequiresAttribute"), new object[0]);
            }
            ...
            if (this.AttributeCount != 1)
            {
                ...
                this.MoveToElement();
                this.Error(System.Windows.SR.Get("XCRInvalidAttribInElement"), new object[] { localName, this.Choice });
            }
        ...
        private void HandleFallback(int elementDepth, ref bool more)
        {
            if (!this.Scope.InAlternateContent)
            {
                this.Error(System.Windows.SR.Get("XCRFallbackOnlyInAC"), new object[0]);
            }
            ...
            if (this.Scope.FallbackSeen)
            {
                this.Error(System.Windows.SR.Get("XCRMultipleFallbackFound"), new object[0]);
            }
            ...
            if (this.AttributeCount != 0)
            {
                ...
                string localName = base.Reader.LocalName;
                this.MoveToElement();
                this.Error(System.Windows.SR.Get("XCRInvalidAttribInElement"), new object[] { localName, this.Fallback });
            }
            if (choiceTaken)
            {
                this.ScanForEndCompatibility(elementDepth);
        ...
        private void HandleMustUnderstand(int elementDepth)
        {
            foreach (string str in this.PrefixesToNamespaces(base.Reader.Value))
            {
                ...
                {
                    this.Error(System.Windows.SR.Get("XCRMustUnderstandFailed"), new object[] { str });
                }
            }
        }
        ...
        private bool IsNamespaceKnown(string namespaceName) => 
            this._knownNamespaces.ContainsKey(namespaceName);

        private bool IsSubsumingNamespace(string namespaceName) => 
            this._subsumingNamespaces.ContainsKey(namespaceName);
        ...
        private IEnumerable<NamespaceElementPair> ParseContentToNamespaceElementPair(string content, string callerContext)
        {
            foreach (string iteratorVariable0 in content.Trim().Split(new char[] { ' ' }))
            {
                if (!string.IsNullOrEmpty(iteratorVariable0))
                {
                    ...
                    {
                        this.Error(System.Windows.SR.Get("XCRInvalidFormat"), new object[] { callerContext });
                    }
                    ...
                    if (namespaceName == null)
                    {
                        this.Error(System.Windows.SR.Get("XCRUndefinedPrefix"), new object[] { prefix });
                    }
                    else if ((str != "*") && !XmlReader.IsName(str))
                    {
                        this.Error(System.Windows.SR.Get("XCRInvalidXMLName"), new object[] { iteratorVariable0 });
                    }
                    else
                    {
        ...
        private IEnumerable<string> PrefixesToNamespaces(string prefixes)
        {
            foreach (string iteratorVariable0 in prefixes.Trim().Split(new char[] { ' ' }))
            {
                if (!string.IsNullOrEmpty(iteratorVariable0))
                {
                    ...
                    {
                        this.Error(System.Windows.SR.Get("XCRUndefinedPrefix"), new object[] { iteratorVariable0 });
                    }
                    else
                    {
        ...
        private bool ReadStartElement(ref bool more)
        {
            ...
            if (object.ReferenceEquals(namespaceURI, this.CompatibilityUri))
            {
                ...
                {
                    this.Error(System.Windows.SR.Get("XCRUnknownCompatElement"), new object[] { localName });
                }
            ...
            else
            {
                ...
                else
                {
                    if (this.Scope.InAlternateContent)
                    {
                        this.Error(System.Windows.SR.Get("XCRInvalidACChild"), new object[] { base.Reader.Name });
                    }
                    flag2 = true;
                }
            }
                ...
                do
                {
                    ...
                    if (this.ShouldIgnoreNamespace(namespaceURI))
                    {
                        if (object.ReferenceEquals(namespaceURI, this.CompatibilityUri))
                        {
                            ...
                            {
                                this.Error(System.Windows.SR.Get("XCRUnknownCompatAttrib"), new object[] { localName });
                            }
        ...
        public override int AttributeCount =>
            (base.Reader.AttributeCount - this._ignoredAttributeCount);
        ...
        public override int Depth =>
            (base.Reader.Depth - this._depthOffset);
        ...
        public override string NamespaceURI =>
            this.GetMappedNamespace(base.Reader.NamespaceURI);
        ...
        private class CompatibilityScope
        {
            ...
            public bool IsIgnorableAtCurrentScope(string namespaceName) => 
                ((this._ignorables != null) && this._ignorables.ContainsKey(namespaceName));
            ...
            public void Verify()
            {
                if (this._processContents != null)
                {
                    foreach (string str in this._processContents.Keys)
                    {
                        ...
                        {
                            this._reader.Error(System.Windows.SR.Get("XCRNSProcessContentNotIgnorable"), new object[] { str });
                        }
                    }
                }
                if (this._preserveElements != null)
                {
                    foreach (string str2 in this._preserveElements.Keys)
                    {
                        ...
                        {
                            this._reader.Error(System.Windows.SR.Get("XCRNSPreserveNotIgnorable"), new object[] { str2 });
                        }
                    }
                }
                if (this._preserveAttributes != null)
                {
                    foreach (string str3 in this._preserveAttributes.Keys)
                    {
                        ...
                        {
                            this._reader.Error(System.Windows.SR.Get("XCRNSPreserveNotIgnorable"), new object[] { str3 });
                        }
                    }
                }
            }
        ...
        private class PreserveItemSet
        {
            ...
            public void Add(string itemName)
            {
                if (this.ShouldPreserveItem(itemName))
                {
                    if (itemName == "*")
                    {
                        this._reader.Error(System.Windows.SR.Get("XCRDuplicateWildcardPreserve"), new object[] { this._namespaceName });
                    }
                    else
                    {
                        this._reader.Error(System.Windows.SR.Get("XCRDuplicatePreserve"), new object[] { itemName, this._namespaceName });
                    }
                }
                if (itemName == "*")
                {
                    if (this._names != null)
                    {
                        this._reader.Error(System.Windows.SR.Get("XCRInvalidPreserve"), new object[] { this._namespaceName });
                    }
                    else
                    {
                        this._all = true;
            ...
            public bool ShouldPreserveItem(string itemName) => 
                (this._all || ((this._names != null) && this._names.ContainsKey(itemName)));
        }
        ...
        private class ProcessContentSet
        {
            ...
            public void Add(string elementName)
            {
                if (this.ShouldProcessContent(elementName))
                {
                    if (elementName == "*")
                    {
                        this._reader.Error(System.Windows.SR.Get("XCRDuplicateWildcardProcessContent"), new object[] { this._namespaceName });
                    }
                    else
                    {
                        this._reader.Error(System.Windows.SR.Get("XCRDuplicateProcessContent"), new object[] { this._namespaceName, elementName });
                    }
                }
                if (elementName == "*")
                {
                    if (this._names != null)
                    {
                        this._reader.Error(System.Windows.SR.Get("XCRInvalidProcessContent"), new object[] { this._namespaceName });
                    }
                    else
                    {
                        this._all = true;
            ...
            public bool ShouldProcessContent(string elementName) => 
                (this._all || ((this._names != null) && this._names.ContainsKey(elementName)));
        }
    }
}
