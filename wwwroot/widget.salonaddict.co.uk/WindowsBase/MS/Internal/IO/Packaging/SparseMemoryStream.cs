namespace MS.Internal.IO.Packaging
{
    ...
    internal class SparseMemoryStream : Stream
    {
        ...
        internal SparseMemoryStream(long lowWaterMark, long highWaterMark) : this(lowWaterMark, highWaterMark, true)
        {
        }
        ...
        private void CheckDisposed()
        {
            ...
            {
                throw new ObjectDisposedException(System.Windows.SR.Get("StreamObjectDisposed"));
            }
        }
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            int num = (int) Math.Min((long) count, this._currentStreamLength - this._currentStreamPosition);
            if (this._isolatedStorageMode)
            {
                this._isolatedStorageStream.Seek(this._currentStreamPosition, SeekOrigin.Begin);
                num2 = this._isolatedStorageStream.Read(buffer, offset, num);
            }
            else
            {
                Array.Clear(buffer, offset, num);
                int num3 = this._memoryStreamList.BinarySearch(this.GetSearchBlockForOffset(this._currentStreamPosition));
                if (num3 < 0)
                {
                    num3 = ~num3;
                }
                while (num3 < this._memoryStreamList.Count)
                {
                    long num4;
                    long num5;
                    MemoryStreamBlock block = this._memoryStreamList[num3];
                    PackagingUtilities.CalculateOverlap(block.Offset, (long) ((int) block.Stream.Length), this._currentStreamPosition, (long) num, out num4, out num5);
                    if (num5 <= 0L)
                    {
                        break;
                    }
                    Array.Copy(block.Stream.GetBuffer(), (int) (num4 - block.Offset), buffer, (int) ((offset + num4) - this._currentStreamPosition), (int) num5);
                    num3++;
                }
                num2 = num;
            }
            ...
            if (this._currentStreamLength != newLength)
            {
                ...
                else if (this._currentStreamLength > newLength)
                {
                    ...
                    else
                    {
                        ...
                        long num2 = newLength - block.Offset;
                        if (num2 > 0L)
                        {
                            block.Stream.SetLength(num2);
                            index++;
                        }
                    }
