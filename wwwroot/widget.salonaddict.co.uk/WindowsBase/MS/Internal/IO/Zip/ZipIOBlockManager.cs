namespace MS.Internal.IO.Zip
{
    ...
    internal class ZipIOBlockManager : IDisposable, IEnumerable
    {
        ...
        internal ZipIOBlockManager(System.IO.Stream archiveStream, bool streaming, bool ownStream)
        {
        ...
        internal static PreSaveNotificationScanControlInstruction CommonPreSaveNotificationHandler(System.IO.Stream stream, long offset, long size, long onDiskOffset, long onDiskSize, ref SparseMemoryStream cachePrefixStream)
        {
            if (size != 0L)
            {
                long num;
                ...
                PackagingUtilities.CalculateOverlap(onDiskOffset, onDiskSize, offset, size, out num, out num2);
                if (num2 <= 0L)
                {
                ...
                long bytesToCopy = (num + num2) - onDiskOffset;
                if (cachePrefixStream == null)
                {
        ...
        internal static ulong ConvertToUInt64(uint loverAddressValue, uint higherAddressValue) => 
            (loverAddressValue + (higherAddressValue << 0x20));
        ...
        private void MapBlock(IZipIOBlock block)
        {
            for (int i = this._blockList.Count - 1; i >= 0; i--)
            {
                ...
                if ((block2 != null) && block2.DiskImageContains(block))
                {
                    ...
                    block2.SplitIntoPrefixSuffix(block, out block3, out block4);
        ...
        internal void MoveData(long moveBlockSourceOffset, long moveBlockTargetOffset, long moveBlockSize)
        {
            if ((moveBlockSize != 0L) && (moveBlockSourceOffset != moveBlockTargetOffset))
            {
                ...
                for (long i = 0L; i < moveBlockSize; i += num4)
                {
                    long num2;
                    long num3;
                    num4 = (int) Math.Min((long) buffer.Length, moveBlockSize - i);
                    if (moveBlockSourceOffset > moveBlockTargetOffset)
                    {
                        num2 = moveBlockSourceOffset + i;
                        num3 = moveBlockTargetOffset + i;
                    }
                    else
                    {
                        num2 = ((moveBlockSourceOffset + moveBlockSize) - i) - num4;
                        num3 = ((moveBlockTargetOffset + moveBlockSize) - i) - num4;
                    }
                    this._archiveStream.Seek(num2, SeekOrigin.Begin);
                    if (PackagingUtilities.ReliableRead(this._archiveStream, buffer, 0, num4) != num4)
                    {
                        throw new FileFormatException(System.Windows.SR.Get("CorruptedData"));
                    }
                    this._archiveStream.Seek(num3, SeekOrigin.Begin);
                    this._archiveStream.Write(buffer, 0, num4);
                }
            }
        }
        ...
        private void SaveContainer(bool closingFlag)
        {
            ...
            if (closingFlag || this.DirtyFlag)
            {
                ...
                {
                    IZipIOBlock block2 = (IZipIOBlock) this._blockList[i];
                    if (block2.GetDirtyFlag(closingFlag))
                    {
                        flag = true;
                        long offset = block2.Offset;
                        long size = block2.Size;
                        if (size > 0L)
                        {
                ...
                if (flag && (this.Stream.Length > num))
                {
                    this.Stream.SetLength(num);
                }
                this.Stream.Flush();
        ...
        internal System.IO.BinaryReader BinaryReader
        {
            get
            {
                this.CheckDisposed();
                if (this._binaryReader == null)
                {
                    this._binaryReader = new System.IO.BinaryReader(this.Stream, this.Encoding);
                }
                return this._binaryReader;
            }
        }

        internal System.IO.BinaryWriter BinaryWriter
        {
            get
            {
                this.CheckDisposed();
                if (this._binaryWriter == null)
                {
                    this._binaryWriter = new System.IO.BinaryWriter(this.Stream, this.Encoding);
                }
