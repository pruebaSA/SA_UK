namespace MS.Internal.IO.Zip
{
    ...
    internal class ZipIOZip64EndOfCentralDirectoryBlock : IZipIOBlock
    {
        ...
        internal static ZipIOZip64EndOfCentralDirectoryBlock SeekableLoad(ZipIOBlockManager blockManager)
        {
            ...
            ZipIOZip64EndOfCentralDirectoryBlock block2 = new ZipIOZip64EndOfCentralDirectoryBlock(blockManager);
            blockManager.Stream.Seek(offset, SeekOrigin.Begin);
            block2.ParseRecord(blockManager.BinaryReader, offset);
            return block2;
        }
        ...
        public void UpdateReferences(bool closingFlag)
        {
            if (this._blockManager.IsCentralDirectoryBlockLoaded && (this._blockManager.Streaming || this._blockManager.CentralDirectoryBlock.GetDirtyFlag(closingFlag)))
            {
                if (this._blockManager.CentralDirectoryBlock.IsZip64BitRequiredForStoring)
                {
                    ...
                    long num4 = (((long) this._sizeOfZip64EndOfCentralDirectory) + 0x38L) - 0x2cL;
                    if (((this._dirtyFlag || (this._totalNumberOfEntriesInTheCentralDirectoryOnThisDisk != count)) || ((this._totalNumberOfEntriesInTheCentralDirectory != count) || (this._sizeOfTheCentralDirectory != size))) || ((this._offsetOfStartOfCentralDirectoryWithRespectToTheStartingDiskNumber != offset) || (this._size != num4)))
                    {
        ...
        internal long OffsetOfStartOfCentralDirectory =>
            ((long) this._offsetOfStartOfCentralDirectoryWithRespectToTheStartingDiskNumber);

        public long Size =>
            this._size;

        internal long SizeOfCentralDirectory =>
            ((long) this._sizeOfTheCentralDirectory);

        internal int TotalNumberOfEntriesInTheCentralDirectory =>
            ((int) this._totalNumberOfEntriesInTheCentralDirectory);
    }
}
