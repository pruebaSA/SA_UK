namespace MS.Internal.IO.Zip
{
    ...
    internal sealed class ZipArchive : IDisposable
    {
        ...
        internal ZipFileInfo AddFile(string zipFileName, CompressionMethodEnum compressionMethod, DeflateOptionEnum deflateOption)
        {
            ...
            {
                throw new InvalidOperationException(System.Windows.SR.Get("AttemptedToCreateDuplicateFileName"));
            }
        ...
        private void CheckDisposed()
        {
            ...
            {
                throw new ObjectDisposedException(null, System.Windows.SR.Get("ZipArchiveDisposed"));
            }
        }
        ...
        internal ZipFileInfo GetFile(string zipFileName)
        {
            ...
            {
                throw new InvalidOperationException(System.Windows.SR.Get("CanNotReadInWriteOnlyMode"));
            }
            zipFileName = ZipIOBlockManager.ValidateNormalizeFileName(zipFileName);
            ...
            if (!this.FileExists(zipFileName))
            {
                throw new InvalidOperationException(System.Windows.SR.Get("FileDoesNotExists"));
            }
        ...
        internal static ZipArchive OpenOnFile(string path, FileMode mode, FileAccess access, FileShare share, bool streaming)
        {
            if ((mode == FileMode.OpenOrCreate) || (mode == FileMode.Open))
            {
                ...
                {
                    throw new FileFormatException(System.Windows.SR.Get("ZipZeroSizeFileIsNotValidArchive"));
                }
            }
            ...
            if (stream.CanSeek)
            {
                ...
                switch (mode)
                {
                    case FileMode.CreateNew:
                        if (!flag)
                        {
                            throw new IOException(System.Windows.SR.Get("CreateNewOnNonEmptyStream"));
                        }
                        break;
        ...
        private static void ValidateModeAccessShareStreaming(Stream stream, FileMode mode, FileAccess access, FileShare share, bool streaming)
        {
            ...
            int num = Convert.ToInt32(mode, CultureInfo.InvariantCulture);
            int num2 = Convert.ToInt32(access, CultureInfo.InvariantCulture);
            int num3 = Convert.ToInt32(share, CultureInfo.InvariantCulture);
            int num4 = Convert.ToInt32(streaming, CultureInfo.InvariantCulture);
            for (int i = 0; i < _validOpenParameters.GetLength(0); i++)
            {
                if (((_validOpenParameters[i, 0] == num) && (_validOpenParameters[i, 1] == num2)) && ((_validOpenParameters[i, 2] == num3) && (_validOpenParameters[i, 3] == num4)))
                {
                    return;
                }
            }
            throw new ArgumentException(System.Windows.SR.Get("UnsupportedCombinationOfModeAccessShareStreaming"));
        }
        ...
        private static void ValidateModeAccessShareSupportedEnums(FileMode mode, FileShare share)
        {
            ...
            {
                throw new NotSupportedException(System.Windows.SR.Get("TruncateAppendModesNotSupported"));
            }
            if ((share != FileShare.Read) && (share != FileShare.None))
            {
                throw new NotSupportedException(System.Windows.SR.Get("OnlyFileShareReadAndFileShareNoneSupported"));
            }
        }
        ...
        private static void ValidateModeAccessStreamStreamingCombinations(Stream stream, FileAccess access, bool streaming)
        {
            ...
            {
                throw new ArgumentException(System.Windows.SR.Get("CanNotReadDataFromStreamWhichDoesNotSupportReading"));
            }
            if (((access == FileAccess.Write) || (access == FileAccess.ReadWrite)) && !stream.CanWrite)
            {
                throw new ArgumentException(System.Windows.SR.Get("CanNotWriteDataToStreamWhichDoesNotSupportWriting"));
            }
            if (!streaming && !stream.CanSeek)
            {
                throw new ArgumentException(System.Windows.SR.Get("CanNotOperateOnStreamWhichDoesNotSupportSeeking"));
            }
        }
        ...
        internal static void VerifyVersionNeededToExtract(ushort version)
        {
            ...
            throw new NotSupportedException(System.Windows.SR.Get("NotSupportedVersionNeededToExtract"));
        }
