namespace MS.Utility
{
    ...
    internal sealed class ArrayItemList<T> : FrugalListBase<T>
    {
        ...
        public override FrugalListBase<T>.Compacter NewCompacter(int newCount) => 
            new ArrayCompacter<T>((ArrayItemList<T>) this, newCount);
        ...
        public void Promote(ArrayItemList<T> oldList)
        {
            ...
            {
                throw new ArgumentException(System.Windows.SR.Get("FrugalList_TargetMapCannotHoldAllData", new object[] { oldList.ToString(), this.ToString() }), "oldList");
            }
        ...
        internal class ArrayCompacter : FrugalListBase<T>.Compacter
        {
            ...
            public ArrayCompacter(ArrayItemList<T> store, int newCount) : base(store, newCount)
            {
                this._sourceArray = store._entries;
                int size = Math.Max(newCount + (newCount >> 2), 9);
                if ((size + (size >> 2)) >= this._sourceArray.Length)
                {
                    this._targetStore = store;
                }
                else
                {
                    this._targetStore = new ArrayItemList<T>(size);
                }
            ...
            public override FrugalListBase<T> Finish()
            {
                ...
                {
                    int index = base._validItemCount;
                    int count = base._store.Count;
                    while (index < count)
                    {
                        this._sourceArray[index] = local;
                        index++;
                    }
                }
                this._targetStore._count = base._validItemCount;
            ...
            protected override void IncludeOverride(int start, int end)
            {
                ...
                Array.Copy(this._sourceArray, start, this._targetArray, base._validItemCount, length);
                base._validItemCount += length;
            }
        }
    }
}
