    ...
    internal static class ProcessManager
    {
        static ProcessManager()
        {
            ...
            if (NativeMethods.LookupPrivilegeValue(null, "SeDebugPrivilege", out lpLuid))
            {
                ...
                try
                {
                    if (NativeMethods.OpenProcessToken(new HandleRef(null, NativeMethods.GetCurrentProcess()), 0x20, out zero))
                    {
        ...
        public static bool IsProcessRunning(int processId) => 
            IsProcessRunning(processId, GetProcessIds());

        public static bool IsProcessRunning(int processId, string machineName) => 
            IsProcessRunning(processId, GetProcessIds(machineName));
        ...
        {
            Microsoft.Win32.SafeHandles.SafeProcessHandle handle = NativeMethods.OpenProcess(access, false, processId);
        ...
        public static Microsoft.Win32.SafeHandles.SafeThreadHandle OpenThread(int threadId, int access)
        {
            ...
            try
            {
                Microsoft.Win32.SafeHandles.SafeThreadHandle handle = NativeMethods.OpenThread(access, false, threadId);
                int error = Marshal.GetLastWin32Error();
                if (handle.IsInvalid)
                {
                    if (error == 0x57)
        ...
        public static bool IsOSOlderThanXP =>
            ((Environment.OSVersion.Version.Major < 5) || ((Environment.OSVersion.Version.Major == 5) && (Environment.OSVersion.Version.Minor == 0)));
    }
}
