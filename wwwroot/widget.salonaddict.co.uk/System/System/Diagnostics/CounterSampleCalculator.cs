namespace System.Diagnostics
{
    ...
    public static class CounterSampleCalculator
    {
        ...
        public static float ComputeCounterValue(CounterSample newSample) => 
            ComputeCounterValue(CounterSample.Empty, newSample);
        ...
        public static float ComputeCounterValue(CounterSample oldSample, CounterSample newSample)
        {
            ...
            LoadPerfCounterDll();
            NativeMethods.PDH_FMT_COUNTERVALUE pFmtValue = new NativeMethods.PDH_FMT_COUNTERVALUE();
        ...
        private static void LoadPerfCounterDll()
        {
            if (!perfCounterDllLoaded)
            {
                ...
                do
                {
                    ...
                    capacity = Microsoft.Win32.UnsafeNativeMethods.GetModuleFileName(hModule, builder, capacity);
                    if (capacity == 0)
                    {
                        throw new Win32Exception();
                    }
                }
                while (capacity == builder.Capacity);
                if (Microsoft.Win32.SafeNativeMethods.LoadLibrary(Path.Combine(Path.GetDirectoryName(builder.ToString()), "perfcounter.dll")) == IntPtr.Zero)
                {
                    throw new Win32Exception();
                }
