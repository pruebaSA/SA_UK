namespace System.Diagnostics
{
    ...
    internal class PerformanceCounterLib
    {
        ...
        internal bool CategoryExists(string category) => 
            this.CategoryTable.ContainsKey(category);
        ...
        internal static bool CategoryExists(string machine, string category)
        {
            ...
            return GetPerformanceCounterLib(machine, new CultureInfo(9)).CategoryExists(category);
        }
        ...
        internal static bool CounterExists(string machine, string category, string counter)
        {
            ...
            if (!flag2)
            {
                flag2 = GetPerformanceCounterLib(machine, new CultureInfo(9)).CounterExists(category, counter, ref categoryExists);
            }
            if (!categoryExists)
            {
        ...
        internal bool FindCustomCategory(string category, out PerformanceCounterCategoryType categoryType)
        {
            ...
            try
            {
                ...
                if (key != null)
                {
                    object obj2 = key.GetValue("Library", null, RegistryValueOptions.DoNotExpandEnvironmentNames);
                    if (((obj2 != null) && (obj2 is string)) && ((string.Compare((string) obj2, "netfxperf.dll", StringComparison.OrdinalIgnoreCase) == 0) || ((string) obj2).EndsWith(@"\netfxperf.dll", StringComparison.OrdinalIgnoreCase)))
                    {
        ...
        internal static CategorySample GetCategorySample(string machine, string category)
        {
            ...
            categorySample = GetPerformanceCounterLib(machine, new CultureInfo(9)).GetCategorySample(category);
            if (categorySample == null)
            {
                throw new InvalidOperationException(SR.GetString("MissingCategory"));
            }
        ...
        internal static PerformanceCounterCategoryType GetCategoryType(string machine, string category)
        {
            ...
            GetPerformanceCounterLib(machine, new CultureInfo(9)).FindCustomCategory(category, out unknown);
            return unknown;
        }
        ...
        internal static string[] GetCounters(string machine, string category)
        {
            ...
            counters = GetPerformanceCounterLib(machine, new CultureInfo(9)).GetCounters(category, ref categoryExists);
            if (!categoryExists)
            {
                throw new InvalidOperationException(SR.GetString("MissingCategory"));
            }
        ...
        internal static PerformanceCounterLib GetPerformanceCounterLib(string machineName, CultureInfo culture)
        {
            ...
            if ((culture.LCID & 0xff00) == 0)
            {
                str = culture.LCID.ToString("X3", CultureInfo.InvariantCulture);
            }
            else
            {
                str = culture.LCID.ToString("X4", CultureInfo.InvariantCulture);
            }
        ...
        private Hashtable GetStringTable(bool isHelp)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < (strArray.Length / 2); i++)
                {
                    ...
                    hashtable[int.Parse(strArray[i * 2], CultureInfo.InvariantCulture)] = str;
                }
            }
            finally
            {
        ...
        internal static bool IsCustomCategory(string machine, string category)
        {
            ...
            return GetPerformanceCounterLib(machine, new CultureInfo(9)).IsCustomCategory(category);
        }
                    ...
                    lock (this.CategoryTableLock)
                    {
                        if (this.categoryTable == null)
                        {
                            fixed (byte* numRef = this.GetPerformanceData("Global"))
                            {
                                ...
                                NativeMethods.PERF_DATA_BLOCK structure = new NativeMethods.PERF_DATA_BLOCK();
                                Marshal.PtrToStructure(ptr, structure);
                                ptr = (IntPtr) (((long) ptr) + structure.HeaderLength);
                                int numObjectTypes = structure.NumObjectTypes;
                                long num2 = ((long) ((ulong) numRef)) + structure.TotalByteLength;
