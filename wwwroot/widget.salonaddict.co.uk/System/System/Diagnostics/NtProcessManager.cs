namespace System.Diagnostics
{
    ...
    internal static class NtProcessManager
    {
        ...
        public static ModuleInfo[] GetModuleInfos(int processId) => 
            GetModuleInfos(processId, false);
        ...
        private static ModuleInfo[] GetModuleInfos(int processId, bool firstModuleOnly)
        {
            ...
            Label_0045:
                ...
                try
                {
                    ...
                    for (int j = 0; j < 10; j++)
                    {
                        flag = NativeMethods.EnumProcessModules(invalidHandle, handle2.AddrOfPinnedObject(), ptrArray.Length * IntPtr.Size, ref needed);
                        if (flag)
                        {
                            goto Label_008F;
                        }
            ...
            Label_008F:
                ...
                for (int i = 0; i < needed; i++)
                {
                    ...
                    if (!NativeMethods.GetModuleInformation(invalidHandle, new HandleRef(null, handle), ntModuleInfo, Marshal.SizeOf(ntModuleInfo)))
                    {
                        throw new Win32Exception();
                    }
                    ...
                    if (NativeMethods.GetModuleBaseName(invalidHandle, new HandleRef(null, handle), baseName, baseName.Capacity * 2) == 0)
                    {
                        throw new Win32Exception();
                    }
                    info.baseName = baseName.ToString();
                    StringBuilder builder2 = new StringBuilder(0x400);
                    if (NativeMethods.GetModuleFileNameEx(invalidHandle, new HandleRef(null, handle), builder2, builder2.Capacity * 2) == 0)
                    {
                        throw new Win32Exception();
                    }
        ...
        public static int GetProcessIdFromHandle(Microsoft.Win32.SafeHandles.SafeProcessHandle processHandle)
        {
            ...
            int error = NativeMethods.NtQueryInformationProcess(processHandle, 0, info, Marshal.SizeOf(info), null);
            if (error != 0)
            {
        ...
        Label_000B:
            if (!NativeMethods.EnumProcesses(processIds, processIds.Length * 4, out num))
            {
                throw new Win32Exception();
            }
        ...
        private static unsafe ProcessInfo[] GetProcessInfos(PerformanceCounterLib library, int processIndex, int threadIndex, byte[] data)
        {
            ...
            fixed (byte* numRef = data)
            {
                ...
                for (int j = 0; j < structure.NumObjectTypes; j++)
                {
                    ...
                    NativeMethods.PERF_COUNTER_DEFINITION[] perf_counter_definitionArray = new NativeMethods.PERF_COUNTER_DEFINITION[list2.Count];
