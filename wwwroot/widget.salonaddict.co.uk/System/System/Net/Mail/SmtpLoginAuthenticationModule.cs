            ...
            try
            {
                lock (this.sessions)
                {
                    ...
                    if (credential2 == null)
                    {
                        ...
                        return new Authorization(Convert.ToBase64String(Encoding.ASCII.GetBytes(userName)), false);
                    }
                    this.sessions.Remove(sessionCookie);
                    authorization = new Authorization(Convert.ToBase64String(Encoding.ASCII.GetBytes(credential2.Password)), true);
                }
            }
            finally
            {
