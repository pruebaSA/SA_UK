namespace System.Net
{
    ...
    internal class ConnectStream : Stream, ICloseEx
    {
        ...
        private int ExchangeCallNesting(int value, int comparand) => 
            Interlocked.CompareExchange(ref this.m_CallNesting, value, comparand);
        ...
        {
            NestedSingleAsyncResult result = state as NestedSingleAsyncResult;
            ConnectStream asyncObject = result.AsyncObject as ConnectStream;
            try
            {
                if (!asyncObject.m_Draining && asyncObject.IsClosed)
                {
                ...
                else if (asyncObject.m_ErrorException != null)
                {
                    result.InvokeCallback(asyncObject.m_ErrorException);
                }
                else
                {
                    if (asyncObject.m_ChunkedNeedCRLFRead)
                    {
                        asyncObject.ReadCRLF(asyncObject.m_TempBuffer);
                        asyncObject.m_ChunkedNeedCRLFRead = false;
                    }
                    StreamChunkBytes readByteBuffer = new StreamChunkBytes(asyncObject);
                    asyncObject.m_ChunkSize = asyncObject.ProcessReadChunkedSize(readByteBuffer);
                    if (asyncObject.m_ChunkSize != 0)
                    {
                        asyncObject.m_ChunkedNeedCRLFRead = true;
                        int size = Math.Min(result.Size, asyncObject.m_ChunkSize);
                        int num2 = 0;
                        if (asyncObject.m_ReadBufferSize > 0)
                        {
                            num2 = asyncObject.FillFromBufferedData(result.Buffer, ref result.Offset, ref size);
                            if (size == 0)
                            {
                                result.InvokeCallback(num2);
                                return;
                            }
                        }
                        if (asyncObject.ErrorInStream)
                        {
                            throw asyncObject.m_ErrorException;
                        }
                        asyncObject.m_BytesAlreadyTransferred = num2;
                        if (asyncObject.m_Connection.BeginRead(result.Buffer, result.Offset, size, m_ReadCallbackDelegate, result) == null)
                        {
                            asyncObject.m_BytesAlreadyTransferred = 0;
                            asyncObject.m_ErrorException = new WebException(NetRes.GetWebStatusString("net_requestaborted", WebExceptionStatus.RequestCanceled), WebExceptionStatus.RequestCanceled);
                            throw asyncObject.m_ErrorException;
                        }
                    }
                    else
                    {
                        asyncObject.ReadCRLF(asyncObject.m_TempBuffer);
                        asyncObject.RemoveTrailers(readByteBuffer);
                        asyncObject.m_ReadBytes = 0L;
                        asyncObject.m_ChunkEofRecvd = true;
                        asyncObject.CallDone();
        ...
        private int ReadWithoutValidation(byte[] buffer, int offset, int size) => 
            this.ReadWithoutValidation(buffer, offset, size, true);
        ...
        internal long BytesLeftToWrite =>
            this.m_BytesLeftToWrite;

        public override bool CanRead =>
            (!this.WriteStream && !this.IsClosed);
        ...
        internal bool ErrorInStream =>
            (this.m_ErrorException != null);

        internal bool IgnoreSocketErrors =>
            this.m_IgnoreSocketErrors;
        ...
        private bool WriteStream =>
            (this.m_HttpWriteMode != HttpWriteMode.Unknown);
