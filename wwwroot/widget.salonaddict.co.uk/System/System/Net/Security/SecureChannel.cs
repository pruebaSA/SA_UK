namespace System.Net.Security
{
    ...
    internal class SecureChannel
    {
        ...
        private bool AcquireClientCredentials(ref byte[] thumbPrint)
        {
            ...
            try
            {
                byte[] certHash = certificate4?.GetCertHash();
                SafeFreeCredentials credentials = SslSessionsCache.TryCachedCredential(certHash, this.m_ProtocolFlags);
                ...
                SecureCredential.Flags flags = SecureCredential.Flags.NoDefaultCred | SecureCredential.Flags.ValidateManual;
                ...
                SecureCredential secureCredential = new SecureCredential(4, certificate4, flags, this.m_ProtocolFlags);
        ...
        private bool AcquireServerCredentials(ref byte[] thumbPrint)
        {
            ...
            try
            {
                ...
                SecureCredential.Flags zero = SecureCredential.Flags.Zero;
                if (!ServicePointManager.DisableSendAuxRecord)
                {
                    zero |= SecureCredential.Flags.SendAuxRecord;
                }
                SecureCredential secureCredential = new SecureCredential(4, certificate2, zero, this.m_ProtocolFlags);
        ...
        internal SecurityStatus Decrypt(byte[] payload, ref int offset, ref int count)
        {
            ...
            SecurityStatus status = (SecurityStatus) SSPIWrapper.DecryptMessage(GlobalSSPI.SSPISecureChannel, this.m_SecurityContext, input, 0);
            count = 0;
            for (int i = 0; i < input.Length; i++)
            {
                if (((status == SecurityStatus.OK) && (input[i].type == BufferType.Data)) || ((status != SecurityStatus.OK) && (input[i].type == BufferType.Extra)))
                {
        ...
        internal SecurityStatus Encrypt(byte[] buffer, int offset, int size, ref byte[] output, out int resultSize)
        {
            ...
            output = buffer2;
            resultSize = (input[0].size + input[1].size) + input[2].size;
        ...
        internal unsafe bool VerifyRemoteCertificate(RemoteCertValidationCallback remoteCertValidationCallback)
        {
            ...
            try
            {
                ...
                else
                {
                    chain = new X509Chain {
                        ChainPolicy = { 
                            RevocationMode = this.m_CheckCertRevocation ? X509RevocationMode.Online : X509RevocationMode.NoCheck,
                            RevocationFlag = X509RevocationFlag.ExcludeRoot
                        }
                    };
                    if (!ServicePointManager.DisableCertificateEKUs)
                    {
                        chain.ChainPolicy.ApplicationPolicy.Add(this.m_ServerMode ? this.m_ClientAuthOid : this.m_ServerAuthOid);
                    }
        ...
        internal bool CheckCertRevocationStatus =>
            this.m_CheckCertRevocation;

        internal X509CertificateCollection ClientCertificates =>
            this.m_ClientCertificates;
        ...
        internal bool IsRemoteCertificateAvailable =>
            this.m_IsRemoteCertificateAvailable;

        internal bool IsServer =>
            this.m_ServerMode;

        internal bool IsValidContext =>
            ((this.m_SecurityContext != null) && !this.m_SecurityContext.IsInvalid);

        internal X509Certificate LocalClientCertificate =>
            this.m_SelectedClientCertificate;

        internal X509Certificate LocalServerCertificate =>
            this.m_ServerCertificate;

        internal int MaxDataSize =>
            this.m_MaxDataSize;

        internal bool RemoteCertRequired =>
            this.m_RemoteCertRequired;
