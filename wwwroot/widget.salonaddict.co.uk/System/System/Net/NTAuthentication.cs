namespace System.Net
{
    ...
    internal class NTAuthentication
    {
        ...
        internal NTAuthentication(bool isServer, string package, NetworkCredential credential, string spn, ContextFlags requestedContextFlags, ContextAwareResult context, System.Security.Authentication.ExtendedProtection.ChannelBinding channelBinding)
        {
            ...
            {
                WindowsIdentity identity = context?.Identity;
                try
                {
                    IDisposable disposable = identity?.Impersonate();
                    if (disposable != null)
                    {
                        using (disposable)
                        {
                    ...
                    ExecutionContext contextCopy = context?.ContextCopy;
                    if (contextCopy == null)
                    {
                        this.Initialize(isServer, package, credential, spn, requestedContextFlags, channelBinding);
                    }
                    ...
                    {
                        ExecutionContext.Run(contextCopy, s_InitializeCallback, new InitializeCallbackContext(this, isServer, package, credential, spn, requestedContextFlags, channelBinding));
                    }
                    return;
                }
                catch
        ...
        private string GetClientSpecifiedSpn() => 
            (SSPIWrapper.QueryContextAttributes(GlobalSSPI.SSPIAuth, this.m_SecurityContext, ContextAttribute.ClientSpecifiedSpn) as string);
        ...
        {
            ContextFlags flags = ContextFlags.Connection | ContextFlags.Confidentiality | ContextFlags.SequenceDetect | ContextFlags.ReplayDetect;
            ...
            return (flags | ContextFlags.MutualAuth);
        }
        ...
        {
            this.m_RequestedContextFlags |= ContextFlags.SequenceDetect | ContextFlags.ReplayDetect;
            SecurityBuffer[] inputBuffers = null;
                    ...
                    if (!isClientPreAuth)
                    {
                        ...
                        if (incomingBlob != null)
                        {
                            List<SecurityBuffer> list = new List<SecurityBuffer>(5) {
                                ...
                                new SecurityBuffer(Encoding.Unicode.GetBytes(this.m_Spn), BufferType.TargetHost)
                            };
                            if (this.m_ChannelBinding != null)
                            {
                                list.Add(new SecurityBuffer(this.m_ChannelBinding));
        ...
        internal bool IsConfidentialityFlag =>
            ((this.m_ContextFlags & ContextFlags.Confidentiality) != ContextFlags.Zero);

        internal bool IsDelegationFlag =>
            ((this.m_ContextFlags & ContextFlags.Delegate) != ContextFlags.Zero);

        internal bool IsIdentifyFlag =>
            ((this.m_ContextFlags & (this.m_IsServer ? ContextFlags.AcceptIdentify : ContextFlags.AcceptIntegrity)) != ContextFlags.Zero);

        internal bool IsIntegrityFlag =>
            ((this.m_ContextFlags & (this.m_IsServer ? ContextFlags.AcceptIntegrity : ContextFlags.AcceptStream)) != ContextFlags.Zero);
        ...
        internal bool IsMutualAuthFlag =>
            ((this.m_ContextFlags & ContextFlags.MutualAuth) != ContextFlags.Zero);
