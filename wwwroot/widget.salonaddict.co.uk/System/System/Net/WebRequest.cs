namespace System.Net
{
    ...
    public abstract class WebRequest : MarshalByRefObject, ISerializable
    {
        ...
        internal static IWebProxy InternalGetSystemWebProxy() => 
            new WebProxyWrapperOpaque(new WebProxy(true));
        ...
        {
            get => 
                RequestCacheManager.GetBinding(string.Empty).Policy;
            set
            {
                ExceptionHelper.WebPermissionUnrestricted.Demand();
        ...
        internal class WebProxyWrapperOpaque : IAutoWebProxy, IWebProxy
        {
            ...
            public ProxyChain GetProxies(Uri destination) => 
                ((IAutoWebProxy) this.webProxy).GetProxies(destination);

            public Uri GetProxy(Uri destination) => 
                this.webProxy.GetProxy(destination);

            public bool IsBypassed(Uri host) => 
                this.webProxy.IsBypassed(host);

            public ICredentials Credentials
            {
                get => 
                    this.webProxy.Credentials;
                set
                {
