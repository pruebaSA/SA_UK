namespace System.Net
{
    ...
    internal class ConnectionPool
    {
        ...
        Label_00EC:
            while (this.m_QueuedRequests.Count > 0)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    {
                        pooledStream.Close();
                        this.PutConnection(pooledStream, request.OwningObject, request.CreationTimeout);
                    }
                ...
                catch
                {
                    ...
                    request.AsyncCallback(request.OwningObject, new Exception(SR.GetString("net_nonClsCompliantException")));
                    continue;
                }
            }
            Thread.Sleep(500);
            ...
            lock (this.m_ObjectList.SyncRoot)
            {
                ...
                for (int i = 0; i < objArray.Length; i++)
                {
                    ...
                    if (stream != null)
                    {
                        ...
                        try
                        {
                            flag2 = Monitor.TryEnter(stream);
                            if (flag2 && stream.IsEmancipated)
                            {
        ...
        private Mutex CreationMutex =>
            ((Mutex) this.m_WaitHandles[2]);

        private ManualResetEvent ErrorEvent =>
            ((ManualResetEvent) this.m_WaitHandles[1]);
        ...
        private System.Net.Semaphore Semaphore =>
            ((System.Net.Semaphore) this.m_WaitHandles[0]);
