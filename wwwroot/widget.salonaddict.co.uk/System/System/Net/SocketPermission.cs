namespace System.Net
{
    ...
    public sealed class SocketPermission : CodeAccessPermission, IUnrestrictedPermission
    {
        ...
        public override IPermission Copy() => 
            new SocketPermission(this.m_noRestriction) { 
        ...
        private static void ParseAddXmlElement(SecurityElement et, ArrayList listToAdd, string accessStr)
        {
            foreach (SecurityElement element in et.Children)
            {
                if (element.Tag.Equals("ENDPOINT"))
                {
                    ...
                    {
                        throw new ArgumentException(SR.GetString("net_perm_invalid_val", new object[] { accessStr + "port", str }), new Exception(SR.GetString("net_nonClsCompliantException")));
                    }
                    if (!ValidationHelper.ValidateTcpPort(num) && (num != -1))
                    {
                        throw new ArgumentOutOfRangeException(SR.GetString("net_perm_invalid_val", new object[] { accessStr + "port", str }));
                    }
        ...
        public IEnumerator AcceptList =>
            this.m_acceptList.GetEnumerator();

        public IEnumerator ConnectList =>
            this.m_connectList.GetEnumerator();
    }
}
