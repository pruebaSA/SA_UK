    ...
    internal static class UnsafeNclNativeMethods
    {
        ...
        internal static class HttpApi
        {
            ...
            internal static unsafe WebHeaderCollection GetHeaders(byte[] memoryBlob, IntPtr originalAddress)
            {
                ...
                fixed (byte* numRef = memoryBlob)
                {
                    ...
                    if (http_requestPtr->Headers.UnknownHeaderCount != 0)
                    {
                        ...
                        {
                            if (((http_unknown_headerPtr->pName != null) && (http_unknown_headerPtr->NameLength > 0)) && ((http_unknown_headerPtr->pRawValue != null) && (http_unknown_headerPtr->RawValueLength > 0)))
                            {
                                string name = new string(http_unknown_headerPtr->pName + ((sbyte*) num), 0, http_unknown_headerPtr->NameLength);
                                string str2 = new string(http_unknown_headerPtr->pRawValue + ((sbyte*) num), 0, http_unknown_headerPtr->RawValueLength);
                    ...
                    {
                        if ((http_known_headerPtr->RawValueLength != 0) && (http_known_headerPtr->pRawValue != null))
                        {
            ...
            internal static unsafe string GetKnownHeader(HTTP_REQUEST* request, int headerIndex) => 
                GetKnownHeader(request, 0L, headerIndex);
            ...
            internal static extern unsafe uint HttpAddUrl(SafeCloseHandle RequestQueueHandle, ushort* pFullyQualifiedUrl, void* pReserved);
            [DllImport("httpapi.dll", CallingConvention=CallingConvention.StdCall, SetLastError=true, ExactSpelling=true)]
            internal static extern unsafe uint HttpInitialize(HTTPAPI_VERSION Version, uint Flags, void* Reserved);
            [DllImport("httpapi.dll", CallingConvention=CallingConvention.StdCall, SetLastError=true, ExactSpelling=true)]
            internal static extern unsafe uint HttpReceiveClientCertificate(SafeCloseHandle RequestQueueHandle, ulong ConnectionId, uint Flags, byte* pSslClientCertInfo, uint SslClientCertInfoSize, uint* pBytesReceived, NativeOverlapped* pOverlapped);
            [DllImport("httpapi.dll", CallingConvention=CallingConvention.StdCall, SetLastError=true, ExactSpelling=true)]
            internal static extern unsafe uint HttpReceiveClientCertificate(SafeCloseHandle RequestQueueHandle, ulong ConnectionId, uint Flags, HTTP_SSL_CLIENT_CERT_INFO* pSslClientCertInfo, uint SslClientCertInfoSize, uint* pBytesReceived, NativeOverlapped* pOverlapped);
            [DllImport("httpapi.dll", CallingConvention=CallingConvention.StdCall, SetLastError=true, ExactSpelling=true)]
            internal static extern unsafe uint HttpReceiveHttpRequest(SafeCloseHandle RequestQueueHandle, ulong RequestId, uint Flags, HTTP_REQUEST* pRequestBuffer, uint RequestBufferLength, uint* pBytesReturned, NativeOverlapped* pOverlapped);
            [DllImport("httpapi.dll", CallingConvention=CallingConvention.StdCall, SetLastError=true, ExactSpelling=true)]
            internal static extern unsafe uint HttpReceiveRequestEntityBody(SafeCloseHandle RequestQueueHandle, ulong RequestId, uint Flags, void* pEntityBuffer, uint EntityBufferLength, uint* pBytesReturned, NativeOverlapped* pOverlapped);
            [DllImport("httpapi.dll", CallingConvention=CallingConvention.StdCall, SetLastError=true, ExactSpelling=true)]
            internal static extern unsafe uint HttpRemoveUrl(SafeCloseHandle RequestQueueHandle, ushort* pFullyQualifiedUrl);
            [DllImport("httpapi.dll", CallingConvention=CallingConvention.StdCall, SetLastError=true, ExactSpelling=true)]
            internal static extern unsafe uint HttpSendHttpResponse(SafeCloseHandle RequestQueueHandle, ulong RequestId, uint Flags, HTTP_RESPONSE* pHttpResponse, void* pCachePolicy, uint* pBytesSent, SafeLocalFree pRequestBuffer, uint RequestBufferLength, NativeOverlapped* pOverlapped, void* pLogData);
            [DllImport("httpapi.dll", CallingConvention=CallingConvention.StdCall, SetLastError=true, ExactSpelling=true)]
            internal static extern unsafe uint HttpSendResponseEntityBody(SafeCloseHandle RequestQueueHandle, ulong RequestId, uint Flags, ushort EntityChunkCount, HTTP_DATA_CHUNK* pEntityChunks, uint* pBytesSent, SafeLocalFree pRequestBuffer, uint RequestBufferLength, NativeOverlapped* pOverlapped, void* pLogData);
            [DllImport("httpapi.dll", CallingConvention=CallingConvention.StdCall, SetLastError=true, ExactSpelling=true)]
            internal static extern unsafe uint HttpWaitForDisconnect(SafeCloseHandle RequestQueueHandle, ulong ConnectionId, NativeOverlapped* pOverlapped);
            ...
            {
                private static Hashtable m_Hashtable = new Hashtable(0x29);
                ...
                if (s_RasSupported)
                {
                    ...
                    if ((lpcConnections != 0) && (num3 == 0))
                    {
                        for (uint i = 0; i < lpcConnections; i++)
                        {
                            ...
                            if ((RasGetConnectStatus(lprasconn[i].hrasconn, ref structure) == 0) && (structure.rasconnstate == RASCONNSTATE.RASCS_Connected))
                            {
