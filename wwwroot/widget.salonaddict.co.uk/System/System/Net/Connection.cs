    ...
    internal class Connection : PooledStream
    {
        ...
        internal bool AbortOrDisassociate(HttpWebRequest request, WebException webException)
        {
            ...
            lock (this)
            {
                ...
                if (index == -1)
                {
                    index = this.m_WaitList.IndexOf(request);
                    if (index != -1)
                    {
                        this.m_WaitList.RemoveAt(index);
                    }
                    return true;
                }
                if (index != 0)
        ...
        private HttpWebRequest CheckNextRequest()
        {
            ...
            HttpWebRequest request = this.m_WaitList[0] as HttpWebRequest;
            if (this.m_IsPipelinePaused)
            {
                this.m_IsPipelinePaused = this.m_WriteList.Count > s_MinPipelinedCount;
            }
            if (((!request.Pipelined || request.RequireBody) || ((!this.m_CanPipeline || !this.m_Pipelining) || this.m_IsPipelinePaused)) && (this.m_WriteList.Count != 0))
            {
                request = null;
            }
            if (request != null)
        ...
        private void CompleteConnection(bool async, HttpWebRequest request)
        {
            ...
            try
            {
                try
                {
                    ...
                    {
                        TlsStream stream = new TlsStream(request.Address.Host, base.NetworkStream, request.ClientCertificates, this.ServicePoint, request, request.Async ? request.GetConnectingContext().ContextCopy : null);
        ...
        private void PrepareCloseConnectionSocket(ref ConnectionReturnResult returnResult)
        {
            ...
            if ((this.m_WriteList.Count != 0) || (this.m_WaitList.Count != 0))
            {
                ...
                if (lockedRequest != null)
                {
                    ...
                    if (!flag)
                    {
                        foreach (HttpWebRequest request3 in this.m_WaitList)
                        {
                            if (request3 == lockedRequest)
                            {
                                flag = true;
                                break;
                            }
                ...
                if (this.m_WaitList.Count != 0)
                {
                    array = new HttpWebRequest[this.m_WaitList.Count];
                ...
                if (this.m_WriteList.Count != 0)
                {
                    ...
                    if (!(innerException is WebException) && !(innerException is SecurityException))
                    {
                        ...
                        else
                        {
                            ...
                            {
                                HttpWebRequest request4 = (HttpWebRequest) this.m_WriteList[0];
                                if (!request4.BodyStarted)
                                {
                                    flag3 = true;
                                }
                            }
                    ...
                    array = new HttpWebRequest[this.m_WriteList.Count];
                    this.m_WriteList.CopyTo(array, 0);
                    ConnectionReturnResult.AddExceptionRange(ref returnResult, array, exception, innerException);
                }
                            ...
                            else
                            {
                                ...
                                if (this.m_WriteDone)
                                {
                                    ...
                                    if (request != null)
                                    {
                                        ...
                                        unspecified = this.StartRequest(request);
                                    }
                                    else
                                    {
                                        this.m_Free = true;
        ...
        private bool TunnelThroughProxy(Uri proxy, HttpWebRequest originalRequest, bool async)
        {
            ...
            try
            {
                ...
                request.Credentials = originalRequest.InternalProxy?.Credentials;
        ...
        internal bool AtLeastOneResponseReceived =>
            this.m_AtLeastOneResponseReceived;

        internal int BusyCount =>
            (((this.m_ReadDone ? 0 : 1) + (2 * (this.m_WaitList.Count + this.m_WriteList.Count))) + this.m_ReservedCount);
        ...
        internal HttpWebRequest LockedRequest
        {
            ...
            set
            {
                ...
                {
                    object unlockConnectionDelegate = lockedRequest?.UnlockConnectionDelegate;
                    if ((unlockConnectionDelegate != null) && ((value != null) || (this.m_ConnectionUnlock != unlockConnectionDelegate)))
                    {
                        throw new InternalException();
                    }
                    if (value == null)
                    ...
                    {
                        UnlockConnectionDelegate delegate2 = value.UnlockConnectionDelegate;
                        if (delegate2 != null)
                        {
                            if (delegate2 == this.m_ConnectionUnlock)
                            {
                                throw new InternalException();
                            }
                            delegate2();
        ...
        internal override System.Net.ServicePoint ServicePoint =>
            this.ConnectionGroup.ServicePoint;
