namespace System.Net
{
    ...
    public sealed class Cookie
    {
        ...
        public override int GetHashCode() => 
            string.Concat(new object[] { this.Name, "=", this.Value, ";", this.Path, "; ", this.Domain, "; ", this.Version }).GetHashCode();
        ...
        internal string ToServerString()
        {
            ...
            TimeSpan span = (TimeSpan) (this.Expires - DateTime.UtcNow);
            int seconds = span.Seconds;
            if (seconds > 0)
            {
                str = str + "; Max-Age=" + seconds.ToString(NumberFormatInfo.InvariantInfo);
            }
        ...
        internal bool VerifySetDefaults(CookieVariant variant, Uri uri, bool isLocalDomain, string localDomain, bool set_default, bool isThrow)
        {
            ...
            else
            {
                if (!this.m_domain_implicit)
                {
                    ...
                    else if ((domain.Length < 4) || (!(flag2 = string.Compare(domain, ".local", StringComparison.OrdinalIgnoreCase) == 0) && (domain.IndexOf('.', 1, domain.Length - 2) == -1)))
                    {
                        flag = false;
                    }
                    ...
                    {
                        if ((((host.Length + 1) != domain.Length) || (string.Compare(host, 0, domain, 1, host.Length, StringComparison.OrdinalIgnoreCase) != 0)) && ((host.Length <= domain.Length) || (string.Compare(host, host.Length - domain.Length, domain, 0, domain.Length, StringComparison.OrdinalIgnoreCase) != 0)))
                        {
                            flag = false;
                        }
                    }
                    else if (!flag2 && (((index == -1) || (domain.Length != (host.Length - index))) || (string.Compare(host, index, domain, 0, domain.Length, StringComparison.OrdinalIgnoreCase) != 0)))
                    {
                        flag = false;
                    }
                    if (flag)
        ...
        {
            get => 
                ((this.m_expires <= DateTime.Now) && (this.m_expires != DateTime.MinValue));
            set
            {
                if (value)
                {
        ...
        internal bool Plain =>
            (this.Variant == CookieVariant.Plain);
        ...
        public string Port
        {
            ...
            set
            {
                ...
                else
                {
                    ...
                    this.m_port_list = new int[strArray.Length];
                    for (int i = 0; i < strArray.Length; i++)
                    {
                        this.m_port_list[i] = -1;
                        if ((strArray[i].Length != 0) && !int.TryParse(strArray[i], out this.m_port_list[i]))
                        {
                            throw new CookieException(SR.GetString("net_cookie_attribute", new object[] { "Port", this.m_port }));
                        }
                    }
