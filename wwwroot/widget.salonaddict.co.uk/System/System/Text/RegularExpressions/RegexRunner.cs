namespace System.Text.RegularExpressions
{
    ...
    public abstract class RegexRunner
    {
        ...
        protected int Crawlpos() => 
            (this.runcrawl.Length - this.runcrawlpos);
        ...
        protected bool IsBoundary(int index, int startpos, int endpos) => 
            (((index > startpos) && RegexCharClass.IsWordChar(this.runtext[index - 1])) != ((index < endpos) && RegexCharClass.IsWordChar(this.runtext[index])));

        protected bool IsECMABoundary(int index, int startpos, int endpos) => 
            (((index > startpos) && RegexCharClass.IsECMAWordChar(this.runtext[index - 1])) != ((index < endpos) && RegexCharClass.IsECMAWordChar(this.runtext[index])));

        protected bool IsMatched(int cap) => 
            this.runmatch.IsMatched(cap);

        protected int MatchIndex(int cap) => 
            this.runmatch.MatchIndex(cap);

        protected int MatchLength(int cap) => 
            this.runmatch.MatchLength(cap);

        protected int Popcrawl() => 
            this.runcrawl[this.runcrawlpos++];
