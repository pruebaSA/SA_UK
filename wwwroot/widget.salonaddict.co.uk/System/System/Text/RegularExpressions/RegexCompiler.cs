namespace System.Text.RegularExpressions
{
    ...
    internal abstract class RegexCompiler
    {
        ...
        internal int AddTrack(int flags) => 
            this.AddBacktrackNote(flags, this.DefineLabel(), this._codepos);

        internal int AddUniqueTrack(int i) => 
            this.AddUniqueTrack(i, 0x80);
        ...
        {
            RegexTypeCompiler compiler = new RegexTypeCompiler(an, attribs, resourceFile, evidence);
        ...
        internal bool IsCi() => 
            ((this._regexopcode & 0x200) != 0);

        internal bool IsRtl() => 
            ((this._regexopcode & 0x40) != 0);
        ...
        internal int NextCodepos() => 
            (this._codepos + RegexCode.OpcodeSize(this._codes[this._codepos]));

        internal int Operand(int i) => 
            this._codes[(this._codepos + i) + 1];
        ...
        private static FieldInfo RegexRunnerField(string fieldname) => 
            typeof(RegexRunner).GetField(fieldname, BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Static | BindingFlags.Instance);

        private static MethodInfo RegexRunnerMethod(string methname) => 
            typeof(RegexRunner).GetMethod(methname, BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Static | BindingFlags.Instance);
