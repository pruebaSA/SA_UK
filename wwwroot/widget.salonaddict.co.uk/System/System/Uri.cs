namespace System
{
    ...
    public class Uri : ISerializable
    {
        ...
        private bool AllowIdnStatic(UriParser syntax, Flags flags)
        {
            ...
            return ((s_IdnScope == UriIdnScope.All) || ((s_IdnScope == UriIdnScope.AllExceptIntranet) && StaticNotAny(flags, Flags.HostNotParsed | Flags.IntranetUri)));
        }
        ...
        private unsafe ushort CheckAuthorityHelper(char* pString, ushort idx, ushort length, ref ParsingError err, ref Flags flags, UriParser syntax, ref string newHost)
        {
            ...
            else if ((((flags2 & UriSyntaxFlags.AllowDnsHost) != 0) && !iriParsing) && DomainNameHelper.IsValid(pString, index, ref end, ref notCanonical, StaticNotAny(flags, Flags.HostNotParsed | Flags.ImplicitFile)))
            {
                ...
                {
                    if ((s_IdnScope == UriIdnScope.AllExceptIntranet) && this.IsIntranet(new string(pString, 0, end)))
                    {
            ...
            else if ((((iriParsing || (s_IdnScope != UriIdnScope.None)) && ((flags2 & UriSyntaxFlags.AllowDnsHost) != 0)) && ((iriParsing && flag4) || this.AllowIdnStatic(syntax, flags))) && DomainNameHelper.IsValidByIri(pString, index, ref end, ref notCanonical, StaticNotAny(flags, Flags.HostNotParsed | Flags.ImplicitFile)))
            {
            ...
            else if ((((s_IdnScope == UriIdnScope.None) && !s_IriParsing) && (((flags2 & UriSyntaxFlags.AllowUncHost) != 0) && UncNameHelper.IsValid(pString, index, ref end, StaticNotAny(flags, Flags.HostNotParsed | Flags.ImplicitFile)))) && ((end - index) <= 0x100))
            {
        ...
        private static char[] Compress(char[] dest, ushort start, ref int destLength, UriParser syntax)
        {
            ...
            while (index != start)
            {
                ...
                if (!flag && (ch == '/'))
                {
                    ...
                    System.Buffer.BlockCopy(dest, num2 << 1, dest, (index + 1) << 1, (destLength - num2) << 1);
                ...
                if ((!flag && (num4 == 0)) && ((num2 == ((index + num3) + 1)) || ((num2 == 0) && (((index + num3) + 1) == destLength))))
                {
                    ...
                    System.Buffer.BlockCopy(dest, num3 << 1, dest, (index + 1) << 1, (destLength - num3) << 1);
            ...
            if (((((ushort) destLength) > start) && syntax.InFact(UriSyntaxFlags.CanonicalizeAsFilePath)) && (num <= 1))
            {
                ...
                {
                    num2 = (ushort) (num2 + 1);
                    System.Buffer.BlockCopy(dest, num2 << 1, dest, start << 1, (destLength - num2) << 1);
                    destLength -= num2;
                    return dest;
                }
                if ((num3 != 0) && ((num2 == (num3 + 1)) || ((num2 == 0) && ((num3 + 1) == destLength))))
                {
                    ...
                    System.Buffer.BlockCopy(dest, num3 << 1, dest, start << 1, (destLength - num3) << 1);
                    destLength -= num3;
                }
            }
            return dest;
        ...
        private static bool ConfigStringEqual(string string1, string string2) => 
            (string.Compare(string1, string2, StringComparison.OrdinalIgnoreCase) == 0);
        ...
        private void CreateThisFromUri(Uri otherUri)
        {
            ...
            if (this.InFact(Flags.HostNotParsed | Flags.MinimalUriInfoSet))
            {
                ...
                this.m_Flags |= (Flags) otherUri.m_Info.Offset.Path;
            }
        ...
        private unsafe void CreateUriInfo(Flags cF)
        {
            ...
            UriInfo info = new UriInfo {
                Offset = { End = (ushort) this.m_String.Length }
            };
        ...
        private static unsafe char[] EnsureDestinationSize(char* pStr, char[] dest, int currentInputPos, short charsToAdd, short minReallocateChars, ref int destPos, int prevInputPos)
        {
            if ((dest == null) || (dest.Length < ((destPos + (currentInputPos - prevInputPos)) + charsToAdd)))
            {
                ...
                {
                    System.Buffer.BlockCopy(dest, 0, dst, 0, destPos << 1);
                }
                dest = dst;
            }
        ...
        private static unsafe char[] EscapeString(string input, int start, int end, char[] dest, ref int destPos, bool isUriString, char force1, char force2, char rsvd)
        {
            ...
            byte* bytes = stackalloc byte[1 * 160];
        ...
        internal unsafe string EscapeUnescapeIri(char* pInput, int start, int end, UriComponents component)
        {
            ...
            while (index < end)
            {
                ...
                if (num < 12)
                {
                    char[] chArray4;
                    int num8 = 0;
                    num8 = chArray.Length + 90;
                    num += 90;
                    char[] chArray3 = new char[num8];
                    ...
                    Label_0324:
                        num9 = 0;
                        while (num9 < destOffset)
                        {
                            chRef[num9] = pDest[num9];
                            num9++;
                        }
                    }
                ...
                int num10 = Encoding.UTF8.GetBytes(pInput + index, surrogatePair ? 2 : 1, numRef, 4);
                num -= num10 * 3;
        ...
        private unsafe char[] GetCanonicalPath(char[] dest, ref int pos, UriFormat formatAs)
        {
            ...
            Label_0272:
                ...
                Label_04CE:
                    ...
                    System.Buffer.BlockCopy(dest, 0, chArray, 0, destinationIndex << 1);
        ...
        private static void GetConfig(ref UriIdnScope idnScope, ref bool iriParsing)
        {
            ...
            IdnScopeFromConfig config;
            IdnScopeFromConfig config2;
            IriParsingFromConfig config3;
            IriParsingFromConfig config4;
        ...
        internal string GetParts(UriComponents uriParts, UriFormat formatAs) => 
            this.GetComponents(uriParts, formatAs);
        ...
        private bool InFact(Flags flags) => 
            ((this.m_Flags & flags) != Flags.HostNotParsed);
        ...
        {
            if (!(AppDomain.CurrentDomain.GetData(".appVPath") is string) && ((appConfigFile == null) || (!appConfigFile.StartsWith("http://", true, CultureInfo.InvariantCulture) && !appConfigFile.StartsWith("https://", true, CultureInfo.InvariantCulture))))
            {
                return false;
            }
            return true;
        ...
        public static bool operator ==(Uri uri1, Uri uri2) => 
            ((uri1 == uri2) || (((uri1 != null) && (uri2 != null)) && uri2.Equals(uri1)));
        ...
        private static void ParseConfigFile(string file, out IdnScopeFromConfig idnStateConfig, out IriParsingFromConfig iriParsingConfig)
        {
            ...
            try
            {
                ...
                using (input)
                {
                    ...
                    using (reader)
                    {
                        if (reader.ReadToFollowing("configuration") && reader.ReadToFollowing("uri"))
                        {
                            do
                            {
                                if ((reader.NodeType == XmlNodeType.EndElement) && ConfigStringEqual(reader.Name, "uri"))
                                {
                                    return;
                                }
                                if (reader.NodeType == XmlNodeType.Element)
                                {
                                    ...
                                    {
                                        attribute = reader.GetAttribute("enabled");
                                        if (attribute != null)
                                        {
        ...
        private unsafe void ParseRemaining()
        {
            ...
            if (!this.UserDrivenParsing)
            {
                ...
                if (flag)
                {
                    ...
                    scheme = this.m_Info.Offset.Path;
                    ushort start = path;
                    if (this.IsImplicitFile || ((flags & (UriSyntaxFlags.MayHaveFragment | UriSyntaxFlags.MayHaveQuery)) == 0))
                    {
                ...
                fixed (char* str5 = ((char*) this.m_String))
                {
                    ...
                    {
                        none = this.CheckCanonical(chPtr2, ref scheme, length, ((flags & UriSyntaxFlags.MayHaveQuery) != 0) ? '?' : (this.m_Syntax.InFact(UriSyntaxFlags.MayHaveFragment) ? '#' : ((char) 0xfffe)));
                    }
                    if ((((this.m_Flags & Flags.AuthorityFound) != Flags.HostNotParsed) && ((flags & UriSyntaxFlags.PathIsRooted) != 0)) && ((this.m_Info.Offset.Path == length) || ((chPtr2[this.m_Info.Offset.Path] != '/') && (chPtr2[this.m_Info.Offset.Path] != '\\'))))
                    {
                        hostNotParsed |= Flags.FirstSlashAbsent;
                    }
                }
                bool flag2 = false;
                if (this.IsDosPath || (((this.m_Flags & Flags.AuthorityFound) != Flags.HostNotParsed) && ((flags & (UriSyntaxFlags.UnEscapeDotsAndSlashes | UriSyntaxFlags.CompressPath | UriSyntaxFlags.ConvertPathSlashes)) != 0)))
                {
                    if (((flags & UriSyntaxFlags.UnEscapeDotsAndSlashes) != 0) && ((none & Check.DotSlashEscaped) != Check.None))
                    {
                        hostNotParsed |= Flags.E_PathNotCanonical | Flags.PathNotCanonical;
                        flag2 = true;
                    }
                    if (((flags & UriSyntaxFlags.ConvertPathSlashes) != 0) && ((none & Check.BackslashInPath) != Check.None))
                    {
                        hostNotParsed |= Flags.E_PathNotCanonical | Flags.PathNotCanonical;
                        flag2 = true;
                    }
                    if (((flags & UriSyntaxFlags.CompressPath) != 0) && (((hostNotParsed & Flags.E_PathNotCanonical) != Flags.HostNotParsed) || ((none & Check.DotSlashAttn) != Check.None)))
                    {
                ...
                if (flag)
                {
                    ...
                    {
                        path = (ushort) (path + 1);
                        this.FindEndOfComponent(this.m_originalUnicodeString, ref path, (ushort) this.m_originalUnicodeString.Length, ((flags & UriSyntaxFlags.MayHaveFragment) != 0) ? '#' : ((char) 0xfffe));
                ...
                fixed (char* str6 = ((char*) this.m_String))
                {
                    ...
                    if ((scheme < length) && (chPtr3[scheme] == '?'))
                    {
                        ...
                        none = this.CheckCanonical(chPtr3, ref scheme, length, ((flags & UriSyntaxFlags.MayHaveFragment) != 0) ? '#' : ((char) 0xfffe));
                        if ((none & Check.DisplayCanonical) == Check.None)
                        {
        ...
        private static unsafe ushort ParseSchemeCheckImplicitFile(char* uriString, ushort length, ref ParsingError err, ref Flags flags, ref UriParser syntax)
        {
            ...
            if (((IntPtr.Size != 4) || (num2 == length)) || ((num2 < (index + 3)) || !CheckKnownSchemes((long*) (uriString + index), (ushort) (num2 - index), ref syntax)))
            {
                ...
                char* ptr = (char*) stackalloc byte[(2 * (num2 - index))];
                length = 0;
                while (index < num2)
                {
                    length = (ushort) (length + 1);
        ...
        private unsafe ParsingError PrivateParseMinimal()
        {
            ...
            if ((s_IdnScope != UriIdnScope.None) || this.m_iriParsing)
            {
        ...
        private static bool StaticInFact(Flags allFlags, Flags checkFlags) => 
            ((allFlags & checkFlags) != Flags.HostNotParsed);

        private static bool StaticIsFile(UriParser syntax) => 
            syntax.InFact(UriSyntaxFlags.FileLikeUri);
        ...
        private bool AllowIdn
        {
            get
            {
                if ((this.m_Syntax == null) || ((this.m_Syntax.Flags & UriSyntaxFlags.AllowIdn) == 0))
                {
                    return false;
                }
                return ((s_IdnScope == UriIdnScope.All) || ((s_IdnScope == UriIdnScope.AllExceptIntranet) && this.NotAny(Flags.HostNotParsed | Flags.IntranetUri)));
            }
        }
        ...
        public string DnsSafeHost
        {
            get
            {
                ...
                if ((this.HostType == Flags.BasicHostType) && this.InFact(Flags.E_HostNotCanonical | Flags.HostNotCanonical))
                {
        ...
        internal bool HasAuthority =>
            this.InFact(Flags.AuthorityFound);
        ...
        private Flags HostType =>
            (this.m_Flags & (Flags.BasicHostType | Flags.IPv4HostType));
        ...
        private bool IsDosPath =>
            ((this.m_Flags & Flags.DosPath) != Flags.HostNotParsed);
        ...
        private bool IsImplicitFile =>
            ((this.m_Flags & (Flags.HostNotParsed | Flags.ImplicitFile)) != Flags.HostNotParsed);
        ...
        private bool IsUncOrDosPath =>
            ((this.m_Flags & (Flags.DosPath | Flags.UncPath)) != Flags.HostNotParsed);

        private bool IsUncPath =>
            ((this.m_Flags & (Flags.HostNotParsed | Flags.UncPath)) != Flags.HostNotParsed);
        ...
        internal bool UserDrivenParsing =>
            ((this.m_Flags & (Flags.HostNotParsed | Flags.UserDrivenParsing)) != Flags.HostNotParsed);

        public bool UserEscaped =>
            this.InFact(Flags.HostNotParsed | Flags.UserEscaped);
