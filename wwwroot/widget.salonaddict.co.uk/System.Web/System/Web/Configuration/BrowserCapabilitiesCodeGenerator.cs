namespace System.Web.Configuration
{
    ...
    public class BrowserCapabilitiesCodeGenerator
    {
        ...
        private void GenerateAssembly()
        {
            ...
            declaration = new CodeAttributeDeclaration("System.Reflection.AssemblyVersion", new CodeAttributeArgument[] { new CodeAttributeArgument(new CodePrimitiveExpression("2.0.0.0")) });
            compileUnit.AssemblyCustomAttributes.Add(declaration);
            CodeNamespace namespace2 = new CodeNamespace("ASP") {
                Imports = { 
                    new CodeNamespaceImport("System"),
                    new CodeNamespaceImport("System.Web"),
                    new CodeNamespaceImport("System.Web.Configuration"),
            ...
            bool debug = RuntimeConfig.GetAppConfig().Compilation.Debug;
                ...
                foreach (BrowserDefinition definition in this._browserDefinitionCollection)
                {
                    if (definition.IsDeviceNode)
                    {
                        CodeAssignStatement statement = new CodeAssignStatement {
                            ...
                            Right = new CodeObjectCreateExpression(typeof(Triplet), new CodeExpression[] { 
                                new CodePrimitiveExpression(definition.ParentName),
                                new CodePropertyReferenceExpression(new CodeTypeReferenceExpression(typeof(string)), "Empty"),
                                new CodePrimitiveExpression(definition.Depth)
                            })
                        };
                        method.Statements.Add(statement);
                    }
                    ...
                    foreach (BrowserDefinition definition2 in (BrowserDefinitionCollection) this._customBrowserDefinitionCollections[i])
                    {
                        if (definition2.IsDeviceNode)
                        {
                            CodeAssignStatement statement2 = new CodeAssignStatement {
                                ...
                                Right = new CodeObjectCreateExpression(typeof(Triplet), new CodeExpression[] { 
                                    new CodePrimitiveExpression(definition2.ParentName),
                                    new CodePropertyReferenceExpression(new CodeTypeReferenceExpression(typeof(string)), "Empty"),
                                    new CodePrimitiveExpression(definition2.Depth)
                                })
                            };
        ...
        internal CodeStatementCollection GenerateTrackedSingleProcessCall(CodeStatementCollection stmts, BrowserDefinition bd, CodeMemberMethod cmm) => 
            this.GenerateTrackedSingleProcessCall(stmts, bd, cmm, string.Empty);
        ...
        protected void ProcessBrowserFiles(bool useVirtualPath, string virtualDir)
        {
            ...
            foreach (string str5 in this._browserFileList)
            {
                ...
                catch (XmlSchemaException exception2)
                {
                    ...
                    int startIndex = str5.LastIndexOf(@"\", StringComparison.Ordinal);
                    if (startIndex > -1)
                    {
                        str5.Substring(startIndex);
                    }
            ...
            for (int k = 0; k < this._customBrowserFileLists.Count; k++)
            {
                ...
                foreach (string str in list2)
                {
                    ...
                    catch (XmlSchemaException exception2)
                    {
                        ...
                        int startIndex = str.LastIndexOf(@"\", StringComparison.Ordinal);
                        if (startIndex > -1)
                        {
        ...
        internal bool UninstallInternal()
        {
            ...
            if (!util.GacUnInstall("ASP.BrowserCapsFactory, Version=2.0.0.0, Culture=neutral"))
            {
                return false;
            }
            return true;
        ...
        internal ArrayList CustomTreeList =>
            this._customTreeList;

        internal ArrayList CustomTreeNames =>
            this._customTreeNames;
