namespace System.Web.Configuration
{
    ...
    internal class HttpConfigurationSystem : IInternalConfigSystem
    {
            ...
            if (!s_inited)
            {
                lock (s_initLock)
                {
                    if (!s_inited)
                    {
                        ...
                        s_configSystem = (IConfigSystem) Activator.CreateInstance(Type.GetType("System.Configuration.Internal.ConfigSystem, System.Configuration, Version=2.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a", true), true);
                        ...
                        s_configSettingsFactory = (IInternalConfigSettingsFactory) Activator.CreateInstance(Type.GetType("System.Configuration.Internal.InternalConfigSettingsFactory, System.Configuration, Version=2.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a", true), true);
        ...
        internal static object GetApplicationSection(string sectionName) => 
            CachedPathData.GetApplicationPathData().ConfigRecord.GetSection(sectionName);
        ...
        internal static object GetSection(string sectionName, string path) => 
            GetSection(sectionName, VirtualPath.CreateNonRelativeAllowNull(path));

        internal static object GetSection(string sectionName, VirtualPath path) => 
            CachedPathData.GetVirtualPathData(path, true).ConfigRecord.GetSection(sectionName);
