namespace System.Web.Configuration
{
    ...
    internal sealed class ProcessHostServerConfig : IServerConfig
    {
        ...
        internal bool IsWithinApp(string virtualPath) => 
            UnsafeIISMethods.MgdIsWithinApp(this._siteNameForCurrentApplication, HttpRuntime.AppDomainAppVirtualPathString, virtualPath);
        ...
        bool IServerConfig.GetUncUser(IApplicationHost appHost, VirtualPath path, out string username, out string password)
        {
            ...
            try
            {
                if (UnsafeIISMethods.MgdGetVrPathCreds(appHost.GetSiteName(), path.VirtualPathString, out zero, out cchUserName, out bstrPassword, out cchPassword) == 0)
                {
        ...
        string[] IServerConfig.GetVirtualSubdirs(VirtualPath path, bool inApp)
        {
            ...
            {
                int count = 0;
                int num4 = UnsafeIISMethods.MgdGetAppCollection(this._siteNameForCurrentApplication, virtualPathString, out bstrPath, out cchPath, out zero, out count);
