namespace System.Web.Configuration
{
    ...
    internal static class ProcessHostConfigUtils
    {
        ...
        internal static string GetSiteNameFromId(uint siteId)
        {
            ...
            try
            {
                str = ((UnsafeIISMethods.MgdGetSiteNameFromId(siteId, out zero, out cchSiteName) == 0) && (zero != IntPtr.Zero)) ? StringUtil.StringFromWCharPtr(zero, cchSiteName) : string.Empty;
            }
            finally
            {
                if (zero != IntPtr.Zero)
        ...
        {
            if ((!HostingEnvironment.IsUnderIISProcess && !ServerConfig.UseMetabase) && (Interlocked.Exchange(ref s_InitedExternalConfig, 1) == 0))
            {
        ...
        internal static string MapPathActual(string siteName, VirtualPath path)
        {
            ...
            try
            {
                if (UnsafeIISMethods.MgdMapPathDirect(siteName, path.VirtualPathString, out zero, out cchPath) < 0)
                {
