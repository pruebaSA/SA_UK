    ...
    public sealed class IdentitySection : ConfigurationSection
    {
        ...
        internal static IntPtr CreateUserToken(string name, string password, out string error)
        {
            ...
            if (VersionInfo.ExeName == "aspnet_wp")
            {
                ...
                if (System.Web.UnsafeNativeMethods.PMCallISAPI(IntPtr.Zero, System.Web.UnsafeNativeMethods.CallISAPIFunc.GenerateToken, dst, dst.Length, bufferOut, bufferOut.Length) == 1)
                {
                    long num = 0L;
            ...
            if (zero == IntPtr.Zero)
            {
                ...
                zero = System.Web.UnsafeNativeMethods.CreateUserToken(name, password, 1, strError, 0x100);
