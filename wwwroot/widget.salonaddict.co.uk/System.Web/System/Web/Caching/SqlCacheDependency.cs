    ...
    public sealed class SqlCacheDependency : CacheDependency
    {
        ...
        internal static CacheDependency CreateOutputCacheDependency(string depString)
        {
            ...
            if (StringUtil.EqualsIgnoreCase(depString, "CommandNotification"))
            {
                ...
                current.SqlDependencyCookie = dependency._sqlYukonDep.Id;
                return dependency;
            }
            AggregateCacheDependency dependency2 = null;
            ArrayList list = ParseSql7OutputCacheDependency(depString);
            if (list.Count == 1)
            {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                dependency2.Add(new CacheDependency[] { CreateSql7SqlCacheDependencyForOutputCache(info._database, info._table, depString) });
            }
            return dependency2;
        }
