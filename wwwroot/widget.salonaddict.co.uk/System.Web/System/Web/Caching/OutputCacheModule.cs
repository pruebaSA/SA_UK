namespace System.Web.Caching
{
    ...
    internal sealed class OutputCacheModule : IHttpModule
    {
        ...
        private string CreateOutputCachedItemKey(HttpContext context, CachedVary cachedVary) => 
            CreateOutputCachedItemKey(context.Request.Path, context.Request.HttpVerb, context, cachedVary);
                ...
                for (int i = 0; i <= 2; i++)
                {
                    ...
                    switch (i)
                    {
                        ...
                        case 1:
                            ...
                            {
                                serverVariables = request.QueryString;
                        ...
                        default:
                            ...
                            if (verb == HttpVerb.POST)
                            {
                                ...
                                {
                                    serverVariables = request.Form;
                                    flag = cachedVary._varyByAllParams;
                                }
                            }
                            break;
                    }
                    if (flag && (serverVariables.Count > 0))
                    {
                        array = serverVariables.AllKeys;
                        num = array.Length - 1;
                        while (num >= 0)
                        {
                            if (array[num] != null)
                    ...
                    if (array != null)
                    {
                        ...
                        while (num < length)
                        {
                            ...
                            {
                                varyByCustomString = serverVariables[str];
        ...
        internal bool InsertResponse(HttpResponse response, HttpContext context, string keyRawResponse, HttpCachePolicySettings settings, CachedVary cachedVary, CachedRawResponse memoryRawResponse)
        {
            ...
            {
                utcTimestampCreated = settings.UtcTimestampCreated;
            }
            else
            {
                utcTimestampCreated = context.UtcTimestamp;
            }
            DateTime maxValue = DateTime.MaxValue;
            if (settings.SlidingExpiration)
            {
                slidingDelta = settings.SlidingDelta;
                maxValue = Cache.NoAbsoluteExpiration;
            }
            else
            {
                ...
                {
                    maxValue = utcTimestampCreated + settings.MaxAge;
                }
                else if (settings.IsExpiresSet)
                {
                    maxValue = settings.UtcExpires;
                }
            }
            if (maxValue < DateTime.UtcNow)
            {
                return false;
            }
            if (cachedVary == null)
            ...
            {
                CachedVary vary = (CachedVary) cacheInternal.UtcAdd(this._key, cachedVary, null, Cache.NoAbsoluteExpiration, Cache.NoSlidingExpiration, CacheItemPriority.Normal, null);
                if (vary != null)
                {
                ...
                dependency = new CacheDependency(0, null, new string[] { this._key });
            }
            CacheDependency dependencies = response.CreateCacheDependencyForResponse(dependency);
            Interlocked.Increment(ref s_cEntries);
            try
            {
                cacheInternal.UtcInsert(keyRawResponse, memoryRawResponse, dependencies, maxValue, slidingDelta, CacheItemPriority.Normal, s_cacheItemRemovedCallback);
            ...
            if (s_cEntries > 0)
            {
                ...
                switch (request.HttpVerb)
                {
                    ...
                    case HttpVerb.POST:
                    {
                        ...
                        CacheInternal cacheInternal = HttpRuntime.CacheInternal;
                        object obj2 = cacheInternal.Get(str);
                        if (obj2 != null)
                        {
                            int num;
                            int length;
                                ...
                                else
                                {
                                    ...
                                    if (knownRequestHeader != null)
                                    {
                                        ...
                                        while (!flag4)
                                        {
                                            ...
                                            {
                                                flag3 = false;
                                                obj2 = cacheInternal.Get(str + contentEncodings[index]);
                                                if (obj2 == null)
                                                {
                                                    startIndex = index + 1;
        ...
        internal void OnLeave(object source, EventArgs eventArgs)
        {
            ...
            bool flag = false;
            if (response3.HasCachePolicy)
            {
                cache = response3.Cache;
                if (((cache.IsModified() && (response3.Cookies.Count <= 0)) && (response3.StatusCode == 200)) && (((request.HttpVerb == HttpVerb.GET) || (request.HttpVerb == HttpVerb.POST)) && response3.IsBuffered()))
                {
                    bool flag3 = false;
            ...
            if (flag)
            {
                ...
                string[] varyByParams;
                this.RecordCacheMiss();
                HttpCachePolicySettings currentSettings = cache.GetCurrentSettings(response3);
                ...
                else
                {
                    ...
                    if (!response3.IsBuffered())
                    {
                        return;
                    }
                }
                HttpRawResponse snapshot = response3.GetSnapshot();
                string kernelCacheUrl = response3.SetupKernelCaching(null);
                CachedRawResponse memoryRawResponse = new CachedRawResponse(snapshot, currentSettings, kernelCacheUrl);
                this.InsertResponse(response3, context, str, currentSettings, vary, memoryRawResponse);
                this._key = null;
            }
        }
