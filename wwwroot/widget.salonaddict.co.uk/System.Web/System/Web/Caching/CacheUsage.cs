        ...
        internal int FlushUnderUsedItems(int toFlush, ref int publicEntriesFlushed)
        {
            ...
            if (Interlocked.Exchange(ref this._inFlush, 1) == 0)
            {
                try
                {
                    ...
                    {
                        int num2 = bucket.FlushUnderUsedItems(toFlush - num, false, ref publicEntriesFlushed);
                        num += num2;
                        if (num >= toFlush)
                        {
                            break;
                    ...
                    {
                        int num3 = bucket2.FlushUnderUsedItems(toFlush - num, true, ref publicEntriesFlushed);
                        num += num3;
                        if (num >= toFlush)
                        {
                            return num;
