        ...
        internal static void InitPolling(string database)
        {
            ...
            if (!s_DatabaseNotifStates.ContainsKey(database))
            {
                ...
                lock (s_DatabaseNotifStates)
                {
                    if (!s_DatabaseNotifStates.ContainsKey(database))
                    {
                        DatabaseNotifState state = new DatabaseNotifState(database, connection, databaseConfig.PollTime);
                        state._timer = new Timer(s_timerCallback, state, 0, databaseConfig.PollTime);
