namespace System.Web
{
    ...
    public sealed class HttpRuntime
    {
        ...
        internal static byte[] s_autogenKeys = new byte[0x58];
        ...
        private void CoalesceNotifications()
        {
            ...
            if ((waitChangeNotification != 0) && (maxWaitChangeNotification != 0))
            {
                ...
                try
                {
                    ...
                    {
                        if (DateTime.UtcNow > this.LastShutdownAttemptTime.AddSeconds((double) waitChangeNotification))
                        {
                            return;
                        }
                        Thread.Sleep(250);
        ...
        internal static object CreateNonPublicInstance(Type type) => 
            CreateNonPublicInstance(type, null);

        internal static object CreateNonPublicInstance(Type type, object[] args) => 
            Activator.CreateInstance(type, BindingFlags.CreateInstance | BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance, null, args, null);
        ...
        internal static bool HasAppPathDiscoveryPermission() => 
            HasPathDiscoveryPermission(AppDomainAppPathInternal);
        ...
        internal static bool IsPathWithinAppRoot(string path) => 
            ((AppDomainIdInternal == null) || System.Web.Util.UrlPath.IsEqualOrSubpath(AppDomainAppVirtualPathString, path));
        ...
        internal static RequestNotificationStatus ProcessRequestNotification(IIS7WorkerRequest wr, HttpContext context) => 
            _theRuntime.ProcessRequestNotificationPrivate(wr, context);
        ...
        private static void SetAutogenKeys()
        {
            ...
            if (flag)
            {
                System.Buffer.BlockCopy(bufferOut, 0, s_autogenKeys, 0, s_autogenKeys.Length);
            }
            else
            {
                System.Buffer.BlockCopy(data, 0, s_autogenKeys, 0, s_autogenKeys.Length);
            }
        }
        ...
        private void SetTrustLevel(TrustSection trustSection, SecurityPolicySection securityPolicySection)
        {
            ...
            else
            {
                ...
                PolicyLevel domainPolicy = CreatePolicyLevel(policyFileExpanded, AppDomainAppPathInternal, CodegenDirInternal, trustSection.OriginUrl, out foundGacToken);
                if (foundGacToken)
                {
                ...
                this._namedPermissionSet = domainPolicy.GetNamedPermissionSet("ASP.Net");
                this._trustLevel = str;
        ...
        internal static bool ShutdownAppDomain(ApplicationShutdownReason reason, string message) => 
            ShutdownAppDomainWithStackTrace(reason, message, null);
        ...
        private static void StaticInit()
        {
            if (!s_initialized)
            {
                ...
                if (System.Web.UnsafeNativeMethods.GetModuleHandle("webengine.dll") != IntPtr.Zero)
                {
                    flag = true;
                }
                if (!flag && (System.Web.UnsafeNativeMethods.LoadLibrary(runtimeDirectory + Path.DirectorySeparatorChar + "webengine.dll") != IntPtr.Zero))
                {
                    flag = true;
                    flag2 = true;
                }
                if (flag)
                {
                    System.Web.UnsafeNativeMethods.InitializeLibrary();
                    if (flag2)
                    {
        ...
        internal static string AppDomainAppPathInternal =>
            _theRuntime._appDomainAppPath;

        public static string AppDomainAppVirtualPath =>
            VirtualPath.GetVirtualPathStringNoTrailingSlash(_theRuntime._appDomainAppVPath);
        ...
        internal static string AppDomainAppVirtualPathString =>
            VirtualPath.GetVirtualPathString(_theRuntime._appDomainAppVPath);

        public static string AppDomainId =>
            AppDomainIdInternal;

        internal static string AppDomainIdInternal =>
            _theRuntime._appDomainId;

        internal static byte[] AppOfflineMessage =>
            _theRuntime._appOfflineMessage;
        ...
        internal static string BinDirectoryInternal =>
            (Path.Combine(_theRuntime._appDomainAppPath, "bin") + Path.DirectorySeparatorChar);
        ...
        internal static string ClrInstallDirectoryInternal =>
            HttpConfigurationSystem.MsCorLibDirectory;

        internal static VirtualPath CodeDirectoryVirtualPath =>
            _theRuntime._appDomainAppVPath.SimpleCombineWithDir("App_Code");
        ...
        internal static VirtualPath WebRefDirectoryVirtualPath =>
            _theRuntime._appDomainAppVPath.SimpleCombineWithDir("App_WebReferences");

        internal static string WpUserId =>
            _theRuntime._wpUserId;
    }
}
