namespace System.Web.UI.WebControls
{
    ...
    public abstract class BaseCompareValidator : BaseValidator
    {
        ...
        protected static bool Compare(string leftText, string rightText, ValidationCompareOperator op, ValidationDataType type) => 
            Compare(leftText, false, rightText, false, op, type);
        ...
        protected static bool Convert(string text, ValidationDataType type, out object value) => 
            Convert(text, type, false, out value);
        ...
        protected static int GetFullYear(int shortYear) => 
            DateTimeFormatInfo.CurrentInfo.Calendar.ToFourDigitYear(shortYear);

        internal bool IsInStandardDateFormat(string date) => 
            Regex.Match(date, @"^\s*(\d+)([-/]|\. ?)(\d+)\2(\d+)\s*$").Success;

        [WebSysDescription("BaseCompareValidator_CultureInvariantValues"), DefaultValue(false), WebCategory("Behavior"), Themeable(false)]
        ...
        protected static int CutoffYear =>
            DateTimeFormatInfo.CurrentInfo.Calendar.TwoDigitYearMax;

        [Themeable(false), DefaultValue(0), WebSysDescription("RangeValidator_Type"), WebCategory("Behavior")]
