namespace System.Web.UI
{
    ...
    public abstract class BasePartialCachingControl : Control
    {
        ...
        internal override void InitRecursive(Control namingContainer)
        {
            ...
            object obj2 = HttpRuntime.CacheInternal.Get(this._cacheKey);
            if (obj2 != null)
            {
                ControlCachedVary cachedVary = obj2 as ControlCachedVary;
                if (cachedVary != null)
                {
                    string key = this.ComputeVaryCacheKey(combinedHashCode, cachedVary);
                    entry = (PartialCachingCacheEntry) HttpRuntime.CacheInternal.Get(key);
                }
                else
                {
                    entry = (PartialCachingCacheEntry) obj2;
                }
            }
            if (entry == null)
            {
        ...
        protected internal override void Render(HtmlTextWriter output)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    ControlCachedVary cachedVary = new ControlCachedVary(varyByParams, this._varyByControlsCollection, this._varyByCustom);
                    ...
                    ControlCachedVary vary2 = (ControlCachedVary) cacheInternal.UtcAdd(this._cacheKey, cachedVary, null, Cache.NoAbsoluteExpiration, Cache.NoSlidingExpiration, CacheItemPriority.Normal, null);
                    if ((vary2 != null) && !cachedVary.Equals(vary2))
                    {
                        cacheInternal.UtcInsert(this._cacheKey, cachedVary);
                    }
                    CacheDependency dependency4 = new CacheDependency(0, null, new string[] { this._cacheKey });
                    if (dependencies == null)
                    {
                        dependencies = dependency4;
                    }
                    else
                    {
                        AggregateCacheDependency dependency5 = new AggregateCacheDependency();
                        dependency5.Add(new CacheDependency[] { dependencies, dependency4 });
                        dependencies = dependency5;
                    }
                }
                ...
                cacheInternal.UtcInsert(str, this._cacheEntry, dependencies, noAbsoluteExpiration, noSlidingExpiration);
            }
        }
