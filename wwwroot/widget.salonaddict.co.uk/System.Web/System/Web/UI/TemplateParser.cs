namespace System.Web.UI
{
    ...
    public abstract class TemplateParser : BaseParser, IAssemblyDependencyParser
    {
        ...
        internal Assembly AddAssemblyDependency(string assemblyName) => 
            this.AddAssemblyDependency(assemblyName, false);
                    ...
                    if (match.Success)
                    {
                        ...
                        if (match.Success)
                        {
                            ...
                            if (!match2.Success || (match2.Index >= match.Index))
                            {
                                ...
                                if ((str2 != input) && BaseParser.TagRegex.Match(str2).Success)
                                {
        ...
        private string GetLiteral() => 
            this._literalBuilder?.ToString();

        internal Type GetType(string typeName) => 
            this.GetType(typeName, false);

        internal Type GetType(string typeName, bool ignoreCase) => 
            this.GetType(typeName, ignoreCase, true);
        ...
        internal bool IsExpressionBuilderValue(string val) => 
            ControlBuilder.expressionBuilderRegex.Match(val, 0).Success;
        ...
        internal Type MapStringToType(string typeName, IDictionary attribs) => 
            this.RootBuilder.GetChildControlType(typeName, attribs);
        ...
        internal bool PageParserFilterProcessedDataBindingAttribute(string controlId, string attributeName, string code) => 
            (((this._pageParserFilter != null) && (this.CompilationMode != System.Web.UI.CompilationMode.Never)) && this._pageParserFilter.ProcessDataBindingAttribute(controlId, attributeName, code));

        internal bool PageParserFilterProcessedEventHookupAttribute(string controlId, string eventName, string handlerName) => 
            (((this._pageParserFilter != null) && (this.CompilationMode != System.Web.UI.CompilationMode.Never)) && this._pageParserFilter.ProcessEventHookup(controlId, eventName, handlerName));
        ...
        Label_000B:
            ...
            else
            {
                ...
                if (!(match = BaseParser.commentRegex.Match(text, startat)).Success)
                {
                    ...
                    {
                        if ((!this.flags[2] && (num2 > startat)) && (match = BaseParser.TagRegex.Match(text, startat)).Success)
                        {
                            try
                            {
        ...
        private bool ProcessBeginTag(Match match, string inputText)
        {
            ...
            else if (this.flags[1] && (((PartialCachingAttribute) TypeDescriptor.GetAttributes(childType)[typeof(PartialCachingAttribute)]) != null))
            {
        ...
        internal CompilerParameters CompilParams =>
            this._compilerType.CompilerParameters;
        ...
        internal virtual Type DefaultFileLevelBuilderType =>
            typeof(System.Web.UI.RootBuilder);
        ...
        {
            get => 
                this._designTimeDataBindHandler;
            set
            {
        ...
        private bool HasParserErrors =>
            ((this._parserErrors != null) && (this._parserErrors.Count > 0));
        ...
        internal Hashtable NamespaceEntries =>
            this._namespaceEntries;

        internal ArrayList PageObjectList =>
            this._pageObjectList;
        ...
        internal HttpStaticObjectsCollection SessionObjects =>
            this._sessionObjects;

        internal StringSet SourceDependencies =>
            this._sourceDependencies;
        ...
        internal List<TagNamespaceRegisterEntry> TagRegisterEntries =>
            this.TypeMapper.TagRegisterEntries;
