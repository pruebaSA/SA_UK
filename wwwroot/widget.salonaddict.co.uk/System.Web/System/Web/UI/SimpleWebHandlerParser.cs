namespace System.Web.UI
{
    ...
    public abstract class SimpleWebHandlerParser : IAssemblyDependencyParser
    {
        ...
        internal IDictionary GetLinePragmasTable()
        {
            ...
            return new Hashtable { [this._lineNumber] = info };
        }
        ...
        internal virtual bool IsMainDirective(string directiveName) => 
            (string.Compare(directiveName, this.DefaultDirectiveName, StringComparison.OrdinalIgnoreCase) == 0);
        ...
        private static Regex DirectiveRegex
        {
            get
            {
                if (_directiveRegex == null)
                {
                    ...
                    {
                        _directiveRegex = (Regex) BaseParser.CreateInstanceNoException("System.Web.RegularExpressions.SimpleDirectiveRegex40");
                    }
                    if (_directiveRegex == null)
                    {
                        _directiveRegex = new SimpleDirectiveRegex();
                    }
                }
