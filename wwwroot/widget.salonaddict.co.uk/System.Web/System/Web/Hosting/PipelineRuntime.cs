namespace System.Web.Hosting
{
    ...
    internal sealed class PipelineRuntime : MarshalByRefObject, IPipelineRuntime, IRegisteredObject
    {
        ...
        [AspNetHostingPermission(SecurityAction.Demand, Level=AspNetHostingPermissionLevel.Minimal), SecurityPermission(SecurityAction.Demand, UnmanagedCode=true)]
        ...
        internal static bool RoleHandler(IntPtr pManagedPrincipal, IntPtr pszRole, int cchRole, bool disposing)
        {
            ...
            if (target != null)
            {
                if (!disposing)
                {
                    return target.IsInRole(StringUtil.StringFromWCharPtr(pszRole, cchRole));
                }
        ...
        internal static bool ShouldRethrowException(Exception ex) => 
            ((((ex is NullReferenceException) || (ex is AccessViolationException)) || ((ex is StackOverflowException) || (ex is OutOfMemoryException))) || (ex is ThreadAbortException));
        ...
        private static HttpContext UnwrapContext(IntPtr contextPtr) => 
            ((HttpContext) GCHandle.FromIntPtr(contextPtr).Target);
        ...
        internal bool HostingShutdownInitiated =>
            HostingEnvironment.ShutdownInitiated;
    }
}
