namespace System.Web.Util
{
    ...
    internal class FactoryGenerator
    {
        ...
        internal IWebObjectFactory CreateFactory(Type type) => 
            ((IWebObjectFactory) Activator.CreateInstance(this.GetFactoryTypeWithAssert(type)));
            ...
            if (this._dynamicModule == null)
            {
                lock (this)
                {
                    if (this._dynamicModule == null)
                    {
                        ...
                        this._dynamicModule = Thread.GetDomain().DefineDynamicAssembly(name, AssemblyBuilderAccess.Run, null, null, null, null, null, true).DefineDynamicModule("M_" + uniqueCompilationName);
                    }
                }
            }
