namespace System.Web
{
    ...
    public sealed class HttpCachePolicy
    {
        ...
        internal bool HasValidationPolicy() => 
            (((this._generateLastModifiedFromFiles || this._generateEtagFromFiles) || (this._validationCallbackInfo != null)) || ((this._validUntilExpires == 1) && (this._slidingExpiration != 1)));
                ...
                switch (@private)
                {
                    ...
                    default:
                        ...
                        if (@private != HttpCacheability.Private)
                        {
                            ...
                            if (!omitVaryStar && (((@private == HttpCacheability.ServerAndPrivate) || (this._varyByCustom != null)) || (this._varyByParams.IsModified() && !this._varyByParams.IgnoreParams)))
                            {
                                str3 = "*";
                            }
                            if (str3 == null)
        ...
        private void UpdateFromDependencies(HttpResponse response)
        {
            ...
            if (this._generateLastModifiedFromFiles || this._generateEtagFromFiles)
            {
                ...
                {
                    DateTime utcDate = this._utcLastModified;
                    if (this._generateEtagFromFiles)
                    {
                        builder = new StringBuilder(HttpRuntime.AppDomainIdInternal);
                    }
                    if (fileDependencies != null)
                    {
                        FileChangesMonitor fileChangesMonitor = HttpRuntime.FileChangesMonitor;
                        foreach (string str in fileDependencies)
                        {
                            FileAttributesData fileAttributes = fileChangesMonitor.GetFileAttributes(str);
                            if (fileAttributes == null)
                            {
                                fileAttributes = FileAttributesData.NonExistantAttributesData;
                            }
                            if (utcDate < fileAttributes.UtcLastWriteTime)
                            {
                                utcDate = fileAttributes.UtcLastWriteTime;
                            }
                            if (this._generateEtagFromFiles)
                            {
                                builder.Append(str);
                                builder.Append(fileAttributes.UtcLastWriteTime.Ticks.ToString(CultureInfo.InvariantCulture));
                                builder.Append(fileAttributes.FileSize.ToString(CultureInfo.InvariantCulture));
                            }
                        }
                    }
                    if (cacheItemDependencies != null)
                    {
                        ...
                        foreach (string str2 in cacheItemDependencies)
                        {
                            CacheEntry entry = (CacheEntry) cache.Get(str2, CacheGetOptions.ReturnCacheEntry);
                            if (entry != null)
                            {
                                DateTime utcCreated = entry.UtcCreated;
                                if (utcDate < utcCreated)
                                {
                                    utcDate = utcCreated;
                                }
                                if (this._generateEtagFromFiles)
                                {
                                    builder.Append(str2);
                                    builder.Append(utcCreated.Ticks.ToString(CultureInfo.InvariantCulture));
                                }
                            }
                        }
                    }
                    ...
                    {
                        if (utcDate < cacheDependency.UtcLastModified)
                        {
                            utcDate = cacheDependency.UtcLastModified;
                        }
                        if (this._generateEtagFromFiles)
                        {
                    ...
                    {
                        if (utcDate < virtualPathDependency.UtcLastModified)
                        {
                            utcDate = virtualPathDependency.UtcLastModified;
                        }
                        if (this._generateEtagFromFiles)
                        {
                            string str4 = virtualPathDependency.GetUniqueID();
                            if (str4 == null)
                            {
                                throw new HttpException(System.Web.SR.GetString("No_UniqueId_Cache_Dependency"));
                            }
                    ...
                    {
                        builder.Append("+LM");
                        builder.Append(utcDate.Ticks.ToString(CultureInfo.InvariantCulture));
