namespace System.Web.Compilation
{
    ...
    internal class BuildManagerHost : MarshalByRefObject, IRegisteredObject
    {
        ...
        {
            Util.AddAssembliesToStringCollection(BuildManager.GetReferencedAssemblies(RuntimeConfig.GetConfig(virtualPath).Compilation), compilerParameters.ReferencedAssemblies);
        }
        ...
        private System.Web.Compilation.BuildProvider GetCompilerParamsAndBuildProvider(VirtualPath virtualPath, out Type codeDomProviderType, out CompilerParameters compilerParameters)
        {
            ...
            {
                provider = BuildManager.CreateBuildProvider(virtualPath, compilation, referencedAssemblies, true);
            }
        ...
        internal string[] GetTopLevelAssemblyReferences(VirtualPath virtualPath)
        {
            ...
            {
                virtualPath.CombineWithAppRoot();
                foreach (AssemblyInfo info in RuntimeConfig.GetConfig(virtualPath).Compilation.Assemblies)
                {
                    ...
                    for (int i = 0; i < assemblyInternal.Length; i++)
                    {
                        ...
                        {
                            list.Add(Util.GetAssemblyCodeBase(assemblyInternal[i]));
                        }
                    }
                }
            }
        ...
        internal bool IsCodeAssembly(string assemblyName) => 
            (BuildManager.GetNormalizedCodeAssemblyName(assemblyName) != null);
        ...
        internal IApplicationHost ApplicationHost =>
            HostingEnvironment.ApplicationHost;
        ...
        internal class ClientVirtualPathProvider : VirtualPathProvider
        {
            ...
            public override bool FileExists(string virtualPath) => 
                (this._stringDictionary.Contains(virtualPath) || base.FileExists(virtualPath));
