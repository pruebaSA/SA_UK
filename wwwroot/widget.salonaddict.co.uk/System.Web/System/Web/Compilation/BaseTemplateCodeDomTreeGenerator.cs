namespace System.Web.Compilation
{
    ...
    internal abstract class BaseTemplateCodeDomTreeGenerator : BaseCodeDomTreeGenerator
    {
        ...
        protected CodeMemberMethod BuildBuildMethod(ControlBuilder builder, bool fTemplate, bool fInTemplate, bool topLevelControlInTemplate, PropertyEntry pse, bool fControlSkin)
        {
            ...
            else
            {
                ...
                if (builder.ControlType != null)
                {
                    ...
                    {
                        method.ReturnType = new CodeTypeReference(builder.ControlType, CodeTypeReferenceOptions.GlobalReference);
                    }
                }
        ...
        private void BuildBuildMethodInternal(ControlBuilder builder, Type ctrlType, bool fInTemplate, bool topLevelControlInTemplate, PropertyEntry pse, CodeStatementCollection statements, bool fStandardControl, bool fControlFieldDeclared, string deviceFilter, bool fControlSkin)
        {
            ...
            {
                CodeTypeReference createType = new CodeTypeReference(ctrlType, CodeTypeReferenceOptions.GlobalReference);
            ...
            if (builder is ContentPlaceHolderBuilder)
            {
                ...
                CodeExpression targetObject = new CodeFieldReferenceExpression(new CodeThisReferenceExpression(), fieldName);
                if (builder is ContentPlaceHolderBuilder)
                {
                    CodePropertyReferenceExpression expression11 = new CodePropertyReferenceExpression(new CodeThisReferenceExpression(), "ContentTemplates");
                    CodeAssignStatement statement8 = new CodeAssignStatement {
                        Left = targetObject,
                        Right = new CodeCastExpression(typeof(ITemplate), new CodeIndexerExpression(expression11, new CodeExpression[] { new CodePrimitiveExpression(name) }))
                    };
                    CodeConditionStatement statement9 = new CodeConditionStatement();
                    CodeBinaryOperatorExpression expression12 = new CodeBinaryOperatorExpression(expression11, CodeBinaryOperatorType.IdentityInequality, new CodePrimitiveExpression(null));
                    CodeMethodInvokeExpression expression13 = new CodeMethodInvokeExpression(expression11, "Remove", new CodeExpression[0]);
                ...
                expression9 = new CodeMethodInvokeExpression(targetObject, "InstantiateIn", new CodeExpression[0]);
                expression9.Parameters.Add(expression3);
                statement7 = new CodeConditionStatement {
                    Condition = new CodeBinaryOperatorExpression(targetObject, CodeBinaryOperatorType.IdentityInequality, new CodePrimitiveExpression(null))
                };
                ...
                foreach (object obj3 in builder.SubBuilders)
                {
                    if (((obj3 is ControlBuilder) && !(obj3 is CodeBlockBuilder)) && !(obj3 is ContentBuilderInternal))
                    {
                        ...
                        if (attribute2 == null)
                        {
                            ...
                            CodeTypeReference type = new CodeTypeReference(builder3.ControlType, CodeTypeReferenceOptions.GlobalReference);
                        ...
                        else
                        {
                            CodeMethodInvokeExpression expression18 = new CodeMethodInvokeExpression {
                                Method = { 
                                    TargetObject = new CodeTypeReferenceExpression(typeof(StaticPartialCachingControl)),
                                    MethodName = "BuildCachedControl"
                                }
                            };
                    ...
                    else if (((obj3 is string) && !builder.HasAspCode) && (!fControlSkin || !builder.AllowWhitespaceLiterals()))
                    {
                        ...
                        else
                        {
                            ...
                            int num4;
                            bool flag2;
                            base._stringResourceBuilder.AddString(s, out num3, out num4, out flag2);
                            ...
                            expression2.Parameters.Add(new CodePrimitiveExpression(flag2));
            ...
            if (builder.ComplexPropertyEntries.Count > 0)
            {
                ...
                foreach (ComplexPropertyEntry entry8 in builder.ComplexPropertyEntries)
                {
                    ...
                    {
                        str10 = "__ctrl" + num5++.ToString(CultureInfo.InvariantCulture);
                        CodeTypeReference reference3 = new CodeTypeReference(entry8.Builder.ControlType, CodeTypeReferenceOptions.GlobalReference);
                        statements9.Add(new CodeVariableDeclarationStatement(reference3, str10));
                        CodeVariableReferenceExpression expression25 = new CodeVariableReferenceExpression(str10);
                        expression2 = new CodeMethodInvokeExpression(new CodeThisReferenceExpression(), buildMethodPrefix + entry8.Builder.ID, new CodeExpression[0]);
                        statement = new CodeExpressionStatement(expression2);
            ...
            if (builder.BoundPropertyEntries.Count > 0)
            {
                ...
                {
                    if (!entry10.TwoWayBound || (!flag3 && !entry10.ReadOnlyProperty))
                    {
            ...
            if (builder.HasAspCode && !fControlSkin)
            {
                ...
                if (builder is ContentPlaceHolderBuilder)
                {
                    string str11 = ((ContentPlaceHolderBuilder) builder).Name;
                    propName = MasterPageControlBuilder.AutoTemplatePrefix + str11;
                    string str12 = "__" + propName;
                    CodeExpression expression29 = new CodeFieldReferenceExpression(new CodeThisReferenceExpression(), str12);
                    statement7 = new CodeConditionStatement {
        ...
        private void BuildFieldDeclaration(ControlBuilder builder)
        {
            if (!(builder is ContentBuilderInternal))
            {
                ...
                CodeMemberField field = new CodeMemberField(new CodeTypeReference(builder.DeclareType, CodeTypeReferenceOptions.GlobalReference), builder.ID);
        ...
        internal virtual CodeExpression BuildPagePropertyReferenceExpression() => 
            new CodePropertyReferenceExpression(new CodeThisReferenceExpression(), pagePropertyName);
            ...
            if (builder.SubBuilders != null)
            {
                ...
                for (int i = 0; enumerator.MoveNext(); i++)
                {
                    ...
                    else if (current is CodeBlockBuilder)
                    {
                        ...
                        if (builder2.BlockType == CodeBlockType.Expression)
                        {
        ...
        protected virtual void BuildSourceDataTreeFromBuilder(ControlBuilder builder, bool fInTemplate, bool topLevelControlInTemplate, PropertyEntry pse)
        {
            if (!(builder is CodeBlockBuilder))
            {
                ...
                foreach (TemplatePropertyEntry entry in builder.TemplatePropertyEntries)
                {
                    ...
                    {
                        TemplateInstanceAttribute attribute = (TemplateInstanceAttribute) Attribute.GetCustomAttribute(entry.PropertyInfo, typeof(TemplateInstanceAttribute), false);
                        if (attribute != null)
                        {
                            flag3 = attribute.Instances == TemplateInstance.Multiple;
                        }
                    }
                    this.BuildSourceDataTreeFromBuilder(entry.Builder, flag3, false, entry);
                }
        ...
        private string ExtractMethodName(ControlBuilder builder) => 
            (extractTemplateValuesMethodPrefix + builder.ID);
        ...
        protected virtual bool UseResourceLiteralString(string s) => 
            ((s.Length >= 0x100) && base._codeDomProvider.Supports(GeneratorSupport.Win32Resources));
