namespace System.Web
{
    ...
    internal class StaticFileHandler : IHttpHandler
    {
        ...
        internal static bool ProcessRangeRequest(HttpContext context, string physicalPath, long fileLength, string rangeHeader, string etag, DateTime lastModified)
        {
            ...
            while ((startIndex < rangeHeader.Length) && flag2)
            {
                ...
                if (flag3)
                {
                    if (src == null)
                    {
                        src = new ByteRange[0x10];
                    }
                    if (num5 >= src.Length)
                    {
                        ByteRange[] dst = new ByteRange[src.Length * 2];
                        Buffer.BlockCopy(src, 0, dst, 0, src.Length * Marshal.SizeOf(src[0]));
                        src = dst;
                    }
                    src[num5].Offset = offset;
                    src[num5].Length = length;
                    num5++;
                    num6 += length;
        ...
        internal static void ProcessRequestInternal(HttpContext context)
        {
            HttpRequest request = context.Request;
            HttpResponse response = context.Response;
            if (!ProcessRequestForNonMapPathBasedVirtualFile(request, response))
            {
