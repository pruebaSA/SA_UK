namespace System.Web.SessionState
{
    ...
    public sealed class SessionStateModule : IHttpModule
    {
        ...
        {
            if (IsRequestQueueEnabled && (this._rqId != null))
            {
                ...
                if (queuedRequestCounter.Decrement() == 0)
                {
                    ...
                    try
                    {
                        ...
                        {
                            s_queuedRequestsNumPerSession.Remove(this.SaltedSessionId);
                        }
                    }
                    finally
                    {
        ...
        {
            if (IsRequestQueueEnabled && (this._rqContext.TimeLeft <= 0L))
            {
        ...
        {
            QueuedRequestCounter counter = null;
            s_lock.AcquireReaderLock();
            try
            {
                if (s_queuedRequestsNumPerSession.TryGetValue(this.SaltedSessionId, out counter))
                {
                    return counter;
                }
            }
            ...
            try
            {
                ...
                counter = new QueuedRequestCounter();
                s_queuedRequestsNumPerSession.Add(this.SaltedSessionId, counter);
            }
            finally
            {
                s_lock.ReleaseWriterLock();
        ...
        {
            if (IsRequestQueueEnabled && (s_sessionIDSalt == null))
            {
                try
                {
                    ...
                    s_sessionIDSalt = Convert.ToBase64String(data);
                }
                catch (CryptographicException)
                {
                    s_sessionIDSalt = string.Empty;
                }
            }
        }
        ...
        private void QueueRef()
        {
            if (IsRequestQueueEnabled && (this._rqId != null))
            {
                ...
                {
                    throw new HttpException(System.Web.SR.GetString("Failed_to_process_request"));
                }
        ...
        private static bool IsRequestQueueEnabled =>
            (AppSettings.RequestQueueLimitPerSession != 0x7fffffff);

        private string SaltedSessionId =>
            (this._rqId + s_sessionIDSalt);
