namespace System.Web
{
    ...
    internal sealed class FileChangesMonitor
    {
        ...
        private bool IsBeneathAppPathInternal(string fullPathName) => 
            (((this._appPathInternal != null) && (fullPathName.Length > (this._appPathInternal.Length + 1))) && ((fullPathName.IndexOf(this._appPathInternal, StringComparison.OrdinalIgnoreCase) > -1) && (fullPathName[this._appPathInternal.Length] == Path.DirectorySeparatorChar)));
        ...
        private DirectoryMonitor ListenToSubdirectoryChanges(string dirRoot, string dirToListenTo)
        {
            ...
            if (this.IsBeneathAppPathInternal(str))
            {
                ...
                monitor.StartMonitoringFile(dirToListenTo, new FileChangeEventHandler(this.OnCriticaldirChange), str);
                return monitor;
            }
            if (Directory.Exists(str))
            {
                ...
                try
                {
                    monitor.StartMonitoringFile(null, new FileChangeEventHandler(this.OnCriticaldirChange), str);
                    return monitor;
                }
                catch
                {
                ...
                using (new ApplicationImpersonationContext())
                {
                    ...
                    try
                    {
                        if (!this._disposed)
                        {
                            ...
                            this._dirMonSubdirs = new DirectoryMonitor(fullPath, true, 2);
                            try
                            {
        ...
        internal DateTime StartMonitoringFile(string alias, FileChangeEventHandler callback)
        {
            ...
            using (new ApplicationImpersonationContext())
            {
                ...
                try
                {
                    ...
                    monitor = directoryMonitor.StartMonitoringFile(fileNameLong, callback, alias);
                    if (flag)
                    {
        ...
        internal DateTime StartMonitoringPath(string alias, FileChangeEventHandler callback, out FileAttributesData fad)
        {
            ...
            using (new ApplicationImpersonationContext())
            {
                ...
                try
                {
                    ...
                    {
                        file = monitor.FileNameLong;
                        monitor = monitor.DirectoryMonitor.StartMonitoringFile(file, callback, alias);
                    }
                    else
                    {
                        flag = true;
                        ...
                        if (this.IsBeneathAppPathInternal(fullPath))
                        {
                            ...
                            monitor = monitor2.StartMonitoringFile(file, callback, alias);
                        }
                        else
                        {
                            ...
                            if (monitor2 != null)
                            {
                                monitor = monitor2.StartMonitoringFile(null, callback, alias);
                            }
                            else
                            {
                                string directoryOrRootName = UrlPath.GetDirectoryOrRootName(fullPath);
