    ...
    public class HttpApplication : IHttpAsyncHandler, IHttpHandler, IComponent, IDisposable
    {
        ...
        private HttpModuleCollection GetModuleCollection(IntPtr appContext)
        {
            if (_moduleConfigInfo == null)
            {
                ...
                {
                    int count = 0;
                    int num5 = UnsafeIISMethods.MgdGetModuleCollection(appContext, out zero, out count);
                    if (num5 < 0)
                    {
        ...
        internal IHttpHandler MapIntegratedHttpHandler(HttpContext context, string requestType, VirtualPath path, string pathTranslated, bool useAppConfig, bool convertNativeStaticFileModule)
        {
            ...
            using (new ApplicationImpersonationContext())
            {
                ...
                string str = (context.WorkerRequest as IIS7WorkerRequest).MapHandlerAndGetHandlerTypeString(requestType, virtualPathString, convertNativeStaticFileModule, false);
                if (str == null)
                {
                    PerfCounters.IncrementCounter(AppPerfCounter.REQUESTS_NOT_FOUND);
                    PerfCounters.IncrementCounter(AppPerfCounter.REQUESTS_FAILED);
        ...
        internal ThreadContext OnThreadEnter() => 
            this.OnThreadEnterPrivate(true);

        internal ThreadContext OnThreadEnter(bool setImpersonationContext) => 
            this.OnThreadEnterPrivate(setImpersonationContext);
        ...
        private void SetAppLevelCulture()
        {
            ...
            if ((info != null) && (info != Thread.CurrentThread.CurrentCulture))
            {
        ...
        private PipelineModuleStepContainer CurrentModuleContainer =>
            this.ModuleContainers[this._context.CurrentModuleIndex];
        ...
        private bool IsContainerInitalizationAllowed =>
            ((HttpRuntime.UseIntegratedPipeline && _initSpecialCompleted) && !this._initInternalCompleted);

        internal bool IsRequestCompleted =>
            this._stepManager?.IsCompleted;
        ...
        internal class CallHandlerExecutionStep : HttpApplication.IExecutionStep
        {
            ...
            bool HttpApplication.IExecutionStep.IsCancellable =>
                !(this._application.Context.Handler is IHttpAsyncHandler);
        }
                    ...
                    Label_00AE:
                        ...
                        else
                        {
                            ...
                            if (flag6 || (context.CurrentModuleEventIndex == num))
                            {
                                ...
                                context.CurrentModuleIndex = UnsafeIISMethods.MgdGetCurrentModuleIndex(workerRequest.RequestContext);
                                context.IsPostNotification = UnsafeIISMethods.MgdIsCurrentNotificationPost(workerRequest.RequestContext);
                                context.CurrentNotification = (RequestNotification) UnsafeIISMethods.MgdGetCurrentNotification(workerRequest.RequestContext);
                                context.CurrentModuleEventIndex = -1;
        ...
        internal class ThreadContext
        {
            ...
            {
                CultureInfo dynamicCulture = null;
                ...
                {
                    dynamicCulture = context.CultureFromConfig(globalization.Culture, true);
                }
                ...
                {
                    dynamicCulture = context.DynamicCulture;
                }
                ...
                if (currentHandler != null)
                {
                    ...
                    {
                        dynamicCulture = currentHandler.DynamicCulture;
                    }
                ...
                if ((dynamicCulture != null) && (dynamicCulture != Thread.CurrentThread.CurrentCulture))
                {
                    Thread.CurrentThread.CurrentCulture = dynamicCulture;
                    this._setThreadCulture = dynamicCulture;
                }
        ...
        internal class UrlMappingsExecutionStep : HttpApplication.IExecutionStep
        {
            ...
            void HttpApplication.IExecutionStep.Execute()
            {
                ...
                string str = urlMappings.HttpResolveMapping(request.RawUrl);
                if (str == null)
                {
                    str = urlMappings.HttpResolveMapping(request.Path);
                }
                if (!string.IsNullOrEmpty(str))
                {
                    context.RewritePath(str, false);
                }
            }
