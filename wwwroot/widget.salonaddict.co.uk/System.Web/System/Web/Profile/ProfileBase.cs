    ...
    public class ProfileBase : SettingsBase
    {
        ...
        internal static Hashtable GetPropertiesForCompilation()
        {
            ...
            if (s_PropertiesForCompilation == null)
            {
                lock (s_InitializeLock)
                {
                    ...
                    AddProfilePropertySettingsForCompilation(profile.PropertySettings, ht, null);
                    foreach (ProfileGroupSettings settings in profile.PropertySettings.GroupSettings)
                    {
            ...
            else
            {
                lock (s_InitializeLock)
                {
                    ...
                    try
                    {
                                ...
                                if (values[info2.Name] == null)
                                {
                                    ...
                                    foreach (Attribute attribute in Attribute.GetCustomAttributes(info2, true))
                                    {
                                        ...
                                        else if (attribute is SettingsAllowAnonymousAttribute)
                                        {
                                            ...
                                            {
                                                throw new ConfigurationErrorsException(System.Web.SR.GetString("Annoymous_id_module_not_enabled", new object[] { info2.Name }), profile.ElementInformation.Properties["inherits"].Source, profile.ElementInformation.Properties["inherits"].LineNumber);
                                            }
                                        }
                                        ...
                                        else if (hasLowTrust && (attribute is ProfileProviderAttribute))
                                        {
                                            ...
                                            if (provider == null)
                                            {
                                                throw new ConfigurationErrorsException(System.Web.SR.GetString("Profile_provider_not_found", new object[] { ((ProfileProviderAttribute) attribute).ProviderName }), profile.ElementInformation.Properties["inherits"].Source, profile.ElementInformation.Properties["inherits"].LineNumber);
                                            }
                                        }
                                    }
                                    SettingsAttributeDictionary attributes = new SettingsAttributeDictionary();
                        ...
                        if (profile.PropertySettings != null)
                        {
                            AddPropertySettingsFromConfig(inheritsFromType, fAnonEnabled, hasLowTrust, profile.PropertySettings, null);
                            foreach (ProfileGroupSettings settings in profile.PropertySettings.GroupSettings)
                            {
        ...
        internal static bool InheritsFromCustomType
        {
            get
            {
                ...
                string typeName = profile.Inherits.Trim();
        ...
        internal static Type InheritsFromType
        {
            get
            {
                ...
                {
                    ProfileSection profile = RuntimeConfig.GetAppConfig().Profile;
                    throw new ConfigurationErrorsException(System.Web.SR.GetString("Wrong_profile_base_type"), null, profile.ElementInformation.Properties["inherits"].Source, profile.ElementInformation.Properties["inherit"].LineNumber);
                }
                return propType;
            }
        }
