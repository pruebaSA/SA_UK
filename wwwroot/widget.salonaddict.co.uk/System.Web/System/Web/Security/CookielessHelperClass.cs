namespace System.Web.Security
{
    ...
    internal sealed class CookielessHelperClass
    {
        ...
        internal void RemoveCookielessValuesFromPath()
        {
            ...
            string path = flag ? this._Context.Request.ClientFilePath.VirtualPathString : this._Context.Request.FilePath;
            int count = path.LastIndexOf(")/", StringComparison.Ordinal);
            int length = (count > 2) ? path.LastIndexOf("/(", count - 1, count, StringComparison.Ordinal) : -1;
            if (length >= 0)
            {
                if (this._Headers == null)
                {
                ...
                if ((flag || (this._Headers == null)) && IsValidHeader(path, length + 2, count))
                {
                    if (!flag || (this._Headers == null))
                    {
                        this._Headers = path.Substring(length + 2, (count - length) - 2);
                    }
                    path = path.Substring(0, length) + path.Substring(count + 1);
                    if (!flag)
                    {
                        this._Context.RewritePath(VirtualPath.CreateAbsolute(path), this._Context.Request.PathInfoObject, null, true);
                    }
                    else
                    {
                        this._Context.Request.ClientFilePath = VirtualPath.CreateAbsolute(path);
                        string rawUrl = this._Context.WorkerRequest.GetRawUrl();
                        int index = rawUrl.IndexOf('?');
                        if (index > -1)
                        {
                            path = path + rawUrl.Substring(index);
                        }
                        this._Context.WorkerRequest.SetRawUrl(path);
                    }
                    if (!string.IsNullOrEmpty(this._Headers))
                    {
                        this._Context.Response.SetAppPathModifier("(" + this._Headers + ")");
