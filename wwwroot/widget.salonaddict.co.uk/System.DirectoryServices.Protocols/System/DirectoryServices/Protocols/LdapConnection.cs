namespace System.DirectoryServices.Protocols
{
    ...
    public class LdapConnection : DirectoryConnection, IDisposable
    {
        ...
        public IAsyncResult BeginSendRequest(DirectoryRequest request, PartialResultProcessing partialMode, AsyncCallback callback, object state) => 
            this.BeginSendRequest(request, base.connectionTimeOut, partialMode, callback, state);
        ...
        private void BindHelper(NetworkCredential newCredential, bool needSetCredential)
        {
            ...
            {
                userName = directoryCredential?.UserName;
                domain = directoryCredential?.Domain;
                password = directoryCredential?.Password;
            }
            ...
            else if (this.AuthType == System.DirectoryServices.Protocols.AuthType.Basic)
            {
                ...
                errorCode = Wldap32.ldap_simple_bind_s(this.ldapHandle, builder.ToString(), password);
            }
            else
            {
                ...
                if (directoryCredential != null)
                {
                    ...
                    credentials.userLength = (userName == null) ? 0 : userName.Length;
                    credentials.domain = domain;
                    credentials.domainLength = (domain == null) ? 0 : domain.Length;
                    credentials.password = password;
                    credentials.passwordLength = (password == null) ? 0 : password.Length;
                }
        ...
        internal LdapMod[] BuildAttributes(CollectionBase directoryAttributes, ArrayList ptrToFree)
        {
            ...
            if ((directoryAttributes != null) && (directoryAttributes.Count != 0))
            {
                ...
                for (int i = 0; i < directoryAttributes.Count; i++)
                {
                    ...
                    modArray[i].values = Utility.AllocHGlobalIntPtrArray(count + 1);
        ...
        private void ResponseCallback(IAsyncResult asyncResult)
        {
            ...
            {
                asyncState.exception = new Exception(System.DirectoryServices.Protocols.Res.GetString("NonCLSException"));
                asyncState.response = null;
            }
        ...
        public override DirectoryResponse SendRequest(DirectoryRequest request) => 
            this.SendRequest(request, base.connectionTimeOut);
        ...
        private int SendRequestHelper(DirectoryRequest request, ref int messageID)
        {
            ...
            try
            {
                ...
                {
                    clientcontrol = Utility.AllocHGlobalIntPtrArray(controlArray2.Length + 1);
                ...
                else if ((request is AddRequest) || (request is ModifyRequest))
                {
                    ...
                    size = (modArray == null) ? 1 : (modArray.Length + 1);
                    attrs = Utility.AllocHGlobalIntPtrArray(size);
                ...
                else
                {
                    if (request is SearchRequest)
                    {
                        ...
                        if (num2 != 0)
                        {
                            attributes = Utility.AllocHGlobalIntPtrArray(num2 + 1);
                            int num9 = 0;
                            num9 = 0;
                            while (num9 < num2)
                            {
            ...
            finally
            {
                ...
                {
                    for (int num16 = 0; num16 < (size - 1); num16++)
                    {
