namespace System.Xml.Schema
{
    ...
    internal class XNodeValidator
    {
        ...
        private void ReplaceSchemaInfo(XObject o, XmlSchemaInfo schemaInfo)
        {
            ...
            XmlSchemaInfo info = o.Annotation<XmlSchemaInfo>();
            if (info != null)
            {
                if (!this.schemaInfos.ContainsValue(info))
                {
                    this.schemaInfos.Add(info, info);
                }
                o.RemoveAnnotations<XmlSchemaInfo>();
            }
            if (!this.schemaInfos.TryGetValue(schemaInfo, out info))
            {
                info = schemaInfo;
                this.schemaInfos.Add(info, info);
            }
            o.AddAnnotation(info);
        }
