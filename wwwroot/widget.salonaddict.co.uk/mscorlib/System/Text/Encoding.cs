namespace System.Text
{
    ...
    public abstract class Encoding : ICloneable
    {
        ...
        internal virtual unsafe int GetBytes(char* chars, int charCount, byte* bytes, int byteCount, EncoderNLS encoder) => 
            this.GetBytes(chars, charCount, bytes, byteCount);
        ...
        internal virtual unsafe int GetChars(byte* bytes, int byteCount, char* chars, int charCount, DecoderNLS decoder) => 
            this.GetChars(bytes, byteCount, chars, charCount);
        ...
        public virtual System.Text.Decoder GetDecoder() => 
            new DefaultDecoder(this);

        public virtual System.Text.Encoder GetEncoder() => 
            new DefaultEncoder(this);
        ...
        public override int GetHashCode() => 
            ((this.m_codePage + this.EncoderFallback.GetHashCode()) + this.DecoderFallback.GetHashCode());
        ...
        public virtual string GetString(byte[] bytes, int index, int count) => 
            new string(this.GetChars(bytes, index, count));

        [ComVisible(false)]
        public bool IsAlwaysNormalized() => 
            this.IsAlwaysNormalized(NormalizationForm.FormC);
        ...
        public virtual string EncodingName =>
            Environment.GetResourceString("Globalization.cp_" + this.m_codePage);

        public virtual string HeaderName =>
            this.dataItem?.HeaderName;
        ...
        public virtual bool IsBrowserDisplay =>
            ((this.dataItem?.Flags & 2) != 0);

        public virtual bool IsBrowserSave =>
            ((this.dataItem?.Flags & 0x200) != 0);

        public virtual bool IsMailNewsDisplay =>
            ((this.dataItem?.Flags & 1) != 0);

        public virtual bool IsMailNewsSave =>
            ((this.dataItem?.Flags & 0x100) != 0);
        ...
        public virtual string WebName =>
            this.dataItem?.WebName;

        public virtual int WindowsCodePage =>
            this.dataItem?.UIFamilyCodePage;
        ...
        internal class DefaultDecoder : System.Text.Decoder, ISerializable, IObjectReference
        {
            ...
            public override unsafe int GetCharCount(byte* bytes, int count, bool flush) => 
                this.m_encoding.GetCharCount(bytes, count);

            public override int GetCharCount(byte[] bytes, int index, int count) => 
                this.GetCharCount(bytes, index, count, false);

            public override int GetCharCount(byte[] bytes, int index, int count, bool flush) => 
                this.m_encoding.GetCharCount(bytes, index, count);

            public override unsafe int GetChars(byte* bytes, int byteCount, char* chars, int charCount, bool flush) => 
                this.m_encoding.GetChars(bytes, byteCount, chars, charCount);

            public override int GetChars(byte[] bytes, int byteIndex, int byteCount, char[] chars, int charIndex) => 
                this.GetChars(bytes, byteIndex, byteCount, chars, charIndex, false);

            public override int GetChars(byte[] bytes, int byteIndex, int byteCount, char[] chars, int charIndex, bool flush) => 
                this.m_encoding.GetChars(bytes, byteIndex, byteCount, chars, charIndex);
        ...
        internal class DefaultEncoder : System.Text.Encoder, ISerializable, IObjectReference
        {
            ...
            public override unsafe int GetByteCount(char* chars, int count, bool flush) => 
                this.m_encoding.GetByteCount(chars, count);

            public override int GetByteCount(char[] chars, int index, int count, bool flush) => 
                this.m_encoding.GetByteCount(chars, index, count);

            public override unsafe int GetBytes(char* chars, int charCount, byte* bytes, int byteCount, bool flush) => 
                this.m_encoding.GetBytes(chars, charCount, bytes, byteCount);

            public override int GetBytes(char[] chars, int charIndex, int charCount, byte[] bytes, int byteIndex, bool flush) => 
                this.m_encoding.GetBytes(chars, charIndex, charCount, bytes, byteIndex);
        ...
        internal class EncodingByteBuffer
        {
            ...
            internal bool AddByte(byte b1, byte b2, byte b3) => 
                this.AddByte(b1, b2, b3, 0);

            internal bool AddByte(byte b1, byte b2, int moreBytesExpected) => 
                (this.AddByte(b1, (int) (1 + moreBytesExpected)) && this.AddByte(b2, moreBytesExpected));

            internal bool AddByte(byte b1, byte b2, byte b3, byte b4) => 
                (((this.AddByte(b1, 3) && this.AddByte(b2, 2)) && this.AddByte(b3, 1)) && this.AddByte(b4, 0));

            internal bool AddByte(byte b1, byte b2, byte b3, int moreBytesExpected) => 
                ((this.AddByte(b1, (int) (2 + moreBytesExpected)) && this.AddByte(b2, (int) (1 + moreBytesExpected))) && this.AddByte(b3, moreBytesExpected));

            internal unsafe bool Fallback(char charFallback) => 
                this.fallbackBuffer.InternalFallback(charFallback, ref this.chars);
            ...
            internal int CharsUsed =>
                ((int) ((long) ((this.chars - this.charStart) / 2)));
        ...
        internal class EncodingCharBuffer
        {
            ...
            internal int BytesUsed =>
                ((int) ((long) ((this.bytes - this.byteStart) / 1)));

            internal int Count =>
                this.charCountResult;

            internal bool MoreData =>
                (this.bytes < this.byteEnd);
        }
    }
}
