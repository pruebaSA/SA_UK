    ...
    internal sealed class ObjectReader
    {
        ...
        internal Type FastBindToType(string assemblyName, string typeName)
        {
            ...
            if ((cachedValue == null) || (cachedValue.assemblyName != assemblyName))
            {
                ...
                if (this.bSimpleAssembly)
                {
                    try
                    {
                        ...
                        try
                        {
                            ...
                            assem = Assembly.LoadWithPartialNameInternal(assemblyName, null, ref lookForMe);
                            if ((assem == null) && (assemblyName != null))
                            {
                                assem = Assembly.LoadWithPartialNameInternal(new AssemblyName(assemblyName).Name, null, ref lookForMe);
                            }
                        }
                        finally
                        {
                ...
                if ((typeFromAssembly == null) && this.bSimpleAssembly)
                {
                    try
                    {
                        sfileIOPermission.Assert();
                        try
                        {
                            typeFromAssembly = TypeName.GetType(assem, typeName);
                        }
                        finally
                        {
                            CodeAccessPermission.RevertAssert();
                ...
                CheckTypeForwardedTo(assem, typeFromAssembly.Assembly);
        ...
        internal Type GetType(BinaryAssemblyInfo assemblyInfo, string name)
        {
            ...
            if (typeFromAssembly == null)
            {
                Assembly assem = assemblyInfo.GetAssembly();
                typeFromAssembly = FormatterServices.GetTypeFromAssembly(assem, name);
                if (typeFromAssembly != null)
                {
                    CheckTypeForwardedTo(assem, typeFromAssembly.Assembly);
                }
