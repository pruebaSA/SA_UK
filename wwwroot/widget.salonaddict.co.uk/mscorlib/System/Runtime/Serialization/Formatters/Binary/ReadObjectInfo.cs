        ...
        internal Type[] GetMemberTypes(string[] inMemberNames, Type objectType)
        {
            ...
            for (int i = 0; i < this.cache.memberInfos.Length; i++)
            {
                ...
                if (!flag2)
                {
                    ...
                    {
                        throw new SerializationException(string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("Serialization_MissingMember"), new object[] { this.cache.memberNames[i], objectType, typeof(OptionalFieldAttribute).FullName }));
                    }
                }
            }
            return typeArray;
