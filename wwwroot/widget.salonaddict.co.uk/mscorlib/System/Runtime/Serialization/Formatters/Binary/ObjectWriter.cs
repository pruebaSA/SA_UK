namespace System.Runtime.Serialization.Formatters.Binary
{
    ...
    internal sealed class ObjectWriter
    {
        ...
        private long GetAssemblyId(WriteObjectInfo objectInfo)
        {
            ...
            this.serWriter.WriteAssembly(objectInfo.GetTypeFullName(), objectInfo.objectType, str2, (int) num, isNew, false);
            return num;
        }
        ...
        private object GetNext(out long objID)
        {
            ...
            if (flag)
            {
                throw new SerializationException(string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("Serialization_ObjNoID"), new object[] { obj3 }));
            }
            return obj2;
        }
        ...
        private long Schedule(object obj, bool assignUniqueIdToValueType, Type type) => 
            this.Schedule(obj, assignUniqueIdToValueType, type, null);
        ...
        internal void Serialize(object graph, Header[] inHeaders, __BinaryWriter serWriter, bool fCheck)
        {
            ...
            else
            {
                ...
                while ((obj2 = this.GetNext(out num2)) != null)
                {
                    ...
                    {
                        objectInfo = WriteObjectInfo.Serialize(obj2, this.m_surrogates, this.m_context, this.serObjectInfoInit, this.m_formatterConverter, this);
        ...
        private NameInfo TypeToNameInfo(WriteObjectInfo objectInfo) => 
            this.TypeToNameInfo(objectInfo.objectType, objectInfo, this.ToCode(objectInfo.objectType), null);

        private NameInfo TypeToNameInfo(Type type) => 
            this.TypeToNameInfo(type, null, this.ToCode(type), null);

        private NameInfo TypeToNameInfo(WriteObjectInfo objectInfo, NameInfo nameInfo) => 
            this.TypeToNameInfo(objectInfo.objectType, objectInfo, this.ToCode(objectInfo.objectType), nameInfo);
            ...
            else
            {
                ...
                for (int i = 0; i < typeArray.Length; i++)
                {
                    ...
                    if ((this.ToCode(type) == InternalPrimitiveTypeE.Invalid) && (type != Converter.typeofString))
                    {
                        if (objArray[i] != null)
                        {
                            memberObjectInfos[i] = WriteObjectInfo.Serialize(objArray[i], this.m_surrogates, this.m_context, this.serObjectInfoInit, this.m_formatterConverter, this);
                            memberObjectInfos[i].assemId = this.GetAssemblyId(memberObjectInfos[i]);
                        }
                        else
                        {
                            memberObjectInfos[i] = WriteObjectInfo.Serialize(typeArray[i], this.m_surrogates, this.m_context, this.serObjectInfoInit, this.m_formatterConverter);
                            memberObjectInfos[i].assemId = this.GetAssemblyId(memberObjectInfos[i]);
                        }
                    }
                }
        ...
        private void WriteArray(WriteObjectInfo objectInfo, NameInfo memberNameInfo, WriteObjectInfo memberObjectInfo)
        {
            ...
            {
                info = WriteObjectInfo.Serialize(elementType, this.m_surrogates, this.m_context, this.serObjectInfoInit, this.m_formatterConverter);
        ...
        private void WriteArrayMember(WriteObjectInfo objectInfo, NameInfo arrayElemTypeNameInfo, object data)
        {
            ...
            if (!this.CheckForNull(objectInfo, arrayElemTypeNameInfo, arrayElemTypeNameInfo, data))
            {
                ...
                if (!this.WriteKnownValueClass(arrayElemTypeNameInfo, typeNameInfo, data))
                {
                    ...
                    if (num < 1L)
                    {
                        WriteObjectInfo info2 = WriteObjectInfo.Serialize(obj2, this.m_surrogates, this.m_context, this.serObjectInfoInit, this.m_formatterConverter, this);
                        info2.objectId = num;
        ...
        private void WriteMembers(NameInfo memberNameInfo, NameInfo memberTypeNameInfo, object memberData, WriteObjectInfo objectInfo, NameInfo typeNameInfo, WriteObjectInfo memberObjectInfo)
        {
            ...
            if (!this.CheckForNull(objectInfo, memberNameInfo, memberTypeNameInfo, memberData))
            {
                ...
                else if (!this.WriteKnownValueClass(memberNameInfo, memberTypeNameInfo, memberData))
                {
                    ...
                    long num2 = this.Schedule(obj2, assignUniqueIdToValueType, type, memberObjectInfo);
                    if (num2 < 0L)
                    {
