namespace System.Runtime.InteropServices
{
    ...
    [ComVisible(true), Guid("475E398F-8AFA-43a7-A3BE-F4EF8D6787C9"), ClassInterface(ClassInterfaceType.None)]
    public class RegistrationServices : IRegistrationServices
    {
                ...
                for (int i = 0; i < length; i++)
                {
                    ...
                    if (info.GetCustomAttributes(attributeType, true).Length != 0)
                    {
                        if (!info.IsStatic)
                        {
                            if (bRegister)
                            {
                                throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("InvalidOperation_NonStaticComRegFunction"), new object[] { info.Name, type3.Name }));
                            }
                            throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("InvalidOperation_NonStaticComUnRegFunction"), new object[] { info.Name, type3.Name }));
                        }
                        ...
                        if (((info.ReturnType != typeof(void)) || (parameters == null)) || ((parameters.Length != 1) || ((parameters[0].ParameterType != typeof(string)) && (parameters[0].ParameterType != typeof(Type)))))
                        {
                            if (bRegister)
                            {
                                throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("InvalidOperation_InvalidComRegFunctionSig"), new object[] { info.Name, type3.Name }));
                            }
                            throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("InvalidOperation_InvalidComUnRegFunctionSig"), new object[] { info.Name, type3.Name }));
                        }
                        if (flag)
                        {
                            if (bRegister)
        ...
        public virtual bool RegisterAssembly(Assembly assembly, AssemblyRegistrationFlags flags)
        {
            ...
            {
                this.RegisterPrimaryInteropAssembly(assembly, strAsmCodeBase, (PrimaryInteropAssemblyAttribute) customAttributes[j]);
            }
        ...
        [ComVisible(false), SecurityPermission(SecurityAction.LinkDemand, Flags=SecurityPermissionFlag.UnmanagedCode)]
        ...
        public virtual bool TypeRequiresRegistration(Type type) => 
            TypeRequiresRegistrationHelper(type);
        ...
        private bool UnregisterValueType(Type type, string strAsmVersion)
        {
            ...
            using (RegistryKey key = Registry.ClassesRoot.OpenSubKey("Record", true))
            {
                ...
                using (RegistryKey key2 = key.OpenSubKey(name, true))
                {
                    if (key2 != null)
                    {
                        key2.DeleteValue("Assembly", false);
                        key2.DeleteValue("Class", false);
                        key2.DeleteValue("CodeBase", false);
