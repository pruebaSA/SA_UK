namespace System.Globalization
{
    ...
    public class UmAlQuraCalendar : Calendar
    {
        ...
        public override DateTime AddMonths(DateTime time, int months)
        {
            ...
            this.CheckYearRange(datePart, 1);
            DateTime time2 = new DateTime((this.GetAbsoluteDateUmAlQura(datePart, month, day) * 0xc92a69c000L) + (time.Ticks % 0xc92a69c000L));
            Calendar.CheckAddResult(time2.Ticks, this.MinSupportedDateTime, this.MaxSupportedDateTime);
            return time2;
        }

        public override DateTime AddYears(DateTime time, int years) => 
            this.AddMonths(time, years * 12);
        ...
        private long GetAbsoluteDateUmAlQura(int year, int month, int day)
        {
            ...
            int mg = 0;
            int dg = 0;
            this.ConvertHijriToGregorian(year, month, day, ref yg, ref mg, ref dg);
        ...
        internal virtual int GetDatePart(DateTime time, int part)
        {
            ...
            this.ConvertGregorianToHijri(time, ref hijriYear, ref hijriMonth, ref hijriDay);
            if (part == 0)
            {
                return hijriYear;
            }
            ...
            return (int) ((this.GetAbsoluteDateUmAlQura(hijriYear, hijriMonth, hijriDay) - this.GetAbsoluteDateUmAlQura(hijriYear, 1, 1)) + 1L);
        }

        public override int GetDayOfMonth(DateTime time) => 
            this.GetDatePart(time, 3);

        public override DayOfWeek GetDayOfWeek(DateTime time) => 
            ((DayOfWeek) (((int) ((time.Ticks / 0xc92a69c000L) + 1L)) % 7));

        public override int GetDayOfYear(DateTime time) => 
            this.GetDatePart(time, 1);
        ...
        {
            this.CheckYearRange(year, era);
            return (this.RealGetDaysInYear(year) == 0x163);
        }
        ...
        internal int RealGetDaysInYear(int year)
        {
            ...
            for (int i = 1; i <= 12; i++)
            {
                num += 0x1d + (hijriMonthsLengthFlags & 1);
                hijriMonthsLengthFlags = hijriMonthsLengthFlags >> 1;
            }
            return num;
        }
        ...
        public override DateTime ToDateTime(int year, int month, int day, int hour, int minute, int second, int millisecond, int era)
        {
            ...
            long num2 = this.GetAbsoluteDateUmAlQura(year, month, day);
            if (num2 < 0L)
            {
