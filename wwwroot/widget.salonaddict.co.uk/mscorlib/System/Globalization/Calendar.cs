namespace System.Globalization
{
    ...
    public abstract class Calendar : ICloneable
    {
        ...
        public virtual DateTime AddDays(DateTime time, int days) => 
            this.Add(time, (double) days, 0x5265c00);

        public virtual DateTime AddHours(DateTime time, int hours) => 
            this.Add(time, (double) hours, 0x36ee80);

        public virtual DateTime AddMilliseconds(DateTime time, double milliseconds) => 
            this.Add(time, milliseconds, 1);

        public virtual DateTime AddMinutes(DateTime time, int minutes) => 
            this.Add(time, (double) minutes, 0xea60);

        public abstract DateTime AddMonths(DateTime time, int months);
        public virtual DateTime AddSeconds(DateTime time, int seconds) => 
            this.Add(time, (double) seconds, 0x3e8);

        public virtual DateTime AddWeeks(DateTime time, int weeks) => 
            this.AddDays(time, weeks * 7);
        ...
        public virtual int GetDaysInMonth(int year, int month) => 
            this.GetDaysInMonth(year, month, 0);

        public abstract int GetDaysInMonth(int year, int month, int era);
        public virtual int GetDaysInYear(int year) => 
            this.GetDaysInYear(year, 0);
        ...
        public virtual int GetHour(DateTime time) => 
            ((int) ((time.Ticks / 0x861c46800L) % 0x18L));

        [ComVisible(false)]
        public virtual int GetLeapMonth(int year) => 
            this.GetLeapMonth(year, 0);
        ...
        public virtual double GetMilliseconds(DateTime time) => 
            ((double) ((time.Ticks / 0x2710L) % 0x3e8L));

        public virtual int GetMinute(DateTime time) => 
            ((int) ((time.Ticks / 0x23c34600L) % 60L));

        public abstract int GetMonth(DateTime time);
        public virtual int GetMonthsInYear(int year) => 
            this.GetMonthsInYear(year, 0);

        public abstract int GetMonthsInYear(int year, int era);
        public virtual int GetSecond(DateTime time) => 
            ((int) ((time.Ticks / 0x989680L) % 60L));
        ...
        public virtual bool IsLeapDay(int year, int month, int day) => 
            this.IsLeapDay(year, month, day, 0);

        public abstract bool IsLeapDay(int year, int month, int day, int era);
        public virtual bool IsLeapMonth(int year, int month) => 
            this.IsLeapMonth(year, month, 0);

        public abstract bool IsLeapMonth(int year, int month, int era);
        public virtual bool IsLeapYear(int year) => 
            this.IsLeapYear(year, 0);

        public abstract bool IsLeapYear(int year, int era);
        internal virtual bool IsValidDay(int year, int month, int day, int era) => 
            ((this.IsValidMonth(year, month, era) && (day >= 1)) && (day <= this.GetDaysInMonth(year, month, era)));

        internal virtual bool IsValidMonth(int year, int month, int era) => 
            ((this.IsValidYear(year, era) && (month >= 1)) && (month <= this.GetMonthsInYear(year, era)));

        internal virtual bool IsValidYear(int year, int era) => 
            ((year >= this.GetYear(this.MinSupportedDateTime)) && (year <= this.GetYear(this.MaxSupportedDateTime)));
        ...
        public virtual DateTime ToDateTime(int year, int month, int day, int hour, int minute, int second, int millisecond) => 
            this.ToDateTime(year, month, day, hour, minute, second, millisecond, 0);
