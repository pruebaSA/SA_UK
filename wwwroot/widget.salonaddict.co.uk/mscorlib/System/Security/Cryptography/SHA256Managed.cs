namespace System.Security.Cryptography
{
    ...
    public class SHA256Managed : SHA256
    {
            ...
            fixed (uint* numRef = this._stateSHA256)
            {
                fixed (byte* numRef2 = this._buffer)
                {
                    fixed (uint* numRef3 = this._W)
                    {
                        ...
                        {
                            Buffer.InternalBlockCopy(partIn, srcOffset, this._buffer, dstOffset, 0x40 - dstOffset);
                        ...
                        while (count >= 0x40)
                        {
                            Buffer.InternalBlockCopy(partIn, srcOffset, this._buffer, 0, 0x40);
                            srcOffset += 0x40;
                            count -= 0x40;
                            SHATransform(numRef3, numRef, numRef2);
                        }
        ...
        private static uint Sigma_0(uint x) => 
            ((RotateRight(x, 2) ^ RotateRight(x, 13)) ^ RotateRight(x, 0x16));

        private static uint sigma_1(uint x) => 
            ((RotateRight(x, 0x11) ^ RotateRight(x, 0x13)) ^ (x >> 10));

        private static uint Sigma_1(uint x) => 
            ((RotateRight(x, 6) ^ RotateRight(x, 11)) ^ RotateRight(x, 0x19));
    }
}
