namespace System.Security.Cryptography
{
    ...
    public class SHA384Managed : SHA384
    {
        ...
        private unsafe void _HashData(byte[] partIn, int ibStart, int cbSize)
        {
            ...
            int dstOffset = (int) (this._count & ((ulong) 0x7fL));
            this._count += count;
            fixed (ulong* numRef = this._stateSHA384)
            {
                fixed (byte* numRef2 = this._buffer)
                {
                    ...
                    {
                        if ((dstOffset > 0) && ((dstOffset + count) >= 0x80))
                        {
                            Buffer.InternalBlockCopy(partIn, srcOffset, this._buffer, dstOffset, 0x80 - dstOffset);
                            srcOffset += 0x80 - dstOffset;
                            count -= 0x80 - dstOffset;
                            SHATransform(numRef3, numRef, numRef2);
                            dstOffset = 0;
                        }
                        while (count >= 0x80)
                        {
                            Buffer.InternalBlockCopy(partIn, srcOffset, this._buffer, 0, 0x80);
                            srcOffset += 0x80;
                            count -= 0x80;
                            SHATransform(numRef3, numRef, numRef2);
                        }
                        if (count > 0)
                        {
                            Buffer.InternalBlockCopy(partIn, srcOffset, this._buffer, dstOffset, count);
                        }
                    }
                }
            }
        ...
        private static ulong Sigma_0(ulong x) => 
            ((RotateRight(x, 0x1c) ^ RotateRight(x, 0x22)) ^ RotateRight(x, 0x27));

        private static ulong sigma_1(ulong x) => 
            ((RotateRight(x, 0x13) ^ RotateRight(x, 0x3d)) ^ (x >> 6));

        private static ulong Sigma_1(ulong x) => 
            ((RotateRight(x, 14) ^ RotateRight(x, 0x12)) ^ RotateRight(x, 0x29));
    }
}
