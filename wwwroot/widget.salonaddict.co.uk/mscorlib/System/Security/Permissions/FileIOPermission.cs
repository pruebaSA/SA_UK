namespace System.Security.Permissions
{
    ...
    public sealed class FileIOPermission : CodeAccessPermission, IUnrestrictedPermission, IBuiltInPermission
    {
        ...
        private bool AccessIsSet(FileIOPermissionAccess access, FileIOPermissionAccess question) => 
            ((access & question) != FileIOPermissionAccess.NoAccess);
        ...
        public string[] GetPathList(FileIOPermissionAccess access)
        {
            ...
            if (this.AccessIsSet(access, FileIOPermissionAccess.Write))
            {
                return this.m_write?.ToStringArray();
            }
            if (this.AccessIsSet(access, FileIOPermissionAccess.Append))
            {
        ...
        public override IPermission Intersect(IPermission target)
        {
            ...
            FileIOAccess access = this.m_read?.Intersect(permission.m_read);
            FileIOAccess access2 = this.m_write?.Intersect(permission.m_write);
            FileIOAccess access3 = this.m_append?.Intersect(permission.m_append);
            FileIOAccess access4 = this.m_pathDiscovery?.Intersect(permission.m_pathDiscovery);
            FileIOAccess access5 = this.m_viewAcl?.Intersect(permission.m_viewAcl);
            FileIOAccess access6 = this.m_changeAcl?.Intersect(permission.m_changeAcl);
