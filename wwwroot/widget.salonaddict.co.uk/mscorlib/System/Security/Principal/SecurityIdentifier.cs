namespace System.Security.Principal
{
    ...
    public sealed class SecurityIdentifier : IdentityReference, IComparable<SecurityIdentifier>
    {
        ...
        public bool IsEqualDomainSid(SecurityIdentifier sid) => 
            Win32.IsEqualDomainSid(this, sid);

        public override bool IsValidTargetType(Type targetType) => 
            IsValidTargetTypeStatic(targetType);

        internal static bool IsValidTargetTypeStatic(Type targetType) => 
            ((targetType == typeof(NTAccount)) || (targetType == typeof(SecurityIdentifier)));

        public bool IsWellKnown(WellKnownSidType type) => 
            Win32.IsWellKnownSid(this, type);
        ...
        public override IdentityReference Translate(Type targetType)
        {
            ...
            return Translate(sourceSids, targetType, 1)[0];
        }
        ...
        private static IdentityReferenceCollection TranslateToNTAccounts(IdentityReferenceCollection sourceSids, out bool someFailed)
        {
            ...
            try
            {
                ...
                if ((num2 == 0) || (num2 == 0x107))
                {
                    ...
                    for (index = 0; index < lsa_referenced_domain_list.Entries; index++)
                    {
                        ...
                        strArray[index] = Marshal.PtrToStringUni(lsa_trust_information.Name.Buffer, lsa_trust_information.Name.Length / 2);
                    }
                    for (index = 0; index < sourceSids.Count; index++)
                    {
                        Win32Native.LSA_TRANSLATED_NAME lsa_translated_name = (Win32Native.LSA_TRANSLATED_NAME) Marshal.PtrToStructure(new IntPtr(((long) names.DangerousGetHandle()) + (index * Marshal.SizeOf(typeof(Win32Native.LSA_TRANSLATED_NAME)))), typeof(Win32Native.LSA_TRANSLATED_NAME));
                        ...
                        someFailed = true;
                        references.Add(sourceSids[index]);
                    }
                }
                else
                {
                    index = 0;
                    while (index < sourceSids.Count)
                    {
                        references.Add(sourceSids[index]);
                        index++;
                    }
            ...
            finally
            {
                for (index = 0; index < sourceSids.Count; index++)
                {
                    if (handleArray[index].IsAllocated)
                    {
                        handleArray[index].Free();
                    }
                }
