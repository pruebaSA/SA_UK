    ...
    public sealed class HashMembershipCondition : ISerializable, IDeserializationCallback, IReportMatchMembershipCondition, IMembershipCondition, ISecurityEncodable, ISecurityPolicyEncodable
    {
        ...
        private void ParseHashValue()
        {
            lock (this.InternalSyncObject)
            {
                if (this.m_element != null)
                {
                    ...
                    if (hexString == null)
                    {
                        throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("Argument_InvalidXMLElement"), new object[] { "HashValue", base.GetType().FullName }));
                    }
        ...
        bool IReportMatchMembershipCondition.Check(Evidence evidence, out object usedEvidence)
        {
            ...
            if (evidence != null)
            {
                ...
                {
                    Hash current = hostEnumerator.Current as Hash;
                    if (current != null)
                    {
                        if ((this.m_value == null) && (this.m_element != null))
                        {
                        ...
                        lock (this.InternalSyncObject)
                        {
                            first = current.GenerateHash(this.m_hashAlg);
                        }
        ...
        public override string ToString()
        {
            ...
            return string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("Hash_ToString"), new object[] { this.m_hashAlg.GetType().AssemblyQualifiedName, Hex.EncodeHexString(this.HashValue) });
        }
