namespace System.ServiceModel.ComIntegration
{
    ...
    internal class DllHostInitializeWorker
    {
        ...
        {
            ComPlusDllHostInitializerTrace.Trace(TraceEventType.Information, TraceCode.ComIntegrationDllHostInitializerStopping, "TraceCodeComIntegrationDllHostInitializerStopping", this.applicationId);
            foreach (ComPlusServiceHost host in this.hosts)
            {
                host.Close();
            }
            ComPlusDllHostInitializerTrace.Trace(TraceEventType.Information, TraceCode.ComIntegrationDllHostInitializerStopped, "TraceCodeComIntegrationDllHostInitializerStopped", this.applicationId);
        }

        public void Startup(IProcessInitControl control)
        {
            this.applicationId = ContextUtil.ApplicationId;
            ComPlusDllHostInitializerTrace.Trace(TraceEventType.Information, TraceCode.ComIntegrationDllHostInitializerStarting, "TraceCodeComIntegrationDllHostInitializerStarting", this.applicationId);
            Thread thread = null;
            try
            {
                ...
                foreach (ServiceElement element in section.Services)
                {
                    ...
                    while (enumerator.MoveNext())
                    {
                        ...
                        Guid clsid = new Guid((string) current.GetValue("CLSID"));
                        if ((clsid == empty) && (this.applicationId == result))
                        {
                            flag3 = true;
                            ComPlusDllHostInitializerTrace.Trace(TraceEventType.Verbose, TraceCode.ComIntegrationDllHostInitializerAddingHost, "TraceCodeComIntegrationDllHostInitializerAddingHost", this.applicationId, clsid, element);
            ...
            ComPlusDllHostInitializerTrace.Trace(TraceEventType.Information, TraceCode.ComIntegrationDllHostInitializerStarted, "TraceCodeComIntegrationDllHostInitializerStarted", this.applicationId);
        }
    }
}
