            ...
            if (this.monikerSupportLibrary == null)
            {
                lock (this)
                {
                    if (this.monikerSupportLibrary == null)
                    {
                        ...
                        using (RegistryHandle handle = RegistryHandle.GetCorrectBitnessHKLMSubkey(IntPtr.Size == 8, @"SOFTWARE\Microsoft\NET Framework Setup\NDP\v3.0\Setup\Windows Communication Foundation"))
                        {
                            string libFilename = handle.GetStringValue("RuntimeInstallPath").TrimEnd(new char[1]) + @"\ServiceMonikerSupport.dll";
