namespace System.ServiceModel.ComIntegration
{
    ...
    internal class ComPlusInstanceContextInitializer : IInstanceContextInitializer
    {
        ...
        private static Assembly ResolveAssembly(object sender, ResolveEventArgs args)
        {
            ...
            string g = args.Name.Substring(0, index).Trim().ToLowerInvariant();
            return TypeCacheManager.Provider.ResolveAssembly(new Guid(g));
        }
            ...
            if (this.info.HasUdts())
            {
                ...
                lock (manifestLock)
                {
                    ...
                    if (!Directory.Exists(path))
                    {
                        ...
                        {
                            Type[] types = this.info.GetTypes(guid);
                            if (types.Length > 0)
                            {
