namespace System.ServiceModel.ComIntegration
{
    ...
    internal class TypeCacheManager : ITypeCacheManager
    {
        ...
        private bool NoCoClassAttributeOnType(ICustomAttributeProvider attrProvider) => 
            (ServiceReflector.GetCustomAttributes(attrProvider, typeof(CoClassAttribute), false).Length == 0);
        ...
        {
            Assembly assembly;
            ComPlusTLBImportTrace.Trace(TraceEventType.Verbose, TraceCode.ComIntegrationTLBImportStarting, "TraceCodeComIntegrationTLBImportStarting", iid, typeLibraryID);
            bool flag = false;
            ...
            try
            {
                lock (this.assemblyTableLock)
                {
                    ...
                    if (assembly == null)
                    {
                        ...
                        if (!flag)
                        {
                            ComPlusTLBImportTrace.Trace(TraceEventType.Verbose, TraceCode.ComIntegrationTLBImportFromAssembly, "TraceCodeComIntegrationTLBImportFromAssembly", iid, typeLibraryID, str);
                            assembly = Assembly.Load(str);
                        }
                        else
                        {
                            ComPlusTLBImportTrace.Trace(TraceEventType.Verbose, TraceCode.ComIntegrationTLBImportFromTypelib, "TraceCodeComIntegrationTLBImportFromTypelib", iid, typeLibraryID);
            ...
            ComPlusTLBImportTrace.Trace(TraceEventType.Verbose, TraceCode.ComIntegrationTLBImportFinished, "TraceCodeComIntegrationTLBImportFinished", iid, typeLibraryID);
            return assembly;
        }
        ...
        internal Type VerifyType(Guid iid)
        {
            ...
            this.FindOrCreateType(iid, out interfaceType, false, true);
