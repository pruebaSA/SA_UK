namespace System.ServiceModel.Channels
{
    ...
    internal abstract class CommunicationPool<TKey, TItem> where TKey: class where TItem: class
    {
        ...
        protected virtual EndpointConnectionPool<TKey, TItem> CreateEndpointConnectionPool(TKey key) => 
            new EndpointConnectionPool<TKey, TItem>((CommunicationPool<TKey, TItem>) this, key);
        ...
        private void OnClose(TimeSpan timeout)
        {
            ...
            foreach (EndpointConnectionPool<TKey, TItem> pool in this.endpointPools.Values)
            {
                ...
                catch (TimeoutException exception2)
                {
                    if (DiagnosticUtility.ShouldTraceError)
                    {
                        TraceUtility.TraceEvent(TraceEventType.Error, TraceCode.ConnectionPoolCloseException, this, exception2);
                    }
                }
            }
        ...
        protected class EndpointConnectionPool
        {
            ...
            protected virtual CommunicationPool<TKey, TItem>.IdleConnectionPool GetIdleConnectionPool() => 
                new PoolIdleConnectionPool<TKey, TItem>(this.parent.MaxIdleConnectionPoolCount);
            ...
            protected class PoolIdleConnectionPool : CommunicationPool<TKey, TItem>.IdleConnectionPool
            {
                ...
                private bool ReturnToPool(TItem connection)
                {
                    ...
                    {
                        DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Information, TraceCode.ConnectionPoolMaxOutboundConnectionsPerEndpointQuotaReached, System.ServiceModel.SR.GetString("TraceCodeConnectionPoolMaxOutboundConnectionsPerEndpointQuotaReached", new object[] { this.maxCount }), null, null, this);
                    }
                    return flag;
                }
