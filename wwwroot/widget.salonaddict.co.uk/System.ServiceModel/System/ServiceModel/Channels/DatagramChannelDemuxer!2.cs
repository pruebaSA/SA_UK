namespace System.ServiceModel.Channels
{
    ...
    internal abstract class DatagramChannelDemuxer<TInnerChannel, TInnerItem> : TypedChannelDemuxer, IChannelDemuxer where TInnerChannel: class, IChannel where TInnerItem: class, IDisposable
    {
        ...
        private ItemDequeuedCallback onItemDequeued;
        ...
        private static WaitCallback startReceivingStatic;

        static DatagramChannelDemuxer()
        {
            DatagramChannelDemuxer<TInnerChannel, TInnerItem>.onReceiveComplete = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(DatagramChannelDemuxer<TInnerChannel, TInnerItem>.OnReceiveCompleteStatic));
            DatagramChannelDemuxer<TInnerChannel, TInnerItem>.startReceivingStatic = new WaitCallback(DatagramChannelDemuxer<TInnerChannel, TInnerItem>.StartReceivingStatic);
        }
        ...
        protected abstract void EnqueueAndDispatch(IChannelListener listener, TInnerItem item, ItemDequeuedCallback dequeuedCallback, bool canDispatchOnThisThread);
        protected abstract void EnqueueAndDispatch(IChannelListener listener, Exception exception, ItemDequeuedCallback dequeuedCallback, bool canDispatchOnThisThread);
        ...
        private bool HandleReceiveResult(IAsyncResult result)
        {
            ...
            if (local == null)
            {
                ...
                {
                    TraceUtility.TraceEvent(TraceEventType.Error, TraceCode.PrematureDatagramEof, null, this.innerChannel, null);
                }
                return true;
            }
            try
        ...
        protected void HandleUnknownException(Exception exception)
        {
            ...
            lock (this.ThisLock)
            {
                if (this.filterTable.Count > 0)
                {
                    ...
                    {
                        this.onItemDequeued = new ItemDequeuedCallback(this.OnItemDequeued);
                    }
        ...
        public IAsyncResult OnBeginOuterListenerOpen(ChannelDemuxerFilter filter, IChannelListener listener, TimeSpan timeout, AsyncCallback callback, object state) => 
            new OpenAsyncResult<TInnerChannel, TInnerItem>((DatagramChannelDemuxer<TInnerChannel, TInnerItem>) this, filter, listener, timeout, callback, state);
            ...
            try
            {
                if (this.ShouldOpenInnerListener(filter, listener))
                {
                    try
                    {
                        ...
                        IOThreadScheduler.ScheduleCallback(DatagramChannelDemuxer<TInnerChannel, TInnerItem>.startReceivingStatic, this);
                        return;
                    }
        ...
        private class CloseAsyncResult : AsyncResult
        {
            ...
            {
                DatagramChannelDemuxer<TInnerChannel, TInnerItem>.CloseAsyncResult.sharedCallback = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(DatagramChannelDemuxer<TInnerChannel, TInnerItem>.CloseAsyncResult.SharedCallback));
            }
        ...
        private class OpenAsyncResult : AsyncResult
        {
            ...
            private TimeoutHelper timeoutHelper;
            private static WaitCallback waitCallback;

            static OpenAsyncResult()
            {
                DatagramChannelDemuxer<TInnerChannel, TInnerItem>.OpenAsyncResult.waitCallback = new WaitCallback(DatagramChannelDemuxer<TInnerChannel, TInnerItem>.OpenAsyncResult.WaitOverCallback);
                DatagramChannelDemuxer<TInnerChannel, TInnerItem>.OpenAsyncResult.openListenerCallback = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(DatagramChannelDemuxer<TInnerChannel, TInnerItem>.OpenAsyncResult.OpenListenerCallback));
                DatagramChannelDemuxer<TInnerChannel, TInnerItem>.OpenAsyncResult.acceptChannelCallback = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(DatagramChannelDemuxer<TInnerChannel, TInnerItem>.OpenAsyncResult.AcceptChannelCallback));
                DatagramChannelDemuxer<TInnerChannel, TInnerItem>.OpenAsyncResult.openChannelCallback = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(DatagramChannelDemuxer<TInnerChannel, TInnerItem>.OpenAsyncResult.OpenChannelCallback));
            }
            ...
            public OpenAsyncResult(DatagramChannelDemuxer<TInnerChannel, TInnerItem> channelDemuxer, ChannelDemuxerFilter filter, IChannelListener listener, TimeSpan timeout, AsyncCallback callback, object state) : base(callback, state)
            {
                ...
                this.timeoutHelper = new TimeoutHelper(timeout);
                if (this.channelDemuxer.openSemaphore.Enter(DatagramChannelDemuxer<TInnerChannel, TInnerItem>.OpenAsyncResult.waitCallback, this))
                {
                    bool flag = false;
                    bool flag2 = false;
                    try
            ...
            private void OnEndOpenChannel(IAsyncResult result)
            {
                ...
                IOThreadScheduler.ScheduleCallback(DatagramChannelDemuxer<TInnerChannel, TInnerItem>.startReceivingStatic, this.channelDemuxer);
            }
            ...
            private static void WaitOverCallback(object state)
            {
