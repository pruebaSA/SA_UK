namespace System.ServiceModel.Channels
{
    ...
    internal class ClientFramingDuplexSessionChannel : FramingDuplexSessionChannel
    {
        ...
        private IAsyncResult BeginSendPreamble(IConnection connection, ArraySegment<byte> preamble, ref TimeoutHelper timeoutHelper, AsyncCallback callback, object state) => 
            new SendPreambleAsyncResult(this, connection, preamble, this.flowIdentity, ref timeoutHelper, callback, state);
        ...
        private IConnection EndSendPreamble(IAsyncResult result) => 
            SendPreambleAsyncResult.End(result);
        ...
        protected override IAsyncResult OnBeginOpen(TimeSpan timeout, AsyncCallback callback, object state) => 
            new OpenAsyncResult(this, timeout, callback, state);
        ...
        private class DuplexConnectionPoolHelper : ConnectionPoolHelper
        {
            ...
            protected override IConnection AcceptPooledConnection(IConnection connection, ref TimeoutHelper timeoutHelper) => 
                this.channel.SendPreamble(connection, this.preamble, ref timeoutHelper);
            ...
            protected override IAsyncResult BeginAcceptPooledConnection(IConnection connection, ref TimeoutHelper timeoutHelper, AsyncCallback callback, object state) => 
                this.channel.BeginSendPreamble(connection, this.preamble, ref timeoutHelper, callback, state);

            protected override TimeoutException CreateNewConnectionTimeoutException(TimeSpan timeout, TimeoutException innerException) => 
                new TimeoutException(System.ServiceModel.SR.GetString("OpenTimedOutEstablishingTransportSession", new object[] { timeout, this.channel.Via.AbsoluteUri }), innerException);

            protected override IConnection EndAcceptPooledConnection(IAsyncResult result) => 
                this.channel.EndSendPreamble(result);
        }

        private class OpenAsyncResult : AsyncResult
        {
            private ClientFramingDuplexSessionChannel duplexChannel;
            private static AsyncCallback onEstablishConnection = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ClientFramingDuplexSessionChannel.OpenAsyncResult.OnEstablishConnection));
        ...
        private class SendPreambleAsyncResult : AsyncResult
        {
            ...
            private static AsyncCallback onWritePreamble = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ClientFramingDuplexSessionChannel.SendPreambleAsyncResult.OnWritePreamble));
            ...
            public static IConnection End(IAsyncResult result) => 
                AsyncResult.End<ClientFramingDuplexSessionChannel.SendPreambleAsyncResult>(result).connection;
            ...
            private bool HandleInitiatorClose(IAsyncResult result)
            {
                ...
                {
                    onWritePreambleEnd = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ClientFramingDuplexSessionChannel.SendPreambleAsyncResult.OnWritePreambleEnd));
                }
            ...
            private bool HandleInitiatorOpen(IAsyncResult result)
            {
                ...
                if (onUpgrade == null)
                {
                    onUpgrade = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ClientFramingDuplexSessionChannel.SendPreambleAsyncResult.OnUpgrade));
                }
            ...
            private bool HandlePreambleAck()
            {
                ...
                {
                    IAsyncResult result = ConnectionUpgradeHelper.BeginDecodeFramingFault(this.channel.decoder, this.connection, this.channel.Via, this.channel.MessageEncoder.ContentType, ref this.timeoutHelper, DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(this.OnFailedPreamble)), this);
                    if (!result.CompletedSynchronously)
                    {
                        return false;
                    }
            ...
            private bool HandleUpgrade(IAsyncResult result)
            {
                ...
                {
                    onUpgradeInitiatorClose = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ClientFramingDuplexSessionChannel.SendPreambleAsyncResult.OnUpgradeInitiatorClose));
                }
            ...
            private bool HandleWritePreamble(IAsyncResult result)
            {
                ...
                {
                    onUpgradeInitiatorOpen = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ClientFramingDuplexSessionChannel.SendPreambleAsyncResult.OnUpgradeInitiatorOpen));
                }
