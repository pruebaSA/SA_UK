namespace System.ServiceModel.Channels
{
    ...
    internal class DuplexSessionOneWayChannelListener : DelegatingChannelListener<IInputChannel>
    {
        ...
        private WaitCallback handleAcceptCallback;
        ...
        private static AsyncCallback onAcceptInnerChannel = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(DuplexSessionOneWayChannelListener.OnAcceptInnerChannel));
        private ItemDequeuedCallback onExceptionDequeued;
        ...
        public DuplexSessionOneWayChannelListener(OneWayBindingElement bindingElement, BindingContext context) : base(true, context.Binding, context.BuildInnerChannelListener<IDuplexSessionChannel>())
        {
            ...
            this.onOpenInnerChannel = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(this.OnOpenInnerChannel));
            ...
            lock (this.acceptLock)
            {
                while (this.IsAcceptNecessary)
                {
                    ...
                    try
                    {
                        ...
                        {
                            this.handleAcceptCallback = new WaitCallback(this.HandleAcceptCallback);
                        }
                        if (channel != null)
                        {
                            IOThreadScheduler.ScheduleCallback(this.handleAcceptCallback, channel);
                            channel = null;
                        }
                        ...
                        if (this.ProcessEndAccept(result, out channel2))
                        {
                            if (channel2 != null)
                            {
                                IOThreadScheduler.ScheduleCallback(this.handleAcceptCallback, channel2);
                            }
                        }
                        else
                        {
        ...
        {
            base.OnOpened();
            IOThreadScheduler.ScheduleCallback(new WaitCallback(this.AcceptLoop), null);
        }
        ...
        private bool ProcessEndAccept(IAsyncResult result, out IDuplexSessionChannel channel)
        {
            ...
            if (flag)
            {
                ...
                {
                    TraceUtility.TraceEvent(TraceEventType.Warning, TraceCode.MaxAcceptedChannelsReached, new StringTraceRecord("MaxAcceptedChannels", this.maxAcceptedChannels.ToString(CultureInfo.InvariantCulture)), this, null);
                }
            }
            else if (exception != null)
            {
                ...
                {
                    this.onExceptionDequeued = new ItemDequeuedCallback(this.OnExceptionDequeued);
                }
        ...
        private bool IsAcceptNecessary =>
            ((!this.acceptPending && (this.activeChannels < this.maxAcceptedChannels)) && (this.innerChannelListener.State == CommunicationState.Opened));
        ...
        private class ChannelReceiver
        {
            ...
            private TimeSpan idleTimeout;
            private WaitCallback onDispatchItemsLater;
            private ItemDequeuedCallback onMessageDequeued;
            private static AsyncCallback onReceive = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(DuplexSessionOneWayChannelListener.ChannelReceiver.OnReceive));
            private WaitCallback onStartReceiveLater;
            private static WaitCallback startReceivingCallback;
            ...
            private void OnMessageDequeued()
            {
                ...
                if (result != null)
                {
                    if (result.CompletedSynchronously)
                    {
                        ...
                        if (this.OnCompleteReceive(result, out flag))
                        {
                            ...
                            {
                                this.onStartReceiveLater = new WaitCallback(this.OnStartReceiveLater);
                            }
                            IOThreadScheduler.ScheduleCallback(this.onStartReceiveLater, null);
                        }
                        if (flag)
                        {
                            if (this.onDispatchItemsLater == null)
                            {
                                this.onDispatchItemsLater = new WaitCallback(this.OnDispatchItemsLater);
                            }
                            IOThreadScheduler.ScheduleCallback(this.onDispatchItemsLater, null);
                        }
                    }
                }
                else if (exception != null)
                ...
                else if (this.channel.State == CommunicationState.Opened)
                {
                    ...
                    {
                        startReceivingCallback = new WaitCallback(this.StartReceivingCallback);
                    }
        ...
        private class DuplexSessionOneWayInputChannelAcceptor : InputChannelAcceptor
        {
            ...
            protected override IAsyncResult OnBeginOpen(TimeSpan timeout, AsyncCallback callback, object state) => 
                new ChainedOpenAsyncResult(timeout, callback, state, new ChainedBeginHandler(this.OnBeginOpen), new ChainedEndHandler(this.OnEndOpen), new ICommunicationObject[] { this.receivers, this.innerChannelListener });
            ...
            protected override InputChannel OnCreateChannel() => 
                new DuplexSessionOneWayInputChannel(base.ChannelManager, null);
