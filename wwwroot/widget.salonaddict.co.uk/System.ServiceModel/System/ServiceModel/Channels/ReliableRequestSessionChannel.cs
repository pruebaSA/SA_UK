namespace System.ServiceModel.Channels
{
    ...
    internal sealed class ReliableRequestSessionChannel : RequestChannel, IRequestSessionChannel, IRequestChannel, IChannel, ICommunicationObject, ISessionChannel<IOutputSession>
    {
        ...
        private static AsyncCallback onPollingComplete = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ReliableRequestSessionChannel.OnPollingComplete));
        ...
        protected override IAsyncRequest CreateAsyncRequest(Message message, AsyncCallback callback, object state) => 
            new AsyncRequest(this, callback, state);
        ...
        protected override IAsyncResult OnBeginOpen(TimeSpan timeout, AsyncCallback callback, object state) => 
            new ReliableChannelOpenAsyncResult(this.binder, this.session, timeout, callback, state);
        ...
        private class AsyncRequest : AsyncResult, ReliableRequestSessionChannel.IReliableRequest, IAsyncRequest, IAsyncResult, IRequestBase
        {
            ...
            {
                this.parent.connection.BeginAddMessage(message, timeout, this, DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(this.AddCompleted)), null);
            }
            ...
            public Message WaitForReply(TimeSpan timeout)
            {
                ...
                try
                {
                    ...
                    if (!this.completed)
                    {
                        ...
                        if (flag3)
                        {
                            flag2 = !TimeoutHelper.WaitOne(this.completedHandle, timeout, false);
                            lock (this.ThisLock)
                            {
                                if (!this.completed)
                                {
