namespace System.ServiceModel.Channels
{
    ...
    internal class SingletonChannelListener<TChannel, TQueuedChannel, TQueuedItem> : DelegatingChannelListener<TChannel>, IChannelDemuxerFilter where TChannel: class, IChannel where TQueuedChannel: InputQueueChannel<TQueuedItem> where TQueuedItem: class, IDisposable
    {
        ...
        public void EnqueueAndDispatch(TQueuedItem item, ItemDequeuedCallback dequeuedCallback, bool canDispatchOnThisThread)
        {
        ...
        protected override IAsyncResult OnBeginClose(TimeSpan timeout, AsyncCallback callback, object state) => 
            new ChainedAsyncResult(timeout, callback, state, new ChainedBeginHandler(this.OnBeginOuterListenerClose), new ChainedEndHandler(this.OnEndOuterListenerClose), new ChainedBeginHandler(this.OnBeginClose), new ChainedEndHandler(this.OnEndClose));

        protected override IAsyncResult OnBeginOpen(TimeSpan timeout, AsyncCallback callback, object state) => 
            new ChainedAsyncResult(timeout, callback, state, new ChainedBeginHandler(this.OnBeginOuterListenerOpen), new ChainedEndHandler(this.OnEndOuterListenerOpen), new ChainedBeginHandler(this.OnBeginOpen), new ChainedEndHandler(this.OnEndOpen));

        private IAsyncResult OnBeginOuterListenerClose(TimeSpan timeout, AsyncCallback callback, object state) => 
            this.channelDemuxer.OnBeginOuterListenerClose(this.filter, timeout, callback, state);

        private IAsyncResult OnBeginOuterListenerOpen(TimeSpan timeout, AsyncCallback callback, object state) => 
            this.channelDemuxer.OnBeginOuterListenerOpen(this.filter, this, timeout, callback, state);
        ...
        {
            get => 
                ((SingletonChannelAcceptor<TChannel, TQueuedChannel, TQueuedItem>) base.Acceptor);
            set
            {
                base.Acceptor = value;
            }
