namespace System.ServiceModel.Channels
{
    ...
    internal sealed class UriPrefixTable<TItem> where TItem: class
    {
        ...
        private MruCache<BaseUriWithWildcard, TItem> lookupCache;
        private SegmentHierarchyNode<TItem> root;
        private bool useWeakReferences;
        private MruCache<BaseUriWithWildcard, WeakReference> weakLookupCache;
        ...
        public UriPrefixTable(bool includePortInComparison, bool useWeakReferences)
        {
            ...
            {
                this.weakLookupCache = new MruCache<BaseUriWithWildcard, WeakReference>(0x80);
            }
            else
            {
                this.lookupCache = new MruCache<BaseUriWithWildcard, TItem>(0x80);
            }
        }
        ...
        private bool TryCacheLookup(BaseUriWithWildcard key, out TItem item)
        {
            ...
            if (this.weakLookupCache.TryGetValue(key, out reference))
            {
                item = reference.Target as TItem;
                if (((TItem) item) != null)
                {
                    return true;
                }
        ...
        public bool TryLookupUri(Uri uri, HostNameComparisonMode hostNameComparisonMode, out TItem item)
        {
            ...
            lock (this.ThisLock)
            {
                ...
                {
                    bool flag2;
                    SegmentHierarchyNode<TItem> node = this.FindDataNode(UriSegmenter<TItem>.ToPath(key.BaseAddress, hostNameComparisonMode, this.includePortInComparison), out flag2);
                    if (node != null)
                    {
                        item = node.Data;
                    }
