namespace System.ServiceModel.Channels
{
    ...
    internal class PipeConnectionInitiator : IConnectionInitiator
    {
        ...
        public IAsyncResult BeginConnect(Uri uri, TimeSpan timeout, AsyncCallback callback, object state) => 
            new ConnectAsyncResult(this, uri, timeout, callback, state);
        ...
        public IConnection Connect(Uri remoteUri, TimeSpan timeout)
        {
            ...
            while (connection == null)
            {
                ...
                if (connection == null)
                {
                    ...
                    {
                        DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Information, TraceCode.FailedPipeConnect, System.ServiceModel.SR.GetString("TraceCodeFailedPipeConnect", new object[] { helper2.RemainingTime(), remoteUri }));
                    }
                }
            }
            return connection;
        ...
        private Exception CreateConnectFailedException(Uri remoteUri, PipeException innerException) => 
            new CommunicationException(System.ServiceModel.SR.GetString("PipeConnectFailed", new object[] { remoteUri.AbsoluteUri }), innerException);
        ...
        private void PrepareConnect(Uri remoteUri, TimeSpan timeout, out string resolvedAddress, out BackoffTimeoutHelper backoffHelper)
        {
            ...
            if (DiagnosticUtility.ShouldTraceInformation)
            {
                TraceUtility.TraceEvent(TraceEventType.Information, TraceCode.InitiatingNamedPipeConnection, new StringTraceRecord("Uri", remoteUri.ToString()), this, null);
            }
            resolvedAddress = GetPipeName(remoteUri);
            backoffHelper = new BackoffTimeoutHelper(TimeoutHelper.ReserveTimeAtEnd(timeout), TimeSpan.FromMinutes(5.0));
        }
        ...
        private class ConnectAsyncResult : AsyncResult
        {
            ...
            private static WaitCallback waitCompleteCallback;
            ...
            private bool ConnectAndWait()
            {
                ...
                if (!flag)
                {
                    ...
                    {
                        waitCompleteCallback = new WaitCallback(PipeConnectionInitiator.ConnectAsyncResult.OnWaitComplete);
                    }
            ...
            public static IConnection End(IAsyncResult result) => 
                AsyncResult.End<PipeConnectionInitiator.ConnectAsyncResult>(result).connection;
            ...
            private static void OnWaitComplete(object state)
            {
                ...
                try
                {
                    if (DiagnosticUtility.ShouldTraceInformation)
                    {
                        DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Information, TraceCode.FailedPipeConnect, System.ServiceModel.SR.GetString("TraceCodeFailedPipeConnect", new object[] { result.timeoutHelper.RemainingTime(), result.remoteUri }));
                    }
