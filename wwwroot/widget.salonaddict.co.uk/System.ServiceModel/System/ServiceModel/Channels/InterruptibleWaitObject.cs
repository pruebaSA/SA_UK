namespace System.ServiceModel.Channels
{
    ...
    internal class InterruptibleWaitObject
    {
        ...
        public IAsyncResult BeginTryWait(TimeSpan timeout, AsyncCallback callback, object state) => 
            this.BeginWait(timeout, false, callback, state);

        public IAsyncResult BeginWait(TimeSpan timeout, AsyncCallback callback, object state) => 
            this.BeginWait(timeout, this.throwTimeoutByDefault, callback, state);
        ...
        public IAsyncResult BeginWait(TimeSpan timeout, bool throwTimeoutException, AsyncCallback callback, object state)
        {
            ...
            lock (this.thisLock)
            {
                if (!this.set)
                {
                    ...
                    this.Aborted = (WaitAsyncResult.AbortHandler) Delegate.Combine(this.Aborted, new WaitAsyncResult.AbortHandler(result.OnAborted));
                    this.Faulted = (WaitAsyncResult.AbortHandler) Delegate.Combine(this.Faulted, new WaitAsyncResult.AbortHandler(result.OnFaulted));
                    this.Signaled = (WaitAsyncResult.SignaledHandler) Delegate.Combine(this.Signaled, new WaitAsyncResult.SignaledHandler(result.OnSignaled));
                    result.Begin();
                    return result;
                }
        ...
        public bool Wait(TimeSpan timeout) => 
            this.Wait(timeout, this.throwTimeoutByDefault);
        ...
        public bool Wait(TimeSpan timeout, bool throwTimeoutException)
        {
            ...
            try
            {
                if (!TimeoutHelper.WaitOne(this.handle, timeout, false))
                {
