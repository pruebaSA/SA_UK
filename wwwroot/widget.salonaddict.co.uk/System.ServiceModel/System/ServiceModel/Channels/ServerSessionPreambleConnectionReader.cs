namespace System.ServiceModel.Channels
{
    ...
    internal class ServerSessionPreambleConnectionReader : InitialServerConnectionReader
    {
        ...
        public ServerSessionPreambleConnectionReader(IConnection connection, ItemDequeuedCallback connectionDequeuedCallback, long streamPosition, int offset, int size, TransportSettingsCallback transportSettingsCallback, ConnectionClosedCallback closedCallback, ServerSessionPreambleCallback callback) : base(connection, closedCallback)
        {
        ...
        private void ContinueReading()
        {
            ...
            catch (Exception exception5)
            {
                ...
                if (!ExceptionHandler.HandleTransportExceptionHelper(exception5))
                {
                    throw;
                }
            }
        ...
        public IDuplexSessionChannel CreateDuplexSessionChannel(ConnectionOrientedTransportChannelListener channelListener, EndpointAddress localAddress, bool exposeConnectionProperty, ConnectionDemuxer connectionDemuxer) => 
            new ServerFramingDuplexSessionChannel(channelListener, this, localAddress, exposeConnectionProperty, connectionDemuxer);
        ...
        private TimeSpan GetRemainingTimeout() => 
            this.receiveTimeoutHelper.RemainingTime();
        ...
        private static void ReadCallback(object state)
        {
            ...
            catch (Exception exception3)
            {
                ...
                if (!ExceptionHandler.HandleTransportExceptionHelper(exception3))
                {
                    throw;
                }
            }
        ...
        private class ServerFramingDuplexSessionChannel : FramingDuplexSessionChannel
        {
            ...
            private static string GetIdentityNameFromContext(SecurityMessageProperty clientSecurity) => 
                System.ServiceModel.Security.SecurityUtils.GetIdentityNamesFromContext(clientSecurity.ServiceSecurityContext.AuthorizationContext);
            ...
            private class OpenAsyncResult : AsyncResult
            {
                        ...
                        do
                        {
                            ...
                            switch (this.channel.decoder.CurrentState)
                            {
                                case ServerSessionDecoder.State.UpgradeRequest:
                                {
                                    ...
                                    {
                                        onWriteUpgradeResponse = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ServerSessionPreambleConnectionReader.ServerFramingDuplexSessionChannel.OpenAsyncResult.OnWriteUpgradeResponse));
                                    }
                                ...
                                case ServerSessionDecoder.State.Start:
                                {
                                    ...
                                    {
                                        onWriteAckResponse = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ServerSessionPreambleConnectionReader.ServerFramingDuplexSessionChannel.OpenAsyncResult.OnWriteAckResponse));
                                    }
                ...
                private bool HandleWriteUpgradeResponseComplete(IAsyncResult result)
                {
                    ...
                    {
                        onUpgradeConnection = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ServerSessionPreambleConnectionReader.ServerFramingDuplexSessionChannel.OpenAsyncResult.OnUpgradeConnection));
                    }
                    try
                    {
                    ...
                    while (!isAtEof && (size > 0))
                    {
                        ...
                        if (count > 0)
                        {
                            if (base.EnvelopeBuffer != null)
                            {
                                ...
                                {
                                    System.Buffer.BlockCopy(buffer, offset, base.EnvelopeBuffer, base.EnvelopeOffset, count);
                                }
