namespace System.ServiceModel.Channels
{
    ...
    internal static class StreamingConnectionHelper
    {
        public static IAsyncResult BeginWriteMessage(Message message, IConnection connection, bool isRequest, IConnectionOrientedTransportFactorySettings settings, ref TimeoutHelper timeoutHelper, AsyncCallback callback, object state) => 
            new WriteMessageAsyncResult(message, connection, isRequest, settings, ref timeoutHelper, callback, state);
        ...
        private class WriteMessageAsyncResult : AsyncResult
        {
            ...
            private static AsyncCallback onWriteEndBytes = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(StreamingConnectionHelper.WriteMessageAsyncResult.OnWriteEndBytes));
            private static AsyncCallback onWriteStartBytes;
            private static WaitCallback onWriteStartBytesScheduled;
                ...
                else
                {
                    try
                    {
                        ...
                        if (flag3)
                        {
                            ...
                            {
                                onWriteStartBytes = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(StreamingConnectionHelper.WriteMessageAsyncResult.OnWriteStartBytes));
                            }
                            ...
                            if (result.CompletedSynchronously)
                            {
                                ...
                                {
                                    onWriteStartBytesScheduled = new WaitCallback(StreamingConnectionHelper.WriteMessageAsyncResult.OnWriteStartBytesScheduled);
                                }
                                IOThreadScheduler.ScheduleCallback(onWriteStartBytesScheduled, result);
                            }
                        }
                        else
                        {
                            ...
                            {
                                onWriteBufferedMessage = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(StreamingConnectionHelper.WriteMessageAsyncResult.OnWriteBufferedMessage));
                            }
