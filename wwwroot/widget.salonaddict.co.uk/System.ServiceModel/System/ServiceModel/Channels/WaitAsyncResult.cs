    ...
    internal class WaitAsyncResult : AsyncResult
    {
        ...
        public void Begin()
        {
            lock (this.thisLock)
            {
                ...
                {
                    this.timer = new IOThreadTimer(new WaitCallback(this.OnTimerElapsed), null, true);
        ...
        public static bool End(IAsyncResult result) => 
            !AsyncResult.End<WaitAsyncResult>(result).timedOut;

        protected virtual string GetTimeoutString(TimeSpan timeout) => 
            System.ServiceModel.SR.GetString("TimeoutOnOperation", new object[] { timeout });
