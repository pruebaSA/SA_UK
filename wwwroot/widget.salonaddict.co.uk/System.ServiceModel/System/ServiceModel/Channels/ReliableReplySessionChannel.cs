namespace System.ServiceModel.Channels
{
    ...
    internal sealed class ReliableReplySessionChannel : ReplyChannel, IReplySessionChannel, IReplyChannel, IChannel, ICommunicationObject, ISessionChannel<IInputSession>
    {
        ...
        private static WaitCallback asyncReceiveComplete = new WaitCallback(ReliableReplySessionChannel.AsyncReceiveCompleteStatic);
        ...
        private static AsyncCallback onReceiveCompleted = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ReliableReplySessionChannel.OnReceiveCompletedStatic));
        ...
        private IAsyncResult BeginCloseBinder(TimeSpan timeout, AsyncCallback callback, object state) => 
            this.binder.BeginClose(timeout, MaskingMode.Handled, callback, state);
        ...
        private IAsyncResult BeginUnregisterChannel(TimeSpan timeout, AsyncCallback callback, object state) => 
            this.listener.OnReliableChannelBeginClose(this.session.InputID, this.session.OutputID, timeout, callback, state);
        ...
        private Message CreateAcknowledgement(SequenceRangeCollection ranges) => 
            WsrmUtilities.CreateAcknowledgmentMessage(this.MessageVersion, this.listener.ReliableMessagingVersion, this.session.InputID, ranges, this.connection.IsLastKnown, this.listener.MaxTransferWindowSize - this.deliveryStrategy.EnqueuedCount);
        ...
        protected override void OnClose(TimeSpan timeout)
        {
            ...
            this.CloseOutput(helper.RemainingTimeExpireZero());
            if (this.listener.ReliableMessagingVersion == ReliableMessagingVersion.WSReliableMessagingFebruary2005)
            {
        ...
        private void ProcessSequencedMessage(RequestContext context, string action, WsrmSequencedMessageInfo info)
        {
            ...
            if (fault == null)
            {
                ...
                else
                {
                    ...
                    if (allAdded)
                    {
                        IOThreadScheduler.ScheduleCallback(new WaitCallback(this.ShutdownCallback), null);
                    }
                }
            }
            else
        ...
        private class ReliableRequestContext : RequestContextBase
        {
            ...
            protected override IAsyncResult OnBeginReply(Message reply, TimeSpan timeout, AsyncCallback callback, object state) => 
                this.BeginReplyInternal(reply, timeout, callback, state);
            ...
            private class ReplyAsyncResult : AsyncResult
            {
                ...
                private static AsyncCallback replyCompleteStatic = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ReliableReplySessionChannel.ReliableRequestContext.ReplyAsyncResult.ReplyCompleteStatic));
        ...
        private class ReplyHelper
        {
            ...
            internal IAsyncResult BeginWaitAndReply(TimeSpan timeout, AsyncCallback callback, object state)
            {
                ...
                OperationWithTimeoutBeginCallback[] beginOperations = new OperationWithTimeoutBeginCallback[] { (this.waitHandle != null) ? new OperationWithTimeoutBeginCallback(this.waitHandle.BeginWait) : null, new OperationWithTimeoutBeginCallback(this.BeginReply) };
                OperationEndCallback[] endOperations = new OperationEndCallback[] { (this.waitHandle != null) ? new OperationEndCallback(this.waitHandle.EndWait) : null, new OperationEndCallback(this.EndReply) };
                return OperationWithTimeoutComposer.BeginComposeAsyncOperations(timeout, beginOperations, endOperations, callback, state);
            }
