namespace System.ServiceModel.Channels
{
    ...
    internal class UtilityExtension : IExtension<IPeerNeighbor>
    {
        ...
        {
            this.ackTimer = new IOThreadTimer(new WaitCallback(this.AcknowledgeLoop), null, false);
            this.pendingSends = 0;
            this.pruneTimer = new IOThreadTimer(new WaitCallback(this.VerifyCheckPoint), null, false);
        ...
        private void SendUtilityMessage(uint useful, uint total)
        {
            ...
            if (((owner != null) && PeerNeighborStateHelper.IsConnected(owner.State)) && (total != 0))
            {
                ...
                try
                {
                    result = owner.BeginSend(message, DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(this.UtilityMessageSent)), new AsyncUtilityState(message, typedMessage));
