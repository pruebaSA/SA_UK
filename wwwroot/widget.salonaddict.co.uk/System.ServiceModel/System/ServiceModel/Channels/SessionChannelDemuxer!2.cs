namespace System.ServiceModel.Channels
{
    ...
    internal abstract class SessionChannelDemuxer<TInnerChannel, TInnerItem> : TypedChannelDemuxer, IChannelDemuxer where TInnerChannel: class, IChannel where TInnerItem: class, IDisposable
    {
        ...
        private WaitCallback onStartAccepting;
        ...
        private static WaitCallback startAcceptStatic;
        private FlowThrottle throttle;

        static SessionChannelDemuxer()
        {
            SessionChannelDemuxer<TInnerChannel, TInnerItem>.onAcceptComplete = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(SessionChannelDemuxer<TInnerChannel, TInnerItem>.OnAcceptCompleteStatic));
            SessionChannelDemuxer<TInnerChannel, TInnerItem>.onPeekComplete = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(SessionChannelDemuxer<TInnerChannel, TInnerItem>.OnPeekCompleteStatic));
            ...
            SessionChannelDemuxer<TInnerChannel, TInnerItem>.startAcceptStatic = new WaitCallback(SessionChannelDemuxer<TInnerChannel, TInnerItem>.StartAcceptStatic);
        }
        ...
        public override IChannelListener<TChannel> BuildChannelListener<TChannel>(ChannelDemuxerFilter filter) where TChannel: class, IChannel => 
            new InputQueueChannelListener<TChannel>(filter, this) { InnerChannelListener = this.innerListener };
        ...
        public IAsyncResult OnBeginOuterListenerOpen(ChannelDemuxerFilter filter, IChannelListener listener, TimeSpan timeout, AsyncCallback callback, object state) => 
            new OpenAsyncResult<TInnerChannel, TInnerItem>((SessionChannelDemuxer<TInnerChannel, TInnerItem>) this, filter, listener, timeout, callback, state);
        ...
        {
            IOThreadScheduler.ScheduleCallback(SessionChannelDemuxer<TInnerChannel, TInnerItem>.startAcceptStatic, state);
        }
        ...
        private void StartAccepting(bool requiresThrottle)
        {
            ...
            {
                IOThreadScheduler.ScheduleCallback(this.OnStartAccepting, result);
            }
        }
        ...
        private WaitCallback OnStartAccepting
        {
            get
            {
                ...
                {
                    this.onStartAccepting = new WaitCallback(this.OnStartAcceptingCallback);
                }
        ...
        private class OpenAsyncResult : AsyncResult
        {
            ...
            {
                SessionChannelDemuxer<TInnerChannel, TInnerItem>.OpenAsyncResult.waitCallback = new WaitCallback(SessionChannelDemuxer<TInnerChannel, TInnerItem>.OpenAsyncResult.WaitOverCallback);
                SessionChannelDemuxer<TInnerChannel, TInnerItem>.OpenAsyncResult.openListenerCallback = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(SessionChannelDemuxer<TInnerChannel, TInnerItem>.OpenAsyncResult.OpenListenerCallback));
            }
            ...
            public OpenAsyncResult(SessionChannelDemuxer<TInnerChannel, TInnerItem> channelDemuxer, ChannelDemuxerFilter filter, IChannelListener listener, TimeSpan timeout, AsyncCallback callback, object state) : base(callback, state)
            {
                ...
                if (this.channelDemuxer.openSemaphore.Enter(SessionChannelDemuxer<TInnerChannel, TInnerItem>.OpenAsyncResult.waitCallback, this))
                {
                    bool flag = false;
                    bool flag2 = false;
                    try
        ...
        private class PeekAsyncResult : AsyncResult
        {
            ...
            {
                SessionChannelDemuxer<TInnerChannel, TInnerItem>.PeekAsyncResult.onOpenComplete = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(SessionChannelDemuxer<TInnerChannel, TInnerItem>.PeekAsyncResult.OnOpenCompleteStatic));
                SessionChannelDemuxer<TInnerChannel, TInnerItem>.PeekAsyncResult.onReceiveComplete = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(SessionChannelDemuxer<TInnerChannel, TInnerItem>.PeekAsyncResult.OnReceiveCompleteStatic));
            }
            ...
            private bool HandleOpenComplete(IAsyncResult result)
            {
                ...
                if (this.demuxer.peekTimeout == TimeSpan.MinValue)
                {
