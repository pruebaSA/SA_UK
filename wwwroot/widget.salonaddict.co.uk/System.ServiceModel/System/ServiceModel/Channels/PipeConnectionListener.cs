namespace System.ServiceModel.Channels
{
    ...
    internal class PipeConnectionListener : IConnectionListener, IDisposable
    {
        ...
        private unsafe PipeHandle CreatePipe()
        {
            ...
            fixed (byte* numRef = SecurityDescriptorHelper.FromSecurityIdentifiers(this.allowedSids, -1073741824))
            {
            ...
            if (handle.IsInvalid)
            {
                ...
                Exception innerException = new PipeException(System.ServiceModel.SR.GetString("PipeListenFailed", new object[] { this.pipeUri.AbsoluteUri, PipeError.GetErrorString(num2) }), num2);
                switch (num2)
                {
                    case 5:
        ...
        private class PendingAccept : AsyncResult
        {
            ...
            private static WaitCallback onStartAccept;
            ...
            public PendingAccept(PipeConnectionListener listener, PipeHandle pipeHandle, bool isBoundToCompletionPort, AsyncCallback callback, object state) : base(callback, state)
            {
                ...
                if (!Thread.CurrentThread.IsThreadPoolThread)
                {
                    ...
                    {
                        onStartAccept = new WaitCallback(PipeConnectionListener.PendingAccept.OnStartAccept);
                    }
