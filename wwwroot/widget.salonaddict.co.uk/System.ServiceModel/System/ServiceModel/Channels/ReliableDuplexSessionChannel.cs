namespace System.ServiceModel.Channels
{
    ...
    internal abstract class ReliableDuplexSessionChannel : DuplexChannel, IDuplexSessionChannel, IDuplexChannel, IInputChannel, IOutputChannel, IChannel, ICommunicationObject, ISessionChannel<IDuplexSession>
    {
        ...
        private static WaitCallback asyncReceiveComplete = new WaitCallback(ReliableDuplexSessionChannel.AsyncReceiveCompleteStatic);
        ...
        private static AsyncCallback onReceiveCompleted = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ReliableDuplexSessionChannel.OnReceiveCompletedStatic));
        ...
        protected ReliableDuplexSessionChannel(ChannelManagerBase manager, IReliableFactorySettings settings, IReliableChannelBinder binder) : base(manager, binder.LocalAddress)
        {
            ...
            this.acknowledgementTimer = new IOThreadTimer(new WaitCallback(this.OnAcknowledgementTimeoutElapsed), null, true);
        ...
        protected override IAsyncResult OnBeginSend(Message message, TimeSpan timeout, AsyncCallback callback, object state) => 
            this.outputConnection.BeginAddMessage(message, timeout, null, callback, state);
        ...
        protected void ProcessDuplexMessage(WsrmMessageInfo info)
        {
            ...
            try
            {
                ...
                else if (flag2 && (info.TerminateSequenceInfo != null))
                {
                    ...
                    lock (base.ThisLock)
                    {
                        flag12 = !this.inputConnection.Terminate();
                    }
                    if (flag12)
                    {
                ...
                else if (flag3)
                {
                    if (((info.TerminateSequenceInfo != null) && (info.TerminateSequenceInfo.Identifier == this.session.InputID)) || (info.CloseSequenceInfo != null))
                    {
                        ...
                        lock (base.ThisLock)
                        {
                            ...
                            {
                                flag15 = last == this.inputConnection.Last;
                                if ((flag13 && flag15) && this.inputConnection.IsSequenceClosed)
                                {
                                    flag7 = true;
                                }
                            }
                        ...
                        {
                            message = flag13 ? WsrmUtilities.CreateTerminateResponseMessage(this.settings.MessageVersion, info2.MessageId, this.session.InputID) : WsrmUtilities.CreateCloseSequenceResponse(this.settings.MessageVersion, info2.MessageId, this.session.InputID);
                            flag6 = true;
                        }
                    }
        ...
        protected void SetConnections()
        {
            ...
            this.deliveryStrategy.DequeueCallback = new ItemDequeuedCallback(this.OnDeliveryStrategyItemDequeued);
        }
        ...
        public override EndpointAddress RemoteAddress =>
            this.binder.RemoteAddress;

        public IDuplexSession Session =>
            ((IDuplexSession) this.session);
