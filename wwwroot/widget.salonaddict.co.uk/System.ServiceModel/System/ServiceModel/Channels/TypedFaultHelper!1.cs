namespace System.ServiceModel.Channels
{
    ...
    internal abstract class TypedFaultHelper<TState> : FaultHelper
    {
        ...
        private WaitCallback sendFaultCallback;
        ...
        protected abstract void AbortState(TState state);
        ...
        private void AsyncCloseBinder(IReliableChannelBinder binder)
        {
            ...
            {
                this.onBinderCloseComplete = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(this.OnBinderCloseComplete));
            }
            ...
            try
            {
                ...
                lock (base.ThisLock)
                {
                    if (this.faultList != null)
                    {
                        ...
                        {
                            this.onSendFaultComplete = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(this.OnSendFaultComplete));
                        }
                    }
                }
                if (flag)
                ...
                else
                {
                    ...
                    {
                        this.sendFaultCallback = new WaitCallback(this.SendFaultCallback);
                    }
                    IOThreadScheduler.ScheduleCallback(this.sendFaultCallback, binder);
                }
            }
            catch (Exception exception)
            {
