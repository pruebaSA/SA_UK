namespace System.ServiceModel.Channels
{
    ...
    internal abstract class ReliableChannelListener<TChannel, TReliableChannel, TInnerChannel> : ReliableChannelListenerBase<TChannel> where TChannel: class, IChannel where TReliableChannel: class, IChannel where TInnerChannel: class, IChannel
    {
        ...
        {
            ReliableChannelListener<TChannel, TReliableChannel, TInnerChannel>.onAcceptCompleted = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ReliableChannelListener<TChannel, TReliableChannel, TInnerChannel>.OnAcceptCompletedStatic));
        }
        ...
        private IServerReliableChannelBinder CreateBinder(TInnerChannel channel, EndpointAddress localAddress, EndpointAddress remoteAddress) => 
            ServerReliableChannelBinder<TInnerChannel>.CreateBinder(channel, localAddress, remoteAddress, TolerateFaultsMode.IfNotSecuritySession, this.DefaultCloseTimeout, this.DefaultSendTimeout);
        ...
        protected bool EnqueueWithoutDispatch(TChannel channel) => 
            this.inputQueueChannelAcceptor.EnqueueWithoutDispatch(channel, null);
        ...
        protected bool HandleException(Exception e, ICommunicationObject o) => 
            (((e is CommunicationException) || (e is TimeoutException)) && (o.State == CommunicationState.Opened));

        protected override bool HasChannels() => 
            ((this.channelsByInput != null) && (this.channelsByInput.Count > 0));
        ...
        protected override void OnOpened()
        {
            ...
            {
                IOThreadScheduler.ScheduleCallback(new WaitCallback(ReliableChannelListener<TChannel, TReliableChannel, TInnerChannel>.StartAccepting), this);
            }
        }
