namespace System.ServiceModel.Channels
{
    ...
    public abstract class Message : IDisposable
    {
        ...
        public void Close()
        {
            ...
            {
                ServiceModelActivity activity = System.ServiceModel.DiagnosticUtility.ShouldUseActivity ? TraceUtility.ExtractAndRemoveActivity(this) : null;
                this.state = MessageState.Closed;
                this.OnClose();
                if (System.ServiceModel.DiagnosticUtility.ShouldTraceVerbose)
                {
                    Guid activityId = (activity == null) ? Guid.Empty : activity.Id;
                    System.ServiceModel.DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Verbose, TraceCode.MessageClosed, System.ServiceModel.SR.GetString("TraceCodeMessageClosed"), null, null, activityId, this);
                }
            }
            else if (System.ServiceModel.DiagnosticUtility.ShouldTraceVerbose)
            {
                TraceUtility.TraceEvent(TraceEventType.Verbose, TraceCode.MessageClosedAgain, this, this);
            }
        }
        ...
        public static Message CreateMessage(MessageVersion version, string action, object body) => 
            CreateMessage(version, action, body, DataContractSerializerDefaults.CreateSerializer(GetObjectType(body), 0x7fffffff));
        ...
        public static Message CreateMessage(MessageVersion version, string action, XmlReader body) => 
            CreateMessage(version, action, XmlDictionaryReader.CreateDictionaryReader(body));
        ...
        public static Message CreateMessage(XmlReader envelopeReader, int maxSizeOfHeaders, MessageVersion version) => 
            CreateMessage(XmlDictionaryReader.CreateDictionaryReader(envelopeReader), maxSizeOfHeaders, version);
        ...
        internal Exception CreateMessageDisposedException() => 
            new ObjectDisposedException("", System.ServiceModel.SR.GetString("MessageClosed"));

        public T GetBody<T>() => 
            this.GetBody<T>(DataContractSerializerDefaults.CreateSerializer(typeof(T), 0x7fffffff));
        ...
        internal static bool IsFaultStartElement(XmlDictionaryReader reader, EnvelopeVersion version) => 
            reader.IsStartElement(XD.MessageDictionary.Fault, version.DictionaryNamespace);
        ...
        protected virtual MessageBuffer OnCreateBufferedCopy(int maxBufferSize) => 
            this.OnCreateBufferedCopy(maxBufferSize, XmlDictionaryReaderQuotas.Max);
        ...
        public void WriteBodyContents(XmlDictionaryWriter writer)
        {
            ...
            switch (this.state)
            {
                case MessageState.Created:
                    ...
                    if (System.ServiceModel.DiagnosticUtility.ShouldTraceVerbose)
                    {
                        TraceUtility.TraceEvent(TraceEventType.Verbose, TraceCode.MessageWritten, this);
                    }
                    this.OnWriteBodyContents(writer);
                    return;
