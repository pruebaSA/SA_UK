namespace System.ServiceModel.Channels
{
    ...
    internal class MsmqQueue : IDisposable
    {
        ...
        public IAsyncResult BeginPeek(NativeMsmqMessage message, TimeSpan timeout, AsyncCallback callback, object state) => 
            new TryReceiveAsyncResult(this, message, timeout, -2147483648, callback, state);

        public IAsyncResult BeginTryReceive(NativeMsmqMessage message, TimeSpan timeout, AsyncCallback callback, object state) => 
            new TryReceiveAsyncResult(this, message, timeout, 0, callback, state);
        ...
        public ReceiveResult EndPeek(IAsyncResult result) => 
            TypedAsyncResult<ReceiveResult>.End(result);
        ...
        private MsmqQueueHandle GetHandle()
        {
            lock (this.ThisLock)
            {
                if (this.handle == null)
        ...
        private IDtcTransaction GetNativeTransaction(MsmqTransactionMode transactionMode)
        {
        ...
        private void HandleIsStale(MsmqQueueHandle handle)
        {
            lock (this.ThisLock)
            {
        ...
        private static bool IsErrorDueToStaleHandle(int error)
        {
            switch (error)
            {
                case -1072824314:
        ...
        public static bool IsReadable(string formatName, out MsmqException ex) => 
            SupportsAccessMode(formatName, 1, out ex);

        private static bool IsReceiveErrorDueToInsufficientBuffer(int error)
        {
            switch (error)
            {
                case -1072824289:
        ...
        internal MsmqQueueHandle OpenQueue()
        {
            MsmqQueueHandle handle;
            int error = UnsafeNativeMethods.MQOpenQueue(this.formatName, this.accessMode, 0, out handle);
            if (error != 0)
            {
        ...
        private int ReceiveByLookupIdCoreDtcTransacted(MsmqQueueHandle handle, long lookupId, NativeMsmqMessage message, MsmqTransactionMode transactionMode, int action)
        {
            int num;
        ...
        public ReceiveResult TryPeek(NativeMsmqMessage message, TimeSpan timeout) => 
            this.TryReceive(message, timeout, MsmqTransactionMode.None, -2147483648);

        public ReceiveResult TryReceive(NativeMsmqMessage message, TimeSpan timeout, MsmqTransactionMode transactionMode) => 
            this.TryReceive(message, timeout, transactionMode, 0);

        private ReceiveResult TryReceive(NativeMsmqMessage message, TimeSpan timeout, MsmqTransactionMode transactionMode, int action)
        {
        ...
        public MoveReceiveResult TryReceiveByLookupId(long lookupId, NativeMsmqMessage message, MsmqTransactionMode transactionMode)
        {
            ...
            try
            {
                error = this.ReceiveByLookupIdCore(handle, lookupId, message, transactionMode, 0x40000020);
            }
            catch (ObjectDisposedException exception)
            {
                MsmqDiagnostics.ExpectedException(exception);
        ...
        private class TryReceiveAsyncResult : TypedAsyncResult<MsmqQueue.ReceiveResult>
        {
            private int action;
            private MsmqQueueHandle handle;
            ...
            private static IOCompletionCallback onPortedCompletion = DiagnosticUtility.Utility.ThunkCallback(new IOCompletionCallback(MsmqQueue.TryReceiveAsyncResult.OnPortedCompletion));
            ...
            private void OnCompletion(int error, bool completedSynchronously)
            {
                ...
                try
                {
                    if (error != 0)
                    {
                        if (error != -1072824293)
                        {
                            ...
                            messageReceived = MsmqQueue.ReceiveResult.OperationCancelled;
                        }
                        else
                        {
                            messageReceived = MsmqQueue.ReceiveResult.Timeout;
                        }
                    }
                }
                ...
                if (exception == null)
                {
                    base.Complete(messageReceived, completedSynchronously);
                }
                else
                {
