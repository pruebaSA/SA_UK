namespace System.ServiceModel.Channels
{
    ...
    internal class SocketConnectionInitiator : IConnectionInitiator
    {
        ...
        public IAsyncResult BeginConnect(Uri uri, TimeSpan timeout, AsyncCallback callback, object state)
        {
            ...
            {
                TraceUtility.TraceEvent(TraceEventType.Information, TraceCode.InitiatingTcpConnection, new StringTraceRecord("Uri", uri.ToString()), this, null);
            }
        ...
        private IConnection CreateConnection(Socket socket) => 
            new SocketConnection(socket, this.bufferSize, false);
        ...
        public static void TraceConnectFailure(Socket socket, SocketException socketException, Uri remoteUri, TimeSpan timeSpentInConnect)
        {
            if (DiagnosticUtility.ShouldTraceWarning)
            {
                ...
                TraceUtility.TraceEvent(TraceEventType.Warning, TraceCode.TcpConnectError, socket, exception);
            }
        }
        ...
        private class ConnectAsyncResult : AsyncResult
        {
            ...
            private static AsyncCallback onConnect = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(SocketConnectionInitiator.ConnectAsyncResult.OnConnect));
            private int port;
            private Socket socket;
            private static WaitCallback startConnectCallback;
            ...
            public ConnectAsyncResult(Uri uri, TimeSpan timeout, AsyncCallback callback, object state) : base(callback, state)
            {
                ...
                else
                {
                    ...
                    {
                        startConnectCallback = new WaitCallback(SocketConnectionInitiator.ConnectAsyncResult.StartConnectCallback);
                    }
            ...
            public static Socket End(IAsyncResult result) => 
                AsyncResult.End<SocketConnectionInitiator.ConnectAsyncResult>(result).socket;
