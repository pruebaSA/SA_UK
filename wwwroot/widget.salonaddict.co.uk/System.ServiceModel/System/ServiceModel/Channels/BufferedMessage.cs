namespace System.ServiceModel.Channels
{
    ...
    internal sealed class BufferedMessage : ReceivedMessage
    {
        ...
        public BufferedMessage(IBufferedMessageData messageData, System.ServiceModel.Channels.RecycledMessageState recycledMessageState) : this(messageData, recycledMessageState, null)
        {
        }

        public BufferedMessage(IBufferedMessageData messageData, System.ServiceModel.Channels.RecycledMessageState recycledMessageState, bool[] understoodHeaders)
        {
            bool flag = true;
            try
            {
                ...
                else
                {
                    ...
                    if (ReceivedMessage.HasHeaderElement(messageReader, envelopeVersion))
                    {
                        ...
                        {
                            this.headers = new MessageHeaders(messageVersion, messageReader, messageData, recycledMessageState, understoodHeaders);
                        }
                        else
                        {
                            this.headers.Init(messageVersion, messageReader, messageData, recycledMessageState, understoodHeaders);
                        }
                    }
                    else
                    {
        ...
        public XmlDictionaryReader GetMessageReader() => 
            this.messageData.GetMessageReader();
        ...
        protected override MessageBuffer OnCreateBufferedCopy(int maxBufferSize)
        {
            if (this.headers.ContainsOnlyBufferedMessageHeaders)
            {
                ...
                return new BufferedMessageBuffer(this.messageData, array, understoodHeaders);
            }
            if (this.reader != null)
            {
