namespace System.ServiceModel.Channels
{
    ...
    internal abstract class ReliableRequestor
    {
        ...
        public IAsyncResult BeginRequest(TimeSpan timeout, AsyncCallback callback, object state) => 
            new RequestAsyncResult(this, timeout, callback, state);
        ...
        private TimeSpan GetNextRequestTimeout(TimeSpan remainingTimeout, out TimeoutHelper iterationTimeout, out bool lastIteration)
        {
            ...
            lastIteration = remainingTimeout <= ReliableMessagingConstants.RequestorIterationTime;
            iterationTimeout.SetDeadline();
        ...
        private bool HandleException(Exception exception, bool lastIteration)
        {
            ...
            return ((this.binder.IsHandleable(exception) && (this.binder.State == CommunicationState.Opened)) && !lastIteration);
        }
        ...
        Label_0014:
            ...
            {
                if (DiagnosticUtility.IsFatal(exception) || !this.HandleException(exception, flag))
                {
                    throw;
                }
        ...
        private class RequestAsyncResult : AsyncResult
        {
            ...
            private static AsyncCallback requestCallback = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ReliableRequestor.RequestAsyncResult.RequestCallback));
            private ReliableRequestor requestor;
            private Message response;
            private TimeoutHelper timeoutHelper;
            private static AsyncCallback waitCallback = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ReliableRequestor.RequestAsyncResult.WaitCallback));
            ...
            public static Message End(IAsyncResult result) => 
                AsyncResult.End<ReliableRequestor.RequestAsyncResult>(result).response;
            ...
            Label_0000:
                ...
                {
                    if (DiagnosticUtility.IsFatal(exception) || !this.requestor.HandleException(exception, this.lastIteration))
                    {
                        throw;
                    }
                    if (DiagnosticUtility.ShouldTraceInformation)
