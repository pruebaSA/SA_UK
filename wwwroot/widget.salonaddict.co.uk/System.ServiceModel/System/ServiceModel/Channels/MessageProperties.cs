namespace System.ServiceModel.Channels
{
    ...
    public sealed class MessageProperties : IDictionary<string, object>, ICollection<KeyValuePair<string, object>>, IEnumerable<KeyValuePair<string, object>>, IEnumerable, IDisposable
    {
        ...
        public void CopyProperties(MessageProperties properties)
        {
            if (properties == null)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentNullException("properties"));
            }
            if (this.disposed)
            {
                this.ThrowDisposed();
            ...
            if (properties.properties != null)
            {
                for (int i = 0; i < properties.properties.Length; i++)
                {
                    ...
                    this[property.Name] = this.CreateCopyOfPropertyValue(property.Value);
                }
            }
        ...
        internal void CopyProperties(KeyValuePair<string, object>[] array)
        {
            ...
            for (int i = 0; i < array.Length; i++)
            {
                ...
                this[pair.Key] = this.CreateCopyOfPropertyValue(pair.Value);
            }
        }
                ...
                if (index < 0)
                {
                    switch (index)
                    {
                        ...
                        case -4:
                            ...
                            this.Security = (SecurityMessageProperty) value;
                            return;
