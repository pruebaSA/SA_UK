namespace System.ServiceModel.Channels
{
    ...
    internal abstract class ConnectionPoolHelper
    {
        ...
        public IAsyncResult BeginEstablishConnection(TimeSpan timeout, AsyncCallback callback, object state) => 
            new EstablishConnectionAsyncResult(this, timeout, callback, state);
        ...
        public IConnection EndEstablishConnection(IAsyncResult result) => 
            EstablishConnectionAsyncResult.End(result);
                ...
                else
                {
                    ...
                    finally
                    {
                        if (!flag2)
                        {
                            if (DiagnosticUtility.ShouldTraceInformation)
                            {
                                DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Information, TraceCode.FailedAcceptFromPool, System.ServiceModel.SR.GetString("TraceCodeFailedAcceptFromPool", new object[] { timeoutHelper.RemainingTime() }));
                            }
        ...
        private IConnection TakeConnection(TimeSpan timeout) => 
            this.connectionPool.TakeConnection(null, this.via, timeout, out this.connectionKey);
        ...
        private class EstablishConnectionAsyncResult : AsyncResult
        {
            ...
            private static AsyncCallback onProcessConnection = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ConnectionPoolHelper.EstablishConnectionAsyncResult.OnProcessConnection));
            ...
            private void Cleanup()
            {
                if (this.cleanupConnection)
                {
                    ...
                    else if (this.rawConnection != null)
                    {
                        if (DiagnosticUtility.ShouldTraceInformation)
                        {
                            DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Information, TraceCode.FailedAcceptFromPool, System.ServiceModel.SR.GetString("TraceCodeFailedAcceptFromPool", new object[] { this.timeoutHelper.RemainingTime() }));
                        }
            ...
            public static IConnection End(IAsyncResult result) => 
                AsyncResult.End<ConnectionPoolHelper.EstablishConnectionAsyncResult>(result).currentConnection;
            ...
            private bool OpenUsingNewConnection()
            {
                ...
                try
                {
                    ...
                    if (onConnect == null)
                    {
                        onConnect = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ConnectionPoolHelper.EstablishConnectionAsyncResult.OnConnect));
                    }
