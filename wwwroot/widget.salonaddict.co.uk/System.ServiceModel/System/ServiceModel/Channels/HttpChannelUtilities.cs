namespace System.ServiceModel.Channels
{
    ...
    internal static class HttpChannelUtilities
    {
        ...
        public static Exception CreateNullReferenceResponseException(NullReferenceException nullReferenceException) => 
            TraceResponseException(new ProtocolException(System.ServiceModel.SR.GetString("NullReferenceOnHttpResponse"), nullReferenceException));
        ...
        public static Exception CreateRequestIOException(IOException ioException, HttpWebRequest request) => 
            CreateRequestIOException(ioException, request, null);
        ...
        private static string CreateRequestTimedOutMessage(HttpWebRequest request) => 
            System.ServiceModel.SR.GetString("HttpRequestTimedOut", new object[] { request.RequestUri, TimeSpan.FromMilliseconds((double) request.Timeout) });
        ...
        public static HttpWebResponse ProcessGetResponseWebException(WebException webException, HttpWebRequest request, HttpAbortReason abortReason)
        {
            ...
            switch (webException.Status)
            {
                ...
                case WebExceptionStatus.ReceiveFailure:
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new CommunicationException(System.ServiceModel.SR.GetString("HttpReceiveFailure", new object[] { request.RequestUri }), webException));

                case WebExceptionStatus.SendFailure:
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new CommunicationException(System.ServiceModel.SR.GetString("HttpSendFailure", new object[] { request.RequestUri }), webException));

                case WebExceptionStatus.RequestCanceled:
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(CreateRequestCanceledException(webException, request, abortReason));

                case WebExceptionStatus.TrustFailure:
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new SecurityNegotiationException(System.ServiceModel.SR.GetString("TrustFailure", new object[] { request.RequestUri.Authority }), webException));

                case WebExceptionStatus.SecureChannelFailure:
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new SecurityNegotiationException(System.ServiceModel.SR.GetString("SecureChannelFailure", new object[] { request.RequestUri.Authority }), webException));

                case WebExceptionStatus.Timeout:
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new TimeoutException(CreateRequestTimedOutMessage(request), webException));
            }
            if (response == null)
            {
        ...
        private static Exception TraceResponseException(Exception exception)
        {
            ...
            {
                TraceUtility.TraceEvent(TraceEventType.Error, TraceCode.HttpChannelUnexpectedResponse, null, exception);
            }
            return exception;
        }
