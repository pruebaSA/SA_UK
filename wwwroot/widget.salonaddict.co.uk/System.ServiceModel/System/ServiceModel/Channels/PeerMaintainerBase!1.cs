namespace System.ServiceModel.Channels
{
    ...
    internal class PeerMaintainerBase<TConnectAlgorithms> : IPeerMaintainer where TConnectAlgorithms: IConnectAlgorithms, new()
    {
        ...
        public PeerMaintainerBase(PeerNodeConfig config, PeerNeighborManager neighborManager, PeerFlooder flooder)
        {
            ...
            this.maintainerTimer = new IOThreadTimer(new WaitCallback(this.OnMaintainerTimer), this, false);
        }
            ...
            if (this.isOpen)
            {
                ...
                if (flag)
                {
                    if (DiagnosticUtility.ShouldTraceInformation)
                    {
                        ...
                        TraceUtility.TraceEvent(TraceEventType.Information, TraceCode.PeerMaintainerActivity, extendedData, this, null);
                    }
                ...
                if (flag)
                {
                            ...
                            using (IConnectAlgorithms algorithms = (default(TConnectAlgorithms) == null) ? ((IConnectAlgorithms) Activator.CreateInstance<TConnectAlgorithms>()) : ((IConnectAlgorithms) default(TConnectAlgorithms)))
                            {
                                ...
                                if (connectedNeighborCount > this.config.IdealNeighbors)
                                {
                                    if (DiagnosticUtility.ShouldTraceInformation)
                                    {
                                        ...
                                        TraceUtility.TraceEvent(TraceEventType.Information, TraceCode.PeerMaintainerActivity, record2, this, null);
                                    }
                                ...
                                if (this.neighborManager.ConnectedNeighborCount < this.config.IdealNeighbors)
                                {
                                    ...
                                    if (DiagnosticUtility.ShouldTraceInformation)
                                    {
                                        ...
                                        TraceUtility.TraceEvent(TraceEventType.Information, TraceCode.PeerMaintainerActivity, record3, this, null);
                                    }
                                    algorithms.Connect(helper.RemainingTime());
                                }
                            }
                    ...
                    finally
                    {
                        if (DiagnosticUtility.ShouldTraceInformation)
                        {
                            ...
                            TraceUtility.TraceEvent(TraceEventType.Information, TraceCode.PeerMaintainerActivity, record4, this, null);
                        }
                    }
        ...
        {
            IOThreadScheduler.ScheduleCallback(new WaitCallback(this.MaintainConnections), null);
        }
        ...
        public void ScheduleConnect(ConnectCallback<TConnectAlgorithms> connectCallback)
        {
            ...
            IOThreadScheduler.ScheduleCallback(new WaitCallback(this.InitialConnection), null);
        }
        ...
        IPeerNeighbor IPeerMaintainer.EndOpenNeighbor(IAsyncResult result) => 
            this.neighborManager.EndOpenNeighbor(result);

        IPeerNeighbor IPeerMaintainer.FindDuplicateNeighbor(PeerNodeAddress address) => 
            this.neighborManager.FindDuplicateNeighbor(address);
        ...
        PeerNodeAddress IPeerMaintainer.GetListenAddress() => 
            this.config.GetListenAddress(true);

        private void SystemEvents_PowerModeChanged(object sender, PowerModeChangedEventArgs e)
        {
            if ((e.Mode == PowerModes.Resume) && this.isOpen)
            {
                IOThreadScheduler.ScheduleCallback(new WaitCallback(this.PingAndRefresh), null);
            }
        }

        int IPeerMaintainer.ConnectedNeighborCount =>
            this.neighborManager.ConnectedNeighborCount;

        bool IPeerMaintainer.IsOpen =>
            this.isOpen;

        int IPeerMaintainer.NonClosingNeighborCount =>
            this.neighborManager.NonClosingNeighborCount;
