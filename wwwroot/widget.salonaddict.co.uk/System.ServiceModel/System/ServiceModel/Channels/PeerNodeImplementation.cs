namespace System.ServiceModel.Channels
{
    ...
    internal class PeerNodeImplementation : IPeerNodeMessageHandling
    {
        ...
        public IAsyncResult BeginClose(TimeSpan timeout, AsyncCallback callback, object state) => 
            this.stateManager.BeginClose(timeout, callback, state);

        public IAsyncResult BeginOpen(TimeSpan timeout, AsyncCallback callback, object state, bool waitForOnline) => 
            this.stateManager.BeginOpen(timeout, callback, state, waitForOnline);
        ...
        public IAsyncResult BeginSend(object registrant, Message message, Uri via, ITransportFactorySettings settings, TimeSpan timeout, AsyncCallback callback, object state, SecurityProtocol securityProtocol)
        {
            ...
            AsyncCallback callback2 = System.ServiceModel.DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(result.OnFloodComplete));
            try
            {
                PeerFlooder flooder;
        ...
        private void CloseCore(TimeSpan timeout, bool graceful)
        {
            ...
            try
            {
                ...
                {
                    IOThreadScheduler.ScheduleCallback(new WaitCallback(this.UnregisterAddress), config.UnregisterTimeout);
                }
            }
            ...
            try
            {
                ...
                if (maintainer != null)
                {
                    ...
                    {
                        if (System.ServiceModel.DiagnosticUtility.IsFatal(exception4))
                        {
                            throw;
                        }
                ...
                if (ipHelper != null)
                {
                    ...
                    {
                        if (System.ServiceModel.DiagnosticUtility.IsFatal(exception5))
                        {
                            throw;
                        }
                ...
                if (neighborManager != null)
                {
                    ...
                    {
                        if (System.ServiceModel.DiagnosticUtility.IsFatal(exception6))
                        {
                            throw;
                        }
                ...
                if (connector != null)
                {
                    ...
                    {
                        if (System.ServiceModel.DiagnosticUtility.IsFatal(exception7))
                        {
                            throw;
                        }
                ...
                if (flooder != null)
                {
                    ...
                    {
                        if (System.ServiceModel.DiagnosticUtility.IsFatal(exception8))
                        {
                            throw;
                        }
            ...
            {
                if (System.ServiceModel.DiagnosticUtility.IsFatal(exception9))
                {
                    throw;
                }
                if (exception == null)
            ...
            if (!graceful && (aborted != null))
            {
                ...
                {
                    if (System.ServiceModel.DiagnosticUtility.IsFatal(exception10))
                    {
                        throw;
                    }
        ...
        private bool CompareVia(Uri via1, Uri via2) => 
            (Uri.Compare(via1, via2, UriComponents.Path | UriComponents.SchemeAndServer | UriComponents.UserInfo, UriFormat.SafeUnescaped, StringComparison.OrdinalIgnoreCase) == 0);
        ...
        {
            bool aborted = false;
            EventHandler b = (source, args) => this.connectCompletedEvent.Set();
            ...
            try
            {
                this.NeighborConnected = (EventHandler) Delegate.Combine(this.NeighborConnected, b);
                this.Aborted = (EventHandler) Delegate.Combine(this.Aborted, handler2);
                this.OpenCore(timeout);
                if (waitForOnline && !TimeoutHelper.WaitOne(this.connectCompletedEvent, helper.RemainingTime(), false))
                {
            ...
            {
                this.NeighborConnected = (EventHandler) Delegate.Remove(this.NeighborConnected, b);
                this.Aborted = (EventHandler) Delegate.Remove(this.Aborted, handler2);
            }
        }
        ...
        private void OpenCore(TimeSpan timeout)
        {
            ...
            lock (this.ThisLock)
            {
                ...
                Dictionary<Type, object> serviceHandlers = this.serviceHandlers;
                if (serviceHandlers == null)
                {
                    serviceHandlers = new Dictionary<Type, object> {
                        { 
                            typeof(IPeerConnectorContract),
                            this.connector
                        },
                        { 
                            typeof(IPeerFlooderContract<Message, UtilityInfo>),
                            this.flooder
                        }
                    };
                }
        ...
        void IPeerNodeMessageHandling.HandleIncomingMessage(MessageBuffer messageBuffer, PeerMessagePropagation propagateFlags, int index, MessageHeader hopHeader, Uri via, Uri to)
        {
            ...
            else if (System.ServiceModel.DiagnosticUtility.ShouldTraceVerbose)
            {
                ...
                {
                    TraceUtility.TraceEvent(TraceEventType.Verbose, TraceCode.PeerFloodedMessageNotPropagated, this.traceRecord, this, null, message2);
                }
            }
        }
        ...
        bool IPeerNodeMessageHandling.HasMessagePropagation =>
            (this.messagePropagationFilter != null);
        ...
        private class SimpleStateManager
        {
            ...
            private void RunQueue()
            {
                if (!this.queueRunning)
                {
                    ...
                    IOThreadScheduler.ScheduleCallback(new WaitCallback(this.RunQueueCallback), null);
                }
            }
                ...
                protected void Complete(Exception exception)
                {
                    if (!this.completed)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                IOThreadScheduler.ScheduleCallback(new WaitCallback(this.AsyncComplete), exception);
                            }
                            else
                            {
