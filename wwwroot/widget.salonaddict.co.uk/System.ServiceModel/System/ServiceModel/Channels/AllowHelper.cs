            ...
            if (singleton == null)
            {
                lock (ThisLock)
                {
                    if (singleton == null)
                    {
                        ...
                        {
                            ListenerUnsafeNativeMethods.ICorRuntimeHost host;
                            object obj2;
                            Guid rclsid = new Guid("CB2F6723-AB3A-11D2-9C40-00C04FA30A3E");
                            Guid riid = new Guid("CB2F6722-AB3A-11D2-9C40-00C04FA30A3E");
                            ListenerUnsafeNativeMethods.CorBindToRuntimeEx(IntPtr.Zero, IntPtr.Zero, 0, ref rclsid, ref riid, out host);
                            host.GetDefaultDomain(out obj2);
