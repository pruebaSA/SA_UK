    ...
    internal static class ServiceMemoryGates
    {
        ...
        internal static void Check(int minFreeMemoryPercentage)
        {
            if ((minFreeMemoryPercentage >= 1) && (minFreeMemoryPercentage <= 0x63))
            {
                ...
                if (memoryStatus.ullAvailPageFile < num)
                {
                    ...
                    if (memoryStatus.ullAvailPageFile < num)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InsufficientMemoryException(System.ServiceModel.SR.GetString("Hosting_MemoryGatesCheckFailed", new object[] { memoryStatus.ullAvailPageFile, minFreeMemoryPercentage })));
                    }
                }
            }
        }
        ...
        private static void GetGlobalMemoryStatus(ref UnsafeNativeMethods.MEMORYSTATUSEX memoryStatus)
        {
            ...
            if (!UnsafeNativeMethods.GlobalMemoryStatusEx(ref memoryStatus))
            {
                ...
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(System.ServiceModel.SR.GetString("Hosting_GetGlobalMemoryFailed"), new Win32Exception(error)));
            }
        }
    }
}
