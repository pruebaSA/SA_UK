namespace System.ServiceModel
{
    ...
    public sealed class TransactionFlowAttribute : Attribute, IOperationBehavior
    {
        ...
        {
            EnsureDictionary(parameters).Add(new DirectionalAction(description.Messages[0].Direction, description.Messages[0].Action), this.transactions);
        }

        private static Dictionary<DirectionalAction, TransactionFlowOption> EnsureDictionary(BindingParameterCollection parameters)
        {
            Dictionary<DirectionalAction, TransactionFlowOption> item = parameters.Find<Dictionary<DirectionalAction, TransactionFlowOption>>();
            if (item == null)
            {
                item = new Dictionary<DirectionalAction, TransactionFlowOption>();
                parameters.Add(item);
            }
            return item;
        }
