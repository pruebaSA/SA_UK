    ...
    internal static class MessageLogger
    {
            ...
            if (numberOfMessagesToLog > 0)
            {
                lock (syncObject)
                {
                    if (numberOfMessagesToLog > 0)
                    {
                        ...
                        {
                            TraceUtility.TraceEvent(TraceEventType.Information, TraceCode.MessageCountLimitExceeded, data);
                        }
                    }
                }
            }
        ...
        private static void InitializeMessageTraceSource()
        {
            try
            {
                ...
                PiiTraceSource source = new PiiTraceSource("System.ServiceModel.MessageLogging", "System.ServiceModel 3.0.0.0") {
            ...
            catch (SecurityException exception)
            {
                ...
                {
                    DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Warning, TraceCode.TraceHandledException, System.ServiceModel.SR.GetString("PartialTrustMessageLoggingNotEnabled"), null, exception);
                }
        ...
        private static void LogInternal(object data)
        {
        ...
        private static void LogInternal(MessageLogTraceRecord record)
        {
            ...
            try
            {
                ...
                if (!messageTraceSource.ShouldLogPii)
                {
        ...
        internal static void LogMessage(string data)
        {
            try
            {
                ...
                lock (syncObject)
                {
                    foreach (XPathMessageFilter filter in Filters)
                    {
                        ...
                        catch (MessageFilterException exception)
                        {
                            ...
                            {
                                TraceUtility.TraceEvent(TraceEventType.Information, TraceCode.FilterNotMatchedNodeQuotaExceeded, (Exception) exception, message);
                            }
                        }
                    }
        ...
        private static bool ShouldLogMessages(MessageLoggingSource source) => 
            (((source & Sources) != MessageLoggingSource.None) && (null != MessageTraceSource));
        ...
        {
            get => 
                ((Sources & MessageLoggingSource.Malformed) != MessageLoggingSource.None);
            set
            {
                lock (syncObject)
                {
        ...
        {
            get => 
                ((Sources & MessageLoggingSource.ServiceLevel) != MessageLoggingSource.None);
            set
            {
                lock (syncObject)
                {
        ...
        {
            get => 
                ((Sources & MessageLoggingSource.Transport) != MessageLoggingSource.None);
            set
            {
                lock (syncObject)
                {
        ...
        internal static bool ShouldLogMalformed =>
            ShouldLogMessages(MessageLoggingSource.Malformed);
