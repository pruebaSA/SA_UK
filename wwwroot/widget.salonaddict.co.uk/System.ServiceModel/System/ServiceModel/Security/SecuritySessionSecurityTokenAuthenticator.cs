namespace System.ServiceModel.Security
{
    ...
    internal class SecuritySessionSecurityTokenAuthenticator : CommunicationObjectSecurityTokenAuthenticator, IIssuanceSecurityTokenAuthenticator, ILogonTokenCacheManager
    {
        ...
        protected override bool CanValidateTokenCore(SecurityToken token) => 
            (token is SecurityContextSecurityToken);
        ...
        internal static ReadOnlyCollection<IAuthorizationPolicy> CreateSecureConversationPolicies(SecurityMessageProperty security, DateTime expirationTime) => 
            CreateSecureConversationPolicies(security, expirationTime, null);

        private static ReadOnlyCollection<IAuthorizationPolicy> CreateSecureConversationPolicies(SecurityMessageProperty security, DateTime expirationTime, SecurityContextSecurityToken currentToken)
        {
            if (security == null)
            {
                throw System.ServiceModel.DiagnosticUtility.ExceptionUtility.ThrowHelperArgumentNull("security");
            }
            ReadOnlyCollection<IAuthorizationPolicy> initiatorTokenAuthorizationPolicies = security.GetInitiatorTokenAuthorizationPolicies(false, currentToken);
            Collection<IAuthorizationPolicy> list = new Collection<IAuthorizationPolicy>();
            for (int i = 0; i < initiatorTokenAuthorizationPolicies.Count; i++)
            {
                IAuthorizationPolicy policy;
                if (initiatorTokenAuthorizationPolicies[i].GetType() == typeof(UnconditionalPolicy))
                {
                    UnconditionalPolicy policy2 = (UnconditionalPolicy) initiatorTokenAuthorizationPolicies[i];
                    policy = new UnconditionalPolicy(policy2.PrimaryIdentity, policy2.Issuances, expirationTime);
                }
                else
                {
                    policy = initiatorTokenAuthorizationPolicies[i];
                }
                list.Add(policy);
            }
        ...
        private static SecurityContextSecurityToken GetMatchingEndorsingSct(SecurityContextKeyIdentifierClause sctSkiClause, SecurityMessageProperty supportingTokenProperty)
        {
        ...
        private SecurityContextSecurityToken IssueToken(RequestSecurityToken rst, Message request, SecurityContextSecurityToken currentToken, out RequestSecurityTokenResponse rstr)
        {
            ...
            ReadOnlyCollection<IAuthorizationPolicy> authorizationPolicies = (security != null) ? CreateSecureConversationPolicies(security, keyExpirationTime, currentToken) : EmptyReadOnlyCollection<IAuthorizationPolicy>.Instance;
        ...
        protected virtual Message ProcessIssueRequest(Message request)
        {
            ...
            try
            {
                ...
                SecurityContextSecurityToken newSessionToken = this.IssueToken(token, request, null, out response);
                if (address != null)
                {
                    if (request.Version.Addressing != AddressingVersion.WSAddressing10)
                    {
                ...
                if (this.StandardsManager.MessageSecurityVersion.TrustVersion == TrustVersion.WSTrust13)
                {
                    ...
                    RequestSecurityTokenResponseCollection responses = new RequestSecurityTokenResponseCollection(rstrCollection, this.StandardsManager);
                    body = responses;
                }
        ...
        protected virtual Message ProcessRenewRequest(Message request)
        {
            ...
            try
            {
                ...
                SecurityContextSecurityToken matchingEndorsingSct = GetMatchingEndorsingSct(renewTarget, security);
                ...
                SecurityContextSecurityToken newSessionToken = this.IssueToken(token, request, matchingEndorsingSct, out response);
                response.MakeReadOnly();
                BodyWriter body = response;
                if (this.StandardsManager.MessageSecurityVersion.TrustVersion == TrustVersion.WSTrust13)
                {
                ...
                this.NotifyOperationCompletion(SecuritySessionOperation.Renew, newSessionToken, matchingEndorsingSct, request.Headers.ReplyTo);
        ...
        private void SetupSessionListener()
        {
            ...
            MessageFilter filter = new SessionActionFilter(this.standardsManager, new string[] { this.IssueAction.Value, this.RenewAction.Value });
        ...
        public virtual XmlDictionaryString CloseAction =>
            this.standardsManager.SecureConversationDriver.CloseAction;

        public virtual XmlDictionaryString CloseResponseAction =>
            this.standardsManager.SecureConversationDriver.CloseResponseAction;
        ...
        public virtual XmlDictionaryString IssueResponseAction =>
            this.standardsManager.SecureConversationDriver.IssueResponseAction;
        ...
        public virtual XmlDictionaryString RenewResponseAction =>
            this.standardsManager.SecureConversationDriver.RenewResponseAction;
        ...
        private class SecuritySessionAuthenticatorBindingElement : BindingElement
        {
            ...
            public override BindingElement Clone() => 
                new SecuritySessionSecurityTokenAuthenticator.SecuritySessionAuthenticatorBindingElement(this.authenticator);
        ...
        private class SecuritySessionHost : ServiceHostBase
        {
            ...
            protected override void InitializeRuntime()
            {
                ...
                Type[] supportedChannels = new Type[] { typeof(IReplyChannel), typeof(IDuplexChannel), typeof(IReplySessionChannel), typeof(IDuplexSessionChannel) };
                ...
                EndpointDispatcher dispatcher2 = new EndpointDispatcher(new EndpointAddress(this.listenUri, new AddressHeader[0]), "IssueAndRenewSession", "http://tempuri.org/") {
                    DispatchRuntime = { 
        ...
        public class SessionRenewSecurityTokenManager : SecurityTokenManager
        {
            ...
            public override SecurityTokenProvider CreateSecurityTokenProvider(SecurityTokenRequirement requirement) => 
                this.innerTokenManager.CreateSecurityTokenProvider(requirement);

            public override SecurityTokenSerializer CreateSecurityTokenSerializer(SecurityTokenVersion version) => 
                this.innerTokenManager.CreateSecurityTokenSerializer(version);
        }
    }
}
