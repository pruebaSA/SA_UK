namespace System.ServiceModel.Security
{
    ...
    internal abstract class ReceiveSecurityHeader : SecurityHeader
    {
        ...
        internal XmlDictionaryReader CreateDecryptedReader(byte[] decryptedBuffer) => 
            ContextImportHelper.CreateSplicedReader(decryptedBuffer, this.SecurityVerifiedMessage.GetEnvelopeAttributes(), this.SecurityVerifiedMessage.GetHeaderAttributes(), this.securityElementAttributes, this.ReaderQuotas);

        internal XmlDictionaryReader CreateSecurityHeaderReader() => 
            this.securityVerifiedMessage.GetReaderAtSecurityHeader();
        ...
        internal void ExecuteDerivedKeyTokenStubPass(bool isFinalPass)
        {
            for (int i = 0; i < this.elementManager.Count; i++)
            {
                if (this.elementManager.GetElementCategory(i) == ReceiveSecurityHeaderElementCategory.Token)
                {
                    ...
                    if (element != null)
                    {
                        SecurityToken tokenToDerive = this.universalTokenResolver.ResolveToken(element.TokenToDeriveIdentifier);
                        if (tokenToDerive != null)
                        {
                            this.EnsureDerivedKeyLimitNotReached();
                            DerivedKeySecurityToken token2 = element.CreateToken(tokenToDerive, this.maxDerivedKeyLength);
        ...
        public SignatureConfirmations GetSentSignatureConfirmations() => 
            this.receivedSignatureConfirmations;

        public SignatureConfirmations GetSentSignatureValues() => 
            this.receivedSignatureValues;
        ...
        public SecurityTokenResolver CombinedUniversalTokenResolver =>
            this.combinedUniversalTokenResolver;
        ...
        {
            get => 
                this.expectSignatureConfirmation;
            set
            {
                base.ThrowIfProcessingStarted();
        ...
        {
            get => 
                this.hasAtLeastOneItemInsideSecurityHeaderEncrypted;
            set
            {
        ...
        private struct OrderTracker
        {
            ...
            public bool AllSignaturesEncrypted =>
                (this.unencryptedSignatureCount == 0);
            public bool EncryptBeforeSignMode =>
                (this.enforce && (this.protectionOrder == MessageProtectionOrder.EncryptBeforeSign));
            public bool EncryptBeforeSignOrderRequirementMet =>
                ((this.state != ReceiverProcessingOrder.DecryptVerify) && (this.state != ReceiverProcessingOrder.Mixed));
            public bool PrimarySignatureDone =>
                (this.signatureCount > 0);
            public bool SignBeforeEncryptOrderRequirementMet =>
                ((this.state != ReceiverProcessingOrder.VerifyDecrypt) && (this.state != ReceiverProcessingOrder.Mixed));
