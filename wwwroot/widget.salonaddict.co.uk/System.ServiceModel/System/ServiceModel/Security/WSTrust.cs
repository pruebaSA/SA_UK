    ...
    internal abstract class WSTrust : System.ServiceModel.Security.WSSecurityTokenSerializer.SerializerEntries
    {
        ...
        internal class BinarySecretClauseEntry : WSSecurityTokenSerializer.KeyIdentifierClauseEntry
        {
            ...
            public override SecurityKeyIdentifierClause ReadKeyIdentifierClauseCore(XmlDictionaryReader reader) => 
                new BinarySecretKeyIdentifierClause(reader.ReadElementContentAsBase64(), false);
        ...
        private class BinarySecretTokenEntry : WSSecurityTokenSerializer.TokenEntry
        {
            ...
            protected override Type[] GetTokenTypesCore() => 
                new Type[] { typeof(BinarySecretSecurityToken) };
        ...
        public abstract class Driver : TrustDriver
        {
            ...
            public override bool IsAppliesTo(string localName, string namespaceUri) => 
                ((localName == this.DriverDictionary.AppliesTo.Value) && (namespaceUri == "http://schemas.xmlsoap.org/ws/2004/09/policy"));

            public override bool IsAtRequestSecurityTokenResponse(XmlReader reader) => 
                reader?.IsStartElement(this.DriverDictionary.RequestSecurityTokenResponse.Value, this.DriverDictionary.Namespace.Value);

            public override bool IsAtRequestSecurityTokenResponseCollection(XmlReader reader) => 
                reader?.IsStartElement(this.DriverDictionary.RequestSecurityTokenResponseCollection.Value, this.DriverDictionary.Namespace.Value);

            internal override bool IsCanonicalizationAlgorithmElement(XmlElement element, out string canonicalizationAlgorithm) => 
                WSTrust.CheckElement(element, this.DriverDictionary.CanonicalizationAlgorithm.Value, this.DriverDictionary.Namespace.Value, out canonicalizationAlgorithm);

            internal override bool IsEncryptionAlgorithmElement(XmlElement element, out string encryptionAlgorithm) => 
                WSTrust.CheckElement(element, this.DriverDictionary.EncryptionAlgorithm.Value, this.DriverDictionary.Namespace.Value, out encryptionAlgorithm);

            internal override bool IsEncryptWithElement(XmlElement element, out string encryptWithAlgorithm) => 
                WSTrust.CheckElement(element, this.DriverDictionary.EncryptWith.Value, this.DriverDictionary.Namespace.Value, out encryptWithAlgorithm);

            public override bool IsRequestedProofTokenElement(string name, string nameSpace) => 
                ((name == this.DriverDictionary.RequestedProofToken.Value) && (nameSpace == this.DriverDictionary.Namespace.Value));

            public override bool IsRequestedSecurityTokenElement(string name, string nameSpace) => 
                ((name == this.DriverDictionary.RequestedSecurityToken.Value) && (nameSpace == this.DriverDictionary.Namespace.Value));

            internal override bool IsSignWithElement(XmlElement element, out string signatureAlgorithm) => 
                WSTrust.CheckElement(element, this.DriverDictionary.SignWith.Value, this.DriverDictionary.Namespace.Value, out signatureAlgorithm);
            ...
            protected virtual void ReadReferences(XmlElement rstrXml, out SecurityKeyIdentifierClause requestedAttachedReference, out SecurityKeyIdentifierClause requestedUnattachedReference)
            {
                ...
                if (childElement != null)
                {
                    ...
                    if (requestedUnattachedReference == null)
                    {
                        ...
                        {
                            throw System.ServiceModel.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException(System.ServiceModel.SR.GetString("TrustApr2004DoesNotSupportCertainIssuedTokens")));
                        }
                    }
                }
            }
            ...
            private bool TryParsePublicKeyElement(XmlElement element) => 
                (((element?.LocalName == this.DriverDictionary.KeyType.Value) && (element.NamespaceURI == this.DriverDictionary.Namespace.Value)) && (element.InnerText == this.DriverDictionary.PublicKeyType.Value));
            ...
            public bool TryParseSymmetricKeyElement(XmlElement element) => 
                (((element?.LocalName == this.DriverDictionary.KeyType.Value) && (element.NamespaceURI == this.DriverDictionary.Namespace.Value)) && (element.InnerText == this.DriverDictionary.SymmetricKeyType.Value));
            ...
            public override XmlDictionaryString RequestSecurityTokenAction =>
                this.DriverDictionary.RequestSecurityTokenIssuance;

            public override XmlDictionaryString RequestSecurityTokenResponseAction =>
                this.DriverDictionary.RequestSecurityTokenIssuanceResponse;

            public override string RequestTypeIssue =>
                this.DriverDictionary.RequestTypeIssue.Value;
