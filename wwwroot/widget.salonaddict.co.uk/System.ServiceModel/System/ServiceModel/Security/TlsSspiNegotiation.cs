namespace System.ServiceModel.Security
{
    ...
    internal sealed class TlsSspiNegotiation : ISspiNegotiation, IDisposable
    {
        ...
        internal void DecryptInPlace(byte[] encryptedContent, out int dataStartOffset, out int dataLen)
        {
            ...
            SecurityBuffer[] input = new SecurityBuffer[] { new SecurityBuffer(encryptedContent, 0, encryptedContent.Length, BufferType.Data), new SecurityBuffer(data, BufferType.Empty), new SecurityBuffer(buffer2, BufferType.Empty), new SecurityBuffer(buffer3, BufferType.Empty) };
        ...
        internal void EncryptInPlace(byte[] buffer, int bufferStartOffset, int dataLen, out int encryptedDataLen)
        {
            ...
            SecurityBuffer[] input = new SecurityBuffer[] { new SecurityBuffer(buffer, bufferStartOffset, this.StreamSizes.header, BufferType.Header), new SecurityBuffer(buffer, bufferStartOffset + this.StreamSizes.header, dataLen, BufferType.Data), new SecurityBuffer(buffer, offset, this.StreamSizes.trailer, BufferType.Trailer), new SecurityBuffer(data, BufferType.Empty) };
            ...
            for (int i = 0; i < input.Length; i++)
            {
                if (input[i].type == BufferType.Trailer)
                {
                    size = input[i].size;
        ...
        private SafeFreeCertContext ExtractCertificateHandle(ContextAttribute contextAttribute) => 
            (SspiWrapper.QueryContextAttributes(this.securityContext, contextAttribute) as SafeFreeCertContext);
        ...
        internal bool TryGetContextIdentity(out WindowsIdentity mappedIdentity)
        {
            ...
            try
            {
                ...
                mappedIdentity = new WindowsIdentity(token.DangerousGetHandle());
                flag = true;
            }
            finally
            {
        ...
        {
            if ((this.clientCertificate != null) && !HasPrivateKey(this.clientCertificate))
            {
                throw System.ServiceModel.DiagnosticUtility.ExceptionUtility.ThrowHelperArgument(System.ServiceModel.SR.GetString("SslClientCertMustHavePrivateKey", new object[] { this.clientCertificate.SubjectName.Name }));
            }
        }
        ...
        private void ValidateServerCertificate()
        {
            ...
            if (!HasPrivateKey(this.serverCertificate))
            {
                throw System.ServiceModel.DiagnosticUtility.ExceptionUtility.ThrowHelperArgument(System.ServiceModel.SR.GetString("SslServerCertMustDoKeyExchange", new object[] { this.serverCertificate.SubjectName.Name }));
            }
        }
        ...
        public bool IsValidContext =>
            ((this.securityContext != null) && !this.securityContext.IsInvalid);
