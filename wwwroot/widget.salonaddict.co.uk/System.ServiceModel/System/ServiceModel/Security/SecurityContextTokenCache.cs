namespace System.ServiceModel.Security
{
    ...
    internal sealed class SecurityContextTokenCache : TimeBoundedCache
    {
        ...
        public SecurityContextTokenCache(int capacity, bool replaceOldestEntries) : base(lowWaterMark, capacity, null, PurgingMode.TimerBasedPurge, purgingInterval, true)
        {
        ...
        public bool TryAddContext(SecurityContextSecurityToken token) => 
            this.TryAddContext(token, false);
        ...
        private bool TryAddContext(SecurityContextSecurityToken token, bool throwOnFailure)
        {
            ...
            if ((utcNow < token.ValidFrom) || (utcNow > token.ValidTo))
            {
            ...
            if ((utcNow < token.KeyEffectiveTime) || (utcNow > token.KeyExpirationTime))
            {
                if (token.KeyGeneration == null)
                {
        ...
        private struct ContextAndGenerationKey
        {
            ...
            public override int GetHashCode() => 
                (this.contextId.GetHashCode() ^ this.generation.GetHashCode());
