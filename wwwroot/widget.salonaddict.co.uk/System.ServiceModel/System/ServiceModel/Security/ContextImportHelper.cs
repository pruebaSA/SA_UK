namespace System.ServiceModel.Security
{
    ...
    internal static class ContextImportHelper
    {
        ...
        private static bool IsNamespaceDeclaration(string prefix, string localName) => 
            (GetPrefixIfNamespaceDeclaration(prefix, localName) != null);
        ...
        internal static byte[] SpliceBuffers(byte[] middle, byte[] wrapper, int wrapperLength, int wrappingDepth)
        {
            ...
            int dstOffset = 0;
            int count = index - 1;
            System.Buffer.BlockCopy(wrapper, 0, dst, dstOffset, count);
            dstOffset += count;
            count = middle.Length;
            System.Buffer.BlockCopy(middle, 0, dst, dstOffset, count);
            dstOffset += count;
            count = wrapperLength - index;
            System.Buffer.BlockCopy(wrapper, index, dst, dstOffset, count);
            return dst;
        }
