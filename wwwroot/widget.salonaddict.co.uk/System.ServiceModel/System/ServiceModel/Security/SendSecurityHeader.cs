namespace System.ServiceModel.Security
{
    ...
    internal abstract class SendSecurityHeader : SecurityHeader, IMessageHeaderWithSharedNamespace
    {
        ...
        {
            ISignatureValueSecurityElement item = this.CompletePrimarySignatureCore(this.elementContainer.GetSignatureConfirmations(), this.elementContainer.GetSignedEndorsingSupportingTokens(), this.elementContainer.GetSignedSupportingTokens(), this.elementContainer.GetBasicSupportingTokens(), true);
            if (item != null)
            {
        ...
        protected override void OnWriteHeaderContents(XmlDictionaryWriter writer, MessageVersion messageVersion)
        {
            ...
            if (signedEndorsingSupportingTokens != null)
            {
                for (int n = 0; n < signedEndorsingSupportingTokens.Length; n++)
                {
                    ...
                    this.WriteSecurityTokenReferencyEntry(writer, signedEndorsingSupportingTokens[n], this.signedEndorsingTokenParameters[n]);
                }
            }
        ...
        {
            IssuedSecurityTokenParameters parameters = null;
            keyIdentifierClause = null;
            switch (mode)
            {
                case SecurityTokenAttachmentMode.Signed:
                    parameters = this.signedTokenParameters[position] as IssuedSecurityTokenParameters;
                    break;

                case SecurityTokenAttachmentMode.SignedEndorsing:
                    parameters = this.signedEndorsingTokenParameters[position] as IssuedSecurityTokenParameters;
                    break;

                case SecurityTokenAttachmentMode.SignedEncrypted:
                    parameters = this.basicSupportingTokenParameters[position] as IssuedSecurityTokenParameters;
                    break;

                default:
                    return false;
            }
            if ((parameters == null) || !parameters.UseStrTransform)
            {
                return false;
            }
            keyIdentifierClause = parameters.CreateKeyIdentifierClause(securityToken, this.GetTokenReferenceStyle(parameters));
            ...
            if (endorsingSupportingTokens != null)
            {
                for (int i = 0; i < endorsingSupportingTokens.Length; i++)
                {
                    ...
                    if (this.endorsingTokenParameters[i].RequireDerivedKeys && !this.endorsingTokenParameters[i].HasAsymmetricKey)
                    {
                        ...
                        token2 = token3;
                        clause2 = new LocalIdKeyIdentifierClause(token3.Id);
            ...
            if (signedEndorsingSupportingTokens != null)
            {
                for (int j = 0; j < signedEndorsingSupportingTokens.Length; j++)
                {
                    ...
                    if (this.signedEndorsingTokenParameters[j].RequireDerivedKeys && !this.signedEndorsingTokenParameters[j].HasAsymmetricKey)
                    {
                        ...
                        token5 = token6;
                        clause4 = new LocalIdKeyIdentifierClause(token6.Id);
        ...
        private void StartEncryption()
        {
            if (this.elementContainer.SourceEncryptionToken != null)
            {
                ...
                if (!System.ServiceModel.Security.SecurityUtils.HasSymmetricSecurityKey(this.elementContainer.SourceEncryptionToken))
                {
                    ...
                    clause2 = new LocalIdKeyIdentifierClause(token2.Id);
                    flag = true;
                }
                else
                {
                ...
                if (this.encryptingTokenParameters.RequireDerivedKeys)
                {
                    ...
                    clause3 = new LocalIdKeyIdentifierClause(token3.Id);
                }
                else
                {
        ...
        private void StartSignature()
        {
            if (this.elementContainer.SourceSigningToken != null)
            {
                ...
                if (this.signingTokenParameters.RequireDerivedKeys && !this.signingTokenParameters.HasAsymmetricKey)
                {
                    ...
                    clause2 = new LocalIdKeyIdentifierClause(sourceSigningToken.Id);
                }
                else
                {
        ...
        public override string Name =>
            base.StandardsManager.SecurityVersion.HeaderName.Value;

        public override string Namespace =>
            base.StandardsManager.SecurityVersion.HeaderNamespace.Value;

        internal byte[] PrimarySignatureValue =>
            this.primarySignatureValue;

        protected System.ServiceModel.Security.SecurityAppliedMessage SecurityAppliedMessage =>
            ((System.ServiceModel.Security.SecurityAppliedMessage) base.Message);

        public bool ShouldProtectTokens
        {
            get => 
                this.shouldProtectTokens;
            set
            {
                base.ThrowIfProcessingStarted();
        ...
        XmlDictionaryString IMessageHeaderWithSharedNamespace.SharedNamespace =>
            System.ServiceModel.XD.UtilityDictionary.Namespace;

        XmlDictionaryString IMessageHeaderWithSharedNamespace.SharedPrefix =>
            System.ServiceModel.XD.UtilityDictionary.Prefix;
