namespace System.ServiceModel.Security
{
    ...
    internal sealed class WindowsSspiNegotiation : ISspiNegotiation, IDisposable
    {
        ...
        internal WindowsSspiNegotiation(string package, SafeFreeCredentials credentialsHandle, string defaultServiceBinding) : this(true, package, credentialsHandle, TokenImpersonationLevel.Delegation, defaultServiceBinding, false)
        {
        }

        internal WindowsSspiNegotiation(string package, SafeFreeCredentials credentialsHandle, TokenImpersonationLevel impersonationLevel, string servicePrincipalName, bool doMutualAuth) : this(false, package, credentialsHandle, impersonationLevel, servicePrincipalName, doMutualAuth)
        {
        }

        private WindowsSspiNegotiation(bool isServer, string package, SafeFreeCredentials credentialsHandle, TokenImpersonationLevel impersonationLevel, string servicePrincipalName, bool doMutualAuth)
        {
        ...
        public string GetRemoteIdentityName()
        {
            ...
            if (this.IsValidContext)
            {
                ...
                {
                    using (WindowsIdentity identity = new WindowsIdentity(handle.DangerousGetHandle()))
                    {
