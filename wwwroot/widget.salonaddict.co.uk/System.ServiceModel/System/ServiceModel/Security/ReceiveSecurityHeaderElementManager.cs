    ...
    internal sealed class ReceiveSecurityHeaderElementManager : ISignatureReaderProvider
    {
        ...
        internal bool TryGetTokenElementIndexFromStrId(string strId, out int index)
        {
            ...
            for (int i = 0; i < this.Count; i++)
            {
                ...
                {
                    keyIdentifierClause = this.GetElement(i) as SecurityKeyIdentifierClause;
            ...
            if (keyIdentifierClause != null)
            {
                for (int j = 0; j < this.Count; j++)
                {
                    ...
                    {
                        SecurityToken element = this.GetElement(j) as SecurityToken;
                        if (element.MatchesKeyIdentifierClause(keyIdentifierClause))
                        {
                            index = j;
                            return true;
                        }
