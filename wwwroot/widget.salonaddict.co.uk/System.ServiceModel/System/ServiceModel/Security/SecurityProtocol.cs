namespace System.ServiceModel.Security
{
    ...
    internal abstract class SecurityProtocol : ISecurityCommunicationObject
    {
        ...
        private void AddSupportingTokenProviders(SupportingTokenParameters supportingTokenParameters, bool isOptional, IList<SupportingTokenProviderSpecification> providerSpecList)
        {
            for (int i = 0; i < supportingTokenParameters.Endorsing.Count; i++)
            {
                ...
                try
                {
                    ...
                    {
                        SupportingTokenProviderSpecification item = new SupportingTokenProviderSpecification(tokenProvider, SecurityTokenAttachmentMode.Endorsing, supportingTokenParameters.Endorsing[i]);
                        providerSpecList.Add(item);
                    }
                }
                catch (Exception exception)
                {
                    if (!isOptional || DiagnosticUtility.IsFatal(exception))
                    {
                        throw;
                    }
                }
            ...
            for (int j = 0; j < supportingTokenParameters.SignedEndorsing.Count; j++)
            {
                ...
                try
                {
                    if (isOptional)
                    {
                        requirement2.IsOptionalToken = true;
                    }
                    SecurityTokenProvider provider2 = this.factory.SecurityTokenManager.CreateSecurityTokenProvider(requirement2);
                    if (provider2 != null)
                    {
                        SupportingTokenProviderSpecification specification2 = new SupportingTokenProviderSpecification(provider2, SecurityTokenAttachmentMode.SignedEndorsing, supportingTokenParameters.SignedEndorsing[j]);
                        providerSpecList.Add(specification2);
                    }
                }
                catch (Exception exception2)
                {
                    if (!isOptional || DiagnosticUtility.IsFatal(exception2))
                    {
                        throw;
                    }
                }
            ...
            for (int k = 0; k < supportingTokenParameters.SignedEncrypted.Count; k++)
            {
                ...
                try
                {
                    if (isOptional)
                    {
                        requirement3.IsOptionalToken = true;
                    }
                    SecurityTokenProvider provider3 = this.factory.SecurityTokenManager.CreateSecurityTokenProvider(requirement3);
                    if (provider3 != null)
                    {
                        SupportingTokenProviderSpecification specification3 = new SupportingTokenProviderSpecification(provider3, SecurityTokenAttachmentMode.SignedEncrypted, supportingTokenParameters.SignedEncrypted[k]);
                        providerSpecList.Add(specification3);
                    }
                }
                catch (Exception exception3)
                {
                    if (!isOptional || DiagnosticUtility.IsFatal(exception3))
                    {
                        throw;
                    }
                }
            ...
            for (int m = 0; m < supportingTokenParameters.Signed.Count; m++)
            {
                ...
                try
                {
                    if (isOptional)
                    {
                        requirement4.IsOptionalToken = true;
                    }
                    SecurityTokenProvider provider4 = this.factory.SecurityTokenManager.CreateSecurityTokenProvider(requirement4);
                    if (provider4 != null)
                    {
                        SupportingTokenProviderSpecification specification4 = new SupportingTokenProviderSpecification(provider4, SecurityTokenAttachmentMode.Signed, supportingTokenParameters.Signed[m]);
                        providerSpecList.Add(specification4);
                    }
                }
                catch (Exception exception4)
                {
                    if (!isOptional || DiagnosticUtility.IsFatal(exception4))
                    {
                        throw;
                    }
                }
        ...
        public virtual IAsyncResult BeginSecureOutgoingMessage(Message message, TimeSpan timeout, AsyncCallback callback, object state)
        {
            ...
            return new TypedCompletedAsyncResult<Message>(message, callback, state);
        }
        ...
        public virtual IAsyncResult BeginSecureOutgoingMessage(Message message, TimeSpan timeout, SecurityProtocolCorrelationState correlationState, AsyncCallback callback, object state) => 
            new TypedCompletedAsyncResult<Message, SecurityProtocolCorrelationState>(message, this.SecureOutgoingMessage(ref message, timeout, correlationState), callback, state);
        ...
        public virtual IAsyncResult BeginVerifyIncomingMessage(Message message, TimeSpan timeout, SecurityProtocolCorrelationState[] correlationStates, AsyncCallback callback, object state) => 
            new TypedCompletedAsyncResult<Message, SecurityProtocolCorrelationState>(message, this.VerifyIncomingMessage(ref message, timeout, correlationStates), callback, state);
        ...
        protected SendSecurityHeader CreateSendSecurityHeader(Message message, string actor, System.ServiceModel.Security.SecurityProtocolFactory factory) => 
            this.CreateSendSecurityHeader(message, actor, factory, true);
        ...
        protected SendSecurityHeader CreateSendSecurityHeaderForTransportProtocol(Message message, string actor, System.ServiceModel.Security.SecurityProtocolFactory factory) => 
            this.CreateSendSecurityHeader(message, actor, factory, false);
        ...
        {
            message = TypedCompletedAsyncResult<Message>.End(result);
        }
        ...
        {
            message = TypedCompletedAsyncResult<Message, SecurityProtocolCorrelationState>.End(result, out newCorrelationState);
        }
        ...
        protected abstract class GetSupportingTokensAsyncResult : AsyncResult
        {
            ...
            private static AsyncCallback getSupportingTokensCallback = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(SecurityProtocol.GetSupportingTokensAsyncResult.GetSupportingTokenCallback));
