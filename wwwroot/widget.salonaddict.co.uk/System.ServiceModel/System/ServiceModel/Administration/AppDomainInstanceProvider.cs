namespace System.ServiceModel.Administration
{
    ...
    internal class AppDomainInstanceProvider : ProviderBase, IWmiProvider
    {
        ...
        internal static void FillAppDomainInfo(IWmiInstance instance)
        {
            ...
            if (ServiceHostingEnvironment.IsHosted && (ServiceHostingEnvironment.CurrentVirtualPath != null))
            {
                configurationFile = ServiceHostingEnvironment.CurrentVirtualPath + "web.config";
            }
            else
            {
                configurationFile = AppDomain.CurrentDomain.SetupInformation.ConfigurationFile;
            }
            instance.SetProperty("ServiceConfigPath", configurationFile);
            FillListenersInfo(instance);
        }

        private static void FillListenersInfo(IWmiInstance instance)
        {
            TraceSource traceSource = DiagnosticUtility.DiagnosticTrace?.TraceSource;
            if (traceSource != null)
            {
        ...
        internal static string GetReference() => 
            string.Format(CultureInfo.InvariantCulture, "AppDomainInfo.AppDomainId={0},Name='{1}',ProcessId={2}", new object[] { AppDomainInfo.Current.Id, AppDomainInfo.Current.Name, AppDomainInfo.Current.ProcessId });
        ...
        bool IWmiProvider.PutInstance(IWmiInstance instance)
        {
            ...
            try
            {
                ...
                if (DiagnosticUtility.Level != newValue)
                {
                    ...
                    {
                        TraceUtility.TraceEvent(TraceEventType.Verbose, TraceCode.WmiPut, new WmiPutTraceRecord("DiagnosticTrace.Level", DiagnosticUtility.Level, newValue), instance, null);
                    }
                ...
                if (MessageLogger.LogMalformedMessages != property)
                {
                    if (DiagnosticUtility.ShouldTraceVerbose)
                    {
                        TraceUtility.TraceEvent(TraceEventType.Verbose, TraceCode.WmiPut, new WmiPutTraceRecord("MessageLogger.LogMalformedMessages", MessageLogger.LogMalformedMessages, property), instance, null);
                    }
                ...
                if (MessageLogger.LogMessagesAtServiceLevel != flag3)
                {
                    if (DiagnosticUtility.ShouldTraceVerbose)
                    {
                        TraceUtility.TraceEvent(TraceEventType.Verbose, TraceCode.WmiPut, new WmiPutTraceRecord("MessageLogger.LogMessagesAtServiceLevel", MessageLogger.LogMessagesAtServiceLevel, flag3), instance, null);
                    }
                ...
                if (MessageLogger.LogMessagesAtTransportLevel != flag4)
                {
                    if (DiagnosticUtility.ShouldTraceVerbose)
                    {
                        TraceUtility.TraceEvent(TraceEventType.Verbose, TraceCode.WmiPut, new WmiPutTraceRecord("MessageLogger.LogMessagesAtTransportLevel", MessageLogger.LogMessagesAtTransportLevel, flag4), instance, null);
                    }
