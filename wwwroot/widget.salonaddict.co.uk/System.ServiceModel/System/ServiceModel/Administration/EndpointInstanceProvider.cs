namespace System.ServiceModel.Administration
{
    ...
    {
        private static bool WMISupportsExtendedProtection = true;

        internal static string EndpointReference(Uri uri, string contractName) => 
            EndpointReference((null != uri) ? uri.ToString() : string.Empty, contractName, true);
        ...
        {
            instance.SetProperty("CounterInstanceName", PerformanceCounters.PerformanceCountersEnabled ? EndpointPerformanceCounters.CreateFriendlyInstanceName(endpoint.ServiceName, endpoint.Contract.Name, endpoint.Address.AbsoluteUri.ToUpperInvariant()) : string.Empty);
        ...
        {
            if (WMISupportsExtendedProtection)
            {
                try
                {
                ...
                catch (WbemInstanceNotFoundException)
                {
                    WMISupportsExtendedProtection = false;
                }
            }
        }
        ...
        private string GetOperationCounterInstanceName(string operationName, IWmiInstance endpointInstance)
        {
            ...
            {
                str3 = OperationPerformanceCounters.CreateFriendlyInstanceName(info.ServiceName, info.Contract.Name, operationName, info.Address.AbsoluteUri.ToUpperInvariant());
            }
            return str3;
        }

        private bool OwnInstance(IWmiInstance instance) => 
            ((((int) instance.GetProperty("ProcessId")) == AppDomainInfo.Current.ProcessId) && (((int) instance.GetProperty("AppDomainId")) == AppDomainInfo.Current.Id));
