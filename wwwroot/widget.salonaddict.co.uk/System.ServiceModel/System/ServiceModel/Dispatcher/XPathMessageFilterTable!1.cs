namespace System.ServiceModel.Dispatcher
{
    ...
    public class XPathMessageFilterTable<TFilterData> : IMessageFilterTable<TFilterData>, IDictionary<MessageFilter, TFilterData>, ICollection<KeyValuePair<MessageFilter, TFilterData>>, IEnumerable<KeyValuePair<MessageFilter, TFilterData>>, IEnumerable
    {
        ...
        private bool ProcessMatch(QueryResult result, out TFilterData data)
        {
            bool flag = false;
            data = default(TFilterData);
        ...
        private void ProcessMatches(QueryResult result, ICollection<TFilterData> results)
        {
            Collection<MessageFilter> resultList = result.Processor.ResultList;
            int num = 0;
            int count = resultList.Count;
            while (num < count)
            {
                results.Add(this.filters[resultList[num]]);
                num++;
            }
            this.iqMatcher.ReleaseResult(result);
        }
        ...
        private bool CanMatch =>
            ((this.filters.Count > 0) && (null != this.iqMatcher));
