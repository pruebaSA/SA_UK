namespace System.ServiceModel.Dispatcher
{
    ...
    internal class DataContractSerializerOperationFormatter : OperationFormatter
    {
        ...
        private object DeserializeHeaderContents(XmlDictionaryReader reader, MessageVersion version, MessageHeaderDescription headerDescription) => 
            this.serializerFactory.CreateSerializer(headerDescription.Type, headerDescription.Name, headerDescription.Namespace, this.knownTypes).ReadObject(reader);
        ...
        protected override void GetHeadersFromMessage(Message message, MessageDescription messageDescription, object[] parameters, bool isRequest)
        {
            ...
            {
                MessageHeaders headers = message.Headers;
                KeyValuePair<Type, ArrayList>[] pairArray = null;
                ArrayList list = null;
                ...
                for (int i = 0; i < headers.Count; i++)
                {
                    ...
                    if (headerDescription != null)
                    {
                        ...
                        if (headerDescription.Multiple)
                        {
                            if (pairArray == null)
                            {
                                pairArray = new KeyValuePair<Type, ArrayList>[parameters.Length];
                            }
                            if (pairArray[headerDescription.Index].Key == null)
                            {
                                pairArray[headerDescription.Index] = new KeyValuePair<Type, ArrayList>(headerDescription.TypedHeader ? TypedHeaderManager.GetMessageHeaderType(headerDescription.Type) : headerDescription.Type, new ArrayList());
                            }
