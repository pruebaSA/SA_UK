namespace System.ServiceModel.Dispatcher
{
    ...
    {
        private static AsyncCallback acceptCallback = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ListenerHandler.AcceptCallback));
        ...
        private static WaitCallback initiateChannelPump = new WaitCallback(ListenerHandler.InitiateChannelPump);
        ...
        private bool Accept() => 
            (this.acceptor.BeginTryAccept(TimeSpan.MaxValue, acceptCallback, this).CompletedSynchronously && (this.channel != null));
        ...
        private void AcceptReady(IAsyncResult result)
        {
            ...
            bool flag = !result.CompletedSynchronously;
            if (this.channel == null)
            {
                this.DoneAccepting();
            }
            else if (flag && this.AcquireThrottle())
            {
                this.Dispatch();
                this.ChannelPump();
            }
        ...
        Label_000C:
            ...
            else if (this.Accept() && this.AcquireThrottle())
            {
                this.Dispatch();
                goto Label_000C;
            }
            ...
            try
            {
                if ((channel.State != CommunicationState.Closing) && (channel.State != CommunicationState.Closed))
                {
                    ...
                    if (channel is ISessionChannel<IDuplexSession>)
                    {
                        ...
                        IAsyncResult result = session.BeginCloseOutputSession(timeout, DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ListenerHandler.CloseOutputSessionCallback)), state);
                    ...
                    {
                        IAsyncResult result2 = channel.BeginClose(timeout, DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ListenerHandler.CloseChannelCallback)), state);
        ...
        private bool HandleError(Exception e) => 
            this.channelDispatcher.HandleError(e);
        ...
        private bool IsSessionChannel(IChannel channel) => 
            (((channel is ISessionChannel<IDuplexSession>) || (channel is ISessionChannel<IInputSession>)) || (channel is ISessionChannel<IOutputSession>));

        internal void NewChannelPump()
        {
            IOThreadScheduler.ScheduleCallback(initiateChannelPump, this);
        }
        ...
        private bool TransactedAccept(out Transaction tx)
        {
            ...
            try
            {
                ...
                {
                    if (!this.acceptor.TryAccept(transactionalTimeout, out channelBinder))
                    {
                        return false;
                    }
                    scope.Complete();
        ...
        protected override TimeSpan DefaultCloseTimeout =>
            this.host.CloseTimeout;

        protected override TimeSpan DefaultOpenTimeout =>
            this.host.OpenTimeout;
