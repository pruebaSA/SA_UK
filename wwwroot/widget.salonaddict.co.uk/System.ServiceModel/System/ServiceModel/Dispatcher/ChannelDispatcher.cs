    ...
    public class ChannelDispatcher : ChannelDispatcherBase
    {
        ...
        public override void CloseInput()
        {
            lock (base.ThisLock)
            {
                if (DiagnosticUtility.ShouldTraceInformation)
                {
                    for (int i = 0; i < this.endpointDispatchers.Count; i++)
                    {
                        ...
                        this.TraceEndpointLifetime(endpoint, TraceCode.EndpointListenerClose);
                    }
                }
                ListenerHandler listenerHandler = this.listenerHandler;
                if (listenerHandler != null)
                {
                    listenerHandler.CloseInput(this.DefaultCloseTimeout);
                }
                if (PerformanceCounters.PerformanceCountersEnabled)
                {
                    for (int j = 0; j < this.endpointDispatchers.Count; j++)
                    {
                        if (this.endpointDispatchers[j] != null)
                        {
                            this.endpointDispatchers[j].ReleasePerformanceCounters();
                        }
                    }
                }
            }
            if (!this.session)
            {
                ...
                if (handler2 != null)
                {
                    handler2.Close(this.DefaultCloseTimeout);
                }
            }
        }
        ...
        internal SynchronizedCollection<T> NewBehaviorCollection<T>() => 
            new ChannelDispatcherBehaviorCollection<T>(this);
        ...
        protected override void OnOpened()
        {
            ...
            for (int i = 0; i < this.endpointDispatchers.Count; i++)
            {
                ...
                if (DiagnosticUtility.ShouldTraceInformation)
                {
                    this.TraceEndpointLifetime(endpoint, TraceCode.EndpointListenerOpen);
                }
            }
        ...
        private void TraceEndpointLifetime(EndpointDispatcher endpoint, TraceCode traceCode)
        {
            if (DiagnosticUtility.ShouldTraceInformation)
            {
                ...
                TraceUtility.TraceEvent(TraceEventType.Information, traceCode, new DictionaryTraceRecord(dictionary), endpoint, null);
            }
        }
        ...
        public bool IsTransactedAccept =>
            (this.isTransactedReceive && this.session);

        public bool IsTransactedReceive
        {
            get => 
                this.isTransactedReceive;
            set
            {
                this.ThrowIfDisposedOrImmutable();
        ...
        internal bool TransactionIsolationLevelSet =>
            this.transactionIsolationLevelSet;
