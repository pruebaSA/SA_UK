namespace System.ServiceModel.Dispatcher
{
    ...
    internal sealed class QuotaThrottle
    {
        ...
        internal bool Acquire(object o)
        {
            lock (this.mutex)
            {
                if (this.IsEnabled)
                {
                    ...
                    {
                        this.limit--;
                        if (((this.limit == 0) && DiagnosticUtility.ShouldTraceInformation) && !this.didTraceThrottleLimit)
                        {
                            this.didTraceThrottleLimit = true;
                            DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Information, TraceCode.ManualFlowThrottleLimitReached, System.ServiceModel.SR.GetString("TraceCodeManualFlowThrottleLimitReached", new object[] { this.propertyName, this.owner }));
                        }
                        return true;
                    }
        ...
        internal void Release(object[] released)
        {
            ...
            {
                IOThreadScheduler.ScheduleCallback(new WaitCallback(this.ReleaseAsync), released[i]);
            }
        }
