        ...
        internal void Complete(Exception error)
        {
            if (!object.ReferenceEquals(this.Current, null))
            {
                ...
                else if (this.transactionSetComplete)
                {
                    ...
                    {
                        DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Information, TraceCode.TxCompletionStatusCompletedForSetComplete, System.ServiceModel.SR.GetString("TraceCodeTxCompletionStatusCompletedForSetComplete", new object[] { this.Current.TransactionInformation.LocalIdentifier, this.rpc.Operation.Name }));
                    }
                }
