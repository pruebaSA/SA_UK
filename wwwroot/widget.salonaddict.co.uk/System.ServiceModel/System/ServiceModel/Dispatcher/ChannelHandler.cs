namespace System.ServiceModel.Dispatcher
{
    ...
    internal class ChannelHandler
    {
        ...
        private bool isPumpAcquired;
        private readonly ListenerHandler listener;
        private MessageVersion messageVersion;
        private static AsyncCallback onAsyncReceiveComplete = DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(ChannelHandler.OnAsyncReceiveComplete));
        private static WaitCallback onContinueAsyncReceive = new WaitCallback(ChannelHandler.OnContinueAsyncReceive);
        private static WaitCallback onStartAsyncMessagePump = new WaitCallback(ChannelHandler.OnStartAsyncMessagePump);
        private static WaitCallback onStartSingleTransactedBatch = new WaitCallback(ChannelHandler.OnStartSingleTransactedBatch);
        private static WaitCallback onStartSyncMessagePump = new WaitCallback(ChannelHandler.OnStartSyncMessagePump);
        private static WaitCallback openAndEnsurePump = new WaitCallback(ChannelHandler.OpenAndEnsurePump);
        ...
        internal ChannelHandler(MessageVersion messageVersion, IChannelBinder binder, ServiceThrottle throttle, ListenerHandler listener, bool wasChannelThrottled, WrappedTransaction acceptTransaction, ServiceChannel.SessionIdleManager idleManager)
        {
            ...
            {
                this.receiver = new ErrorHandlingReceiver(new MultipleReceiveBinder(binder, channelDispatcher.MaxPendingReceives, !this.isConcurrent), channelDispatcher);
            }
            else
            {
        ...
        private bool DispatchAndReleasePump(RequestContext request, bool cleanThread, OperationContext currentOperationContext)
        {
            ...
            try
            {
                ...
                MessageRpc rpc = new MessageRpc(request, requestMessage, operation, channel, this.host, this, cleanThread, currentOperationContext, this.requestInfo.ExistingInstanceContext) {
                    TransactedBatchContext = this.transactedBatchContext
                };
        ...
        private void EnsurePump()
        {
            if ((this.sharedTransactedBatchContext == null) || this.isMainTransactedBatchHandler)
            {
                if (this.TryAcquirePump())
                {
                    ...
                    {
                        IOThreadScheduler.ScheduleCallback(onStartSyncMessagePump, this);
                    }
                    else if (!Thread.CurrentThread.IsThreadPoolThread)
                    {
                        IOThreadScheduler.ScheduleCallback(onStartAsyncMessagePump, this);
                    }
                    else
                    {
                        ...
                        {
                            IOThreadScheduler.ScheduleCallback(onContinueAsyncReceive, state);
                        }
                    }
                }
            }
            ...
            {
                IOThreadScheduler.ScheduleCallback(onStartSingleTransactedBatch, this);
            }
        }
        ...
        private EndpointDispatcher GetEndpointDispatcher(Message message, out bool addressMatched) => 
            this.listener.Endpoints.Lookup(message, out addressMatched);
        ...
        private bool HandleRequestAsReply(RequestContext request) => 
            ((this.duplexBinder != null) && this.duplexBinder.HandleRequestAsReply(request.RequestMessage));
        ...
        internal void ThrottleAcquiredForCall()
        {
            ...
            bool channelHandlerOwnsCallThrottle = this.requestInfo.ChannelHandlerOwnsCallThrottle;
            this.requestInfo.ChannelHandlerOwnsCallThrottle = true;
            this.TryRetrievingInstanceContext(requestWaitingForThrottle);
            if ((this.requestInfo.Channel != null) && (this.requestInfo.DispatchRuntime != null))
            {
                this.requestInfo.Channel.CompletedIOOperation();
        ...
        private bool TryAcquirePump()
        {
            if (this.isConcurrent)
            {
                return this.TryAcquirePumpCore();
            }
            return true;
        }

        private bool TryAcquirePumpCore()
        {
            lock (this.ThisLock)
            {
        ...
        private void TryRetrievingInstanceContext(RequestContext request)
        {
            bool flag = true;
            try
            {
                ...
                using (TransactionScope scope = new TransactionScope(tx))
                {
                    if (this.sharedTransactedBatchContext != null)
                    {
                        lock (this.sharedTransactedBatchContext.ReceiveLock)
                        {
                            ...
                            flag = this.TryReceive(TimeSpan.Zero, out request);
                            goto Label_005D;
                        }
                    }
                    flag = this.TryReceive(transactionalTimeout, out request);
                Label_005D:
                    scope.Complete();
                }
                return flag;
        ...
        private bool IsOpen =>
            (this.binder.Channel.State == CommunicationState.Opened);
        ...
        private static TimeSpan transactionalTimeout =>
