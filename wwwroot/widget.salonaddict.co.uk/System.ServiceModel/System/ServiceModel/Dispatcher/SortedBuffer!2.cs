namespace System.ServiceModel.Dispatcher
{
    ...
    internal struct SortedBuffer<T, C> where C: IComparer<T>, new()
    {
        private int size;
        private T[] buffer;
        private static readonly DefaultComparer<T, C> Comparer;
        ...
        static SortedBuffer()
        {
            SortedBuffer<T, C>.Comparer = new DefaultComparer<T, C>();
        }
        internal class DefaultComparer : IItemComparer<T, T>
        {
            ...
            {
                SortedBuffer<T, C>.DefaultComparer.Comparer = (default(C) == null) ? ((IComparer<T>) Activator.CreateInstance<C>()) : ((IComparer<T>) default(C));
            }

            public int Compare(T item1, T item2) => 
                SortedBuffer<T, C>.DefaultComparer.Comparer.Compare(item1, item2);
        }
    }
}
