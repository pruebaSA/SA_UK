namespace System.ServiceModel.Dispatcher
{
    ...
    internal class FaultFormatter : IClientFaultFormatter, IDispatchFaultFormatter
    {
        ...
        protected FaultException CreateFaultException(MessageFault messageFault, string action, object detailObj, Type detailType, XmlDictionaryReader detailReader)
        {
            ...
            if (flag)
            {
                return (FaultException) Activator.CreateInstance(typeof(FaultException<>).MakeGenericType(new Type[] { detailType }), new object[] { detailObj, messageFault.Reason, messageFault.Code, action });
            }
            return null;
        }
        ...
        private static MessageFault CreateMessageFault(XmlObjectSerializer serializer, FaultException faultException, Type detailType)
        {
            ...
            return (MessageFault) Activator.CreateInstance(typeof(OperationFault).MakeGenericType(new Type[] { detailType }), new object[] { serializer, faultException });
        }
        ...
        public MessageFault Serialize(FaultException faultException, out string action)
        {
            ...
            Type type2 = null;
            for (Type type3 = faultException.GetType(); type3 != typeof(FaultException); type3 = type3.BaseType)
            {
