namespace System.ServiceModel.Dispatcher
{
    ...
    internal sealed class SecurityImpersonationBehavior
    {
        ...
        {
            if (IsSecurityContextImpersonationRequiredByAspNet())
            {
                return true;
            }
        ...
        private bool IsSecurityContextImpersonationRequired(ref MessageRpc rpc) => 
            ((rpc.Operation.Impersonation == ImpersonationOption.Required) || ((rpc.Operation.Impersonation == ImpersonationOption.Allowed) && this.impersonateCallerForAllOperations));

        private static bool IsSecurityContextImpersonationRequiredByAspNet() => 
            (ServiceHostingEnvironment.IsHosted && ServiceHostingEnvironment.AspNetCompatibilityEnabled);
        ...
        public void StartImpersonation(ref MessageRpc rpc, out IDisposable impersonationContext, out IPrincipal originalPrincipal, out bool isThreadPrincipalSet)
        {
            ...
            if (isSecurityContextImpersonationOn || IsSecurityContextImpersonationRequiredByAspNet())
            {
        ...
        private IDisposable StartImpersonation2(ref MessageRpc rpc, ServiceSecurityContext securityContext, bool isSecurityContextImpersonationOn)
        {
            ...
            try
            {
                if (isSecurityContextImpersonationOn)
                {
                    ...
                    if (windowsIdentity.User == null)
                    {
                        if (securityContext.PrimaryIdentity is WindowsSidIdentity)
                        {
                            ...
                            using (WindowsIdentity identity3 = new WindowsIdentity(this.GetUpnFromDownlevelName(primaryIdentity.Name)))
                            {
                ...
                else if ((IsSecurityContextImpersonationRequiredByAspNet() && (rpc.HostingProperty != null)) && (rpc.HostingProperty.ImpersonationContext != null))
                {
                    disposable = rpc.HostingProperty.ImpersonationContext.Impersonate();
                }
            Label_010B:
        ...
        public void StopImpersonation(ref MessageRpc rpc, IDisposable impersonationContext, IPrincipal originalPrincipal, bool isThreadPrincipalSet)
        {
            try
            {
                if ((this.IsSecurityContextImpersonationRequired(ref rpc) || IsSecurityContextImpersonationRequiredByAspNet()) && (impersonationContext != null))
                {
        ...
        private class WindowsAnonymousIdentity
        {
            public IDisposable Impersonate()
            {
                ...
                if (!SafeNativeMethods.OpenCurrentThreadToken(currentThread, TokenAccessLevels.Impersonate, true, out handle))
                {
                    ...
                    handle = new SafeCloseHandle(IntPtr.Zero, false);
                }
