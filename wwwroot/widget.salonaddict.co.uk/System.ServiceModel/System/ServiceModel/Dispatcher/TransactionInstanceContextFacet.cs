    ...
    internal sealed class TransactionInstanceContextFacet
    {
                    ...
                    if (!isConcurrent)
                    {
                        if (this.shouldReleaseInstance)
                        {
                            ...
                            if (rpc.Error == null)
                            {
                                ...
                                {
                                    DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Information, TraceCode.TxCompletionStatusCompletedForAsyncAbort, System.ServiceModel.SR.GetString("TraceCodeTxCompletionStatusCompletedForAsyncAbort", new object[] { rpc.Transaction.Current.TransactionInformation.LocalIdentifier, rpc.Operation.Name }));
                                }
                            }
                        }
                        if (rpc.Transaction.IsCompleted || (rpc.Error != null))
                        {
                            if (DiagnosticUtility.ShouldTraceInformation)
                            {
                                if (rpc.Error != null)
                                {
                                    DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Information, TraceCode.TxCompletionStatusCompletedForError, System.ServiceModel.SR.GetString("TraceCodeTxCompletionStatusCompletedForError", new object[] { rpc.Transaction.Current.TransactionInformation.LocalIdentifier, rpc.Operation.Name }));
                                }
                                else
                                {
                                    DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Information, TraceCode.TxCompletionStatusCompletedForAutocomplete, System.ServiceModel.SR.GetString("TraceCodeTxCompletionStatusCompletedForAutocomplete", new object[] { rpc.Transaction.Current.TransactionInformation.LocalIdentifier, rpc.Operation.Name }));
                                }
                            }
                            this.Attached = null;
                        ...
                        else
                        {
                            ...
                            if (DiagnosticUtility.ShouldTraceInformation)
                            {
                                DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Information, TraceCode.TxCompletionStatusRemainsAttached, System.ServiceModel.SR.GetString("TraceCodeTxCompletionStatusRemainsAttached", new object[] { rpc.Transaction.Current.TransactionInformation.LocalIdentifier, rpc.Operation.Name }));
                            }
                        }
                    }
                ...
                if (tx.Equals(this.current))
                {
                    if (this.waiting != null)
                    {
                        ...
                        if (this.instanceContext.Behavior.ReleaseServiceInstanceOnTransactionComplete)
                        {
                            ...
                            if (DiagnosticUtility.ShouldTraceInformation)
                            {
                                DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Information, TraceCode.TxReleaseServiceInstanceOnCompletion, System.ServiceModel.SR.GetString("TraceCodeTxReleaseServiceInstanceOnCompletion", new object[] { tx.TransactionInformation.LocalIdentifier }));
                            }
                        }
        ...
        private sealed class RemoveReferenceRM : TransactionInstanceContextFacet.VolatileBase
        {
            ...
            public override void Rollback(Enlistment enlistment)
            {
                ...
                if (DiagnosticUtility.ShouldTraceInformation)
                {
                    DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Information, TraceCode.TxAsyncAbort, System.ServiceModel.SR.GetString("TraceCodeTxAsyncAbort", new object[] { base.Transaction.TransactionInformation.LocalIdentifier }));
                }
