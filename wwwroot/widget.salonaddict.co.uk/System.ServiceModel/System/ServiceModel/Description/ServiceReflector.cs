namespace System.ServiceModel.Description
{
    ...
    internal static class ServiceReflector
    {
        ...
        internal static object[] GetCustomAttributes(ICustomAttributeProvider attrProvider, Type attrType) => 
            GetCustomAttributes(attrProvider, attrType, false);
        ...
        internal static object[] GetCustomAttributes(ICustomAttributeProvider attrProvider, Type attrType, bool inherit)
        {
            ...
            {
                if (DiagnosticUtility.IsFatal(innerException))
                {
                    throw;
                }
        ...
        internal static List<Type> GetInterfaces(Type service)
        {
            ...
            if (!flag)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(System.ServiceModel.SR.GetString("ServicesWithoutAServiceContractAttributeCan2", new object[] { info.Name, service.FullName })));
                }
            }
            foreach (Type type2 in service.GetInterfaces())
            {
                if (type2.IsDefined(typeof(ServiceContractAttribute), false))
                {
                    if (flag)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(System.ServiceModel.SR.GetString("SFxContractInheritanceRequiresInterfaces", new object[] { service, type2 })));
                    }
                    list.Add(type2);
                }
            }
        ...
        internal static string GetLogicalName(MethodInfo method) => 
            GetLogicalName(method, IsBegin(method));
        ...
        {
            OperationContractAttribute requiredSingleAttribute = GetRequiredSingleAttribute<OperationContractAttribute>(method);
            return NamingHelper.GetOperationName(GetLogicalName(method), requiredSingleAttribute.Name);
        }
        ...
        internal static bool IsBegin(MethodInfo method)
        {
            ...
            return IsBegin(singleAttribute, method);
        }
        ...
        private static bool IsParameterDisposable(Type type)
        {
            if (type.IsSealed)
            {
