namespace System.ServiceModel.Description
{
    ...
    public class ServiceMetadataExtension : IExtension<ServiceHostBase>
    {
        ...
        private static readonly Type[] httpGetSupportedChannels = new Type[] { typeof(IReplyChannel) };
        ...
        private ChannelDispatcher CreateGetDispatcher(Uri listenUri, System.ServiceModel.Channels.Binding binding)
        {
            ...
            EndpointDispatcher item = new EndpointDispatcher(address, "IHttpGetHelpPageAndMetadataContract", "http://schemas.microsoft.com/2006/04/http/metadata");
        ...
        {
            foreach (ChannelDispatcher dispatcher in this.owner.ChannelDispatchers)
            {
                if (((dispatcher.Listener.Uri == listenUri) && (dispatcher.Endpoints.Count == 1)) && ((dispatcher.Endpoints[0].DispatchRuntime.SingletonInstanceContext != null) && (dispatcher.Endpoints[0].DispatchRuntime.SingletonInstanceContext.UserObject is HttpGetImpl)))
                {
                    return dispatcher;
                }
            }
        ...
        private static BindingParameterCollection GetBindingParameters(System.ServiceModel.Description.ServiceDescription description)
        {
            ...
            foreach (IServiceBehavior behavior in description.Behaviors)
            {
                ...
                {
                    parameters.Add(behavior);
                }
                else if (behavior is HostedBindingBehavior)
                {
                    parameters.Add(((HostedBindingBehavior) behavior).VirtualPathExtension);
                    parameters.Add(new HostedMetadataProperty());
                }
            }
            return parameters;
        }
        ...
        private class DynamicAddressUpdateWriter : ServiceMetadataExtension.WriteFilter
        {
            ...
            internal DynamicAddressUpdateWriter(Uri listenUri, string requestHost, int requestPort, IDictionary<string, int> updatePortsByScheme, bool removeBaseAddress)
            {
                this.oldHostName = listenUri.Host;
                this.newHostName = requestHost;
                this.updatePortsByScheme = updatePortsByScheme;
                this.removeBaseAddress = removeBaseAddress;
                this.requestScheme = listenUri.Scheme;
        ...
        internal class HttpGetImpl : ServiceMetadataExtension.IHttpGetMetadata, IDispatchMessageInspector
        {
            ...
            public System.ServiceModel.Channels.Message Get(System.ServiceModel.Channels.Message message) => 
                this.ProcessHttpRequest(message);
            ...
            private class InitializationData
            {
                ...
                public static ServiceMetadataExtension.HttpGetImpl.InitializationData InitializeFrom(ServiceMetadataExtension extension)
                {
                    ...
                    return new ServiceMetadataExtension.HttpGetImpl.InitializationData(docFromQuery, queryFromDoc, wsdls, xsds) { 
                ...
                public bool TryQueryLookup(string query, out object doc) => 
                    this.docFromQuery.TryGetValue(query, out doc);
            }
        ...
        internal class WSMexImpl : IMetadataExchange
        {
            ...
            internal WSMexImpl(ServiceMetadataExtension parent, Uri listenUri)
            {
                this.parent = parent;
                this.listenUri = listenUri;
                if ((this.parent.ExternalMetadataLocation != null) && (this.parent.ExternalMetadataLocation != ServiceMetadataExtension.EmptyUri))
