namespace System.ServiceModel.Description
{
    ...
    {
        private readonly Dictionary<Type, ContractDescription> contracts = new Dictionary<Type, ContractDescription>();
        ...
        private static Type[] formatterAttributes = new Type[] { typeof(XmlSerializerFormatAttribute), typeof(DataContractFormatAttribute) };
        private static Type[] knownTypesMethodParamType = new Type[] { typeof(ICustomAttributeProvider) };
        private static Type[] messageContractMemberAttributes = new Type[] { typeof(MessageHeaderAttribute), typeof(MessageBodyMemberAttribute), typeof(MessagePropertyAttribute) };
        private readonly Dictionary<Type, MessageDescriptionItems> messages = new Dictionary<Type, MessageDescriptionItems>();
        private const string ResponseSuffix = "Response";
        private const string ReturnSuffix = "Result";
        ...
        private void AddBehaviors(ContractDescription contractDesc, Type implType, bool implIsCallback, ContractReflectionInfo reflectionInfo)
        {
            ...
            for (int k = 0; k < contractDesc.Operations.Count; k++)
            {
                ...
                else
                {
                    if (callback == null)
                    {
                        callback = delegate (Type currentType, KeyedByTypeCollection<IOperationBehavior> behaviors) {
                    ...
                    if (!flag2)
                    {
                        if (callback2 == null)
                        {
                            callback2 = delegate (Type currentType, KeyedByTypeCollection<IOperationBehavior> behaviors) {
                                KeyedByTypeCollection<IOperationBehavior> types = this.GetIOperationBehaviorAttributesFromType(opDesc, targetIface, null);
                                for (int i = 0; i < types.Count; i++)
                                {
                                    behaviors.Add(types[i]);
        ...
        internal void AddBehaviorsFromImplementationType(ServiceEndpoint serviceEndpoint, Type implementationType)
        {
            ...
            for (int j = 0; j < serviceEndpoint.Contract.Operations.Count; j++)
            {
                ...
                ApplyServiceInheritance<IOperationBehavior, KeyedByTypeCollection<IOperationBehavior>>(implementationType, descriptionBehaviors, delegate (Type currentType, KeyedByTypeCollection<IOperationBehavior> behaviors) {
                    KeyedByTypeCollection<IOperationBehavior> types = this.GetIOperationBehaviorAttributesFromType(opDesc, targetIface, currentType);
        ...
        {
            for (Type type = serviceType; type != null; type = type.BaseType)
            {
        ...
        private ContractDescription CreateContractDescription(ServiceContractAttribute contractAttr, Type contractType, Type serviceType, out ContractReflectionInfo reflectionInfo, object serviceImplementation)
        {
            ...
            {
                Type attrProvider = inheritedContractTypes[i];
        ...
        private MessageHeaderDescription CreateMessageHeaderDescription(Type headerParameterType, ICustomAttributeProvider attrProvider, System.ServiceModel.Description.XmlName defaultName, string defaultNS, int parameterIndex, int serializationPosition)
        {
        ...
        private MessagePartDescription CreateMessagePartDescription(Type bodyType, ICustomAttributeProvider attrProvider, System.ServiceModel.Description.XmlName defaultName, string defaultNS, int parameterIndex, int serializationIndex)
        {
        ...
        {
            OperationContractAttribute singleAttribute = ServiceReflector.GetSingleAttribute<OperationContractAttribute>(methodInfo);
            if (singleAttribute == null)
            {
                return null;
            }
            ...
            bool isAsync = ServiceReflector.IsBegin(singleAttribute, methodInfo);
            System.ServiceModel.Description.XmlName operationName = NamingHelper.GetOperationName(ServiceReflector.GetLogicalName(methodInfo, isAsync), singleAttribute.Name);
            ...
            OperationDescription description3 = new OperationDescription(operationName.EncodedName, declaringContract) {
                IsInitiating = singleAttribute.IsInitiating,
                IsTerminating = singleAttribute.IsTerminating,
                HasNoDisposableParameters = ServiceReflector.HasNoDisposableParameters(methodInfo)
            };
            if (singleAttribute.HasProtectionLevel)
            {
                description3.ProtectionLevel = singleAttribute.ProtectionLevel;
            }
            ...
            if (singleAttribute.IsOneWay && (knownTypeAttributes.Length > 0))
            {
            ...
            foreach (Type type in this.GetKnownTypes(knownTypeAttributes, methodInfo))
            {
            ...
            string action = NamingHelper.GetMessageAction(contractName, description3.CodeName, singleAttribute.Action, false);
            string str6 = NamingHelper.GetMessageAction(contractName, description3.CodeName, singleAttribute.ReplyAction, true);
            ...
            else
            {
                ...
                if (singleAttribute.ReplyAction != null)
                {
        ...
        private static MessagePartDescription CreateParameterPartDescription(System.ServiceModel.Description.XmlName defaultName, string defaultNS, int index, ICustomAttributeProvider attrProvider, Type type)
        {
            ...
            return new MessagePartDescription(name.EncodedName, defaultNS) { 
                Type = type,
                Index = index,
        ...
        internal MessageDescription CreateTypedMessageDescription(Type typedMessageType, ICustomAttributeProvider returnAttrProvider, System.ServiceModel.Description.XmlName returnValueName, string defaultNS, string action, MessageDirection direction)
        {
            MessageDescription description;
            ...
            for (Type type = typedMessageType; ((type != null) && (type != typeof(object))) && (type != typeof(ValueType)); type = type.BaseType)
            {
        ...
        private void EnsureNoInheritanceWithContractClasses(Type actualContractType)
        {
            ...
            {
                for (Type type = actualContractType.BaseType; type != null; type = type.BaseType)
                {
        ...
        {
            foreach (Type type in actualContractType.GetInterfaces())
            {
                this.EnsureNoOperationContractsOnNonServiceContractTypes_Helper(type);
            }
            for (Type type2 = actualContractType.BaseType; type2 != null; type2 = type2.BaseType)
            {
        ...
        private void EnsureNoOperationContractsOnNonServiceContractTypes_Helper(Type aParentType)
        {
            if (ServiceReflector.GetSingleAttribute<ServiceContractAttribute>(aParentType) == null)
            {
                ...
                {
                    if (ServiceReflector.GetSingleAttribute<OperationContractAttribute>(info) != null)
                    {
        ...
        internal static System.ServiceModel.Description.XmlName GetBodyWrapperResponseName(System.ServiceModel.Description.XmlName operationName) => 
            new System.ServiceModel.Description.XmlName(operationName.EncodedName + "Response", true);

        internal static System.ServiceModel.Description.XmlName GetBodyWrapperResponseName(string operationName) => 
            new System.ServiceModel.Description.XmlName(operationName + "Response");
        ...
        private IEnumerable<Type> GetKnownTypes(object[] knownTypeAttributes, ICustomAttributeProvider provider)
        {
            if (knownTypeAttributes.Length == 1)
            {
                ...
                {
                    Type declaringType = attribute.DeclaringType;
                    if (declaringType == null)
                    {
                        declaringType = provider as Type;
                        if (declaringType == null)
                        {
                    ...
                    if (!typeof(IEnumerable<Type>).IsAssignableFrom(info.ReturnType))
                    {
                    ...
                    return (IEnumerable<Type>) info.Invoke(null, new object[] { provider });
                }
            }
        ...
        internal static System.ServiceModel.Description.XmlName GetReturnValueName(System.ServiceModel.Description.XmlName methodName) => 
            new System.ServiceModel.Description.XmlName(methodName.EncodedName + "Result", true);

        internal static System.ServiceModel.Description.XmlName GetReturnValueName(string methodName) => 
            new System.ServiceModel.Description.XmlName(methodName + "Result");
        ...
        private static bool IsMethodOverriding(MethodInfo method) => 
            (method.IsVirtual && ((method.Attributes & MethodAttributes.NewSlot) == MethodAttributes.PrivateScope));

        public ContractDescription LoadContractDescription(Type contractType) => 
            this.LoadContractDescriptionHelper(contractType, null, null);

        public ContractDescription LoadContractDescription(Type contractType, Type serviceType) => 
            this.LoadContractDescriptionHelper(contractType, serviceType, null);

        public ContractDescription LoadContractDescription(Type contractType, Type serviceType, object serviceImplementation) => 
            this.LoadContractDescriptionHelper(contractType, serviceType, serviceImplementation);
        ...
        private ContractDescription LoadContractDescriptionHelper(Type contractType, Type serviceType, object serviceImplementation)
        {
            ...
            Type contractTypeAndAttribute = ServiceReflector.GetContractTypeAndAttribute(contractType, out attribute);
            lock (this.thisLock)
            {
        ...
        {
            Type contractType = typeof(IOutputChannel);
        ...
        {
            Type contractType = typeof(IRequestChannel);
            XmlQualifiedName name = NamingHelper.GetContractName(contractType, null, "http://schemas.microsoft.com/2005/07/ServiceModel");
            ContractDescription declaringContract = new ContractDescription(name.Name, name.Namespace) {
                ContractType = contractType,
                ConfigurationName = contractType.FullName,
        ...
        {
            ApplyServiceInheritance<IContractBehavior, KeyedByTypeCollection<IContractBehavior>>(serviceType, description.Behaviors, delegate (Type currentType, KeyedByTypeCollection<IContractBehavior> behaviors) {
        ...
        private void UpdateOperationsWithInterfaceAttributes(ContractDescription contractDesc, ContractReflectionInfo reflectionInfo)
        {
            ...
            foreach (Type type in this.GetKnownTypes(knownTypeAttributes, reflectionInfo.iface))
            {
            ...
            if (reflectionInfo.callbackiface != null)
            {
                ...
                foreach (Type type2 in this.GetKnownTypes(knownTypeAttributes, reflectionInfo.callbackiface))
                {
