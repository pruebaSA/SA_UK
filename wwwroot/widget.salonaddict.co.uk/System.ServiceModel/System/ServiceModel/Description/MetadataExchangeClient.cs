namespace System.ServiceModel.Description
{
    ...
    public class MetadataExchangeClient
    {
        ...
        public MetadataSet EndGetMetadata(IAsyncResult result) => 
            AsyncMetadataResolver.End(result);
        ...
        internal static void TraceReceiveReply(string sourceUrl, Type metadataType)
        {
            if (DiagnosticUtility.ShouldTraceInformation)
            {
                ...
                TraceUtility.TraceEvent(TraceEventType.Information, TraceCode.MetadataExchangeClientReceiveReply, new DictionaryTraceRecord(dictionary), null, null);
            }
        }
        ...
        private static void TraceSendRequest(TraceCode code, string address, string mode)
        {
            if (DiagnosticUtility.ShouldTraceInformation)
            {
                Hashtable dictionary = new Hashtable(2) {
                ...
                TraceUtility.TraceEvent(TraceEventType.Information, code, new DictionaryTraceRecord(dictionary), null, null);
            }
        }
        ...
        private class AsyncMetadataResolver : AsyncResult
        {
            ...
            internal static MetadataSet End(IAsyncResult result) => 
                AsyncResult.End<MetadataExchangeClient.AsyncMetadataResolver>(result).resolveCallState.MetadataSet;
        ...
        internal class EncodingHelper
        {
            ...
            internal static bool IsApplication(ContentType contentType) => 
                (string.Compare((contentType == null) ? string.Empty : contentType.MediaType, "application", StringComparison.OrdinalIgnoreCase) == 0);
        }
        ...
        private class MetadataLocationRetriever : MetadataExchangeClient.MetadataRetriever
        {
            ...
            internal override IAsyncResult BeginRetrieve(TimeoutHelper timeoutHelper, AsyncCallback callback, object state)
            {
                ...
                try
                {
                    ...
                    retriever = new AsyncMetadataLocationRetriever(request, base.resolver.MaxMessageSize, timeoutHelper, callback, state);
                }
                catch (Exception exception2)
                {
                    if (DiagnosticUtility.IsFatal(exception2))
            ...
            protected override XmlReader DownloadMetadata(TimeoutHelper timeoutHelper)
            {
                ...
                return GetXmlReader(response, base.resolver.MaxMessageSize);
            }
            ...
            public override bool Equals(object obj) => 
                ((obj is MetadataExchangeClient.MetadataLocationRetriever) && (((MetadataExchangeClient.MetadataLocationRetriever) obj).location == this.location));
            ...
            {
                XmlReader reader = XmlDictionaryReader.CreateTextReader(new MaxMessageSizeStream(response.GetResponseStream(), maxMessageSize), MetadataExchangeClient.EncodingHelper.GetDictionaryReaderEncoding(response.ContentType), EncoderDefaults.ReaderQuotas, null);
                reader.Read();
            ...
            protected override string SourceUrl =>
                this.responseLocation.ToString();
            ...
            private class AsyncMetadataLocationRetriever : AsyncResult
            {
                ...
                internal AsyncMetadataLocationRetriever(WebRequest request, long maxMessageSize, TimeoutHelper timeoutHelper, AsyncCallback callback, object state) : base(callback, state)
                {
                    ...
                    ThreadPool.RegisterWaitForSingleObject(result.AsyncWaitHandle, DiagnosticUtility.Utility.ThunkCallback(new WaitOrTimerCallback(MetadataExchangeClient.MetadataLocationRetriever.AsyncMetadataLocationRetriever.RetrieveTimeout)), request, TimeoutHelper.ToMilliseconds(timeoutHelper.RemainingTime()), true);
                    if (result.CompletedSynchronously)
                    {
                ...
                internal static MetadataSection End(IAsyncResult result) => 
                    AsyncResult.End<MetadataExchangeClient.MetadataLocationRetriever.AsyncMetadataLocationRetriever>(result).section;
                ...
                private void HandleResult(IAsyncResult result)
                {
                    ...
                    using (XmlReader reader = MetadataExchangeClient.MetadataLocationRetriever.GetXmlReader((HttpWebResponse) asyncState.EndGetResponse(result), this.maxMessageSize))
                    {
        ...
        private class MetadataReferenceRetriever : MetadataExchangeClient.MetadataRetriever
        {
            ...
            public MetadataReferenceRetriever(EndpointAddress address, MetadataExchangeClient resolver) : this(address, resolver, null, null)
            {
            }

            public MetadataReferenceRetriever(EndpointAddress address, MetadataExchangeClient resolver, string dialect, string identifier) : base(resolver, dialect, identifier)
            {
                if (address == null)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgumentNull("address");
            ...
            internal override IAsyncResult BeginRetrieve(TimeoutHelper timeoutHelper, AsyncCallback callback, object state)
            {
                ...
                try
                {
                    ...
                    lock (base.resolver.ThisLock)
                    {
                        ...
                        exchange = factory.CreateChannel(this.address);
            ...
            private static System.ServiceModel.Channels.Message CreateGetMessage(MessageVersion messageVersion) => 
                System.ServiceModel.Channels.Message.CreateMessage(messageVersion, "http://schemas.xmlsoap.org/ws/2004/09/transfer/Get");
            ...
            public override bool Equals(object obj) => 
                ((obj is MetadataExchangeClient.MetadataReferenceRetriever) && (((MetadataExchangeClient.MetadataReferenceRetriever) obj).address == this.address));
            ...
            private class AsyncMetadataReferenceRetriever : AsyncResult
            {
                ...
                internal AsyncMetadataReferenceRetriever(IMetadataExchange metadataClient, MessageVersion messageVersion, TimeoutHelper timeoutHelper, AsyncCallback callback, object state) : base(callback, state)
                {
                    ...
                    IAsyncResult result = metadataClient.BeginGet(this.message, DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(this.RequestCallback)), metadataClient);
                    if (result.CompletedSynchronously)
                    {
                        this.HandleResult(result);
                        base.Complete(true);
                ...
                internal static MetadataSection End(IAsyncResult result) => 
                    AsyncResult.End<MetadataExchangeClient.MetadataReferenceRetriever.AsyncMetadataReferenceRetriever>(result).section;
        ...
        private abstract class MetadataRetriever
        {
            ...
            private static bool CanReadMetadataSet(XmlReader reader) => 
                ((reader.LocalName == "Metadata") && (reader.NamespaceURI == "http://schemas.xmlsoap.org/ws/2004/09/mex"));

            private static bool CanReadSchema(XmlReader reader) => 
                ((reader.LocalName == "schema") && (reader.NamespaceURI == "http://www.w3.org/2001/XMLSchema"));
        ...
        private class ResolveCallState
        {
            ...
            internal ResolveCallState(int maxResolvedReferences, bool resolveMetadataReferences, System.ServiceModel.TimeoutHelper timeoutHelper, MetadataExchangeClient resolver)
            {
            ...
            internal bool HasBeenUsed(MetadataExchangeClient.MetadataRetriever retriever) => 
                this.usedRetrievers.ContainsKey(retriever);
            ...
            internal bool ResolvedMaxResolvedReferences =>
                (this.usedRetrievers.Count == this.maxResolvedReferences);

            internal Stack<MetadataExchangeClient.MetadataRetriever> StackedRetrievers =>
                this.stackedRetrievers;
