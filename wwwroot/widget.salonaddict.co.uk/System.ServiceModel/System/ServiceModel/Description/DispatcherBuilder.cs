namespace System.ServiceModel.Description
{
    ...
    internal class DispatcherBuilder
    {
        ...
        private static void AddMsmqIntegrationContractInformation(ServiceEndpoint endpoint)
        {
            ...
            if (binding != null)
            {
                Type[] typeArray = ProcessDescriptionForMsmqIntegration(endpoint, binding.TargetSerializationTypes);
            ...
            else
            {
                ...
                if (binding2 != null)
                {
                    ...
                    if (element != null)
                    {
                        Type[] typeArray2 = ProcessDescriptionForMsmqIntegration(endpoint, element.TargetSerializationTypes);
        ...
        private Type BuildChannelListener(StuffPerListenUriInfo stuff, ServiceHostBase serviceHost, Uri listenUri, ListenUriMode listenUriMode, bool supportContextSession, out IChannelListener result)
        {
            Uri uri;
            string str;
            ...
            List<Type> requiredChannels = new List<Type>();
            if (flag3)
            {
            ...
            Type type = MaybeCreateListener(true, requiredChannels.ToArray(), binding2, parameters, uri, str, listenUriMode, serviceHost.ServiceThrottle, out result, supportContextSession && (name != null));
            if (result != null)
            {
                return type;
            }
        ...
        private static EndpointDispatcher BuildDispatcher(ServiceHostBase service, System.ServiceModel.Description.ServiceDescription serviceDescription, ServiceEndpoint endpoint, ContractDescription contractDescription, EndpointFilterProvider provider)
        {
            ...
            EndpointDispatcher dispatcher = new EndpointDispatcher(endpoint.Address, contractDescription.Name, contractDescription.Namespace, endpoint.Id);
        ...
        private Uri EnsureListenUri(ServiceHostBase serviceHost, ServiceEndpoint endpoint)
        {
            ...
            {
                ServiceHostingEnvironment.ProcessNotMatchedEndpointAddress(listenUri, endpoint.Binding.Name);
            }
        ...
        private void EnsureThereAreNonMexEndpoints(System.ServiceModel.Description.ServiceDescription description)
        {
            foreach (ServiceEndpoint endpoint in description.Endpoints)
            {
                if (!ServiceMetadataBehavior.IsMetadataImplementedType(description, endpoint.Contract.ContractType))
                {
                    return;
                }
            }
        ...
        internal static Type[] GetSupportedChannelTypes(ContractDescription contractDescription)
        {
            ...
            Type[] typeArray = ChannelRequirements.ComputeRequiredChannels(ref requirements);
        ...
        {
            this.EnsureThereAreNonMexEndpoints(description);
            ...
            foreach (KeyValuePair<ListenUriInfo, StuffPerListenUriInfo> pair in dictionary)
            {
                ...
                ThreadSafeMessageFilterTable<EndpointAddress> item = new ThreadSafeMessageFilterTable<EndpointAddress>();
                bindingParameters.Add(item);
                ...
                ChannelDispatcher dispatcher = new ChannelDispatcher(listener, name.ToString(), timeouts);
                dispatcher.SetEndpointAddressTable(item);
                pair.Value.ChannelDispatcher = dispatcher;
                bool flag2 = false;
                int num3 = 0x7fffffff;
                for (int m = 0; m < pair.Value.Endpoints.Count; m++)
                {
                    ...
                    {
                        OperationDescription description2 = endpoint3.Contract.Operations[n];
                        OperationBehaviorAttribute attribute = description2.Behaviors.Find<OperationBehaviorAttribute>();
                ...
                if (flag2)
                {
                    foreach (BindingElement element in timeouts.CreateBindingElements())
                    {
                        ...
                        {
                            dispatcher.IsTransactedReceive = true;
                            dispatcher.MaxTransactedBatchSize = num3;
                            break;
                        }
                    }
                }
                serviceHost.ChannelDispatchers.Add(dispatcher);
            }
        ...
        private void InitializeServicePerformanceCounters(ServiceHostBase serviceHost)
        {
            ...
            {
                ServicePerformanceCounters counters = new ServicePerformanceCounters(serviceHost);
                if (counters.Initialized)
                {
                    serviceHost.Counters = counters;
                }
            }
            else if (PerformanceCounters.MinimalPerformanceCountersEnabled)
            {
                DefaultPerformanceCounters counters2 = new DefaultPerformanceCounters(serviceHost);
                if (counters2.Initialized)
                {
                    serviceHost.DefaultCounters = counters2;
                }
            }
        }
        ...
        internal static Type MaybeCreateListener(bool actuallyCreate, Type[] supportedChannels, Binding binding, BindingParameterCollection parameters, Uri listenUriBaseAddress, string listenUriRelativeAddress, ListenUriMode listenUriMode, ServiceThrottle throttle, out IChannelListener result) => 
            MaybeCreateListener(actuallyCreate, supportedChannels, binding, parameters, listenUriBaseAddress, listenUriRelativeAddress, listenUriMode, throttle, out result, false);
        ...
        private static Type MaybeCreateListener(bool actuallyCreate, Type[] supportedChannels, Binding binding, BindingParameterCollection parameters, Uri listenUriBaseAddress, string listenUriRelativeAddress, ListenUriMode listenUriMode, ServiceThrottle throttle, out IChannelListener result, bool supportContextSession)
        {
            ...
            {
                Type type2 = supportedChannels[j];
        ...
        private class ListenUriInfo
        {
            ...
            public override bool Equals(object other) => 
                this.Equals(other as DispatcherBuilder.ListenUriInfo);
            ...
            public override int GetHashCode() => 
                EndpointAddress.UriGetHashCode(this.listenUri, true);
        ...
        private class TransactionContractInformationEndpointBehavior : IEndpointBehavior
        {
            ...
            private static bool UsesTransactionFlowProperties(BindingElementCollection bindingElements, ContractDescription contract) => 
                new BindingElementCollection(bindingElements).Find<TransactionFlowBindingElement>()?.IsFlowEnabled(contract);
