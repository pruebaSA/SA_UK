namespace System.ServiceModel
{
    ...
    public sealed class InstanceContext : CommunicationObject, IExtensibleObject<InstanceContext>
    {
        ...
        internal IAsyncResult BeginCloseInput(TimeSpan timeout, AsyncCallback callback, object state) => 
            this.channels.BeginCloseInput(timeout, callback, state);
        ...
        internal void CompleteAttachedTransaction()
        {
            ...
            if (!this.behavior.TransactionAutoCompleteOnSessionClose)
            {
                ...
                if (DiagnosticUtility.ShouldTraceInformation)
                {
                    DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Information, TraceCode.TxCompletionStatusAbortedOnSessionClose, System.ServiceModel.SR.GetString("TraceCodeTxCompletionStatusAbortedOnSessionClose", new object[] { this.transaction.Attached.TransactionInformation.LocalIdentifier }));
                }
            }
            else if (DiagnosticUtility.ShouldTraceInformation)
            {
                DiagnosticUtility.DiagnosticTrace.TraceEvent(TraceEventType.Information, TraceCode.TxCompletionStatusCompletedForTACOSC, System.ServiceModel.SR.GetString("TraceCodeTxCompletionStatusCompletedForTACOSC", new object[] { this.transaction.Attached.TransactionInformation.LocalIdentifier }));
            }
        ...
        public int IncrementManualFlowControlLimit(int incrementBy) => 
            this.EnsureQuotaThrottle().IncrementLimit(incrementBy);
        ...
        internal bool HasTransaction =>
            ((this.transaction != null) && !object.Equals(this.transaction.Attached, null));
        ...
        {
            get => 
                this.instanceContextManagerIndex;
            set
            {
        ...
        private bool IsSingleton =>
            ((this.behavior != null) && InstanceContextProviderBase.IsProviderSingleton(this.behavior.InstanceContextProvider));
        ...
        {
            get => 
                this.EnsureQuotaThrottle().Limit;
            set
            {
        ...
        private class CloseAsyncResult : AsyncResult
        {
            ...
            {
                IAsyncResult result = this.instanceContext.channels.BeginClose(this.timeoutHelper.RemainingTime(), DiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(this.CloseChannelsCallback)), this);
            ...
            private void CloseChannelsCallback(IAsyncResult result)
            {
                if (!result.CompletedSynchronously)
                {
                    Exception exception = null;
                    InstanceContext.CloseAsyncResult asyncState = (InstanceContext.CloseAsyncResult) result.AsyncState;
                    try
                    {
                        asyncState.instanceContext.channels.EndClose(result);
                        asyncState.instanceContext.Unload();
                    }
