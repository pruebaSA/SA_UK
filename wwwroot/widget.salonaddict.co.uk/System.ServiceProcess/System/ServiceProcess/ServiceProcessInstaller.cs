namespace System.ServiceProcess
{
    ...
    public class ServiceProcessInstaller : ComponentInstaller
    {
        ...
        private static bool AccountHasRight(IntPtr policyHandle, byte[] accountSid, string rightName)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < rightsCount; i++)
                {
                    ...
                    char[] destination = new char[structure.length];
        ...
        {
            System.ServiceProcess.NativeMethods.LSA_UNICODE_STRING userRights = new System.ServiceProcess.NativeMethods.LSA_UNICODE_STRING {
                buffer = rightName
            };
            userRights.length = (short) (userRights.buffer.Length * 2);
            userRights.maximumLength = (short) (userRights.buffer.Length * 2);
            int ntStatus = System.ServiceProcess.NativeMethods.LsaAddAccountRights(policyHandle, accountSid, userRights, 1);
            if (ntStatus != 0)
            {
                throw new Win32Exception(SafeNativeMethods.LsaNtStatusToWinError(ntStatus));
            }
        ...
        private static void RemoveAccountRight(IntPtr policyHandle, byte[] accountSid, string rightName)
        {
            ...
            userRights.length = (short) (userRights.buffer.Length * 2);
            userRights.maximumLength = userRights.length;
            int ntStatus = System.ServiceProcess.NativeMethods.LsaRemoveAccountRights(policyHandle, accountSid, false, userRights, 1);
            if (ntStatus != 0)
            {
                throw new Win32Exception(SafeNativeMethods.LsaNtStatusToWinError(ntStatus));
            }
        ...
        [TypeConverter("System.Diagnostics.Design.StringValueConverter, System.Design, Version=2.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a"), Browsable(false)]
