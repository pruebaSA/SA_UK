    ...
    internal class SqlResolver
    {
        ...
        internal class SqlBubbler : SqlVisitor
        {
            ...
            private void ForceLocal(SqlRow row, string name)
            {
                ...
                {
                    if (base.RefersToColumn(column, this.found))
                    {
                        this.found = column;
                        flag = true;
                        break;
                ...
                if (!flag)
                {
                    SqlColumn item = new SqlColumn(this.found.ClrType, this.found.SqlType, name, this.found.MetaMember, new SqlColumnRef(this.found), row.SourceExpression);
            ...
            private bool IsFoundInGroup(SqlSelect select)
            {
                ...
                {
                    if (base.RefersToColumn(expression, this.found) || base.RefersToColumn(expression, this.match))
                    {
                        return true;
                    }
                }
            ...
            internal override SqlRow VisitRow(SqlRow row)
            {
                foreach (SqlColumn column in row.Columns)
                {
                    if (base.RefersToColumn(column, this.match))
                    {
                        if (this.found != null)
                        {
        ...
        private class Visitor : SqlResolver.SqlScopedVisitor
        {
            ...
            {
                for (SqlResolver.SqlScopedVisitor.Scope scope = base.CurrentScope; scope != null; scope = scope.ContainingScope)
                {
