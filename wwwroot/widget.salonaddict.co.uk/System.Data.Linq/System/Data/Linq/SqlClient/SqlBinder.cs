namespace System.Data.Linq.SqlClient
{
    ...
    internal class SqlBinder
    {
        ...
        internal SqlBinder(Translator translator, SqlFactory sqlFactory, MetaModel model, DataLoadOptions shape, SqlColumnizer columnizer)
        {
            this.sql = sqlFactory;
        ...
        private class Visitor : SqlVisitor
        {
            ...
            private SqlNode AccessMember(SqlMember m, SqlExpression expo)
            {
                ...
                switch (expr.NodeType)
                {
                    ...
                    {
                        SqlSubSelect select = (SqlSubSelect) expr;
                        ...
                        try
                        {
                            SqlSelect select3 = new SqlSelect(selection, alias, select.SourceExpression);
                            ...
                            if (select5 == null)
                            {
                                throw System.Data.Linq.SqlClient.Error.UnexpectedNode(node2.NodeType);
                            }
                    ...
                    {
                        SqlGrouping grouping = (SqlGrouping) expr;
                    ...
                    case SqlNodeType.ClientCase:
                    {
                        ...
                        foreach (SqlClientWhen when in @case.Whens)
                        {
                            ...
                            {
                                throw System.Data.Linq.SqlClient.Error.ExpectedClrTypesToAgree(clrType, item.ClrType);
                            }
                    ...
                    case SqlNodeType.AliasRef:
                    {
                        ...
                        if (table != null)
                        {
                            ...
                            if (column == null)
                            {
                                ProviderType sqlType = this.sql.Default(member);
                        ...
                        if (column2 == null)
                        {
                            ProviderType type4 = this.sql.Default(requiredInheritanceDataMember);
                    ...
                    case SqlNodeType.New:
                    {
                        ...
                        SqlExpression expression9 = new4.Find(m.Member);
                        if (expression9 != null)
                        {
                            return expression9;
                        }
                        ...
                        {
                            throw System.Data.Linq.SqlClient.Error.MemberNotPartOfProjection(m.Member.DeclaringType, m.Member.Name);
                        }
                        goto Label_098C;
                    }
                    ...
                    case SqlNodeType.UserRow:
                    {
                        ...
                        if (column3 == null)
                        {
                            ProviderType type5 = this.sql.Default(member3);
                    ...
                    case SqlNodeType.SimpleCase:
                    {
                        ...
                        foreach (SqlWhen when2 in case2.Whens)
                        {
                            ...
                            {
                                throw System.Data.Linq.SqlClient.Error.ExpectedClrTypesToAgree(type2, expression4.ClrType);
                            }
                    ...
                    switch (nodeType)
                    {
                        case SqlNodeType.ClientCase:
                        {
                            ...
                            foreach (SqlClientWhen when in @case.Whens)
                            {
                                ...
                                {
                                    throw System.Data.Linq.SqlClient.Error.DidNotExpectTypeChange(when.Value.ClrType, @case.ClrType);
                                }
                            }
                            return node;
                        }
            ...
            internal SqlExpression ConvertToExpression(SqlNode node)
            {
                ...
                if (select == null)
                {
                    throw System.Data.Linq.SqlClient.Error.UnexpectedNode(node.NodeType);
                }
            ...
            internal SqlExpression ConvertToFetchedExpression(SqlNode node)
            {
                ...
                switch (node.NodeType)
                {
                    case SqlNodeType.ClientCase:
                    {
                        ...
                        if (flag)
                        {
                            ...
                            while (num < count)
                            {
                                ...
                                {
                                    throw System.Data.Linq.SqlClient.Error.DidNotExpectTypeChange(clientCase.ClrType, expression3.ClrType);
                                }
                    ...
                    case SqlNodeType.Link:
                    {
                        ...
                        {
                            SqlJoinType joinType = (link.Member.Association.IsForeignKey && !link.Member.Association.IsNullable) ? SqlJoinType.Inner : SqlJoinType.LeftOuter;
                            SqlSubSelect select = (SqlSubSelect) node;
                            SqlExpression where = select.Select.Where;
                            ...
                            this.currentSelect.From = this.MakeJoin(joinType, this.currentSelect.From, alias, where, select.SourceExpression);
                            SqlExpression expr = new SqlAliasRef(alias);
            ...
            internal SqlNode ConvertToFetchedSequence(SqlNode node)
            {
                if (node != null)
                {
                    ...
                    if (expression != null)
                    {
                        ...
                        {
                            throw System.Data.Linq.SqlClient.Error.SequenceOperatorsNotSupportedForType(expression.ClrType);
                        }
                        if (expression.NodeType == SqlNodeType.Value)
                        {
                            throw System.Data.Linq.SqlClient.Error.QueryOnLocalCollectionNotSupported();
                        }
            ...
            internal SqlExpression FetchExpression(SqlExpression expr) => 
                this.ConvertToExpression(this.ConvertToFetchedExpression(this.ConvertLinks(this.VisitExpression(expr))));
            ...
            private MetaType[] GetPossibleTypes(SqlExpression typeExpression)
            {
                ...
                {
                    throw System.Data.Linq.SqlClient.Error.UnexpectedNode(typeExpression.NodeType);
                }
            ...
            private static MetaDataMember GetRequiredInheritanceDataMember(MetaType type, MemberInfo mi) => 
                type.GetInheritanceType(mi.DeclaringType)?.GetDataMember(mi);
            ...
            private bool IsOuterDependent(bool isOuterDependent, SqlSource location, HashSet<SqlAlias> consumed, out HashSet<SqlAlias> produced)
            {
                ...
                {
                    HashSet<SqlAlias> set;
                    SqlJoin join = (SqlJoin) location;
                    if (this.IsOuterDependent(isOuterDependent, join.Left, consumed, out produced))
                    {
                        return true;
                    }
                    bool flag = (join.JoinType == SqlJoinType.LeftOuter) || (join.JoinType == SqlJoinType.OuterApply);
                    if (this.IsOuterDependent(flag, join.Right, consumed, out set))
                    {
                        return true;
                    }
                    produced.UnionWith(set);
                }
                else
                {
                    SqlAlias alias = location as SqlAlias;
                    if (alias != null)
                    {
                        SqlSelect node = alias.Node as SqlSelect;
                        if (((node != null) && !isOuterDependent) && ((node.From != null) && this.IsOuterDependent(false, node.From, consumed, out produced)))
                        {
                            return true;
                        }
                    }
                ...
                return (consumed.IsSubsetOf(produced) && isOuterDependent);
            }

            private SqlJoin MakeJoin(SqlJoinType joinType, SqlSource location, SqlAlias alias, SqlExpression condition, Expression source)
            {
                if (joinType == SqlJoinType.LeftOuter)
                {
                    SqlSelect node = alias.Node as SqlSelect;
                    if (((node != null) && (node.Selection != null)) && (node.Selection.NodeType != SqlNodeType.OptionalValue))
                    {
                        node.Selection = new SqlOptionalValue(new SqlColumn("test", this.sql.Unary(SqlNodeType.OuterJoinedValue, this.sql.Value(typeof(int?), this.typeProvider.From(typeof(int)), 1, false, source))), node.Selection);
                    }
                }
            ...
            internal override SqlExpression VisitBinaryOperator(SqlBinary bo)
            {
                ...
                switch (bo.NodeType)
                {
                    ...
                    case SqlNodeType.Or:
                    {
                        ...
                        if ((value6 == null) || (value7 != null))
                        {
                            ...
                            if ((value6 != null) && (value7 != null))
                            {
                                return this.sql.ValueFromObject(((bool) value6.Value) ? ((object) 1) : ((object) ((bool) value7.Value)), false, bo.SourceExpression);
                            }
                            break;
                        }
                    ...
                    case SqlNodeType.And:
                    {
                        ...
                        return this.sql.ValueFromObject(!((bool) value4.Value) ? ((object) 0) : ((object) ((bool) value5.Value)), false, bo.SourceExpression);
                    }
                }
                ...
                switch (bo.NodeType)
                {
                    ...
                    case SqlNodeType.NE2V:
                    {
                        ...
                        {
                            throw System.Data.Linq.SqlClient.Error.ComparisonNotSupportedForType(bo.Left.ClrType);
                        }
                        if (TypeSystem.IsSequenceType(bo.Right.ClrType))
                        {
                            throw System.Data.Linq.SqlClient.Error.ComparisonNotSupportedForType(bo.Right.ClrType);
                        }
                        return bo;
                    }
                }
            ...
            internal override SqlExpression VisitExpression(SqlExpression expr) => 
                this.ConvertToExpression(this.Visit(expr));
            ...
            internal override SqlNode VisitMember(SqlMember m) => 
                this.AccessMember(m, this.FetchExpression(m.Expression));
            ...
            internal override SqlExpression VisitSearchedCase(SqlSearchedCase c)
            {
                if (((c.ClrType == typeof(bool)) || (c.ClrType == typeof(bool?))) && ((c.Whens.Count == 1) && (c.Else != null)))
                {
                    ...
                    if (((@else != null) && (@else.Value != null)) && !((bool) @else.Value))
                    {
                        return this.VisitExpression(this.sql.Binary(SqlNodeType.And, c.Whens[0].Match, c.Whens[0].Value));
                    }
                    if (((value3 != null) && (value3.Value != null)) && ((bool) value3.Value))
                    {
            ...
            internal override SqlExpression VisitSharedExpressionRef(SqlSharedExpressionRef sref) => 
                ((SqlExpression) SqlDuplicator.Copy(sref.SharedExpression.Expression));
                ...
                switch (nodeType)
                {
                    ...
                    case SqlNodeType.TypeCase:
                    {
                        ...
                        foreach (SqlTypeCaseWhen when2 in case4.Whens)
                        {
                            ...
                            if (item is SqlNew)
                            {
                                throw System.Data.Linq.SqlClient.Error.DidNotExpectTypeBinding();
                            }
                            list3.Add(when2.Match);
                            list4.Add(item);
                        }
