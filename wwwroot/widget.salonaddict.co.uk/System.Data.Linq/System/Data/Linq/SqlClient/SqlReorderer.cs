namespace System.Data.Linq.SqlClient
{
    ...
    internal class SqlReorderer
    {
        ...
        internal SqlNode Reorder(SqlNode node) => 
            new Visitor(this.typeProvider, this.sql).Visit(node);
        ...
        private class Visitor : SqlVisitor
        {
            ...
            {
                SqlSelect node = new SqlSelect(new SqlNop(column.ClrType, column.SqlType, column.SourceExpression), this.currentSelect.From, this.currentSelect.SourceExpression);
            ...
            internal override SqlAlias VisitAlias(SqlAlias a)
            {
                ...
                foreach (MetaDataMember member in node.RowType.IdentityMembers)
                {
                    ...
                    if (item == null)
                    {
                        item = new SqlColumn(member.MemberAccessor.Type, this.typeProvider.From(member.MemberAccessor.Type), mappedName, member, null, node.SourceExpression) {
                            Alias = a
                        };
            ...
            internal override SqlRowNumber VisitRowNumber(SqlRowNumber rowNumber)
            {
                if (rowNumber.OrderBy.Count <= 0)
                {
                    ...
                    if (list.Count == 0)
                    {
                        foreach (SqlColumn column in SqlReorderer.SqlGatherColumnsProduced.GatherColumns(this.currentSelect.From))
                        {
                        ...
                        if (list.Count == 0)
                        {
                            SqlColumn column2 = new SqlColumn("rowNumberOrder", this.sql.Value(typeof(int), this.typeProvider.From(typeof(int)), 1, false, rowNumber.SourceExpression));
                    ...
                    {
                        rowNumber.OrderBy.Add(new SqlOrderExpression(expression2.OrderType, (SqlExpression) duplicator.Duplicate(expression2.Expression)));
                    }
                }
                return rowNumber;
            }
            ...
            internal override SqlSelect VisitSelect(SqlSelect select)
            {
                ...
                if (checker.HasRowNumber(select) && (checker.RowNumberColumn != null))
                {
                    ...
                    this.Orders.Add(new SqlOrderExpression(SqlOrderType.Ascending, new SqlColumnRef(checker.RowNumberColumn)));
                }
                if ((this.topSelect || (select.Top != null)) && ((select.OrderingType != SqlOrderingType.Never) && (this.orders != null)))
                {
                    this.orders = new HashSet<SqlOrderExpression>(this.orders).ToList<SqlOrderExpression>();
                    SqlDuplicator duplicator = new SqlDuplicator(true);
                    foreach (SqlOrderExpression expression in this.orders)
                    {
                        select.OrderBy.Add(new SqlOrderExpression(expression.OrderType, (SqlExpression) duplicator.Duplicate(expression.Expression)));
                    }
                }
