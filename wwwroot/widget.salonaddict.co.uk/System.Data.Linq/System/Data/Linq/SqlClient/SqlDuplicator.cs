namespace System.Data.Linq.SqlClient
{
    ...
    internal class SqlDuplicator
    {
        ...
        internal SqlDuplicator() : this(true)
        {
        }
        ...
        internal class DuplicatingVisitor : SqlVisitor
        {
            ...
            internal override SqlStatement VisitAssign(SqlAssign sa) => 
                new SqlAssign(this.VisitExpression(sa.LValue), this.VisitExpression(sa.RValue), sa.SourceExpression);

            internal override SqlExpression VisitBetween(SqlBetween between) => 
                new SqlBetween(between.ClrType, between.SqlType, this.VisitExpression(between.Expression), this.VisitExpression(between.Start), this.VisitExpression(between.End), between.SourceExpression);
            ...
            internal override SqlExpression VisitCast(SqlUnary c) => 
                new SqlUnary(SqlNodeType.Cast, c.ClrType, c.SqlType, (SqlExpression) this.Visit(c.Operand), c.SourceExpression);
            ...
            {
                SqlSubSelect subquery = (SqlSubSelect) this.VisitExpression(cq.Query);
            ...
            {
                SqlColumn column = new SqlColumn(col.ClrType, col.SqlType, col.Name, col.MetaMember, null, col.SourceExpression);
            ...
            internal override SqlStatement VisitDelete(SqlDelete sd) => 
                new SqlDelete((SqlSelect) this.Visit(sd.Select), sd.SourceExpression);

            internal override SqlExpression VisitDiscriminatedType(SqlDiscriminatedType dt) => 
                new SqlDiscriminatedType(dt.SqlType, this.VisitExpression(dt.Discriminator), dt.TargetType, dt.SourceExpression);

            internal override SqlExpression VisitDiscriminatorOf(SqlDiscriminatorOf dof) => 
                new SqlDiscriminatorOf(this.VisitExpression(dof.Object), dof.ClrType, dof.SqlType, dof.SourceExpression);

            internal override SqlExpression VisitDoNotVisit(SqlDoNotVisitExpression expr) => 
                new SqlDoNotVisitExpression(this.VisitExpression(expr.Expression));

            internal override SqlExpression VisitElement(SqlSubSelect elem) => 
                this.VisitMultiset(elem);

            internal override SqlExpression VisitExists(SqlSubSelect sqlExpr) => 
                new SqlSubSelect(sqlExpr.NodeType, sqlExpr.ClrType, sqlExpr.SqlType, (SqlSelect) this.Visit(sqlExpr.Select));
            ...
            internal override SqlExpression VisitGrouping(SqlGrouping g) => 
                new SqlGrouping(g.ClrType, g.SqlType, this.VisitExpression(g.Key), this.VisitExpression(g.Group), g.SourceExpression);
            ...
            internal override SqlNode VisitIncludeScope(SqlIncludeScope scope) => 
                new SqlIncludeScope(this.Visit(scope.Child), scope.SourceExpression);

            internal override SqlStatement VisitInsert(SqlInsert si) => 
                new SqlInsert(si.Table, this.VisitExpression(si.Expression), si.SourceExpression) { 
                    OutputKey = si.OutputKey,
                    OutputToLocal = si.OutputToLocal,
                    Row = this.VisitRow(si.Row)
                };
            ...
            internal override SqlExpression VisitJoinedCollection(SqlJoinedCollection jc) => 
                new SqlJoinedCollection(jc.ClrType, jc.SqlType, this.VisitExpression(jc.Expression), this.VisitExpression(jc.Count), jc.SourceExpression);

            internal override SqlExpression VisitLift(SqlLift lift) => 
                new SqlLift(lift.ClrType, this.VisitExpression(lift.Expression), lift.SourceExpression);

            internal override SqlExpression VisitLike(SqlLike like) => 
                new SqlLike(like.ClrType, like.SqlType, this.VisitExpression(like.Expression), this.VisitExpression(like.Pattern), this.VisitExpression(like.Escape), like.SourceExpression);
            ...
            internal override SqlNode VisitMember(SqlMember m) => 
                new SqlMember(m.ClrType, m.SqlType, (SqlExpression) this.Visit(m.Expression), m.Member);

            internal override SqlMemberAssign VisitMemberAssign(SqlMemberAssign ma) => 
                new SqlMemberAssign(ma.Member, (SqlExpression) this.Visit(ma.Expression));
            ...
            internal override SqlExpression VisitMultiset(SqlSubSelect sms) => 
                new SqlSubSelect(sms.NodeType, sms.ClrType, sms.SqlType, (SqlSelect) this.Visit(sms.Select));
            ...
            internal override SqlExpression VisitObjectType(SqlObjectType ot) => 
                new SqlObjectType(this.VisitExpression(ot.Object), ot.SqlType, ot.SourceExpression);
            ...
            internal override SqlExpression VisitParameter(SqlParameter p) => 
                new SqlParameter(p.ClrType, p.SqlType, p.Name, p.SourceExpression) { Direction = p.Direction };
            ...
            internal override SqlExpression VisitScalarSubSelect(SqlSubSelect ss) => 
                new SqlSubSelect(SqlNodeType.ScalarSubSelect, ss.ClrType, ss.SqlType, this.VisitSequence(ss.Select));
            ...
            internal override SqlExpression VisitSimpleExpression(SqlSimpleExpression simple) => 
                new SqlSimpleExpression(this.VisitExpression(simple.Expression));
            ...
            internal override SqlExpression VisitTreat(SqlUnary t) => 
                new SqlUnary(SqlNodeType.Treat, t.ClrType, t.SqlType, (SqlExpression) this.Visit(t.Operand), t.SourceExpression);
            ...
            internal override SqlExpression VisitTypeCase(SqlTypeCase tc)
            {
                ...
                {
                    whens.Add(new SqlTypeCaseWhen(this.VisitExpression(when.Match), this.VisitExpression(when.TypeBinding)));
                }
                return new SqlTypeCase(tc.ClrType, tc.SqlType, tc.RowType, discriminator, whens, tc.SourceExpression);
            }

            internal override SqlExpression VisitUnaryOperator(SqlUnary uo) => 
                new SqlUnary(uo.NodeType, uo.ClrType, uo.SqlType, (SqlExpression) this.Visit(uo.Operand), uo.Method, uo.SourceExpression);

            internal override SqlNode VisitUnion(SqlUnion su) => 
                new SqlUnion(this.Visit(su.Left), this.Visit(su.Right), su.All);
            ...
            internal override SqlExpression VisitUserRow(SqlUserRow row) => 
                new SqlUserRow(row.RowType, row.SqlType, (SqlUserQuery) this.Visit(row.Query), row.SourceExpression);
