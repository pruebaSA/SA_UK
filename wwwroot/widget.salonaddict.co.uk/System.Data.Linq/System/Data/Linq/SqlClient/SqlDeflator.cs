    ...
    internal class SqlDeflator
    {
            ...
            internal override SqlSelect VisitSelect(SqlSelect select)
            {
                ...
                try
                {
                    ...
                    for (int i = select.Row.Columns.Count - 1; i >= 0; i--)
                    {
                        ...
                        if (((!forceReferenceAll && !this.referenceMap.ContainsKey(key)) && !select.IsDistinct) && ((select.GroupBy.Count != 0) || !this.aggregateChecker.HasAggregates(key.Expression)))
                        {
        ...
        private class SqlTopSelectDeflator : SqlVisitor
        {
            ...
            internal override SqlSelect VisitSelect(SqlSelect select)
            {
                ...
                foreach (SqlColumn column in select.Row.Columns)
                {
                    SqlColumnRef expression = (SqlColumnRef) column.Expression;
