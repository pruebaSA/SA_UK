    ...
    internal static class LockedFile
    {
        ...
        public static IDisposable AcquireLock(string path, TimeSpan timeout, bool writer)
        {
            ...
            else
            {
                ...
                access2 = -2147483648;
                mode = PlatformSpecific.OnWin9x ? (NativeMethods.ShareMode.FILE_SHARE_NONE | NativeMethods.ShareMode.FILE_SHARE_READ) : (NativeMethods.ShareMode.FILE_SHARE_DELETE | NativeMethods.ShareMode.FILE_SHARE_READ);
            }
            while (true)
            {
