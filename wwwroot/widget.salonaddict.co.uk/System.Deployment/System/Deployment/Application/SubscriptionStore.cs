namespace System.Deployment.Application
{
    ...
    internal class SubscriptionStore
    {
        ...
        public IDisposable AcquireStoreReaderLock() => 
            this.AcquireLock(this.SubscriptionStoreLock, false);

        public IDisposable AcquireStoreWriterLock() => 
            this.AcquireLock(this.SubscriptionStoreLock, true);
        ...
        public TempDirectory AcquireTempDirectory() => 
            new TempDirectory(this._tempPath);

        public TempFile AcquireTempFile(string suffix) => 
            new TempFile(this._tempPath, suffix);
        ...
        private void CheckApplicationPayload(CommitApplicationParams commitParams)
        {
            ...
            {
                byte[] buffer = ComponentVerifier.GenerateDigestValue(directoryName, System.Deployment.Internal.Isolation.Manifest.CMS_HASH_DIGESTMETHOD.CMS_HASH_DIGESTMETHOD_SHA1, System.Deployment.Internal.Isolation.Manifest.CMS_HASH_TRANSFORM.CMS_HASH_TRANSFORM_IDENTITY);
            ...
            using (this.AcquireSubscriptionWriterLock(subState))
            {
                ...
                if (flag)
                {
                    ...
                    foreach (ComponentStore.CrossGroupApplicationData data in list)
                    {
                        ...
                        {
                            if (data.SubState.IsShellVisible)
                            {
                                this.UninstallSubscription(data.SubState);
                            }
                            else if (data.SubState.appType == AppType.CustomHostSpecified)
                            {
                        ...
                        {
                            ComponentStore.CrossGroupApplicationData.GroupType crossGroupType = data.CrossGroupType;
                        }
                    }
                }
        ...
        internal ulong GetOnlineAppQuotaInBytes() => 
            this._compStore.GetOnlineAppQuotaInBytes();
