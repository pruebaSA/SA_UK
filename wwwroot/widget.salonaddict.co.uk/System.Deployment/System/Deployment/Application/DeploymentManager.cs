namespace System.Deployment.Application
{
    ...
    internal class DeploymentManager : IDisposable, IDownloadNotification
    {
        ...
        private ManualResetEvent _platformRequirementsSucceededEvent;
        ...
        internal DeploymentManager(Uri deploymentSource, bool isUpdate, bool isConfirmed, DownloadOptions downloadOptions, AsyncOperation optionalAsyncOp)
        {
            ...
            this._platformRequirementsSucceededEvent = new ManualResetEvent(false);
            ...
            this._assertApplicationReqEvents = new ManualResetEvent[] { this._trustNotGrantedEvent, this._platformRequirementsFailedEvent, this._platformRequirementsSucceededEvent };
        ...
        private bool BindCore(bool blocking, ref TempFile tempDeploy, ref TempDirectory tempAppDir, ref FileStream refTransaction, ref string productName)
        {
            try
            {
                ...
                if (this._subStore.CheckAndReferenceApplication(subscriptionState, appId, num) && appId.Equals(subscriptionState.CurrentBind))
                {
        ...
        private static ActivationContext ConstructActivationContextFromStore(DefinitionAppId defAppId) => 
            ActivationContext.CreatePartialActivationContext(defAppId.ToApplicationIdentity());
        ...
        private bool DeterminePlatformRequirementsCore(bool blocking)
        {
            try
            {
                ...
                {
                    PlatformDetector.VerifyPlatformDependencies(this._actDesc.AppManifest, this._actDesc.DeployManifest.Description.SupportUri, directory.Path);
                }
            }
        ...
        private bool DetermineTrustCore(bool blocking, TrustParams tp)
        {
            try
            {
                ...
                this._actDesc.Trust = System.Deployment.Application.ApplicationTrust.RequestTrust(subscriptionState, this._actDesc.DeployManifest.Deployment.Install, this._actDesc.IsUpdate, this._actCtx, tmc);
            }
            catch (Exception exception)
            {
        ...
        public string ShortcutAppId
        {
            get
            {
                ...
                if (subscriptionState.IsInstalled)
                {
                    str = $"{subscriptionState.DeploymentProviderUri.AbsoluteUri}#{subscriptionState.SubscriptionId.ToString()}";
                }
                return str;
            }
        }
