namespace System.Deployment.Application
{
    ...
    internal class ComponentStore
    {
        ...
        private ArrayList CleanCDFMSFilesInDirectory(DirectoryInfo Folder)
        {
            ...
            foreach (FileInfo info in Folder.GetFiles("*.cdf-ms", SearchOption.AllDirectories))
            {
                try
                {
                    info.Delete();
        ...
        private static void FinalizeSubscriptionState(SubscriptionStateInternal newState)
        {
            ...
            else
            {
                ...
                System.Deployment.Application.DefinitionIdentity identity2 = previousBind?.DeploymentIdentity;
        ...
        private AssemblyManifest GetAssemblyManifest(System.Deployment.Application.DefinitionIdentity asmId) => 
            new AssemblyManifest(this._store.GetAssemblyManifest(0, asmId.ComPointer));
        ...
        public static ComponentStore GetStore(ComponentStoreType storeType, SubscriptionStore subStore) => 
            new ComponentStore(storeType, subStore);
        ...
        public SubscriptionStateInternal GetSubscriptionStateInternal(System.Deployment.Application.DefinitionIdentity subId)
        {
            ...
            if (internal2.IsInstalled)
            {
                ...
                System.Deployment.Application.DefinitionIdentity deploymentIdentity = internal2.PreviousBind?.DeploymentIdentity;
                internal2.RollbackDeployment = ((deploymentIdentity != null) && ((internal2.MinimumRequiredVersion == null) || (deploymentIdentity.Version >= internal2.MinimumRequiredVersion))) ? deploymentIdentity : null;
        ...
        public SubscriptionStateInternal GetSubscriptionStateInternal(SubscriptionState subState) => 
            this.GetSubscriptionStateInternal(subState.SubscriptionId);
        ...
        {
            ArrayList list = new ArrayList {
                subState.CurrentBind,
                subState.PreviousBind,
                subState.PendingBind
            };
        ...
        private void RemoveInvalidCDFMSFiles()
        {
            ...
            try
            {
                key = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\Classes\Software\Microsoft\Windows\CurrentVersion\Deployment", true);
                if (key != null)
                {
                    key2 = key.OpenSubKey("ClickOnce35SP1Update");
                    key3 = key.OpenSubKey(@"SideBySide\2.0");
                    if ((key2 == null) && (key3 != null))
                    {
                        string str = key3.GetValue("ComponentStore_RandomString").ToString();
                        string searchPattern = str.Substring(0, 8) + "." + str.Substring(8, 3);
                        string str3 = str.Substring(11, 8) + "." + str.Substring(0x13, 3);
                        string str4 = string.Empty;
                        DirectoryInfo info = new DirectoryInfo(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData));
                        ArrayList list = new ArrayList();
                        ArrayList list2 = new ArrayList {
                            info.Parent,
                            info
                        };
                        foreach (DirectoryInfo info2 in list2)
                        {
                            foreach (DirectoryInfo info3 in info2.GetDirectories("Apps"))
                            {
                                foreach (DirectoryInfo info4 in info3.GetDirectories("2.0"))
                                {
                                    foreach (DirectoryInfo info5 in info4.GetDirectories(searchPattern))
                                    {
                                        foreach (DirectoryInfo info6 in info5.GetDirectories(str3))
                                        {
                                            list.AddRange(this.CleanCDFMSFilesInDirectory(info6));
                                            str4 = str4 + info6.FullName + "\n";
                                        }
                                    }
                                }
                            }
                        }
                        foreach (FileInfo info7 in list)
                        {
                            if (info7.Exists)
                            {
                                info7.Delete();
                        ...
                        key2 = key.CreateSubKey("ClickOnce35SP1Update");
                        if (key2 != null)
                        {
                            key2.SetValue("Action", "Purged CDF-MS Data");
                            key2.SetValue("AppData", info.FullName.ToString());
                            key2.SetValue("Hits", str4);
                        }
                        flag = true;
                    }
                }
                if (!flag)
                {
                    if (key == null)
                    {
                        key = Registry.CurrentUser.CreateSubKey(@"SOFTWARE\Classes\Software\Microsoft\Windows\CurrentVersion\Deployment");
                    }
                    if ((key != null) && (key2 == null))
                    {
                        ...
                        if (key2 != null)
                        {
                            key2.SetValue("Action", "No cleanup required");
                        }
                    }
                }
            }
        ...
        private class ScavengeContext
        {
            ...
            private class SubInstance : IComparable
            {
                ...
                public int CompareTo(object other) => 
                    ((ComponentStore.ScavengeContext.SubInstance) other).LastAccessTime.CompareTo(this.LastAccessTime);
            }
        }
