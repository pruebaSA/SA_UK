namespace System.ServiceModel.Diagnostics
{
    ...
    internal class EventLogger
    {
        ...
        internal void LogEvent(TraceEventType type, EventLogCategory category, EventLogEventId eventId, bool shouldTrace, params string[] values)
        {
            if (canLogEvent)
            {
                ...
                catch (SecurityException exception)
                {
                    ...
                    {
                        this.diagnosticTrace.TraceEvent(TraceEventType.Warning, TraceCode.TraceHandledException, TraceSR.GetString("TraceHandledException"), null, exception, null);
                    }
                }
            }
        }
        ...
        [SecurityTreatAsSafe, SecurityCritical, SecurityPermission(SecurityAction.Demand, UnmanagedCode=true)]
        internal void SafeLogEvent(TraceEventType type, EventLogCategory category, EventLogEventId eventId, bool shouldTrace, params string[] values)
        {
            this.UnsafeLogEvent(type, category, eventId, shouldTrace, values);
        }

        [SecurityCritical, SecurityTreatAsSafe, SecurityPermission(SecurityAction.Demand, UnmanagedCode=true)]
                ...
                try
                {
                    ...
                    if (shouldTrace && (this.diagnosticTrace != null))
                    {
                        Dictionary<string, string> dictionary = new Dictionary<string, string>(logValues.Length + 4) {
                            ["CategoryID.Name"] = category.ToString()
                        };
                        ...
                        this.diagnosticTrace.TraceEvent(type, TraceCode.EventLog, TraceSR.GetString("TraceCodeEventLog"), new DictionaryTraceRecord(dictionary), null, null);
                    }
                }
        ...
        private void UnsafeWriteEventLog(TraceEventType type, EventLogCategory category, EventLogEventId eventId, string[] logValues, byte[] sidBA, GCHandle stringsRootHandle)
        {
