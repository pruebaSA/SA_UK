namespace System.IdentityModel.Selectors
{
    ...
    public static class CardSpaceSelector
    {
            ...
            try
            {
                ...
                finally
                {
                    try
                    {
                        ...
                        {
                            status = NativeMethods.GetToken(chain.Length, chain.DoMarshal(), out securityToken, out pCryptoHandle);
                        }
                        if (status == 0)
                        {
        ...
        public static void Import(string fileName)
        {
            ...
            int status = NativeMethods.ImportInformationCard(fileName);
            if (status != 0)
            {
                ExceptionHelper.ThrowIfCardSpaceException(status);
                throw InfoCardTrace.ThrowHelperError(new CardSpaceException(Microsoft.InfoCards.SR.GetString("ClientAPIInfocardError")));
        ...
        {
            int status = NativeMethods.ManageCardSpace();
            if (status != 0)
            {
                ExceptionHelper.ThrowIfCardSpaceException(status);
                throw InfoCardTrace.ThrowHelperError(new CardSpaceException(Microsoft.InfoCards.SR.GetString("ClientAPIInfocardError")));
