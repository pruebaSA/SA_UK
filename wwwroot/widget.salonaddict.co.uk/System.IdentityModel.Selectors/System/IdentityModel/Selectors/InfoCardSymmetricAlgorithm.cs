namespace System.IdentityModel.Selectors
{
    ...
    internal class InfoCardSymmetricAlgorithm : SymmetricAlgorithm, IDisposable
    {
        ...
        public override ICryptoTransform CreateDecryptor() => 
            new CryptoTransform(this, CryptoTransform.Direction.Decrypt);
        ...
        public override ICryptoTransform CreateEncryptor() => 
            new CryptoTransform(this, CryptoTransform.Direction.Encrypt);
        ...
        private class CryptoTransform : ICryptoTransform, IDisposable
        {
            ...
            public CryptoTransform(InfoCardSymmetricAlgorithm symAlgo, Direction cryptoDirection)
            {
                ...
                using (HGlobalSafeHandle handle2 = HGlobalSafeHandle.Construct(iV.Length))
                {
                    ...
                    int status = NativeMethods.GetCryptoTransform(symAlgo.m_cryptoHandle.InternalHandle, (int) symAlgo.Mode, (int) symAlgo.Padding, symAlgo.FeedbackSize, (int) cryptoDirection, iV.Length, handle2, out nativeTransformHandle);
                    if (status != 0)
                    {
            ...
            public int TransformBlock(byte[] inputBuffer, int inputOffset, int inputCount, byte[] outputBuffer, int outputOffset)
            {
                ...
                using (HGlobalSafeHandle handle2 = HGlobalSafeHandle.Construct(inputCount))
                {
                    ...
                    int status = NativeMethods.TransformBlock(this.m_transCryptoHandle.InternalHandle, inputCount, handle2, out cbOutData, out pOutData);
                    if (status != 0)
                    {
                        ExceptionHelper.ThrowIfCardSpaceException(status);
                        throw InfoCardTrace.ThrowHelperError(new Win32Exception(status));
            ...
            public byte[] TransformFinalBlock(byte[] inputBuffer, int inputOffset, int inputCount)
            {
                ...
                {
                    Marshal.Copy(inputBuffer, inputOffset, handle2.DangerousGetHandle(), inputCount);
                    int status = NativeMethods.TransformFinalBlock(this.m_transCryptoHandle.InternalHandle, inputCount, handle2, out cbOutData, out pOutData);
                    if (status != 0)
                    {
                        ExceptionHelper.ThrowIfCardSpaceException(status);
                        throw InfoCardTrace.ThrowHelperError(new Win32Exception(status));
            ...
            public bool CanReuseTransform =>
                this.m_param.canReuseTransform;

            public bool CanTransformMultipleBlocks =>
                this.m_param.canTransformMultipleBlocks;

            public int InputBlockSize =>
                this.m_param.inputBlockSize;

            public int OutputBlockSize =>
                this.m_param.outputBlockSize;
